...
```bash
kubectl get gatewayclass -o=jsonpath='{.items[0].spec.parametersRef}'
```

```
{"group":"gateway.envoyproxy.io","kind":"EnvoyProxy","name":"envoy-proxy-config","namespace":"tigera-gateway"}
```
