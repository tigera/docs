...
Several implementations of the Gateway API are available, one of which is the [Envoy Gateway](https://gateway.envoyproxy.io/).
$[prodname] provides support for the Gateway API by integrating a hardened Envoy Gateway image that is based on Envoy Gateway $[envoyVersion].
