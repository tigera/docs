...
instances, so this is the first way that $[prodname] differs from traditional
...
$[prodname]'s connectivity design, based on IP routing, allows unicast IP and
anycast IP.  Anycast IP can be achieved using Neutron floating IPs or
allowed-address-pairs, if you also configure your routing to process multiple
paths for a CIDR (`add_paths` in BIRD) and to program ECMP routes into the
local kernel (`merge_paths` in BIRD).  Multicast IP is not supported, although
possible in principle.  Broadcast IP is not possible because it depends on L2
adjacency.
