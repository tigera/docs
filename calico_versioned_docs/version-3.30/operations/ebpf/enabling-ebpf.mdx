...
- SCTP (either for policy or services). This is due to lack of kernel support for the SCTP checksum in BPF.
  This is because the `Log` action maps to the iptables `LOG` action and BPF programs cannot access that log.
- VLAN-based traffic.
  :::tip
  Under some circumstances, you can use a VLAN device to connect a node to the cluster by using the `bpfDataIfacePattern` Felix configuration variable.
