...
- Either $[prodname]'s eBPF mode or `kube-proxy` must be active on a host for services to function. If you
  ...
  detects that the kernel is not supported, it will fall back to standard data plane mode (which does not support
  services).
...
- Run the tool as follows:
  ...
  Also, it is possible to fetch various counters, like packets dropped by a policy or different errors, from BPF data plane using the same tool.
...
$[prodname]'s eBPF programs contain optional detailed debug logging. Although the logs can be very verbose (because
