...
Starting in Calico v3.30.3, the Calico binaries no longer ship with debug
symbols built in. These symbols are only needed for debugging crashes using
tools like `gdb` or `lldb` and most users will not need them, so they have
been removed to reduce download and installation size. If you required these
symbols for debugging, you can install them with the following steps:

1. Add the `main/debug` component to your `project-calico-ubuntu-master-noble.sources` file
   alongside the `main` component. You can do this by editing the file manually, or through
   the following command:

   ```bash
   add-apt-repository -c main/debug ppa:project-calico/$[ppa_repo_name]
   ```

   If you chose to edit the file manually, be sure to run `apt-get update` to refresh
   package indexes.
