...
import {RadioGroup, RadioGroupButtons, RadioGroupSelection} from '/src/___new___/components/RadioGroup';
import Screenshot from '/src/___new___/components/Screenshot';
export let figCount = 1;

# Calico quickstart guide

This quickstart guide shows you how to install Calico Open Source, secure a cluster with network policy, and monitor network traffic with Calico Whisker.
...
If you want try out Calico with eBPF, select eBPF as your data plane:

<RadioGroup defaultValue='iptables' urlParam='operating-system'>

  <RadioGroupButtons labels={['iptables', 'eBPF']} />

  <RadioGroupSelection value='iptables'>
    {/* iptables instructions here */}
    These are the instructions for iptables.
  </RadioGroupSelection>

  <RadioGroupSelection value='eBPF'>
    {/* eBPF instructions here} */}

    These are ebpf specific instructions.
  </RadioGroupSelection>
...
1. Create a file called `config.yaml` and give it the following content:
  ...
  <RadioGroupSelection value='eBPF'>Setting `kubeProxyMode` to `none` instructs kind to start the cluster without kube-proxy, which is recommended for eBPF data plane mode.</RadioGroupSelection>
...
3. To verify that your cluster is working, run the following command:
   ...
   calico-cluster-control-plane   NotReady   control-plane   113s   v1.32.2
   calico-cluster-worker          NotReady   <none>          99s    v1.32.2
   calico-cluster-worker2         NotReady   <none>          99s    v1.32.2
   ```
...
<ol>
 ...
 <RadioGroupSelection value ='eBPF'>
   <li>
   Download the custom resources so you can make modifications before you apply them to your cluster.
   ...
   <li>
     Open the `custom-resources.yaml` file and add `linuxDataplane: BPF` to the `.spec.calicoNetwork` section of the `Installation` resource.
     This enables the eBPF data plane when you install Calico.

     ```yaml title="Modified custom-resources.yaml file"
     ...
     spec:
       ...
       calicoNetwork:
         // highlight-next-line
         linuxDataplane: BPF # Enables the eBPF data plane.
         ipPools:
   ...
   <li>
      Install $[prodname] by applying the modified custom resources.
