...
Use {variables.prodname} network policy to allow/deny traffic for Kubernetes service accounts.

## Value

Using {variables.prodname} network policy, you can leverage Kubernetes service accounts with RBAC for flexible control over how policies are applied in a cluster. For example, the security team can have RBAC permissions to:
...
Using **Istio-enabled apps** with {variables.prodname} network policy, the cryptographic identity associated with the service account is checked (along with the network identity) to achieve two-factor authentication.
...
Like all other Kubernetes objects, service accounts have labels. You can use labels to create ‘groups’ of service accounts. {variables.prodname} network policy lets you select workloads by their service account using:
