...
a {variables.prodname}-based OpenStack deployment is running correctly.

## Prerequisites

This document requires you have the following things:

- SSH access to the nodes in your {variables.prodname}-based OpenStack deployment.
- Access to an administrator account on your {variables.prodname}-based
...
isn't, it's likely that an error has happened in either nova or {variables.prodname}
...
FIB on the compute node has been correctly populated by {variables.prodname}. To do
...
If all of these tests behave correctly, your {variables.prodname}-based OpenStack
...
A future {variables.prodname} enhancement will remove the requirement to perform this
step.
...
confirm that you have configured BGP sessions with _all_ other {variables.prodname}
nodes.
...
the VM IP addresses. To get full {variables.prodname} functionality the gateway should
