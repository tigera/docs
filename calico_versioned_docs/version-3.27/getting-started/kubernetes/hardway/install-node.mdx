...
- Felix, the {variables.prodname} per-node daemon
- BIRD, a daemon that speaks the BGP protocol to distribute routing information to other nodes
- confd, a daemon that watches the {variables.prodname} datastore for config changes and updates BIRD's config files
...
Provision a cluster role with permissions to read and modify {variables.prodname} datastore objects
