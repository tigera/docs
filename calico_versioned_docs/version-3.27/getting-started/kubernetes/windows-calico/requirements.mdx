...
- At least one Linux Kubernetes worker node to run {{prodname}}'s cluster-wide components that meets [Linux system requirements](../requirements.mdx), and is installed with {{prodname}}. {{prodname}} v3.12+ is required for manual installs, and v3.27+ for Operator installs.
...
For operator-managed Linux {{prodname}} clusters, three Linux worker nodes are required in order to meet the high-availability requirements for Typha, which only runs on Linux nodes.
