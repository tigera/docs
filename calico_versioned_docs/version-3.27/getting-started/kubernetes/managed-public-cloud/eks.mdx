...
When using the Amazon VPC CNI plugin, {variables.prodname} does not support enforcement of network policy on IPv6 pods with `ENABLE_V4_EGRESS` set to `true`.
...
1. Install version {{releaseTitle}} of the {variables.prodname} operator and custom resource definitions.
