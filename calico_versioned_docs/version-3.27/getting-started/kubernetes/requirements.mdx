...
import ReqsSys from "@site/calico_versioned_docs/version-3.27/_includes/components/ReqsSys";
import ReqsKernel from "@site/calico_versioned_docs/version-3.27/_includes/components/ReqsKernel";
...
We test {variables.prodname} {variables.version} against the following Kubernetes versions. Other versions may work, but we are not actively testing them.

- v1.27
- v1.28
- v1.29

Due to changes in the Kubernetes API, {variables.prodname} {variables.version} will not work
on Kubernetes v1.20 or below. v1.21 may work, but is no longer tested.
Newer versions may also work, but we recommend upgrading to a version of {variables.prodname}
that is tested against the newer Kubernetes version.

#### CNI plug-in enabled

{variables.prodname} must be installed as a CNI plugin in the container runtime.
...
Generally, you cannot use {variables.prodname} together with another network provider.
...
If you're working with a cluster that already uses another CNI, you cannot migrate to {variables.prodname}.
