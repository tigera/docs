...
We've added support for Operator-based install and upgrade for Windows nodes using HPC (HostProcess/Privileged containers), {variables.prodname} can now bootstrap Windows worker nodes just like Linux worker nodes.
Platform engineers can scale Windows worker nodes with an auto-scaling node pool without bootstrapping each node manually, and upgrade {variables.prodname} deployments without manually upgrading nodes one at a time.

For more information, see [the {variables.prodnameWindows} Operator guide](../getting-started/kubernetes/windows-calico/operator.mdx).
...
* In {variables.prodname} 3.27, the Felix configuration parameter `BPFConnectTimeLoadBalancingEnabled` is deprecated and is planned to be removed in a future release.
