...
and volume mount the keys into the started `{variables.noderunning}` container.**

Therefore, configuring <code>{variables.nodecontainer}</code> for etcd is easily accomplished by running
