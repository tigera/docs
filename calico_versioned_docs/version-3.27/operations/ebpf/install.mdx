...
To install in eBPF mode, we recommend using the Tigera Operator to install {variables.prodname} so these instructions
use the operator. Installing {variables.prodname} normally consists of the following stages, which are covered by the
...
<CodeBlock language='bash'>
 curl -o custom-resources.yaml {variables.manifestsUrl}/manifests/custom-resources.yaml
</CodeBlock>
...
```yaml
# This section includes base {variables.prodname} installation configuration.
