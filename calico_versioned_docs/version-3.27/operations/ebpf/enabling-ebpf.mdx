...
import EbpfValue from '@site/calico_versioned_docs/version-3.27/_includes/content/_ebpf-value.mdx';
...
- Distributions:
  ...
  - AKS with limitations:
    - [AKS with Azure CNI and Calico network policy](../../getting-started/kubernetes/managed-public-cloud/aks.mdx#install-aks-with-{variables.prodnamedash}-for-network-policy) works, but it is not possible to disable kube-proxy resulting in wasted resources and suboptimal performance.
    - [AKS with {variables.prodname} networking](../../getting-started/kubernetes/managed-public-cloud/aks.mdx#install-aks-with-{variables.prodnamedash}-networking) is in testing with the eBPF dataplane. This should be a better solution overall but, at time of writing, the testing was not complete.
...
Limitations:
  ...
  - IPIP is not supported ({variables.prodname} iptables does not supporte it either). VXLAN is the recommended overlay for eBPF mode.
...
- [Configure {variables.prodname} to talk directly to the API server](#configure-{variables.prodnamedash}-to-talk-directly-to-the-api-server)
...
### Configure {variables.prodname} to talk directly to the API server

In eBPF mode, {variables.prodname} implements Kubernetes service networking directly (rather than relying on `kube-proxy`).
...
To be able to disable `kube-proxy`, {variables.prodname} needs to communicate to the API server _directly_ rather than
...
- If you created a cluster manually (for example by using `kubeadm`) then the right address to use depends on whether you
  ...
  - If you opted for a single control plane node then you can use the address of the control plane node itself. However,
    ...
    causing {variables.prodname} to lose connectivity to the API server.
...
- MKE and Rancher neither allow `kube-proxy` to be disabled nor provide a stable address for the
  ...
  - Let {variables.prodname} connect to the API server as through `kube-proxy` (by skipping the step below to create the
...
**The next step depends on whether you installed {variables.prodname} using the operator, or a manifest:**

<Tabs>
<TabItem label="Operator" value="Operator-0">

If you installed {variables.prodname} using the operator, create the following config map in the `tigera-operator` namespace using the host and port determined above:
...
The operator will pick up the change to the config map automatically and do a rolling update of {variables.prodname} to pass on the change. Confirm that pods restart and then reach the `Running` state with the following command:
...
If you installed {variables.prodname} using a manifest, create the following config map in the `kube-system` namespace using the host and port determined above:
...
Wait 60s for kubelet to pick up the `ConfigMap` (see Kubernetes [issue #30189](https://github.com/kubernetes/kubernetes/issues/30189)); then, restart the {variables.prodname} pods to pick up the change:
...
You can verify that the change was picked up by checking the logs of one of the {variables.nodecontainer} pods.
...
{variables.noderunning}-d6znw                          1/1     Running   0          48m
...
```
...
In eBPF mode {variables.prodname} replaces `kube-proxy` so it wastes resources (and reduces performance) to run both.  
...
If you installed {variables.prodname} using a manifest, change Felix configuration parameter `BPFEnabled` to `true`. This can be done with `calicoctl`, as follows:
