...
Use Prometheus configured for {variables.prodname} components to get valuable metrics about the health of {variables.prodname}.

## Value

Using the open-source Prometheus monitoring and alerting toolkit, you can view time-series metrics from {variables.prodname} components in the Prometheus or Grafana interfaces.

## Concepts

### About Prometheus

The Prometheus monitoring tool scrapes metrics from instrumented jobs and displays time series data in a visualizer (such as Grafana). For {variables.prodname}, the “jobs” that Prometheus can harvest metrics from are the Felix and Typha components.

### About {variables.prodname} Felix, Typha, and kube-controllers components

**Felix** is a daemon that runs on every machine that implements network policy. Felix is the brains of {variables.prodname}. Typha is an optional set of pods that extends Felix to scale traffic between {variables.prodname} nodes and the datastore. The kube-controllers pod runs a set of controllers which are responsible for a variety of control plane functions, such as resource garbage collection and synchronization with the Kubernetes API.

You can configure Felix, Typha, and/or kube-controllers to provide metrics to Prometheus.

## Before you begin...

In this tutorial we assume that you have completed all other introductory tutorials and possess a running Kubernetes cluster with {variables.prodname}. You can either use `kubectl` or `calicoctl` to perform the following steps. Depending on which tool you would like to use, make sure you have the necessary prerequisites as shown below.

<Tabs>
<TabItem label="kubectl" value="kubectl-0">

If you wish to modify {variables.prodname} configurations with `kubectl` binary you need to make sure you have the {variables.prodname} API server in your cluster. The API server allows you to manage resources within the `projectcalico.org/v3` api group.
...
This tutorial will go through the necessary steps to implement basic monitoring of {variables.prodname} with Prometheus.

1. Configure {variables.prodname} to enable the metrics reporting.
...
### 1. Configure {variables.prodname} to enable metrics reporting
...
If running {variables.prodnameWindows}, also create a service for Windows nodes:
...
[See the FelixConfiguration reference for more details](../../reference/resources/felixconfig.mdx). You can also add a Windows firewall rule that allows listening on the Prometheus metrics port instead of having {variables.prodname} manage it.
...
An Operator installation of {variables.prodname} automatically deploys one or more Typha instances depending on the scale of your cluster. By default metrics for these instances are disabled.
...
You need to provide Prometheus a serviceAccount with required permissions to collect information from {variables.prodname}.
...
If running {variables.prodnameWindows}, also clean up the Windows nodes service:
```bash
...
Return {variables.prodname} configurations to their default state.
...
If you enable {variables.prodname} metrics to Prometheus, a best practice is to use network policy to limit access to the {variables.prodname} metrics endpoints. For details, see [Secure {variables.prodname} Prometheus endpoints](../../network-policy/comms/secure-metrics.mdx).
