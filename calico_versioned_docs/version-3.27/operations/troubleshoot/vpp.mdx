...
If you're encountering issues with the VPP dataplane, feel free to reach out to us either on the [#vpp channel](https://calicousers.slack.com/archives/C017220EXU1) on the {variables.prodname} slack, or by opening a new issue in [GitHub](https://github.com/projectcalico/vpp-dataplane/issues)).
...
- With curl
  ...
  curl https://raw.githubusercontent.com/projectcalico/vpp-dataplane/\{variables.vppbranch}/test/scripts/vppdev.sh | tee /usr/bin/calivppctl
  chmod +x /usr/bin/calivppctl
  </CodeBlock>

- With docker (and a cluster with {variables.prodname} VPP running)
...
- With kubectl (and a cluster with {variables.prodname} VPP running)
...
<CodeBlock language='bash'>
# Print the logs for the \{variables.prodname} VPP dataplane agent, programming serviceIPs, BGP, ...
