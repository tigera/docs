...
A {variables.prodname} [Kubernetes controllers](../kube-controllers/configuration.mdx) configuration resource (`KubeControllersConfiguration`) represents configuration options for the {variables.prodname} Kubernetes controllers.
...
- {variables.prodname} automatically creates a resource named `default` containing the configuration settings, only the name `default` is used and only one object of this type is allowed. You can use [calicoctl](../calicoctl/overview.mdx) to view and edit these settings
...
| etcdV3CompactionPeriod | The period between etcdv3 compaction requests. Only applies when using etcd as the {variables.prodname} datastore. | Set to 0 to disable, > 0 to enable  | [Duration string][parse-duration] | 10m     |
...
| Field            | Description                                                                       | Accepted Values   | Schema                            | Default |
| ---------------- | --------------------------------------------------------------------------------- | ----------------- | --------------------------------- | ------- |
| reconcilerPeriod | Period to perform reconciliation with the {variables.prodname} datastore                  |                   | [Duration string][parse-duration] | 5m      |
| syncLabels       | When enabled, Kubernetes node labels will be copied to {variables.prodname} node objects. | Enabled, Disabled | string                            | Enabled |
...
The policy controller syncs Kubernetes network policies to the Calico datastore. This controller is only valid when using etcd as the {variables.prodname} datastore.
...
The workload endpoint controller automatically syncs Kubernetes pod label changes to the {variables.prodname} datastore by updating the corresponding workload
endpoints appropriately. This controller is only valid when using etcd as the {variables.prodname} datastore.
...
The service account controller syncs Kubernetes service account changes to the {variables.prodname} datastore. This controller is only valid when using etcd as
the {variables.prodname} datastore.
...
The namespace controller syncs Kubernetes namespace label changes to the {variables.prodname} datastore. This controller is only valid when using etcd as the
{variables.prodname} datastore.
