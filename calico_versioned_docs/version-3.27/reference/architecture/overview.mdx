...
The following diagram shows the required and optional {variables.prodname} components for a Kubernetes, on-premises deployment with networking and network policy.
...
**Main task**: Lets you manage {variables.prodname} resources directly with `kubectl`.
...
- **BGP route reflector configuration**
  ...
  For redundancy, you can seamlessly deploy multiple BGP route reflectors. BGP route reflectors are involved only in control of the network: no endpoint data passes through them. When the {variables.prodname} BGP client advertises routes from its FIB to the route reflector, the route reflector advertises those routes out to the other nodes in the deployment.

## confd

**Main task**: Monitors {variables.prodname} datastore for changes to BGP configuration and global defaults such as AS number, logging levels, and IPAM information. Open source, lightweight configuration management tool.
...
The {variables.prodname} binary that presents this API to Kubernetes is called the CNI plugin, and must be installed on every node in the Kubernetes cluster. The {variables.prodname} CNI plugin allows you to use {variables.prodname} networking for any orchestrator that makes use of the CNI networking specification. Configured through the standard [CNI configuration mechanism](https://github.com/containernetworking/cni/blob/master/SPEC.md#network-configuration), and [{variables.prodname} CNI plugin](../configure-cni-plugins.mdx).

## Datastore plugin

**Main task**: Increases scale by reducing each node’s impact on the datastore. It is one of the {variables.prodname} [CNI plugins](../configure-cni-plugins.mdx).

- **Kubernetes API datastore (kdd)**

  The advantages of using the Kubernetes API datastore (kdd) with {variables.prodname} are:

  - Simpler to manage because it does not require an extra datastore
  - Use Kubernetes RBAC to control access to {variables.prodname} resources
  - Use Kubernetes audit logging to generate audit logs of changes to {variables.prodname} resources

- **etcd**

  `etcd` is a consistent, highly-available distributed key-value store that provides data storage for the {variables.prodname} network, and for communications between components. `etcd` is supported for protecting only non-cluster hosts (as of {variables.prodname} v3.1). For completeness, `etcd ` advantages are:

  - Lets you run {variables.prodname} on non-Kubernetes platforms
  - Separation of concerns between Kubernetes and {variables.prodname} resources, for example allowing you to scale the datastores independently
  - Lets you run a {variables.prodname} cluster that contains more than just a single Kubernetes cluster, for example, bare metal servers with {variables.prodname} host protection interworking with a Kubernetes cluster; or multiple Kubernetes clusters.

  [etcd admin guide](https://coreos.com/etcd/docs/latest/admin_guide.html#optimal-cluster-size)

## IPAM plugin

**Main task**: Uses {variables.prodname}’s IP pool resource to control how IP addresses are allocated to pods within the cluster. It is the default plugin used by most {variables.prodname} installations. It is one of the {variables.prodname} [CNI plugins](../configure-cni-plugins.mdx).
...
**Main task**: Command line interface to create, read, update, and delete {variables.prodname} objects. `calicoctl` command line is available on any host with network access to the {variables.prodname} datastore as either a binary or a container. Requires separate installation. [calicoctl](../calicoctl/index.mdx).

## Plugins for cloud orchestrators

**Main task**: Translates the orchestrator APIs for managing networks to the {variables.prodname} data-model and datastore.
