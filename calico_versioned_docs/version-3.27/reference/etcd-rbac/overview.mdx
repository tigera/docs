...
- You want to use a single etcd cluster for {variables.prodname} and Kubernetes (rather than
  having an etcd cluster for {variables.prodname} and a separate etcd cluster for Kubernetes.
- You want to restrict the read/write access of the various {variables.prodname} components
...
(like {variables.prodname} and Kubernetes) will need to authenticate with the etcd cluster.
...
- If using only the v2 API with etcd, as {variables.prodname} does, then the minimum etcd
...
4. Configure components. For example:
   - [Setting up Kubernetes with {variables.prodname} utilizing etcd RBAC](kubernetes.mdx).
