...
| `ipPools` _[IPPool](#ippool) array_ | (Optional) IPPools contains a list of IP pools to manage. If nil, a single IP pool<br />will be defaulted. If an empty list is provided, the operator will not create any IP pools and will instead<br />wait for IP pools to be created out-of-band.<br />IP pools in this list will be reconciled by the operator and should not be modified out-of-band. |
