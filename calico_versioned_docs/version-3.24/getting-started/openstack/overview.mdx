...
{variables.prodname}'s integration with OpenStack consists of the following pieces.

- etcd, providing a distributed key/value database that is accessible from all
  compute hosts and Neutron servers.

- Felix (the {variables.prodname} agent) running on each compute host. Felix reads
...
- The {variables.prodname} driver for Neutron, that runs as part of the Neutron server on
...
- The {variables.prodname} DHCP agent, running on each compute host, that configures and
...
The Etcd, Felix and BIRD pieces are the same as in other {variables.prodname} integrations,
and so independent of OpenStack. The {variables.prodname} Neutron driver and DHCP agent are
...
by the Neutron API. Refer to [{variables.prodname}'s interpretation of Neutron API calls](../../networking/openstack/neutron-api.mdx) for more detail about the
