...
1. Install {{prodname}}.
   ...
   ```
   kubectl apply -f {{releases.0.manifests_url}}/manifests/flannel-migration/calico.yaml
   ```

1. Start the migration controller.

   ```
   kubectl apply -f {{releases.0.manifests_url}}/manifests/flannel-migration/migration-job.yaml
   ```
...
1. Delete the migration controller.
   ...
   ```
   kubectl delete -f {{releases.0.manifests_url}}/manifests/flannel-migration/migration-job.yaml
   ```
...
1. Remove the migration controller and {{prodname}}.
   ...
   ```
   kubectl delete -f {{releases.0.manifests_url}}/manifests/flannel-migration/migration-job.yaml
   kubectl delete -f {{releases.0.manifests_url}}/manifests/flannel-migration/calico.yaml
   ```
