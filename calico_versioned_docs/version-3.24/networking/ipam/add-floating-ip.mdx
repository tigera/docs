...
**{variables.prodname} CNI configuration file** with floating_ips enabled
...
- {variables.prodname} CNI plugin

To verify, ssh to one of the Kubernetes nodes and look for at the CNI plugin configuration, usually located at `/etc/cni/net.d/`. If you see the file, `10-calico.conflist`, you are using the {variables.prodname} CNI plugin.
