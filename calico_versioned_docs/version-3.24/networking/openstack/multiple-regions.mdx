...
of OpenStack, you can use {variables.prodname} to facilitate defining security
policy between VMs in different regions.

## Architecture

The way this works is that all of the regions share the same {variables.prodname}
etcd datastore, but each region uses a different {variables.prodname} namespace.
...
the {variables.prodname} datastore in the namespace "openstack-region-xyz-east".

Because all the information is in the same {variables.prodname} datastore, you can
then define {variables.prodname} policy for communication between VMs in different
regions.
...
1.  Only install a single etcd database (instead of one per region) and
    configure all of the regions to use that as their {variables.prodname}
    datastore.
...
configure this {variables.prodname} policy:
...
{variables.prodname} adds to OpenStack VM endpoints, to identify the set of VMs
