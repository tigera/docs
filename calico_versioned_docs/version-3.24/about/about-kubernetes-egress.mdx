...
Sets](../network-policy/policy-rules/external-ips-policy.mdx), or Calico Enterprise's support for domain names in policy rules.
...
](../network-policy/adopt-zero-trust.mdx) guide.
