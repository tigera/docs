...
## Connections from {variables.prodname} components to etcd
...
- Configure each {variables.prodname} component to verify the etcd server's identity and to present
  a certificate to the etcd server that is signed by the etcd certificate authority.
  - [{variables.nodecontainer}](../../reference/configure-calico-node.mdx)
