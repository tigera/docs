...
```batch
curl {{manifestsUrl}}/manifests/alp/istio-inject-configmap-1.9.yaml -o istio-inject-configmap.yaml
kubectl patch configmap -n istio-system istio-sidecar-injector --patch "$(cat istio-inject-configmap.yaml)"
```

[View sample manifest]({{manifestsUrl}}/manifests/alp/istio-inject-configmap-1.9.yaml)
