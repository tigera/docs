...
![Detail of BPF program showing that packets are sent to a separate (generated) policy program,](/img/calico/v3.24/bpf-policy.svg 'Expanded view of tc program showing policy.')
...
![Diagram showing BPF program attached to socket connect call; it does NAT at connect time,](/img/calico/v3.24/bpf-connect-time.svg 'BPF program attached to socket connect call.')
...
- If you'd like to try eBPF mode in your Kubernetes cluster, follow the [Enable the eBPF dataplane](../maintenance/ebpf/enabling-ebpf.mdx) guide.
