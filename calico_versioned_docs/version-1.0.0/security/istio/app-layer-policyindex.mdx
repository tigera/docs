...
Envoy must be able to communicate with the `istio-pilot.istio-system service`. If you apply any egress policies to your pods, you _must_ enable access. For example, you could [apply a network policy]({{url}}{{baseUrl}}/security/calico-network-policy).

```bash-plain-text
kubectl apply -f {{url}}{{baseUrl}}/getting-started/kubernetes/installation/manifests/app-layer-policy/allow-istio-pilot.yaml
```
