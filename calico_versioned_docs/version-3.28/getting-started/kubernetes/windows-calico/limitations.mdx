...
## {variables.prodnameWindows} feature limitations
...
| Networking          | **Supported**:<br />- {variables.prodname} VXLAN, no cross-subnet or VXLAN MTU settings with [limitations](#{variables.prodname}-vxlan-networking-limitations)<br />- {variables.prodname} non-overlay mode with BGP peering with [limitations](#{variables.prodname}-bgp-networking-limitations)<br />- IPv4<br /><br />**Not supported**: <br />- Overlay mode with BGP peering<br />- IP in IP overlay with BGP routing<br />- Cross-subnet support and MTU setting for VXLAN<br />- IPv6 and dual stack<br />- Service advertisement |
...
| Visibility and troubleshooting | Visibility and troubleshooting | **Not supported**:<br/>- 'Log' action in network policy                                                                                                                                                                                      |
| Operations          | **Supported**: <br />- {variables.prodname} node status                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Metrics             | **Supported**: Prometheus monitoring                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| eBPF                | **Not supported**: (Linux-based feature)                                                                                                                                                                                                                                                                                                                                                                                                                                                     |

## {variables.prodname} BGP networking limitations

If you are using {variables.prodname} with BGP, note these current limitations with Windows.

| Feature                  | Limitation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| IP mobility/ borrowing   | {variables.prodname} IPAM allocates IPs to host in blocks for aggregation purposes.<br/>If the IP pool is full, nodes can also "borrow" IPs from another node's block. In BGP terms, the borrower then advertises a more specific "/32" route for the borrowed IP and traffic for that IP only is routed to the borrowing host. <br /><br />Windows nodes do not support this borrowing mechanism; they will not borrow IPs even if the IP pool is full and they mark their blocks so that Linux nodes will not borrow from them.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| IPs reserved for Windows | {variables.prodname} IPAM allocates IPs in CIDR blocks. Due to networking requirements on Windows, four IPs per Windows node-owned block must be reserved for internal purposes.<br /><br/>For example, with the default block size of /26, each block contains 64 IP addresses, 4 are reserved for Windows, leaving 60 for pod networking.<br /><br />To reduce the impact of these reservations, a larger block size can be configured at the IP pool scope (before any pods are created).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Single IP block per host | {variables.prodname} IPAM is designed to allocate blocks of IPs (default size /26) to hosts on demand. While the {variables.prodname} CNI plugin was written to do the same, kube-proxy currently only supports a single IP block per host.<br/><br />To work around the default limit of one /26 per host there some options:<br/><br />- With {variables.prodname} BGP networking and the etcd datastore before creating any blocks, change the block size used by the IP pool so that it is sufficient for the largest number of Pods that are to be used on a single Windows host.<br/>- Use {variables.prodname} BGP networking with the kubernetes datastore. In that mode, {variables.prodname} IPAM is not used and the CNI host-local IPAM plugin is used with the node's Pod CIDR.<br/><br />To allow multiple IPAM blocks per host (at the expense of kube-proxy compatibility), set the `windows_use_single_network` flag to `false` in the `cni.conf.template` before installing {variables.prodname}. Changing that setting after pods are networked is not recommended because it may leak HNS endpoints. |
| IP-in-IP overlay         | {variables.prodname}'s IPIP overlay mode cannot be used in clusters that contain Windows nodes because Windows does not support IP-in-IP.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| NATOutgoing              | {variables.prodname} IP pools support a "NAT outgoing" setting with the following behaviour: <br /><br />- Traffic between {variables.prodname} workloads (in any IP pools) is not NATted. <br />- Traffic leaving the configured IP pools is NATted if the workload has an IP within an IP pool that has NAT outgoing enabled. {variables.prodnameWindows} honors the above setting but it is only applied at pod creation time. If the IP pool configuration is updated after a pod is created, the pod's traffic will continue to be NATted (or not) as before. NAT policy for newly-networked pods will honor the new configuration. {variables.prodnameWindows} automatically adds the host itself and its subnet to the NAT exclusion list. This behaviour can be disabled by setting flag `windows_disable_host_subnet_nat_exclusion` to `true` in `cni.conf.template` before running the install script.                                                                                                                                                                                         |
| Service IP advertisement | This {variables.prodname} feature is not supported on Windows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |

### Check your network configuration

If you are using a networking type that requires layer 2 reachability (such as {variables.prodname} with a BGP mesh and no peering to your fabric), you can check that your network has layer 2 reachability as follows:
...
## {variables.prodname} VXLAN networking limitations

Because of differences between the Linux and Windows dataplane feature sets, the following {variables.prodname} features are not supported on Windows.
...
If you create a Windows host with a cloud provider (AWS for example), the creation of the vSwitch at {variables.prodname} install time can remove the cloud provider's metadata route. If your application relies on the metadata service, you may need to examine the routing table before and after installing {variables.prodname} to reinstate any lost routes.
...
- Delete the {variables.prodname} HNS network:

  <CodeBlock language='powershell'>
  Import-Module -DisableNameChecking \{variables.rootDirWindows}\libs\hns\hns.psm1
  Get-HNSNetwork | ? Name -EQ "\{variables.prodname}" | Remove-HNSNetwork
  </CodeBlock>
...
Under certain conditions, relatively simple {variables.prodname} policies can require significant Windows dataplane resources, that can cause significant CPU and memory usage, and large policy programming latency.
...
- [Install {variables.prodnameWindows} using operator](operator.mdx)
