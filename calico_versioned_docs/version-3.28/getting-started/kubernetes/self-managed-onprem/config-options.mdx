...
- Installs the <code>{variables.nodecontainer}</code> container on each host using a DaemonSet.
...
| etcd_ca        | The file containing the root certificate of the CA that issued the etcd server certificate. Configures <code>{variables.nodecontainer}</code>, the CNI plugin, and the Kubernetes controllers to trust the signature on the certificates provided by the etcd server.  | None                  |
| etcd_key       | The file containing the private key of the <code>{variables.nodecontainer}</code>, the CNI plugin, and the Kubernetes controllers client certificate. Enables these components to participate in mutual TLS authentication and identify themselves to the etcd server. | None                  |
| etcd_cert      | The file containing the client certificate issued to <code>{variables.nodecontainer}</code>, the CNI plugin, and the Kubernetes controllers. Enables these components to participate in mutual TLS authentication and identify themselves to the etcd server.          | None                  |
...
Refer to the
<Link to={variables.manifestsUrl + '/manifests/alp/istio-inject-configmap-1.4.2.yaml'}>Calico ConfigMap manifest</Link> for an
