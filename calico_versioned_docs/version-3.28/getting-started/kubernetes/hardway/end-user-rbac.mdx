...
cover roles for using {variables.prodname}. General RBAC for a production Kubernetes cluster is beyond the scope of this lab.
...
A network admin is a person responsible for configuring and operating the {variables.prodname} network as a whole. As such, they
will need access to all {variables.prodname} custom resources, as well as some associated Kubernetes resources.
...
network policy for their service, but don't need to view or modify any global configuration related to {variables.prodname}.
