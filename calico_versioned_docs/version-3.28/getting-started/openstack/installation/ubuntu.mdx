...
1.  Configure APT to use the {variables.prodname} PPA:

    <CodeBlock language='bash'>
    add-apt-repository ppa:project-calico/{variables.ppa_repo_name}
    </CodeBlock>
...
1.  Run `apt-get upgrade` and `apt-get dist-upgrade`. These commands
    bring in {variables.prodname}-specific updates to the OpenStack packages and
    to `dnsmasq`.
...
1.  Install the {variables.prodname} DHCP agent (which uses etcd, allowing
