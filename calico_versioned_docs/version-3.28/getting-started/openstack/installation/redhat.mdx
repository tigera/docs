...
import OpenStackEtcdAuth from '@site/calico_versioned_docs/version-3.28/_includes/content/_openstack-etcd-auth.mdx';

These instructions will take you through a first-time install of
{variables.prodname}. If you are upgrading an existing system, please see
[Upgrading {variables.prodname} on OpenStack](../../../operations/upgrading/openstack-upgrade.mdx)
instead.

There are two sections to the install: adding {variables.prodname} to OpenStack
control nodes, and adding {variables.prodname} to OpenStack compute nodes. Follow
...
Some steps need to be taken on all machines being installed with {variables.prodname}.
...
1.  Configure the {variables.prodname} repository:

    ```bash
    cat > /etc/yum.repos.d/calico.repo <<EOF
    [calico]
    name=Calico Repository
    baseurl=https://binaries.projectcalico.org/rpm/{variables.ppa_repo_name}/
    enabled=1
    skip_if_unavailable=0
    gpgcheck=1
    gpgkey=https://binaries.projectcalico.org/rpm/{variables.ppa_repo_name}/key
    priority=97
    EOF
    ```

1.  Install version 1.0.1 of the `etcd3gw` Python package.  This is
    needed by {variables.prodname}'s OpenStack driver and DHCP agent.
...
1.  Delete all configured OpenStack state, in particular any instances,
    ...
    The {variables.prodname} install will fail if incompatible state is
    left around.
...
1.  Stop and disable the Neutron DHCP agent, and install the
    {variables.prodname} DHCP agent (which uses etcd, allowing it to scale to higher
    numbers of hosts).
...
1.  Configure BIRD. By default {variables.prodname} assumes that you will deploy a
