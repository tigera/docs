...
instructions explain how to set up a single or multiple node DevStack/{variables.prodname}
system, and then how to see {variables.prodname} connectivity in action.
...
To prepare a single node DevStack/{variables.prodname} system, that is with all controller
...
- follow the steps above, except for the network and subnet creations, to
  install and set up DevStack with {variables.prodname} on that node.

## Demonstrating {variables.prodname} connectivity

Then, to see {variables.prodname} connectivity in action:

1. Launch instances attached to the 'calico' network.

2. Use `ip route` to observe per-instance routes created by the {variables.prodname} agent.
