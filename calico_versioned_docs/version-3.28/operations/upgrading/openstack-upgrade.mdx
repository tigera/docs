...
## {variables.prodname} package update

This page describes how to upgrade to {variables.version} from {variables.prodname} v3.0 or later. The procedure
...
1. On all nodes, change the location of the {variables.prodname} packages to point to the {variables.version} repo:
...
1. On all compute nodes, update packages:
   ...
   We recommend upgrading the whole distribution as shown here. In case you prefer to upgrade particular packages only, those needed for a {variables.prodname} compute node are the following.
...
1. Use the following command on the compute nodes to confirm that Felix has upgraded to {variables.version}.

   ```
   calico-felix --version
   ```

   It should return `{variables.version}`.
...
1. On all control nodes, update packages:
   ...
   We recommend upgrading the whole distribution as shown here. In case you prefer to upgrade particular packages only, those needed for a {variables.prodname} control node are the following.
