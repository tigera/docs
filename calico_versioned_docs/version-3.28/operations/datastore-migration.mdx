...
Switch your {variables.prodname} datastore from etcdv3 to Kubernetes on a live cluster.
...
[{variables.prodname} Datastore](../getting-started/kubernetes/hardway/the-calico-datastore.mdx#using-kubernetes-as-the-datastore)
documentation.
...
1. Verify that the datastore was properly imported. This can be accomplished by using
   `calicoctl` to query for any {variables.prodname} resources that exist in the etcd
...
1. Configure {variables.prodname} to read from the Kubernetes datastore. Follow the
   directions to install {variables.prodname} with the Kubernetes datastore. The
...
1. Unlock the datastore. This allows the {variables.prodname} resources to affect the cluster again.
   ...
   all of the expected {variables.prodname} resources prior to unlocking the datastore.
...
following steps delete the {variables.prodname} resources imported into the Kubernetes datastore and
...
1. Delete all of the {variables.prodname} CRDs. This will remove all of the data imported into
...
1. Configure {variables.prodname} to read from the etcd datastore. Follow the
   directions to install {variables.prodname} with the etcd datastore. The
   installation instructions contain the relevant version of the
   `calico.yaml` file to apply.
...
1. Unlock the etcd datastore. This allows the {variables.prodname} resources to affect the cluster again.
