...
needed to segment etcd so that Kubernetes and {variables.prodname} can only read and write
within their respected subtrees/prefixes. To configure more compartmentalized
configurations of the {variables.prodname} components, see this addon:
...
You are using Kubernetes and {variables.prodname} that share an etcd datastore and you wish
to ensure that {variables.prodname} and Kubernetes are unable to access each others' etcd
data.
...
## Updating a hosted {variables.prodname} manifest

To deploy {variables.prodname} with the CA and {variables.prodname}-specific certificate/key pair,
use <Link to={variables.manifestsUrl + '/manifests/calico-etcd.yaml'}>this manifest template</Link>
