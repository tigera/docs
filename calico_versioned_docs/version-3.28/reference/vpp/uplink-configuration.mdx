...
specific to interfaces in {variables.prodname} VPP:
...
In the specific case of the AVF driver, the PCI id must belong to a VF that can be created with the `avf.sh` <Link to={`https://github.com/projectcalico/vpp-dataplane/blob/${variables.vppbranch}/test/scripts/utils/avf.sh`}>script</Link>. Different drivers will have different requirements.
