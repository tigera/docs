...
## Why use {variables.prodname}?

The problem {variables.prodname} tries to solve is the networking of workloads (VMs,
...
we think {variables.prodname} is more scalable, simpler, and more flexible. We think
you should look into it if you have more than a handful of nodes on a
single site.

{variables.prodname} also provides a rich network security model that
...
## Does {variables.prodname} work with IPv6?

Yes! {variables.prodname}'s core components support IPv6 out of the box. However,
not all orchestrators that we integrate with support IPv6 yet.

## Why does my container have a route to 169.254.1.1?

In a {variables.prodname} network, each host acts as a gateway router for the
workloads that it hosts. In container deployments, {variables.prodname} uses
169.254.1.1 as the address for the {variables.prodname} router. By using a
link-local address, {variables.prodname} saves precious IP addresses and avoids
...
## Why isn't {variables.prodname} working with a containerized Kubelet?

{variables.prodname} hosted install places the necessary CNI binaries and config on each
...
Without the above volume mounts, the kubelet will not call the {variables.prodname} CNI binaries, and so
{variables.prodname} [workload endpoints](resources/workloadendpoint.mdx) will
not be created, and {variables.prodname} policy will not be enforced.

## How do I view {variables.prodname} CNI logs?

The {variables.prodname} CNI plugin emits logs to stderr, which are then logged out by the kubelet. Where these logs end up
...
key `log_level`. See [Configuring the {variables.prodname} CNI plugins](configure-cni-plugins.mdx) for more information.

CNI plugin logs can also be found in `/var/log/calico/cni`.

## How do I configure the pod IP range?

When using {variables.prodname} IPAM, IP addresses are assigned from [IP Pools](resources/ippool.mdx).
...
However, if you do need to assign a particular address to a pod, {variables.prodname} provides two ways of doing this:

- You can request an IP that is available in {variables.prodname} IPAM using the `cni.projectcalico.org/ipAddrs` annotation.
...
{variables.prodname} tries hard to avoid interfering with any other configuration
on the host. Rather than adding the gateway address to the host side
of each workload interface, {variables.prodname} sets the `proxy_arp` flag on the
...
{variables.prodname} assigns a MAC address itself. Since {variables.prodname} uses
...
connections based on the full set of supported {variables.prodname} policy primitives
including labels, Kubernetes namespaces, CIDRs, and ports.

## I've heard {variables.prodname} uses proxy ARP, doesn't proxy ARP cause a lot of problems?

It can, but not in the way that {variables.prodname} uses it.

In container deployments, {variables.prodname} only uses proxy ARP for resolving the
...
## Is {variables.prodname} compliant with PCI/DSS requirements?

PCI certification applies to the whole end-to-end system, of which
{variables.prodname} would be a part. We understand that most current solutions use
VLANs, but after studying the PCI requirements documents, we believe
that {variables.prodname} does meet those requirements and that nothing in the
...
## How does {variables.prodname} maintain saved state?

State is saved in a few places in a {variables.prodname} deployment, depending on
...
The only other state storage in a {variables.prodname} network is in the BGP sessions,
...
This makes the {variables.prodname} design very simple, because we store very little
...
## I heard {variables.prodname} is suggesting layer 2: I thought you were layer 3! What's happening?

It's important to distinguish what {variables.prodname} provides to the workloads
hosted in a data center (a purely layer 3 network) with what the {variables.prodname}
project _recommends_ operators use to build their underlying network
fabric.

{variables.prodname}'s core principle is that _applications_ and _workloads_
...
too. Here, {variables.prodname} has discussed how both a layer 2 (see
[here](architecture/design/l2-interconnect-fabric.mdx))
or a layer 3 (see
[here](architecture/design/l3-interconnect-fabric.mdx))
fabric
could be integrated with {variables.prodname}. This is one of the great strengths of
the {variables.prodname} model: it allows the infrastructure to be decoupled from what
...
route scale, does not mean that {variables.prodname} is "going back to Ethernet" or
...
{variables.prodname} provides an extremely rich security policy model, applying policy at the first and last hop
of the routed traffic within the {variables.prodname} network (the source and
destination compute hosts).

This model is substantially more robust to failure than a centralized
firewall-based model. In particular, the {variables.prodname} approach has no
...
## How does {variables.prodname} interact with the Neutron API?

[{variables.prodname}'s interpretation of Neutron API calls](../networking/openstack/neutron-api.mdx)
goes into extensive detail about how various Neutron API calls translate into
{variables.prodname} actions.
...
If a host's containers are connected to the `docker0` bridge interface, {variables.prodname}
...
## Can {variables.prodname} containers use any IP address within a pool, even subnet network/broadcast addresses?

Yes! {variables.prodname} is fully routed, so all IP address within a {variables.prodname} pool are usable as
...
## How do I get network traffic into and out of my {variables.prodname} cluster?

The recommended way to get traffic to/from your {variables.prodname} network is by peering to
...
in [{variables.prodname} over IP fabrics](architecture/design/l3-interconnect-fabric.mdx).
...
Alternatively you can use {variables.prodname}'s built in outbound NAT capability by enabling it on any
{variables.prodname} IP pool. In this case {variables.prodname} will perform outbound NAT locally on the compute
...
to configure {variables.prodname} to peer with the data center's existing L3 routers.
...
### Can I run {variables.prodname} in a public cloud environment?

Yes. If you are running in a public cloud that doesn't allow either L3 peering or L2 connectivity between {variables.prodname} hosts then you can enable IP-in-IP in your {variables.prodname} IP pool:
...
{variables.prodname} will then route traffic between {variables.prodname} hosts using IP-in-IP.
