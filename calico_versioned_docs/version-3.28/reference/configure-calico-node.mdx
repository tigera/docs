...
The <code>{variables.nodecontainer}</code> container is deployed to every node (on Kubernetes, by a DaemonSet), and runs three internal daemons:
...
For manifest-based installations, <code>{variables.nodecontainer}</code> is primarily configured through environment
variables, typically set in the deployment manifest. Individual nodes may also be updated through the Node
custom resource. <code>{variables.nodecontainer}</code> can also be configured through the Calico Operator.
...
<code>{variables.nodecontainer}</code> does not need to be configured directly when installed by the operator. For a complete operator
...
<code>{variables.nodecontainer}</code> can be configured to create a default IP pool for you, but only if none already
...
| NO_DEFAULT_POOLS                   | Prevents {variables.prodname} from creating a default pool if one does not exist. [Default: `false`]                                                                                                                | boolean                                                                      |
...
<code>{variables.nodecontainer}</code> also exposes some options to allow setting certain fields on these objects, as described
below.
...
When {variables.prodname} is configured to use the Kubernetes API as the datastore, the environments
...
| ETCD_KEY_FILE      | Path to the file containing the private key matching the <code>{variables.nodecontainer}</code> client certificate. Enables <code>{variables.nodecontainer}</code> to participate in mutual TLS authentication and identify itself to the etcd server. Example: `/etc/node/key.pem` (optional)                                                                                                 | string |
| ETCD_CERT_FILE     | Path to the file containing the client certificate issued to <code>{variables.nodecontainer}</code>. Enables <code>{variables.nodecontainer}</code> to participate in mutual TLS authentication and identify itself to the etcd server. Example: `/etc/node/cert.pem` (optional)                                                                                                               | string |
| ETCD_CA_CERT_FILE  | Path to the file containing the root certificate of the certificate authority (CA) that issued the etcd server certificate. Configures <code>{variables.nodecontainer}</code> to trust the CA that signed the root certificate. The file may contain multiple root certificates, causing <code>{variables.nodecontainer}</code> to trust each of the CAs included. Example: `/etc/node/ca.pem` | string |
...
| CALICO_STARTUP_LOGLEVEL     | The log severity above which startup <code>{variables.nodecontainer}</code> logs are sent to the stdout. [Default: `ERROR`] | DEBUG, INFO, WARNING, ERROR, CRITICAL, or NONE (case-insensitive) |

### Configuring CNI Plugin

<code>{variables.nodecontainer}</code> has a few options that are configurable based on the CNI plugin and CNI plugin
...
The <code>{variables.nodecontainer}</code> must know the name of the node on which it is running. The node name is used to
...
When launched, the <code>{variables.nodecontainer}</code> container sets the node name according to the following order of precedence:
...
{variables.nodecontainer} will use "host-a" for its name and will write the value in `/var/lib/calico/nodename`. If {variables.nodecontainer}
...
When {variables.prodname} is used for routing, each node must be configured with an IPv4
address and/or an IPv6 address that will be used to route between
nodes. To eliminate node specific IP address configuration, the <code>{variables.nodecontainer}</code>
