...
requires running {variables.prodname} as a Neutron core plugin (i.e. `core_plugin = calico`) instead of as an ML2 mechanism driver.
...
1. Create a floating IP and associate it with the target VM.
   ...
   Then the {variables.prodname} agents will arrange that the floating IP is routed to the
