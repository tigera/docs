...
If you are wondering about the performance differences between iptables and ipvs, the answers are definitely not straightforward. For a comparison between iptables (including {variables.prodname}’s own use of iptables) and ipvs modes, see [Comparing kube-proxy modes: iptables or IPVS?](https://www.projectcalico.org/comparing-kube-proxy-modes-iptables-or-ipvs/).

### IPVS mode and NodePort ranges

Kube-proxy IPVS mode supports NodePort services and cluster IPs. {variables.prodname} also uses NodePorts for routing traffic to the cluster, including the same default Kubernetes NodePort range (30000:32767). If you change your default NodePort range in Kubernetes, you must also change it on {variables.prodname} to maintain ipvs coverage.

### iptables: when to change mark bits

To police traffic in IPVS mode, {variables.prodname} uses additional iptables mark bits to store an ID for each local {variables.prodname} endpoint. If you are planning to run more than 1,022 pods per host with IPVS enabled, you may need to adjust the mark bit size using the `IptablesMarkMask` parameter in {variables.prodname} [FelixConfiguration](../../reference/felix/configuration.mdx#ipvs-bits).

### {variables.prodname} auto detects ipvs mode

When {variables.prodname} detects that kube-proxy is running in IPVS mode (during or after installation), IPVS support is automatically activated. Detection happens when calico-node starts up, so if you change kube-proxy's mode in a running cluster, you will need to restart your calico-node instances.
...
As previously discussed, there is nothing you need to do in {variables.prodname} to use IPVS mode; if enabled, the mode is automatically detected. However, if your default Kubernetes NodePort range changes, use the following instructions to update {variables.prodname} nodeport ranges to stay in sync. Detection happens when calico-node starts up, so if you change kube-proxy's mode in a running cluster, you will need to restart your calico-node instances.

### Change {variables.prodname} default nodeport range

In the FelixConfiguration resource, change the configuration parameter for the default node port range (`KubeNodePortRange`,) in {variables.prodname} to match your new default range in Kubernetes. For help, see [FelixConfiguration](../../reference/felix/configuration.mdx).
