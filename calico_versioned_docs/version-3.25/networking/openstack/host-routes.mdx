...
## With {variables.prodname}, a host route's next hop IP should be the local host

networking-calico supports host routes, but it's important to note that a host
route is only consistent with {variables.prodname} when its next hop IP represents the local
hypervisor. This is because the local hypervisor, in a {variables.prodname} setup, _always_
...
## When are host routes useful with {variables.prodname}?

Host routes are useful with {variables.prodname} when an instance has multiple NICs and you
