...
When running {variables.prodname} with OpenStack, you also need to configure various
OpenStack components, as follows.

## Nova (/etc/nova/nova.conf)

{variables.prodname} uses the Nova metadata service to provide metadata to VMs,
...
- `/etc/nova/nova.conf` must not set `service_neutron_metadata_proxy`
  ...
  correct for a {variables.prodname} cluster.)
...
| -------------------- | --------- | ---------------------------------------- |
| core_plugin          | calico    | Use the {variables.prodname} core plugin         |
| -------------------- | --------- | ---------------------------------------- |

{variables.prodname} can operate either as a core plugin or as an ML2 mechanism driver. The
...
the {variables.prodname} plugin/driver and DHCP agent connect to the {variables.prodname} etcd
...
| Setting              | Value       | Meaning                           |
| -------------------- | ----------- | --------------------------------- |
| mechanism_drivers    | calico      | Use {variables.prodname}                  |
