...
When {variables.prodname} represents an OpenStack VM as a {variables.prodname} WorkloadEndpoint,
...
configure {variables.prodname} policy that is additional to the policy defined by OpenStack
...
For the VM's {variables.prodname} namespace, the label is:
...
To allow {variables.prodname} to provide the project name and parent ID labels,
...
the Keystone database. This is because {variables.prodname}'s driver runs as part of the
Neutron server, and needs to query the Keystone database for the information for those
labels. If Neutron isn't sufficiently privileged, {variables.prodname} will fall back to
...
{variables.prodname} only allows certain characters in label names and values
...
{variables.prodname} does not support changing project name or security group
...
Now you can configure {variables.prodname} operator policy that will apply before the policy
...
{variables.prodname} will enforce any policy with a specified `order` field (as
