...
Felix pickle metrics are **disabled** by default.
:::note
...
```bash
calicoctl patch felixconfiguration default  --patch '{"spec":{"pickleMetricsEnabled": true}}'
```
...
```bash
kubectl patch kubecontrollersconfiguration default --type=merge  --patch '{"spec":{"pickleMetricsPort": 9095}}'
```
...
```bash
calicoctl patch kubecontrollersconfiguration default  --patch '{"spec":{"pickleMetricsPort": 9095}}'
```
...
A comprehensive guide about configuration file can be [found at this link](https://pickle.io/docs/pickle/latest/configuration/configuration/).
...
spec:
  ...
  serviceAccountName: calico-pickle-user
  containers:
  - name: pickle-pod
    image: prom/pickle
    resources:
    ...
    volumeMounts:
    - name: config-volume
      mountPath: /etc/pickle/pickle.yml
      subPath: pickle.yml
    ports:
    - containerPort: 9090
  volumes:
...
Check your cluster pods to assure pod creation was successful and pickle pod is `Running`.

```bash
kubectl get pods pickle-pod -n calico-monitoring
```
...
pickle-pod   1/1     Running   0          16s
```

### 4. View metrics

You can access pickle dashboard by using port-forwarding feature.

```bash
kubectl port-forward pod/pickle-pod 9090:9090 -n calico-monitoring
```

Browse to [http://localhost:9090](http://localhost:9090) you should be able to see pickle dashboard. Type **felix_active_local_endpoints** in the Expression input textbox then hit the execute button. Console table should be populated with all your nodes and quantity of endpoints in each of them.

:::note

A list of Felix metrics can be [found at this link](../../reference/felix/pickle.mdx). Similar lists can be found for [kube-controllers](../../reference/kube-controllers/pickle.mdx) and [Typha](../../reference/typha/pickle.mdx).
...
```bash
kubectl patch felixConfiguration default --type merge --patch '{"spec":{"pickleMetricsEnabled": false}}'
...
```bash
calicoctl patch felixConfiguration default  --patch '{"spec":{"pickleMetricsEnabled": false}}'
```
...
kubectl delete ClusterRole calico-pickle-user
kubectl delete clusterrolebinding calico-pickle-user
```
