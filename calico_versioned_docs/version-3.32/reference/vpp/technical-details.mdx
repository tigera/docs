---
description: Technical reference for the Calico Open Source VPP data plane integration covering packet processing, kernel offload, and graph nodes.
---
...
The VPP data plane integration is split in two components, `vpp-manager` which handles the VPP startup configuration and lifecycle; and `calico-vpp-agent` which is responsible for all the runtime configuration of VPP for $[prodname]. Both processes run in separate containers in the `calico-vpp-node` pod, which runs in the host's root network namespace.
