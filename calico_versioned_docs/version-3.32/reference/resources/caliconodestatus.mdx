---
description: Reference for the CalicoNodeStatus resource in Calico Open Source that exposes per-node agent, BGP, and routing state.
---
...
Status of BGP agents, BGP sessions, and routes exposed to BGP agents is collected from Linux nodes only. **Windows nodes are not supported at this time.**
