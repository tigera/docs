...
The `{variables.nodecontainer}` container is deployed to every node (on Kubernetes, by a DaemonSet), and runs three internal daemons:
...
For manifest-based installations, `{variables.nodecontainer}` is primarily configured through environment
variables, typically set in the deployment manifest. Individual nodes may also be updated through the Node
custom resource. `{variables.nodecontainer}` can also be configured through the Calico Operator.
...
`{variables.nodecontainer}` does not need to be configured directly when installed by the operator. For a complete operator
...
`{variables.nodecontainer}` can be configured to create a default IP pool for you, but only if none already
...
`{variables.nodecontainer}` also exposes some options to allow setting certain fields on these objects, as described
below.
...
| ETCD_KEY_FILE      | Path to the file containing the private key matching the `{variables.nodecontainer}` client certificate. Enables `{variables.nodecontainer}` to participate in mutual TLS authentication and identify itself to the etcd server. Example: `/etc/node/key.pem` (optional)                                                                                                 | string |
| ETCD_CERT_FILE     | Path to the file containing the client certificate issued to `{variables.nodecontainer}`. Enables `{variables.nodecontainer}` to participate in mutual TLS authentication and identify itself to the etcd server. Example: `/etc/node/cert.pem` (optional)                                                                                                               | string |
| ETCD_CA_CERT_FILE  | Path to the file containing the root certificate of the certificate authority (CA) that issued the etcd server certificate. Configures `{variables.nodecontainer}` to trust the CA that signed the root certificate. The file may contain multiple root certificates, causing `{variables.nodecontainer}` to trust each of the CAs included. Example: `/etc/node/ca.pem` | string |
...
| CALICO_STARTUP_LOGLEVEL     | The log severity above which startup `{variables.nodecontainer}` logs are sent to the stdout. [Default: `ERROR`] | DEBUG, INFO, WARNING, ERROR, CRITICAL, or NONE (case-insensitive) |

### Configuring CNI Plugin

`{variables.nodecontainer}` has a few options that are configurable based on the CNI plugin and CNI plugin
...
The `{variables.nodecontainer}` must know the name of the node on which it is running. The node name is used to
...
When launched, the `{variables.nodecontainer}` container sets the node name according to the following order of precedence:
...
nodes. To eliminate node specific IP address configuration, the `{variables.nodecontainer}`
