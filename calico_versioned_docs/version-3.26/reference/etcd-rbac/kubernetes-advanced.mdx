...
access of individual {variables.prodname} components or to limit calicoctl user access.
It assumes you have followed [this guide](kubernetes.mdx) for initial etcd
RBAC configuration of {variables.prodname} and Kubernetes.

## Why you might be interested in this guide

You want to limit access on a per-{variables.prodname} component level or create limited
...
- [{variables.prodname} Kubernetes controllers](calico-etcdv3-paths.mdx#calicokube-controllers)
- [{variables.nodecontainer}](calico-etcdv3-paths.mdx#caliconode)
...
- [{variables.prodname} Kubernetes controllers](../kube-controllers/configuration.mdx)
- [{variables.nodecontainer}](../configure-calico-node.mdx)
- [calicoctl](../../operations/calicoctl/install.mdx)

Below are examples and suggestions when using a hosted {variables.prodname} install where
the {variables.prodname} components are launched through a Kubernetes manifest file, this
...
hosted manifest. This setup requires a certificate for each different {variables.prodname}
component type listed above (cni-plugin, {variables.prodname} Kubernetes controllers, and
<code>{variables.nodecontainer}</code>).

This setup needs similar updates to the manifest like what is described in
[Using etcd RBAC to segment Kubernetes and {variables.prodname}: Updating a hosted {variables.prodname} manifest](kubernetes.mdx#updating-a-hosted-Calico-manifest),
...
- Installing and starting the {variables.prodname} components with a configuration management
