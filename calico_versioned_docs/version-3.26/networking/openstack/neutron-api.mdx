...
When running in an OpenStack deployment, {variables.prodname} receives and interprets
certain Neutron API actions, to program those actions down into
the network. However, because {variables.prodname} is substantially simpler than much
of what Neutron generally allows (see [IP addressing and connectivity](connectivity.mdx)) and because it's a purely layer 3 model (see [The {variables.prodname} data path](../../reference/architecture/data-path.mdx), not all Neutron API calls will have the same effect as they would with other backends.
...
In {variables.prodname}, because all traffic is L3 and routed, the role of Neutron
network as L2 connectivity domain is not helpful. Therefore, in {variables.prodname},
Neutron networks are simply containers for subnets. Best practices for
operators configuring Neutron networks in {variables.prodname} deployments can be
...
Neutron networks. Although {variables.prodname} will allow non-administrator tenants
...
Network creation events on the API are no-op events in {variables.prodname}: a
positive (2XX) response will be sent but no programming will actually
occur.

## Extended Attributes: Provider Networks

Neutron Provider networks are not used in {variables.prodname} deployments. Setting
...
Best practices for configuring Neutron subnets in {variables.prodname} deployments can
be found in [Set up OpenStack](connectivity.mdx#opens-external-conn-setup).

In {variables.prodname}, these roles for the Neutron subnet are preserved in their
...
in a {variables.prodname} deployment: instead, a port is a connection from a VM to the
shared layer 3 network that {variables.prodname} builds in Neutron.
...
: This is unused in {variables.prodname}.

`binding:vnic_type`

: This field, if used, **must** be set to `normal`. If set to any
other value, {variables.prodname} will not correctly function!
...
In {variables.prodname}, security groups have all the same function. Additionally,
they serve to provide the connectivity-limiting function that in vanilla
OpenStack is provided by Neutron networks.

All the attributes of security groups remain unchanged in {variables.prodname}.
...
Load Balancer as a Service (LBaaS) does not function in a {variables.prodname} network. Any
attempt to create one will fail.

:::note

It is possible that in a future version of {variables.prodname} LBaaS may be
...
purpose in a {variables.prodname} network. See [Layer 3 Routing](#routers) for more.
...
{variables.prodname} network. See [Layer 3 Routing](#routers) for more.
