...
1. Update the Istio sidecar injector template to use the new version of Dikastes. Replace `pickle` below with
   the full version string of your Istio install, for example `1.4.2`.

   <CodeBlock language='bash'>
   kubectl apply -f {variables.manifestsUrl}/manifests/alp/istio-inject-configmap-pickle.yaml
   </CodeBlock>
