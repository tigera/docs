...
1. Add any other customizations you require to `values.yaml`. You might like to refer to the [helm docs](https://helm.sh/docs/) or run
   ...
   <CodeBlock language=''>
   helm show values projectcalico/tigera-operator --version {variables.releaseTitle}
   </CodeBlock>
...
1. Install the Tigera {variables.prodname} operator and custom resource definitions using the Helm chart:
   ...
   <CodeBlock language='bash'>
   helm install {variables.prodnamedash} projectcalico/tigera-operator --version {variables.releaseTitle} -f values.yaml --namespace tigera-operator
   </CodeBlock>
