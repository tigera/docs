...
Kind cluster creation can be modified by [configuration file](https://kind.sigs.k8s.io/docs/user/quick-start/#creating-a-cluster). Here you are providing kind configuration in order to disable simple networking implementation (default CNI) and change the pod ip CIDR.
...
Start your Kind cluster with one control plane and two worker nodes using the following command.
