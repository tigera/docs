...
{variables.prodname} policy integrates with [Istio](https://istio.io) to allow you to write policies that enforce against
...
{variables.prodname} uses a Container Storage Interface (CSI) driver to enable secure connectivity between Felix and the Dikastes container
...
only enforce standard {variables.prodname} network policy.
