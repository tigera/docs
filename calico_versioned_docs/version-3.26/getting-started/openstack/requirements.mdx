---
description: Requirements for installing Calico on OpenStack nodes.
---

import variables from '@site/calico_versioned_docs/version-3.26/variables';

import ReqsSys from "@site/calico_versioned_docs/version-3.26/_includes/components/ReqsSys";
import ReqsKernel from "@site/calico_versioned_docs/version-3.26/_includes/components/ReqsKernel";

# System requirements

<ReqsSys orch='OpenStack' />

## OpenStack requirements

The Calico Neutron driver is written in Python 3 and so requires an OpenStack release that
runs with Python 3. Subject to that, we aim to develop and maintain the Neutron driver
for {variables.prodname} (networking-calico) so that its master code works with OpenStack
master or any previous Python 3 release, on any operating system, independently of the
deployment mechanism that is used to install it.

However, we recommend using OpenStack Yoga or later, and our active support and testing
of {variables.prodname} {variables.version} with OpenStack is with Yoga.

<ReqsKernel />
