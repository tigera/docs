...
the new IP addresses. This limitation can be circumvented using {variables.prodname} [Use external IPs or networks rules in policy](network-policy/policy-rules/external-ips-policy.mdx), or Calico Enterprise's support for domain names in policy rules..
