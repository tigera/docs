...
function ApplyPolicyScript(props) {
  if (props.prodname === 'Calico') {
    return <CodeBlock language='bash'>calicoctl apply -f - {'<'} allow-all-upgrade.yaml</CodeBlock>;
  }
...
export default function HostEndpointsUpgrade(props) {
  return (
    <>
      ...
      <p>
        In versions of {props.prodname} prior to <PriorToVersion prodname={props.prodname} />, all-interfaces host
        endpoints (host endpoints with <code>interfaceName: *</code>) only supported pre-DNAT policy. The default
        behavior of all-interfaces host endpoints, in the absence of any policy, was to allow all traffic.
      </p>
      <p>
        Beginning from <PriorToVersion prodname={props.prodname} />, all-interfaces host endpoints support normal policy
        in addition to pre-DNAT policy. The support for normal policy includes a change in default behavior for
        all-interfaces host endpoints: in the absence of policy the default behavior is to <strong>drop traffic</strong>
        . This default behavior is consistent with &quot;named&quot; host endpoints (which specify a named interface
        such as &quot;eth0&quot;); named host endpoints drop traffic in the absence of policy.
      </p>
      <p>
        {/*
        ...
        */}
        Before upgrading, you must ensure that global network policies are in place that select existing all-interfaces
        host endpoints and explicitly allow existing traffic flows. As a starting point, you can create an allow-all
        policy that selects existing all-interfaces host endpoints. First, we&#39;ll add a label to the existing host
        endpoints. Get a list of the nodes that have an all-interfaces host endpoint:
      </p>
