...
function SubList(props) {
  ...
  return (
    <ul>
      <li key={idx++}>The “base” modules (including the IPv6 versions if IPv6 is enabled in your cluster).</li>
      <li key={idx++}>
      ...
      <li key={idx++}>
        At least the following actions: <code>REJECT</code>,<code>ACCEPT</code>, <code>DROP</code>, <code>LOG</code>.
      </li>
    </ul>
  );
}
...
function MainList(props) {
  ...
  return (
    <ul>
      ...
      <li key={idx++}>
        <p>
          IPIP, VXLAN, Wireguard support, if using {props.prodname}
  ...
  return (
    <>
      ...
      <p>
        ...
        of the particular kernel modules that are required to run {props.prodname}. However, in general, you’ll need:
      </p>
      <MainList {...props} />
    </>
  );
