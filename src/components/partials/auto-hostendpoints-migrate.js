...
export default function AutoHostendpointsMigrate(props) {
  return (
    <>
      ...
      <p>In order to migrate existing all-interfaces host endpoints to {props.prodname}-managed auto host endpoints:</p>
      <ol>
        <li>
          <p>
            Add any labels on existing all-interfaces host endpoints to their corresponding {props.orch} nodes.{' '}
            {props.prodname} manages labels on automatic host endpoints by syncing labels from their nodes. Any labels
            on existing all-interfaces host endpoints should be added to their respective nodes. For example, if your
        ...
        <li>
          <p>
            ...
            <Link
              href={`/docs/${toKebab(props.prodname)}/security/hosts/kubernetes-nodes#enable-automatic-host-endpoints`}
            >
        ...
        <li>
          <p>
            Delete old all-interfaces host endpoints. You can distinguish host endpoints managed by {props.prodname}{' '}
            from others in several ways. First, automatic host endpoints have the label{' '}
