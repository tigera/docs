...
function NodeRequirementsEnt(props) {
    return (
        <>
            <h2 id="node-requirements">Node requirements</h2>
            <ul>
                <li><p>x86-64 processor with at least 2 cores, 8.0GB RAM and 20 GB free disk space</p></li>
                <li><p>Linux kernel 3.10 or later with <Link href="#kernel-dependencies">required dependencies</Link>.
                    The following
                    distributions have the required kernel, its dependencies, and are known
                    to work well with {props.prodname} and {props.orch}.</p>
                ...
                <li><p>{props.prodname} must be able to manage <code>cali*</code>
                    interfaces on the host. When IPIP is enabled (the default),
                    {props.prodname} also needs to be able to manage <code>tunl*</code>
                    interfaces. When VXLAN is enabled, {props.prodname} also needs to be
                    able to manage the <code>vxlan.calico</code> interface.</p>
                    <Admonition type="note">
                        <p>Many Linux distributions, such as most of the
                            above, include NetworkManager. By default, NetworkManager does not allow
                            {props.prodname} to manage interfaces. If your nodes have
                            NetworkManager, complete the steps in <Link
                                href={"/docs/maintenance/troubleshoot/troubleshooting#configure-networkmanager"}>Preventing
                                NetworkManager from controlling {props.prodname} interfaces</Link> before
                            installing {props.prodname}.</p>
                    </Admonition>
                </li>
                <li><p>If your Linux distribution comes with installed Firewalld or
                    another iptables manager it should be disabled. These may interfere with
                    rules added by {props.prodname} and result in unexpected behavior.</p>
                    <Admonition type="note">
                        <p>If a host firewall is needed, it can be
                            configured by {props.prodname} HostEndpoint and GlobalNetworkPolicy.
                            More information about configuration at <Link
                                href={"/docs/security/hosts"}>Security for
                                host</Link>.</p>
                    </Admonition>
                </li>
                <li><p>In order to properly run Elasticsearch, nodes must be configured
                    according to the <Link
                        href="https://www.elastic.co/guide/en/elasticsearch/reference/current/system-config.html">Elasticsearch
                        system configuration documentation.</Link></p></li>
                <li><p>The Typha autoscaler requires a minimum number of Linux worker
                    nodes based on total number of schedulable nodes.</p>
                    <table>
                        <thead>
                        <tr className="header">
            ...
            <div className="note">
                <Admonition type="note">
                    <p>
                        ...
                        GlobalNetworkPolicy. More information about configuration at <Link href={"/docs/security/hosts"}>Security
                        for host</Link>.
                    </p>
                </Admonition>
            </div>
...
function NetworkRequirementsEnt(props) {
    return (
        <>
            <h2 id="network-requirements">Network requirements</h2>
            <p>Ensure that your hosts and firewalls allow the necessary traffic
                based on your configuration. See <Link
                    href={"/docs/calico-enterprise/reference/architecture/overview"}>Component architecture</Link> to view the following components.</p>
            <table>
                <colgroup>
                    <col style={{width: '27%'}}/>
                    <col style={{width: '46%'}}/>
                ...
                <tbody>
                    <tr className="odd">
                        <td><strong>{props.prodname} networking options</strong></td>
                    ...
                    <tr className="even">
                        <td><strong>Cluster scaling</strong></td>
                        <td>Any {props.prodname} networking option above with Typha agents
                            enabled
                        </td>
                        <td>TCP 5473 (default)</td>
                    </tr>
                    ...
                    <tr className="even">
                        <td><strong>Component metrics</strong></td>
                    ...
                    <tr className="even">
                        <td><strong>Logs and storage</strong></td>
                    ...
                    <tr className="odd">
                        <td><strong>Visibility and troubleshooting</strong></td>
                        <td>Kibana</td>
                    ...
                    <tr className="even">
                        <td><strong>Multi-cluster management</strong></td>
            ...
            {props.orch === orchestrators.Kubernetes || props.orch === orchestrators.OpenShift &&
                <>
                    <p>
                        * <em>The value passed to kube-apiserver using the <code>--secure-port</code>
            ...
            {props.orch === orchestrators.OpenStack &&
                <p>
                    * <em>If your compute hosts connect directly and don’t use IP-in-IP, you don’t need to allow
                    IP-in-IP traffic.</em>
                </p>
            }
        </>
            ...
            {props.orch === orchestrators.Kubernetes &&
                <>
                    <p>When installed as a Kubernetes daemon set, {props.prodname} meets this requirement by running
                        ...
                        containers. There are two ways this can be achieved.</p>
                    <ul>
                        <li>
                            Specify <code>--allow-privileged</code> on the kubelet (deprecated).
                        </li>
                        <li>
                            Use a <Link href="https://kubernetes.io/docs/concepts/policy/pod-security-policy/">
                            pod security policy</Link>.
                        </li>
                    </ul>
                </>
            }
...
export default function ReqsSys(props) {
    const prod = props.prodname === openSource.prodname
        ? openSource
    ...
    return (
        <>
            {props.prodname === openSource.prodname &&
                <>
                    <NodeRequirementsOSS {...prod} {...props}/>
                    <NotesOSS {...prod} {...props}/>
                </>
            }

            {(props.prodname === enterprise.prodname || props.prodname === cloud.prodname) &&
                <NodeRequirementsEnt {...prod} {...props}/>
            }

            <KeyValueStore {...prod} {...props}/>

            {props.prodname === openSource.prodname &&
                <NetworkRequirementsOSS {...prod} {...props}/>
            }

            {(props.prodname === enterprise.prodname || props.prodname === cloud.prodname) &&
                <NetworkRequirementsEnt {...prod} {...props}/>
            }

            <Privileges {...prod} {...props}/>
        </>
    )
}
