...
import EbpfValue from '@site/calico-cloud_versioned_docs/version-18-2/_includes/content/_ebpf-value.mdx';
...
eBPF supports AKS with Calico CNI and {variables.prodname} network policy. However, with AKS with Azure CNI and {variables.prodname} network policy, kube-proxy cannot be disabled so the performance benefits of eBPF are lost. However, there are other reasons to use eBPF other than performance gains, as described in [eBPF use cases](use-cases-ebpf.mdx).
<!--TODO-XREFS-CC -->
