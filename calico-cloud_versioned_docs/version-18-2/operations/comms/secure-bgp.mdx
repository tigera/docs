...
including passwords. Therefore, for this {variables.prodname} feature, we use Secrets to
store BGP passwords.
...
1.  Create (or update) a Kubernetes secret in the namespace where {variables.noderunning} is
...
1.  Ensure that {variables.noderunning} has RBAC permissions to access that secret.
...
If {variables.noderunning} in your cluster is running in a namespace other than calico-system,
...
The {variables.noderunning} pod must have permission to access that secret. To allow
{variables.noderunning} to access that secret, you would configure:
...
Node to node mesh must be enabled in order to set node to node mesh
BGP password.
...
peers](../../networking/configuring/bgp.mdx).
