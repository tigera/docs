...
WAF is deployed in your cluster along with Envoy DaemonSet. {variables.prodname} proxies selected service traffic through Envoy, checking HTTP requests using the industry-standard 
...
By default, {variables.prodname} ships with CoreRuleset v3.3.5 with the following setup files pre-loaded:
...
| On     | Denies HTTP traffic. {variables.prodname}  will log the event in Security Events. | Yes          |
