---
description: Calico Cloud lets you combine AWS security groups with network policy to enforce access control between Kubernetes pods and AWS VPC resources.
---

import variables from '@site/calico-cloud_versioned_docs/version-18-2/variables';

# Configure Calico Cloud AWS security groups integration

## Big picture

Configure {variables.prodname} AWS security groups integration.

## Value

AWS security group integration for {variables.prodname} allows you to combine AWS security groups with network policy to enforce granular access control between Kubernetes pods and AWS VPC resources.
...
- Helm install
- Multiple VPCs
- {variables.prodname} non-cluster hosts, and auto-hostendpoint features. Alternatives to secure host endpoints:
  ...
  - Use {variables.prodname} [GlobalNetworkSets](../../../reference/resources/globalnetworkset.mdx) and use custom labels for selection in policy.
...
- You have installed [{variables.prodname} for EKS](/getting-started/install-on-clusters/eks/)
on your cluster, or kubeadm/kops.

Verify {variables.prodname} has been installed by confirming that all tigerastatuses are available:
...
- You have installed {variables.prodname} configured to work with the AWS CNI Plugin. This is done by following the installation guide for EKS on your cluster or configuring the Installation resource during installation and setting the CNI.Type to `AmazonVPC`.
...
- You are not using the {variables.prodname} [auto hostendpoints feature](../../hosts/kubernetes-nodes.mdx), and have not created any [host endpoints](../../../reference/resources/hostendpoint.mdx) that have a `spec.node` value that matches any of your Kubernetes nodes.

  Verify that there are no entries for {variables.prodname} host endpoints using this command:
...
Commands above apply only to EKS clusters with unmanaged nodegroups i.e. [eksctl without --managed](https://eksctl.io/usage/eks-managed-nodes/) option.
...
Because `KOPS_CLUSTER_NAMES` are FQDNs, you must to choose a `CLUSTER_NAME` that does not contain any dot separators for use in the remainder of this guide. See [before you begin](#before-you-begin) for more information.
...
1.  Install AWS per-account resources.
    ...
    --template-body {variables.filesUrl_CE}/manifests/aws/security-group-integration/account-cf.yaml
...
1.  Install AWS per-VPC resources.
    ...
    --template-body {variables.filesUrl_CE}/manifests/aws/security-group-integration/vpc-cf.yaml
...
1.  Install AWS per-cluster resources.
    ...
    --template-body {variables.filesUrl_CE}/manifests/aws/security-group-integration/cluster-cf.yaml
