...
The {variables.prodname} AWS security group integration allows you to manage AWS security groups as you normally do, but adds the ability to apply Security Groups to individual pods.
...
All AWS security groups and their enforcement rules are mapped into equivalent {variables.prodname} network policy in non-editable integration tiers (105-107). The integration tiers only pass traffic, and do not alter security groups. Although you cannot put tiers before them, you can add regular network policy after tier 107.

All pod-to-pod traffic is allowed by default.

The integration supports using both **Kubernetes** and **{variables.prodname} tiered network policy**.
...
We recommend restricting access to integration tiers to ensure they are not modified as they are essential to operations. When you log in to {variables.prodname} Manager, the following integration tiers are displayed.
...
- Although you cannot hide specific tiers from non-admin users, you can use [RBAC for tiered policies](../../policy-tiers/rbac-tiered-policies.mdx), to display only a subset of tiers in the {variables.prodname} UI. For help, see [displaying only the net-sec tier](../../policy-tiers/rbac-tiered-policies.mdx#user-can-only-view-a-specific-tier).
...
Host endpoints are created for all RDS and EC2 instances with labels for the security groups they belong to, as part of normal representation of your {variables.prodname} network policy within the network.
