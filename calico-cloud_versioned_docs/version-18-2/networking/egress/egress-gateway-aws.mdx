...
{variables.prodname} controls the source IP by directing traffic through one or more "egress gateway" pods, which
...
using {variables.prodname} IPAM. {variables.prodname} IPAM allows the IP addresses to be precisely controlled, this allows
for static configuration of external appliances. Using an IP pool backed by a VPC subnet allows {variables.prodname} to
...
- {variables.prodname} CNI and IPAM. The ability to control the egress gateway's IP is a feature of
  {variables.prodname} CNI and IPAM. AWS VPC CNI does not support that feature, so it is incompatible with
  egress gateways.
...
- Felix, {variables.prodname}'s per-host agent.
...
{variables.prodname} supports IP pools that are backed by the AWS fabric. Workloads that use an IP address from an
...
- An IP pool is created with its `awsSubnetID` field set to the ID of a VPC subnet. This "AWS-backed" IP pool's
  ...
  For example, by creating a dedicated VPC subnet for {variables.prodname}. If the CIDR is not reserved; both
  {variables.prodname} and AWS may try to assign the same IP address, resulting in a conflict.

  :::

- Since they are a limited resource, {variables.prodname} IPAM does not use AWS-backed pools by default. To request an
  ...
  {variables.prodname} manages the `projectcalico.org/aws-secondary-ipv4` capacity on the Kubernetes Node resource,
...
- When Felix, {variables.prodname}'s per-host agent spots a local workload with an AWS-backed address it tries to ensure
...
To arrange for AWS to route traffic to and from egress gateways, {variables.prodname} adds _secondary_ Elastic
Network Interfaces (ENIs) to the host. {variables.prodname} supports two modes for provisioning the
...
- Amazon VPC CNI
  ...
  {variables.prodname} CNI and IPAM is required. The ability to control the egress gateway’s IP is a feature of {variables.prodname} CNI and IPAM. AWS VPC CNI does not support that feature, so it is incompatible with egress gateways.
...
Since this feature adds additional network interfaces to nodes, it is important to configure {variables.prodname} to
autodetect the correct primary interface to use for normal pod-to-pod traffic. Otherwise, {variables.prodname} may
...
({variables.prodname} ensures that the secondary ENIs do not have default routes in the main routing table.)

For other AWS clusters, {variables.prodname} may default to `firstFound`, which is **not** suitable.
...
To avoid clashes between AWS IP allocations and {variables.prodname} IP allocations, it is important that the range of
IP addresses assigned to {variables.prodname} IP pools is not used by AWS for automatic allocations. In this guide we
assume that you have created a dedicated VPC subnet per Availability Zone (AZ) that is reserved for {variables.prodname}
...
{variables.prodname} requires a dedicated VPC subnet in each AWS availability zone that you wish to deploy egress
gateways. The subnet must be dedicated to {variables.prodname} so that AWS will not
...
Some IP addresses from the dedicated subnet are reserved for AWS and {variables.prodname} internal use:
...
- _In **Secondary-IP-per-workload** mode_, {variables.prodname} requires one IP address from the subnet per secondary ENI
...
{variables.prodname} allocates ENIs on-demand so each instance will only claim one of those reserved IP addresses when the
...
In order to provision the required AWS resources, each {variables.noderunning} pod in your cluster requires the
...
of resources). In addition, to support elastic IPs, each {variables.noderunning} also requires the following permissions:
...
In order to allow egress gateway traffic into the egress gateway pod's host from the client, the ingress rules of the security
...
prevent {variables.prodname} from _trying_ to use them. For each VPC subnet that you plan to use,
...
- `allowedUse` set to `["Workload"]` to tell {variables.prodname} IPAM to use those pools for the egress gateway workloads.
...
- `blockSize` set to 32. This aligns {variables.prodname} IPAM with the behaviour of the AWS fabric.
- `disableBGPExport` set to `true`. This prevents routing conflicts if your cluster is using IPIP or BGP networking.

It's also recommended to:

- Set `nodeSelector` to `"!all()"`. This prevents {variables.prodname} IPAM from using the pool automatically. It will
...
- One medium-sized IP pool per-Subnet reserved for {variables.prodname} to use for the _primary_ IP addresses of its _secondary_ ENIs.
...
Using the example of the "red" egress gateway cluster, we use several features of Kubernetes and {variables.prodname}
...
- ipPools tells {variables.prodname} IPAM to use one of the "red" IP pools:
  ...
  - name: "egress-red-west-2"
  ```
  Depending on which AZ the pod is scheduled in, {variables.prodname} IPAM will automatically ignore IP pools that
...
Once the update has rolled out, {variables.prodname} will try to add one of the requested Elastic IPs to each pod in
the deployment.
