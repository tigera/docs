...
In this article you will learn how to create {variables.prodname} global network policies to isolate namespaces for ingress traffic.
...
Microsegmentation across namespaces is achieved using {variables.prodname} policy labels and selectors with a **Pass** action rule. If you are unfamiliar with policy action rules, see [Tutorial - Understanding policy tiers](../../network-policy/policy-tiers/tiered-policy.mdx).
