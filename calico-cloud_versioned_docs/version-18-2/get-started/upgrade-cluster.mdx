...
You are required to upgrade your cluster to the latest {{prodname}} version before the cluster reaches two minor releases older than the current release. For example, if the latest {{prodname}} version is based on 3.19, managed clusters with v3.16 or older will not be covered for fixes under the [Support policy](/get-started/suppport-policy).
