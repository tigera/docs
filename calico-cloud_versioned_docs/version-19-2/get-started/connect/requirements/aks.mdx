...
If the AKS cluster already has Calico installed and is managed by AKS it may still be compatible with {variables.prodname}.
If output from the following command includes "EnsureExists" then the install is compatible with {variables.prodname}.
...
is not supported. You will need to reach out to support to create a plan to uninstall {variables.prodname}.

:::

## Remove taints from Linux node pools

Hybrid clusters on AKS may have taints applied to Linux nodes that prevent {variables.prodname} from scheduling pods on those nodes.
These taints must be removed before you connect your cluster to {variables.prodname}.
