...
This guide is a variation of the [simple policy demo](../../tutorials/kubernetes-tutorials/kubernetes-policy-basic.mdx) intended to introduce the extra features of {variables.prodname} to people already familiar with Project Calico for Kubernetes.

It requires a Kubernetes cluster configured with Calico networking and {variables.prodname}, and expects that you have `kubectl` configured to interact with the cluster.
...
The key steps in moving to {variables.prodname} are to change to the {variables.prodname} version of calico-node, update its configuration, download [calicoq](/reference/clis/calicoq/index) and deploy Prometheus.
*/}

This guide assumes that you have installed all the {variables.prodname} components from the
...
1. Inspect the network policies using calicoq. The `host` command displays
   ...
   dynamic aspects of {variables.prodname} Policy: in particular displaying the endpoints actually affected by policies,
   ...
   {variables.prodname} Policies and Kubernetes NetworkPolicies, although this
   ...
   - The first two policies are defined in the monitor-calico.yaml manifest.
     ...
     the {variables.prodname} format (note the addition of the namespace test).
...
1. Use kubectl to see the detail of any particular policy or profile. For
   ...
   Alternatively, you may also use {variables.prodname} Manager to inspect and view information and metrics associated with policies, endpoints, and nodes.

## Enable isolation

Let's turn on isolation in our policy-demo namespace. {variables.prodname} will then prevent connections to pods in this namespace.
...
## {variables.prodname} Metrics

Now would be a great time to take a look at the metrics.

In {variables.prodname} Manager, head to the dashboard view. You will see graphs associated with allowed packets/bytes and denied packets/bytes. The graphs represent the rates at which packets/bytes are being allowed or denied and are time windowed.
...
The `NetworkPolicy` spec for [`default-deny`](#enable-isolation) does not come configured
with any rules. This policy results in a 'default deny' because of how it is [evaluated](../../reference/resources/tier.mdx#how-policy-is-evaluated). A packet will be dropped if the policies (like default-deny in this case) affecting the
endpoint takes no action.
You can review the metrics associated with such behavior in the **Implicit Drops** block.
...
1. Create a network policy `access-nginx` with the following contents:
   ...
   The NetworkPolicy allows traffic from pods with the label `run: access`
...
1. You can clean up the demo by deleting the demo namespace.
   ...
   This was just a simple example of the Kubernetes NetworkPolicy API and how {variables.prodname} can secure your Kubernetes cluster. For more
