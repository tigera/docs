...
**Network policy** is the primary tool for securing a Kubernetes network. It lets you restrict network traffic in your cluster so only the traffic that you want to flow is allowed. {variables.prodname} provides more robust policy than Kubernetes, but you can use them together -- seamlessly. {variables.prodname} supports:

- {variables.prodname} network policy, (namespaced)
- {variables.prodname} global network policy (non-namespaced, global)
...
All {variables.prodname} and Kubernetes network policies reside in tiers. You can start "thinking in tiers" by grouping your teams and types of policies within each group. For example, we recommend these three tiers (platform, security, and application).

![policy types](/img/calico-cloud/policy-types.svg)

Next, you can determine the priority of policies in tiers (from top to bottom). In the following example, that platform and security tiers use {variables.prodname} global network policies that apply to all pods, while developer teams can safely manage pods within namespaces using Kubernetes network policy for their applications and microservices.
...
- Network and global network policies are placed when you upgrade from Project Calico to {variables.prodname}
...
- **allow-tigera** tier contains policies to secure {variables.prodname} components and are controlled by the Tigera Operator. These policies should not be edited, and the tier should not be moved. Inadvertent changes are automatically reverted by the Operator to ensure your cluster is always protected.

:::warning

Although it is possible to change the behavior of the `allow-tigera` using adjacent tiers, it is not a trivial task. You can break critical cluster traffic and impact the operation of {variables.prodname}. To prevent loss of cluster services, see [Change allow-tigera tier behavior](allow-tigera.mdx), and contact Support for help.
...
Instead of IP addresses and IP ranges, network policies in Kubernetes depend on labels and selectors to determine which workloads can talk to each other. Workload identity is the same for Kubernetes and {variables.prodname} network policies: as pods dynamically come and go, network policy is enforced based on the labels and selectors that you define.
...
- The **CNI**, **{variables.prodname} components**, and underlying **dataplane** (iptables/eBPF) all make use of labels and selectors as part of routing traffic.

![tier funnel](/img/calico-cloud/tier-funnel.svg)

### Policy action rules

{variables.prodname} network policy uses action rules to specify how to process traffic/packets:
...
- **Known endpoints** - {variables.prodname} resources that are managed by Felix
...
| ---------------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------ |
| Workload, {variables.prodname} | Deny                                                                    | Deny                                                               |
