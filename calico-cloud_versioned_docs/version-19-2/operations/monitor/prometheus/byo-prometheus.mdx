...
Scrape {variables.prodname} metrics for Bring Your Own (BYO) Prometheus.

## Value

{variables.prodname} uses the Prometheus monitoring tool to scrape metrics from instrumented jobs, and displays time-series data in a visualizer such as Grafana. You can scrape the following time-series metrics for {variables.prodname} components to your own Prometheus:
...
kubectl apply -f \{variables.filesUrl_CE}/manifests/prometheus/elasticsearch-metrics-service-monitor.yaml -n $NAMESPACE
</CodeBlock>
...
<CodeBlock language='bash'>
kubectl apply -f  \{variables.filesUrl_CE}/manifests/prometheus/fluentd-metrics-service-monitor.yaml -n $NAMESPACE
</CodeBlock>
...
<CodeBlock language='bash'>
kubectl apply -f  \{variables.filesUrl_CE}/manifests/prometheus/calico-node-monitor-service-monitor.yaml -n $NAMESPACE
</CodeBlock>
...
<CodeBlock language='bash'>
kubectl apply -f  \{variables.filesUrl_CE}/manifests/prometheus/kube-controller-metrics-service-monitor.yaml -n $NAMESPACE
</CodeBlock>
...
[See the FelixConfiguration reference for more details](../../../reference/resources/felixconfig.mdx). You can also add a Windows firewall rule that allows listening on the Prometheus port(s) instead of having {variables.prodname} manage it.
...
<CodeBlock language='bash'>
kubectl apply -f  \{variables.filesUrl_CE}/manifests/prometheus/felix-metrics-service-monitor.yaml -n $NAMESPACE
</CodeBlock>
...
By default, Typha uses **port 9091** TCP to publish metrics. However, if {variables.prodname} is installed using the Amazon yaml file, this port will be 9093 because it is set manually using the **TYPHA_PROMETHEUSMETRICSPORT** environment variable.
...
<CodeBlock language='bash'>
kubectl apply -f  \{variables.filesUrl_CE}/manifests/prometheus/typha-metrics-service-monitor.yaml -n $NAMESPACE
</CodeBlock>
...
To support zero trust, we recommend that you create {variables.prodname} network policy to allow the traffic between BYO Prometheus pods, and the respective metrics pods. For samples of ingress and egress policies, see [Get started with Calico network policy](../../../network-policy/beginners/calico-network-policy.mdx).
