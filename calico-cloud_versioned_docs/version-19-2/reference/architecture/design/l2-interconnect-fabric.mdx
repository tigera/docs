...
will discuss some of the various interconnect fabric options in a {variables.prodname}
network.

Any technology that is capable of transporting IP packets can be used as
the interconnect fabric in a {variables.prodname} network (the first person to test
and publish the results of using [IP over Avian Carrier](https://datatracker.ietf.org/doc/html/rfc1149)
 as a transport for {variables.prodname}
will earn a very nice dinner on or with the core {variables.prodname} team). This
means that the standard tools used to transport IP, such as MPLS and
Ethernet can be used in a {variables.prodname} network.
...
infrastructure will work for {variables.prodname} as well. However, the concerns that
...
to be viably considered as a {variables.prodname} interconnect, even in large-scale
deployments.
...
First, let's look at how {variables.prodname} uses an Ethernet interconnect fabric.
...
those ISPs' customers' nodes. We leverage the same effect in {variables.prodname}.

To take the issues outlined above, let's revisit them in a {variables.prodname}
context.

1.  Large numbers of end points. In a {variables.prodname} network, the Ethernet
...
2.  High rate of _churn_. In a classical Ethernet data center fabric,
    ...
    hundreds or thousands of events per second. In a {variables.prodname} network,
    however, the _churn_ is very low. The only event that would lead to
    _churn_ in a {variables.prodname} network's Ethernet fabric would be the addition
...
3.  High volume of broadcast traffic. Since the first (and last) hop for
    any traffic in a {variables.prodname} network is an IP hop, and IP hops terminate
...
connection fabric in {variables.prodname} is not only possible, it is practical and
should be seriously considered as the interconnect fabric for a {variables.prodname}
network.

As mentioned in the IP fabric post, an IP fabric is also quite feasible
for {variables.prodname}, but there are more considerations that must be taken into
...
As mentioned elsewhere in the {variables.prodname} documentation, since {variables.prodname} can use
most of the standard IP tooling, some interesting options regarding
fabric topology become possible.

We assume that an Ethernet fabric for {variables.prodname} would most likely be
...
Since {variables.prodname} is an IP routed fabric, a {variables.prodname} network can use
...
{variables.prodname} compute servers, it is possible to build the fabric out of
multiple _independent_ leaf/spine planes using no technologies other
than IP routing in the {variables.prodname} nodes, and basic Ethernet switching in the
...
may be interesting to venture over to Facebook's [blog post](https://code.facebook.com/posts/360346274145943/introducing-data-center-fabric-the-next-generation-facebook-data-center-network/)
...
problem that would occur if the route reflector were "behind" the {variables.prodname}
network.
...
[^1]:
    In this document (and in all {variables.prodname} documents) we tend to use the
    ...
    to a {variables.prodname} network. If we are referring to a specific type of end
