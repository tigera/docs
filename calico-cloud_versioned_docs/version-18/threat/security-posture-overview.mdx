...
Because the dashboard is based on existing {{prodname}} data, no configuration is required. You can start improving the risk of your Kubernetes cluster from day one.
...
Risk calculators run in MongoDB where all data is encrypted using unique keys per tenant. Each API call is authenticated and authorized, and data is retained for 90 days.
