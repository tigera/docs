...
You can include the Image Assurance CLI scanner in your CI/CD pipelines (for example, Jenkins, Github actions). Follow these steps to authenticate your scanning process with {{prodname}} Image Assurance in Manager UI:

1. Download the Image Assurance CLI scanner binary onto your CI runner.
1. If you are running an ephemeral environment in the pipeline, include the download and update the executable steps in your pipeline to download the scanner on every execution.
1. Using the API token and API URL (from Manager UI, Image Assurance, All Scanned Images, Scan Settings), create a secret and make it available in the pipeline (for example, using a <secure_api_token> environment variable)
1. Add the Image Assurance CLI scanner in your pipeline using the following command:

   **Command**: `./tigera-scanner scan --apiurl <api_url> --token <api_token> <image_name>:<tag>`

   **Where**:
   - <api_url> - path to uploading information to the Image Assurance system.
   - <api_token> - key-value pair (variable type=secret) that is set in your pipeline
   - <image_name> - container image name

   **Example**: `./tigera-scanner scan --apiurl``xxxx://qq8psbdn-management.calicocloud.io/eng` `--token $(SECURE_API_TOKEN) my-image-repo:$(tag)`
