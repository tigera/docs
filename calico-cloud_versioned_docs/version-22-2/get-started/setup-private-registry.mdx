...
:::tip Helm chart version is independent of $[prodname] version

The $[prodname] Helm chart uses its own versioning scheme (v1.x) that is independent of the $[prodname] product version shown in the UI.
You do not need to match a specific chart version to a specific $[prodname] release.
Always use the latest Helm chart version — it automatically selects the correct installer image for whichever $[prodname] version you are installing or upgrading to.
