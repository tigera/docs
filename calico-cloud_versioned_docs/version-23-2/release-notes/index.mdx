...
<h2 id="23.0.2">August 27, 2026 (version 23.0.2)</h2>

### Bug fixes

* Fixed an issue where a customer-defined default-deny network policy that covers the `calico-system` namespace could block traffic from the API server to Guardian, causing the policy board in the web console to return an error.
  $[prodname] now includes a network policy that allows this traffic.
* Fixed an issue where changes to how Azure Kubernetes Service (AKS) manages Calico could cause the installer to fail or, on existing clusters, cause the operator to stop managing resources.
  $[prodname] now deploys the additional resources needed for normal operation to continue and installs to proceed.

<h2 id="22.5.1">August 27, 2026 (version 22.5.1)</h2>
...
* ***Breaking change:*** The `allow-tigera` tier is renamed to `calico-system`.
  ...
  Because a policy name is prefixed with the name of its tier, the policies in the tier are renamed with it, from `allow-tigera.<policy>` to `calico-system.<policy>`.
  Before you upgrade, review anything that refers to the tier or to those policy names, including:
  * Network policies that you added to the `allow-tigera` tier.
  * RBAC rules that name `allow-tigera` or `allow-tigera.*` in `resourceNames`.
  * Manifests, scripts, and automation that reference policies by their `allow-tigera.` names.
  * Dashboards, alerts, and queries that filter on `tier="allow-tigera"` or on policy names.
