...
:::note
`installer.calicoCloudVersion` also selects the operator image, and an operator rejects an `Installer` that asks for a different version. To change version, generate a new command rather than editing this value.
:::
