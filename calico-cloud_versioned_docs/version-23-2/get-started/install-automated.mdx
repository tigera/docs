...
:::important
The operator image is pulled at the tag named by `installer.calicoCloudVersion`, so [mirror that version's operator image](setup-private-registry.mdx) first. With `installer.imagePath` unset, it is pulled from `calico-cloud-public` rather than `tigera`.
:::
