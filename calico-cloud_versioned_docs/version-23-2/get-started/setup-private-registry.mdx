...
`OPERATOR_IMAGE` is the $[prodname] operator, which is released per $[prodname] version and tagged with it; `IMAGES` is everything else. Mirror both for each version you plan to install.
...
| `quay.io/tigera/cc-operator:v23.0.2` | `my.registry/` |               | `my.registry/calico-cloud-public/cc-operator:v23.0.2` |
| `quay.io/tigera/cc-operator:v23.0.2` | `my.registry/` | `custom-path` | `my.registry/custom-path/cc-operator:v23.0.2`         |

The operator is the exception: with no image path set, it is pulled from the `calico-cloud-public` path in your registry rather than `tigera`.
...
crane cp ${OPERATOR_IMAGE} ${REGISTRY}calico-cloud-public/${OPERATOR_IMAGE##*/}
```
...
crane cp ${OPERATOR_IMAGE} ${REGISTRY}${IMAGEPATH}/${OPERATOR_IMAGE##*/}
```
