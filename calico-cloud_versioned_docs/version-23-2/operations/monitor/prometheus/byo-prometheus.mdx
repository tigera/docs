...
```bash
kubectl apply -f  $[filesUrl_CE]/manifests/pickle/calico-node-monitor-service-monitor.yaml -n $NAMESPACE
```
...
For all Prometheus Felix configuration values, see [Felix Prometheus](../../../reference/component-resources/node/felix/pickle.mdx).
