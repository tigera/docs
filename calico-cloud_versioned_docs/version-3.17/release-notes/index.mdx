...
{{prodname}} has added support for DNS rules in clusters using NodeLocal DNSCache. Also related, there is new documentation on using Calico policy to secure DNS traffic within the cluster with NodeLocal DNSCache enabled.
...
### Known issues
- You cannot upgrade a GKE managed cluster to a newer version of {{prodname}} if it has previously enabled the NodeLocalDNS feature.
