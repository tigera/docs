...
**Error: rendered manifests contain a resource that already exists. Unable to continue with install: CustomResourceDefinition "installers.operator.calicocloud.io" in namespace "" exists**

(Helm installations only) The `installers.operator.calicocloud.io` CRD was originally installed as an unmanaged CRD by helm. Run the below command to apply the appropriate labels & annotations, and then rerun the installation command.
```bash
