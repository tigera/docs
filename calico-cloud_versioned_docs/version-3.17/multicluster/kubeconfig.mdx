...
- [Install {variables.prodname}](/getting-started/install-on-clusters/kubernetes/index)
...
1. If RBAC is enabled, apply this manifest.
   ...
   ```bash
   kubectl apply -f \
   {variables.filesUrl_CE}/manifests/federation-rem-rbac-kdd.yaml
   ```
...
1. Apply the following manifest to create a service account called `tigera-federation-remote-cluster`.
   ...
   ```bash
   kubectl apply -f \
   {variables.filesUrl_CE}/manifests/federation-remote-sa.yaml
   ```
