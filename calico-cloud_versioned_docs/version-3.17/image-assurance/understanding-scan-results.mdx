...
Scan results for Image Assurance is based on the Common Vulnerability Scoring System v3 (CVSS Scores). The default Pass, Warn, and Fail values will work for the majority of {variables.prodname} deployments. However, you may need to change the defaults because of security requirements.
...
This tab shows CVEs associated with running pods in your cluster so you can assess pod vulnerability and remediate. For example, you can see image exposure by viewing the  **Clusters** and **Running Instances** columns that show the number of running instances in clusters that are connected to {variables.prodname}.
...
If this field is blank, it means that {variables.prodname} cannot access the metadata. For example, images from Docker Hub do not specify the registry in the image metadata.
