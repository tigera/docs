...
Honeypods can optionally be monitored using a {variables.prodname} controller that periodically polls selected honeypods for suspicious activity and scans its traffic. Alerts are generated in the Events tab of {variables.prodname} Manager UI.
...
kubectl apply -f {variables.filesUrl_CE}/manifests/threatdef/honeypod/controller.yaml
</CodeBlock>

For OpenShift deployments, the controller requires privileged access:

<CodeBlock language='bash'>
kubectl apply -f {variables.filesUrl_CE}/manifests/threatdef/honeypod/controller_os.yaml
</CodeBlock>

### Add custom Snort rules

You can add custom Snort rules to the controller using a ConfigMap. By default, {variables.prodname} uses the [Snort Community Ruleset](https://www.snort.org/downloads/#rule-downloads).
