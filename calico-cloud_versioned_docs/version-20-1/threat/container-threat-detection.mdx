...
Detectors are grouped by [MITRE tactics](https://attack.mitre.org/tactics/enterprise/) they protect against. Additional information for each detector can be found by clicking on a detector. 
...
Get the reference `detectorsConfig`:
    ...
    ```
    export detectorConfig="$(kubectl get runtimesecurities.operator.tigera.io default -o jsonpath="{.spec.detectorsConfig}")"
    ```

Alternatively, can enable Container Threat Detection with the required configuration in one command:
    ```
...
If duplicate detector IDs are present in the RuntimeSecurity CR. The last entry will take precedence.
