...
The cluster you are using to install {variables.prodname} does not have a CNI plugin installed, the CNI is incompatible. If your cluster has functional pod networking and you see this message, it is likely that kubelet has been configured to use kubenet networking, which is not compatible with {variables.prodname}. You can use a different cluster, or re-create your cluster with [compatible networking](system-requirements.mdx).
