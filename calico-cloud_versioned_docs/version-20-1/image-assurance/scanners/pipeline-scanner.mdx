...
1. Download the latest version of the scanner.
   ...
   **Linux**
   ...
   {`curl -Lo tigera-scanner ${variables.clouddownloadbase}/tigera-scanner/{variables.cloudversion}/image-assurance-scanner-cli-linux-amd64`}
   </CodeBlock>

   **macOS**

   <CodeBlock language='shell'>
   {`curl -Lo tigera-scanner ${variables.clouddownloadbase}/tigera-scanner/{variables.cloudversion}/image-assurance-scanner-cli-darwin-amd64`}
   </CodeBlock>
...
3. Verify that the scanner works correctly by running the version command.
   ...
   ${variables.imageassuranceversion}`}
   </CodeBlock>
### Integrate the scanner into your build pipeline
...
You can scan images and report results back to {variables.prodname}, or scan images locally without reporting results to {variables.prodname}.
...
- `--apiurl` - {variables.prodname} API URL path. You can get this URL in Manager UI, **Image Assurance**, **Scan settings**.
- `--token` - secure API or authorization token to make requests to {variables.prodname} API URL. You can get this URL in Manager UI, **Image Assurance**, **Scan settings**.
...
| ----------------------- | --------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| --apiurl                | -a        | CC_API_URL               | {variables.prodname} API URL path. You can get this URL in Manager UI, Image Assurance, Scan settings.                              |
| --token                 | -t        | CC_TOKEN                 | Secure API or authorization token to make requests to {variables.prodname} API URL.                                                 |
