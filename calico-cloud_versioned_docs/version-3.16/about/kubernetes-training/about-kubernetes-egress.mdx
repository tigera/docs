...
the new IP addresses. This limitation can be circumvented using {{prodname}} [Use external IPs or networks rules in policy](network-policy/beginners/policy-rules/external-ips-policy), or Calico Enterprise's support for domain names in policy rules..
...
- [Use external IPs or networks rules in policy](network-policy/beginners/policy-rules/external-ips-policy)
