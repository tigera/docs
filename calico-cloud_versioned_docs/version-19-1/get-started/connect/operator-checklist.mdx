...
The cluster you are using to install {variables.prodname} does not have a CNI plugin installed, the CNI is incompatible. If your cluster has functional pod networking and you see this message, it is likely that kubelet has been configured to use kubenet networking, which is not compatible with {variables.prodname}. You can use a different cluster, or re-create your cluster with [compatible networking](requirements/system-requirements.mdx).

### {variables.prodname} cannot be connected to a cluster with FIPS mode enabled

At this time, FIPS mode is not supported in {variables.prodname}. Disable FIPS mode in the cluster and install again.
...
Installing {variables.prodname} on your Kubernetes cluster is managed by the Tigera operator. The Tigera operator is deployed as a ReplicaSet in the `tigera-operator` namespace, and records status in a custom resource named, `tigerastatus`. The operator get its configuration from several Custom Resources (CRs); the central one is the Installation CR.
...
If all components show a status of "Available" = TRUE, {variables.prodname} is properly installed.
...
**Issue: {variables.prodname} is not installed**

If {variables.prodname} is not installed, you'll get the following error. Install {variables.prodname} on the node using the `curl` command that you got from Support.
...
**Issue: {variables.prodname} components are missing or are degraded**

If some of the {variables.prodname} components are Available = FALSE or DEGRADED = TRUE, run the following command and contact Support with the following output.
...
In the following example, the typha component has an issue because it is showing `AVAILABLE: FALSE`, and `DEGRADED: TRUE`. To understand details of {variables.prodname} components, see [Deep dive into custom resources](#deep-dive-into-custom-resources).
...
`log-collector` collects flow and other logs and forwards them to {variables.prodname}. Check the pods and logs in the `tigera-fluentd` namespace. You should have one pod running on each of your nodes.
...
The `management-cluster-connection` is required for your managed clusters to connect to the {variables.prodname} backend. If it is not available or degraded, check the pods and logs in the `tigera-guardian` namespace.
...
The high-level components {variables.prodname} needs to run are:
...
Search for the term `PodSecurityPolicy` in the status message of failed cluster deployments. If a PSP is present, install open source Calico in the cluster before you connect to {variables.prodname}.
