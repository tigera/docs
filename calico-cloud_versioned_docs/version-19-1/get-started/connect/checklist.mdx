...
## Check {variables.prodname} installation

Installing {variables.prodname} on your Kubernetes cluster is managed by the {variables.prodname} operator. The {variables.prodname} operator is deployed as a Deployment in the `calico-cloud` namespace, and records status in a custom resource named `installer`.
...
After `state` is `complete`, {variables.prodname} is properly installed.

## Check logs for fatal errors

Check that the {variables.prodname} operator is running and that logs do not have any fatal errors.
...
To upload diagnostics about a failed installation to {variables.prodname} support, run the following command:
