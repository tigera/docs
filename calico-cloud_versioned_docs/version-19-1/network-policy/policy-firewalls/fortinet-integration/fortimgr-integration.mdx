...
The {variables.prodname}/Fortinet integration lets you control Kubernetes clusters directly and apply policy
...
This {variables.prodname}/Fortinet solution lets you directly control Kubernetes policies using FortiManager.
...
1. Create firewall policies using the address groups for IPv4 Source address and IPv4 Destination Address, and select services and actions as you normally would to allow or deny the traffic. Under the covers, the {variables.prodname} integration controller periodically reads the FortiManager firewall policies for your Kubernetes cluster, converts them to {variables.prodname} global network policies, and applies them to clusters.
1. Use the {variables.prodname} Manager UI to verify the integration, and then FortiManager UI to make all updates to policy rules.
...
<!--//TODO-CC-XREF
- Pull secret that you used during [{variables.prodname} installation](/getting-started/install-on-clusters/index)
- Login access to [{variables.prodname} Manager UI](/operations/cnx/access-the-manager/)
-->
...
Create a [{variables.prodname} tier](../../policy-tiers/tiered-policy.mdx) in the {variables.prodname} Manager UI for each Kubernetes cluster you want to secure. We recommend that you create a new tier (rather than reusing an existing tier) for all global network policies created by the {variables.prodname} integration controller.
...
1. In this [FortiManager ConfigMap manifest]({variables.filesUrl_CE}/manifests/fortimanager-device-configmap.yaml), add your FortiManager device information in the data section: `tigera.firewall.fortimanager-policies`. For example:
   ...
   | tier                       | Tier name you created in {variables.prodname} Manager UI                                                            |
...
1. Apply the manifest.
   ...
   kubectl apply -f {variables.filesUrl_CE}/manifests/fortimanager-device-configmap.yaml
   </CodeBlock>
...
1. Apply the manifest.
   ...
   <CodeBlock language='bash'>
   kubectl apply -f {variables.filesUrl_CE}/manifests/fortimanager.yaml
   </CodeBlock>
...
5. Log in to the {variables.prodname} Manager UI, and under the tier that you specified in the ConfigMap, verify that the GlobalNetworkPolicies are created.
