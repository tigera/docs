...
```
For details, see [Image Assurance installation reference](../reference/installation/api.mdx#image-assurance.operator.tigera.io/v1.ImageAssuranceSpec).

:::

Other notes:

- The columns, **Clusters** and **Running Instances**, show the number of running instances in clusters that are connected to {variables.prodname}.
...
- In the All Scanned Images and Running Images tabs, the **Registry path** field may be blank if {variables.prodname} cannot access this metadata. For example, images from Docker Hub do not specify the registry in the image metadata.
...
The following default threshold values will work for the majority of {variables.prodname} deployments. However, you may need to change the defaults because of security requirements.
