...
{variables.prodname} provides an end-to-end IP network that interconnects the
...
which {variables.prodname} operates [^2].

While {variables.prodname} is designed to work with any underlying interconnect fabric
...
or will be deployed, and it makes sense for {variables.prodname} to operate in those
environments.

However, since {variables.prodname} is, itself, a routed infrastructure, there are
more engineering, architecture, and operations considerations that have
to be weighed when running {variables.prodname} with an IP routed interconnection
fabric. We will briefly outline those in the rest of this post. That
said, {variables.prodname} operates equally well with Ethernet or IP interconnect
fabrics.
...
<!--TODO-XREFS-CC
A description of the {variables.prodname} architecture can be found in our
...
In a {variables.prodname} network, each compute server acts as a router for all of the
...
plane by a BGP protocol server, and management plane by {variables.prodname}'s
on-server agent, _Felix_.

Each endpoint can only communicate through its local vRouter, and the
first and last _hop_ in any {variables.prodname} packet flow is an IP router hop
...
components in the {variables.prodname} architecture, but they are irrelevant to the
...
the {variables.prodname} model, that function is pushed to the compute server itself.
...
1.  The routing infrastructure is based on some form of IGP. Due to the
 ...
 for discussion of this
    ...
    scale in a {variables.prodname} environment. However, it is possible to use a
    combination of IGP and BGP in the interconnect fabric, where an IGP
    communicates the path to the _next-hop_ router (in {variables.prodname}, this is
...
2.  The other model, and the one that this note concerns itself with, is
    ...
    routers in the network (in a {variables.prodname} network this includes the
...
1.  A BGP fabric where each of the TOR switches (and their subsidiary
    compute servers) are a unique [Autonomous     System (AS)](https:/en.wikipedia.org/wiki/Autonomous_System_(Internet)
>)
...
Ethernet connection _plane_ as in the {variables.prodname} Ethernet interconnect
...
For example, the BGP configuration on a {variables.prodname} compute server is
...
requirements. A default {variables.prodname} network does not venture into those
...
{variables.prodname} network. These BGP design requirements _can_ be worked around, if
...
: In a {variables.prodname} network, each endpoint is a route. Hardware networking
...
Every router within the rack, which, in the case of {variables.prodname} is every
...
aggregating and routing element. In {variables.prodname}, the ToR, if it is an L3
router, is actually the second tier. Remember, in {variables.prodname}, the compute
...
servers in a {variables.prodname} fabric would be limited to the approximately five
...
available. This should be sufficient for any given {variables.prodname} fabric.
...
In the diagram above, we are showing that all {variables.prodname} nodes share the same
...
peer (the {variables.prodname} routers to their ToR, the ToRs to the spine switches).
However, in return, the upstream router only announces a default route.
In this case, a given {variables.prodname} router only has routes for the endpoints
that are locally hosted on it, as well as the default from the ToR.
Since the ToR is the only route for the {variables.prodname} network the rest of the
...
that are for endpoints that are hosted on its downstream {variables.prodname} nodes.
Even if we were to host 200 endpoints per {variables.prodname} node, and stuff 80
{variables.prodname} nodes in each rack, that would still limit the routing table on
...
the {variables.prodname} network routes, just as they do in the routed spines in the
...
the number of routes in the {variables.prodname} nodes, but, as we have discussed
before, that is not a concern in most deployments as the amount of
memory consumed by a full routing table in {variables.prodname} is a fraction of the
...
the {variables.prodname} nodes, then the user should consider the Ethernet fabric as
detailed in [this post](l2-interconnect-fabric.mdx).

If a {variables.prodname} user is interested in the AS per compute server, the Project
...
There may be {variables.prodname} architectures where there are similar models where
the routers within a given AS are not directly adjacent. In those
models, the use of an IGP in {variables.prodname} may be warranted. The configuration
...
model, all of the compute servers (the routers in a {variables.prodname} network) are
directly connected over one or more Ethernet network(s) and therefore
are directly reachable. In this case, a router in the {variables.prodname} network
...
may traverse a non-{variables.prodname} router are eBGP routes, and therefore _next
hop self_ is automatically set correctly. If a deployment of {variables.prodname} in
...
with 1000's or 10,000's of routers (the potential size of a {variables.prodname}
network).
...
(in {variables.prodname}'s case, the compute server), depending on the route reflector
...
The final consideration is the number of endpoints in a {variables.prodname} network.
...
destination IP address in the packet, which, in {variables.prodname}'s case, is the
...
reflector clients in the BGP mesh, just as the {variables.prodname} vRouter/compute
server does.
...
without other routing _tricks_, such as aggregation, a {variables.prodname}
...
[^1]:
    In {variables.prodname}'s terminology, an endpoint is an IP address and
...
[^2]:
    ...
    {variables.prodname} (v)Router (in almost all cases, the compute servers) nodes,
...
[^4]:
    However those tools are available if a given {variables.prodname} instance needs
