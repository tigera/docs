...
| ---------- | ------------------------------------------------------------ |
| Prometheus | Monitoring tool that scrapes metrics from instrumented jobs and displays time series data in a visualizer (such as Grafana). For {variables.prodname}, the “jobs” that Prometheus can harvest metrics from the Fluentd component. |
