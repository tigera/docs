...
1. Grant permission to access the {{prodname}} Manager to users in your cluster. Issue one of the following
   commands, replacing `{{user}}` with the {{type}} you wish to grant access.

   The ClusterRole `tigera-ui-user` grants permission to use the {{prodname}} Manager UI, view flow
   ...
   The ClusterRole `tigera-network-admin` grants permission to use the {{prodname}} Manager UI, view flow
   ...
   To grant access to additional tiers, or create your own roles consult the [RBAC documentation](../../network-policy/policy-tiers/rbac-tiered-policies.mdx).
