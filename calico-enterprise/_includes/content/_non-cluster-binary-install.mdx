...
{% tabs %}
  <label: Startup script,active:true>
  <%
...
%>
  <label: Manually configure Felix>
  <%

Configure Felix by creating a file at `/kubernetes/calico/felix.cfg`. 
See [Felix configuration]({{site.baseurl}}/reference/felix/configuration) for help with environment variables.

>**Note**: Felix tries to detect whether IPv6 is available on your platform but
...
>**Note**: For the Kubernetes datastore, Felix works in policy-only mode. Even though pod networking is
