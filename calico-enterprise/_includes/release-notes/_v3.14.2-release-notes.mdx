...
#### Enhancements
- {{site.prodname}} can now be installed on RKE2 clusters with a CIS Profile enabled.
- The [Compliance resource]({{site.baseurl}}/reference/installation/api#operator.tigera.io/v1.Compliance) is no longer a requirement for other components and can be removed by users who have no need for compliance reports.
- Added [remote cluster status metrics]({{site.baseurl}}/calico/reference/kube-controllers/prometheus) to the `calico-kube-controllers` deployment that lets users monitor the connectivity status for federated clusters.
