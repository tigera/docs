...
| `audit`  | Audit logs for $[prodname] resources                                                          | $[prodname] API server                      | `audit_ee`   | `tigera_secure_ee_audit_ee.*`  |
|          | Audit logs for Kubernetes resources                                                            | Kubernetes API server                        | `audit_kube` | `tigera_secure_ee_audit_kube.*`|
...
Fluent Bit buffers logs on each node's filesystem, under `/var/log/calico/calico-fluent-bit/storage`, so chunks that have not been delivered yet survive a log collector restart. Each destination gets its own buffer with its own cap: 500 MB for flow logs and for each additional destination, and 100 MB for each of the other log types. Account for that disk usage when you size your nodes. Fluent Bit retries a chunk until it is delivered, so a destination that stays unreachable long enough to fill its buffer starts losing data for that destination only; the others keep shipping.
...
### Log ingestion compression

Fluent Bit compresses each batch of logs it sends to the log ingestion endpoint. Set `ingestionCompression` on the LogCollector resource to `Zstd` (the default), `Gzip`, or `None`:

```bash
kubectl patch logcollector tigera-secure --type=merge \
  --patch='{"spec":{"ingestionCompression":"Zstd"}}'
```

The setting applies only to batches in transit. It does not change the compression of the rotated log files on disk, the [additional destinations](archive-storage.mdx), or the OpenTelemetry exporters.
