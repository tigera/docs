...
2. Verify that L7 logs are being collected by looking for output in the default log file location (`/var/log/calico/l7logs/l7.log`) on each node:

   ```bash
   tail -f /var/log/calico/l7logs/l7.log
   ```

   Logs are flushed to the file on the interval set by the [L7LogsFlushInterval](../../../reference/component-resources/node/felix/configuration.mdx#L7LogsFlushInterval) field, so allow one interval to pass before you expect output; the log directory itself is controlled by the [L7LogsFileDirectory](../../../reference/component-resources/node/felix/configuration.mdx#L7LogsFileDirectory) field on `FelixConfiguration`.
