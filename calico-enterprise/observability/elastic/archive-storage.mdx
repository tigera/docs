...
Note: both flow logs from within the cluster and flow logs from [non-cluster hosts](../../getting-started/bare-metal/about) are supported for archival.
...
The [`hostScope`](../../reference/installation/api.mdx#operator.tigera.io/v1.AdditionalLogStoreSpec) field on the LogCollector resource can be used to control which hosts have their logs archived. This field can be used to configure $[prodname] to only archive [non-cluster host](../../getting-started/bare-metal/about) logs.
