...
1. Remove the `ManagementClusterConnection` from your cluster and delete the managed cluster connection secret.
   ...
   Follow the following step to also disconnect your managed cluster from your management cluster. This will not result in any
   data loss. If you reconnect your cluster using the same name, existing data will be available again.

   1. In your management cluster, delete the `ManagedCluster` resource for your managed cluster.
   ```bash
