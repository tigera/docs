...
# Create a Calico Enterprise managed cluster with Helm
...
1. Export the management cluster certificate and managed cluster certificate and key.
  ...
  If you haven't already done so already, generate the base64 encoded CRT and KEY for this managed cluster:

  ```bash
  openssl genrsa 2048 | base64 -w 0 > my-managed-cluster.key.base64
  openssl req -new -key <(base64 -d my-managed-cluster.key.base64) -subj "/CN=tigera-voltron" | \
  ...
  ```bash
  kubectl get secret tigera-pull-secret -n tigera-operator -o jsonpath='{.data.tls\.crt}' | base64 -d > management-cluster.crt.base64
  ```
