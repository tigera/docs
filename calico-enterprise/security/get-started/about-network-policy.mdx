...
![Example mix of network policy types](/img/calico-enterprise/master/example-k8s-calico-policy-mix.svg)
...
[Calico Enterprise](/security/policy-tiers/tiered-policy/) supports hierarchical network policy using policy tiers. RBAC
for each tier can be defined to restrict who can interact with each tier. This can be used to delegate trust across
multiple teams.

![Example tiers](/img/calico-enterprise/master/example-tiers.svg)
