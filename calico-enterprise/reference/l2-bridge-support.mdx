...
| Kernel | Support for the bridge port `locked` flag, which arrived upstream in 5.18 |

The requirement is the capability, not the version number.
$[prodname] locks the bridge ports it creates, and a kernel without that flag cannot run this feature.
It is a higher floor than the eBPF data plane needs on its own.

Distributions that backport the flag satisfy the requirement at a lower reported version.
RHEL 9 and RHCOS are the notable case: they report kernel 5.14 and the feature works, because Red Hat has backported the flag.
To check a node directly, look for `locked on` in `bridge -d link show` for a port on a $[prodname] bridge.
...
- Multus needs the API server's real address on a cluster running the eBPF data plane.
  With `kube-proxy` disabled, which eBPF mode requires, the cluster IP address of the API server does not resolve until the data plane is programmed.
  Multus therefore cannot reach the API server on a freshly booted node, and every pod sandbox on that node fails until it can.
...
A bridge you prepare yourself must meet three requirements, which $[prodname] checks and will not correct for you.
Those are VLAN filtering enabled, an explicitly set MAC address, and the 802.1Q tag protocol rather than 802.1ad.
For the full contract, including what $[prodname] does not require, see the [Network resource](resources/network.mdx).
For the procedure, see [Prepare a bridge on single-uplink nodes](../networking/l2-bridge/byo-bridge.mdx).

## Addressing

IPv4 only.
$[prodname] IPAM is the authority for workload addresses, but the guest does not have to know that.
A VM can still receive its address over DHCP, from KubeVirt's DHCP service inside the pod.
...
A Linux bridge takes the lowest MTU of its ports, so the bridge drops to that value when the first workload attaches.
Any VLAN device above the bridge drops with it.
...
A cluster IP address can be resolved and translated on the way out of the pod's primary interface, then dropped by reverse path filtering.
