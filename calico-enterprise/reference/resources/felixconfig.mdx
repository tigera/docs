---
description: Reference for the FelixConfiguration resource in Calico Enterprise that controls Felix data plane behavior cluster-wide.
...
- The `nodeSelector` field uses the same [selector syntax](../resources/networkpolicy.mdx#selector) as other Calico resources.
...
On Windows, or when using the eBPF data plane, this setting is ignored. Windows always uses `NoDelay` while eBPF has its own [BPFDNSPolicyMode](#bpfdnspolicymode) option.
