...
Policy recommendation scope represents a collection of configuration options used to control the [policy recommendation](/network-policy/recommendations/policy-recommendations/policy-recommendations.mdx) engine.
...
| selector               | Selects the namespaces for which to generate recommendations for. |                 |        | `!(projectcalico.org/name starts with ''tigera-'') && !(projectcalico.org/name starts with ''calico-'') && !(projectcalico.org/name starts with ''kube-'')`    |
