...
While there is no extra setup configuration required by the user to generate a benchmark report for Openshift, the result sets will be different than a report generated for regular Kubernetes clusters. Use the {% include open-new-window.html text='Openshift Container Platform Security Guide' url='https://static.open-scap.org/ssg-guides/ssg-ocp4-guide-index.html' %} to cross-reference the benchmark results.
