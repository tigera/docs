...
SecurityEventWebhooks provide an easy way to add arbitrary data to the webhook generated HTTP payload through the metadata annotation.
...
SecurityEventWebhooks use a domain-specific query language to select which records
...
Data required to be present in the `config` section of the SecurityEventWebhook `spec` depends on the intended consumer for the HTTP
