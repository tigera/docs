...
| hostEndpoint     | Configures the host endpoint controlle                                           |                   | [HostEndpoint](#hostendpoint)     |         |
...
| interfaceCIDRs            | Used to determine which addresses from node.Spec.Addresses, IPv4/IPv6 tunnel addresses and Wireguard addresses will be included in the generated HostEndpoint if they are within the specified list of CIDRs. If no interface matches, the HostEndpoint will not be created | List of valid CIDRs | List string                         |          |
