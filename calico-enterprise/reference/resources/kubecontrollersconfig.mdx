...
| interfaceCIDRs            | Matches Addresses from node.Spec.Addresses, IPv4/IPv6 tunnel addresses and Wireguard addresses on the selected Node objects based on CIDR, if no interfaces matche HostEndpoint will not be created  | List of valid CIDRs | List string                         |          |
