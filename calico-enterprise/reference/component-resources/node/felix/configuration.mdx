...
| `PolicySyncPathPrefix`            | `FELIX_POLICYSYNCPATHPREFIX`            | File system path where Felix notifies services of policy changes over Unix domain sockets. This is required only if you're configuring [L7 logs]({{site.baseurl}}/visibility/elastic/l7/configure), or [egress gateways]({{site.baseurl}}/networking/egress/). Set to `""` to disable. [Default: `""`] | string |
...
| `FlowLogsCollectProcessPath` | `FELIX_FLOWLOGSCOLLECTPROCESSPATH` | `true` | If enabled, along with FlowLogsCollectProcessInfo, each flow log will contain the full path of the process executable and the arguments with which the executable was invoked. If path or arguments cannot be determined, Felix will fallback to using task names and arguments will be empty. For full functionality, this feature should be enabled via operator see [Enabling process path]({{site.baseurl}}/visibility/elastic/flow/processpath)|
