...
The `policy_setup_timeout_seconds` option makes the {variables.prodname} CNI plugin wait to start a new pod until one of the following conditions occurs:
...
The {variables.prodname} CNI plugin uses {variables.prodname}'s `endpoint-status` directory to determine when the dataplane has been programmed for a pod.
By default, the {variables.prodname} CNI plugin will look for the directory at: `/var/run/calico/endpoint-status`.
To enable and adjust where {variables.prodname} writes `endpoint-status`, the `endpointStatusPathPrefix` option must be configured for [Felix](./felix/configuration.mdx).

To adjust where the {variables.prodname} CNI plugin looks for {variables.prodname}'s `endpoint-status` directory, you must set the `endpoint_status_dir` option.
