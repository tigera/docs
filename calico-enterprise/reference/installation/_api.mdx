...
| `readinessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) ReadinessProbe allows customization of the readiness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
| `livenessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) LivenessProbe allows customization of the liveness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
...
| `calicoWebhooksDeployment` _[CalicoWebhooksDeployment](#calicowebhooksdeployment)_ | (Optional) CalicoWebhooksDeployment configures the calico-webhooks Deployment. |
...
| Field | Description |
| --- | --- |
| `replicas` _integer_ | (Optional) Replicas defines the number of Alertmanager replicas. When set to 0, Alertmanager is not rendered.<br />Default: 0 |
...
| `clusterRoutingMode` _[ClusterRoutingMode](#clusterroutingmode)_ | (Optional) ClusterRoutingMode controls which component programs the routes that a node needs in order to reach workloads on other nodes. It only applies to IP Pools with vxlanMode: Never; the routes for IP Pools with vxlanMode: Always or vxlanMode: CrossSubnet are always programmed by Felix. In BIRD mode, confd and BIRD program the routes for both IPIP and unencapsulated IP Pools. In Felix mode, Felix programs the routes for both IPIP and unencapsulated IP Pools. In FelixIPIPOnly mode, Felix programs the routes for IPIP IP Pools, and confd and BIRD program them for unencapsulated IP Pools. If not specified, the operator writes neither FelixConfiguration.programClusterRoutes nor BGPConfiguration.programClusterRoutes, so Calico's own defaults apply; as of Calico v3.33 those defaults are equivalent to FelixIPIPOnly. Note that BIRD programming of IPIP routes, which the BIRD mode selects, is deprecated as of Calico v3.33 and is intended for removal in v3.35. |
...
| `readinessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) ReadinessProbe allows customization of the readiness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
| `livenessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) LivenessProbe allows customization of the liveness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
| `startupProbe` _[ProbeOverride](#probeoverride)_ | (Optional) StartupProbe allows customization of the startup probe timing parameters. The probe handler is set by the operator. |
...
| `resources` _[ResourceRequirements](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#resourcerequirements-v1-core)_ | (Optional) Resources allows customization of limits and requests for compute resources such as cpu and memory. If specified, this overrides the named DaemonSet container's resources. If omitted, the DaemonSet will use its default value for this container's resources. If used in conjunction with the deprecated ComponentResources, then this value takes precedence. |
| `readinessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) ReadinessProbe allows customization of the readiness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
| `livenessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) LivenessProbe allows customization of the liveness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
...
| --- | --- |
| `metadata` _[Metadata](#metadata)_ | (Optional) Refer to Kubernetes API documentation for fields of `metadata`. |
| `spec` _[CalicoWebhooksDeploymentSpec](#calicowebhooksdeploymentspec)_ | (Optional) Spec is the specification of the calico-webhooks Deployment. |
...
_Appears in:_
- [CalicoWebhooksDeploymentPodSpec](#calicowebhooksdeploymentpodspec)

| Field | Description |
| --- | --- |
| `name` _string_ | Name is an enum which identifies the calico-webhooks Deployment container by name.<br />Supported values are: calico-webhooks |
...
_Appears in:_
- [CalicoWebhooksDeploymentContainer](#calicowebhooksdeploymentcontainer)

| Field | Description |
| --- | --- |
| `name` _string_ | Name is an enum which identifies the calico-webhooks Deployment container port by name.<br />Supported values are: calico-webhooks |
| `containerPort` _integer_ | Number of port to expose on the pod's IP address. This must be a valid port number, 0 < x < 65536. |


### CalicoWebhooksDeploymentPodSpec
...
| `hostNetwork` _boolean_ | (Optional) HostNetwork forces the webhook pod to use the host's network namespace. When true, the webhook pod will run with hostNetwork=true and DNSPolicy=ClusterFirstWithHostNet. When nil or omitted, the operator auto-detects whether host networking is required (e.g., for EKS/TKG with Calico CNI). |


### CalicoWebhooksDeploymentPodTemplateSpec
...
_Appears in:_
- [CalicoWebhooksDeploymentSpec](#calicowebhooksdeploymentspec)

| Field | Description |
| --- | --- |
| `metadata` _[Metadata](#metadata)_ | (Optional) Refer to Kubernetes API documentation for fields of `metadata`. |
| `spec` _[CalicoWebhooksDeploymentPodSpec](#calicowebhooksdeploymentpodspec)_ | (Optional) Spec is the calico-webhooks Deployment's PodSpec. |


### CalicoWebhooksDeploymentSpec
...
| `template` _[CalicoWebhooksDeploymentPodTemplateSpec](#calicowebhooksdeploymentpodtemplatespec)_ | (Optional) Template describes the calico-webhooks Deployment pod that will be created. |
...
| Field | Description |
| --- | --- |
| `name` _string_ | Name is an enum which identifies the EGW Deployment container by name.<br />Supported values are: egress-gateway |
...
| Field | Description |
| --- | --- |
| `spec` _[FluentBitDaemonSetSpec](#fluentbitdaemonsetspec)_ | (Optional) Spec is the specification of the Fluent Bit DaemonSet. |


### FluentBitDaemonSetContainer
...
_Appears in:_
- [FluentBitDaemonSetPodSpec](#fluentbitdaemonsetpodspec)

| Field | Description |
| --- | --- |
| `name` _string_ | Name is an enum which identifies the Fluent Bit DaemonSet container by name.<br />Supported values are: calico-fluent-bit (or the deprecated alias fluentd, kept for one release so existing FluentdDaemonSet overrides continue to validate). |
| `resources` _[ResourceRequirements](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#resourcerequirements-v1-core)_ | (Optional) Resources allows customization of limits and requests for compute resources such as cpu and memory. If specified, this overrides the named Fluent Bit DaemonSet container's resources. If omitted, the Fluent Bit DaemonSet will use its default value for this container's resources. |
| `readinessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) ReadinessProbe allows customization of the readiness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
| `livenessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) LivenessProbe allows customization of the liveness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |


### FluentBitDaemonSetInitContainer
...
| Field | Description |
| --- | --- |
| `name` _string_ | Name is an enum which identifies the Fluent Bit DaemonSet init container by name.<br />Supported values are: calico-fluent-bit-tls-key-cert-provisioner (or the deprecated alias tigera-fluentd-prometheus-tls-key-cert-provisioner, kept for one release so existing FluentdDaemonSet overrides continue to validate). |
| `resources` _[ResourceRequirements](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#resourcerequirements-v1-core)_ | (Optional) Resources allows customization of limits and requests for compute resources such as cpu and memory. If specified, this overrides the named Fluent Bit DaemonSet init container's resources. If omitted, the Fluent Bit DaemonSet will use its default value for this init container's resources. |


### FluentBitDaemonSetPodSpec
...
_Appears in:_
- [FluentBitDaemonSetPodTemplateSpec](#fluentbitdaemonsetpodtemplatespec)

| Field | Description |
| --- | --- |
| `initContainers` _[FluentBitDaemonSetInitContainer](#fluentbitdaemonsetinitcontainer) array_ | (Optional) InitContainers is a list of Fluent Bit DaemonSet init containers. If specified, this overrides the specified Fluent Bit DaemonSet init containers. If omitted, the Fluent Bit DaemonSet will use its default values for its init containers. |
| `containers` _[FluentBitDaemonSetContainer](#fluentbitdaemonsetcontainer) array_ | (Optional) Containers is a list of Fluent Bit DaemonSet containers. If specified, this overrides the specified Fluent Bit DaemonSet containers. If omitted, the Fluent Bit DaemonSet will use its default values for its containers. |


### FluentBitDaemonSetPodTemplateSpec
...
_Appears in:_
- [FluentBitDaemonSetSpec](#fluentbitdaemonsetspec)

| Field | Description |
| --- | --- |
| `spec` _[FluentBitDaemonSetPodSpec](#fluentbitdaemonsetpodspec)_ | (Optional) Spec is the Fluent Bit DaemonSet's PodSpec. |


### FluentBitDaemonSetSpec
...
_Appears in:_
- [FluentBitDaemonSet](#fluentbitdaemonset)

| Field | Description |
| --- | --- |
| `template` _[FluentBitDaemonSetPodTemplateSpec](#fluentbitdaemonsetpodtemplatespec)_ | (Optional) Template describes the Fluent Bit DaemonSet pod that will be created. |
...
| `readinessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) ReadinessProbe allows customization of the readiness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
| `livenessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) LivenessProbe allows customization of the liveness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
| `resources` _[ResourceRequirements](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#resourcerequirements-v1-core)_ | (Optional) container. |
...
| `metricsPort` _integer_ | (Optional) MetricsPort configures the port that Goldmane uses to serve Prometheus metrics. When set to a non-zero value, Goldmane will expose a /metrics endpoint on the given port. Set to zero to disable metrics. If omitted, metrics are disabled. |
...
| Field | Description |
| --- | --- |
| `hostname` _string_ | Hostname for the Gateway listener. Must match the Authentication CR's managerDomain when OIDC is configured (Manager only). |
| `gatewayNamespace` _string_ | (Optional) GatewayNamespace is the namespace where the Gateway and Envoy Proxy pods run. Defaults to "calico-system". |
| `gatewayClassName` _string_ | (Optional) GatewayClassName selects the GatewayClass for the Gateway resource. If not set and the GatewayAPI CR has exactly one class, that class is used. If not set and multiple classes exist, the controller sets a warning requiring the user to specify one. |
...
| Field | Description |
| --- | --- |
| `variant` _[ProductVariant](#productvariant)_ | (Optional) Variant is the product to install - one of Calico or CalicoEnterprise. TigeraSecureEnterprise is also accepted as a deprecated alias for CalicoEnterprise. If left unset, the operator fills in the variant it is running as. |
...
| `typhaPodDisruptionBudget` _[PodDisruptionBudgetOverride](#poddisruptionbudgetoverride)_ | (Optional) TyphaPodDisruptionBudget configures the PodDisruptionBudget for the calico-typha Deployment. Fields left unset fall back to the operator's defaults. The PDB's selector is managed by the operator and cannot be overridden. |
...
| `fipsMode` _[FIPSMode](#fipsmode)_ | (Optional) Deprecated. FIPS mode is no longer supported. Setting fipsMode to Enabled marks the installation degraded. |
...
| `networkPolicy` _[NetworkPolicySpec](#networkpolicyspec)_ | (Optional) NetworkPolicy configures how the operator manages the NetworkPolicies and GlobalNetworkPolicies it installs to protect the Calico components it manages. |
...
| Field | Description |
| --- | --- |
| `variant` _[ProductVariant](#productvariant)_ | Variant is the most recently observed installed variant - one of Calico or CalicoEnterprise. TigeraSecureEnterprise is a deprecated alias for CalicoEnterprise. |
...
| Field | Description |
| --- | --- |
| `replicas` _integer_ | (Optional) Replicas defines the number of Kibana replicas. When set to 0, Kibana is not rendered.<br />Default: 1 |
...
| `fluentdDaemonSet` _[FluentBitDaemonSet](#fluentbitdaemonset)_ | (Optional) FluentdDaemonSet configures the calico-fluent-bit DaemonSet (deprecated alias). Deprecated: use CalicoFluentBitDaemonSet instead. This field is retained as an alias for one release during the Fluentd → Fluent Bit migration; when both are set, CalicoFluentBitDaemonSet takes precedence. |
| `calicoFluentBitDaemonSet` _[FluentBitDaemonSet](#fluentbitdaemonset)_ | (Optional) CalicoFluentBitDaemonSet configures the calico-fluent-bit DaemonSet, the Fluent Bit replacement for the Fluentd DaemonSet. Pod-template override semantics are unchanged from the deprecated FluentdDaemonSet field. |
...
| `openTelemetry` _[OpenTelemetrySpec](#opentelemetryspec)_ | (Optional) OpenTelemetry configures OpenTelemetry export of logs and metrics via OTLP. It is not a passthrough for OpenTelemetry Collector configuration: the fields here describe what Calico Enterprise exports and where, and the operator translates that into a Collector deployment. Unlike AdditionalStores entries (S3, Syslog, Splunk), which point at external systems, that Collector is operator-managed infrastructure (StatefulSet, ConfigMap, RBAC, certs) with its own lifecycle, so this lives at the top level rather than under AdditionalStores. |
...
| Field | Description |
| --- | --- |
| `name` _string_ | Name is an enum which identifies the Manager Deployment container by name.<br />Supported values are: calico-voltron, calico-manager, calico-ui-apis, calico-dashboard-api, tigera-voltron (deprecated), tigera-manager (deprecated), tigera-ui-apis (deprecated), tigera-es-proxy (deprecated) |
...
| `ingressGateway` _[IngressGatewaySpec](#ingressgatewayspec)_ | (Optional) IngressGateway configures Calico Ingress Gateway access to the Manager UI. When set, the operator renders Gateway API resources (Gateway, HTTPRoute, Backend, ReferenceGrant, TLS Secret) to expose Manager via CIG. Requires a GatewayAPI CR to be present. |
...
- [GoldmaneDeployment](#goldmanedeployment)
- [GoldmaneDeploymentPodTemplateSpec](#goldmanedeploymentpodtemplatespec)
- [OpenTelemetryCollectorStatefulSet](#opentelemetrycollectorstatefulset)
- [OpenTelemetryCollectorStatefulSetPodTemplateSpec](#opentelemetrycollectorstatefulsetpodtemplatespec)
- [PodDisruptionBudgetOverride](#poddisruptionbudgetoverride)
...
| Field | Description |
| --- | --- |
| `managePolicies` _[NetworkPolicyManagement](#networkpolicymanagement)_ | (Optional) ManagePolicies controls whether the operator creates and reconciles the NetworkPolicies and GlobalNetworkPolicies it uses to protect the Calico components it installs. When set to Disabled, the operator stops creating or updating these policies and deletes any it has already created, leaving policy management to the user. Defaults to Enabled. |
...
### OpenTelemetryCollectorStatefulSet
...
| --- | --- |
| `metadata` _[Metadata](#metadata)_ | (Optional) Refer to Kubernetes API documentation for fields of `metadata`. |
| `spec` _[OpenTelemetryCollectorStatefulSetSpec](#opentelemetrycollectorstatefulsetspec)_ | (Optional) Spec is the specification of the OpenTelemetry Collector StatefulSet. |


### OpenTelemetryCollectorStatefulSetContainer
...
_Appears in:_
- [OpenTelemetryCollectorStatefulSetPodSpec](#opentelemetrycollectorstatefulsetpodspec)

| Field | Description |
| --- | --- |
| `name` _string_ | Name is an enum which identifies the OpenTelemetry Collector StatefulSet container by name.<br />Supported values are: otel-collector |
| `resources` _[ResourceRequirements](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#resourcerequirements-v1-core)_ | (Optional) Resources allows customization of limits and requests for compute resources such as cpu and memory. If specified, this overrides the named OpenTelemetry Collector StatefulSet container's resources. If omitted, the OpenTelemetry Collector StatefulSet will use its default value for this container's resources. |
| `readinessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) ReadinessProbe allows customization of the readiness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |
| `livenessProbe` _[ProbeOverride](#probeoverride)_ | (Optional) LivenessProbe allows customization of the liveness probe timing parameters. The probe handler is set by the operator and cannot be overridden. |


### OpenTelemetryCollectorStatefulSetPodSpec
...
| Field | Description |
| --- | --- |
| `affinity` _[Affinity](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#affinity-v1-core)_ | (Optional) Affinity is a group of affinity scheduling rules for the OpenTelemetry Collector pods. |
| `containers` _[OpenTelemetryCollectorStatefulSetContainer](#opentelemetrycollectorstatefulsetcontainer) array_ | (Optional) Containers is a list of OpenTelemetry Collector containers. If specified, this overrides the specified OpenTelemetry Collector StatefulSet containers. If omitted, the OpenTelemetry Collector StatefulSet will use its default values for its containers. |
| `nodeSelector` _object (keys:string, values:string)_ | (Optional) NodeSelector gives more control over the nodes where the OpenTelemetry Collector pods will run on. |
| `topologySpreadConstraints` _[TopologySpreadConstraint](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#topologyspreadconstraint-v1-core) array_ | (Optional) TopologySpreadConstraints describes how a group of pods ought to spread across topology domains. Scheduler will schedule pods in a way which abides by the constraints. All topologySpreadConstraints are ANDed. |
| `tolerations` _[Toleration](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#toleration-v1-core) array_ | (Optional) Tolerations is the OpenTelemetry Collector pod's tolerations. If specified, this overrides any tolerations that may be set on the OpenTelemetry Collector StatefulSet. If omitted, the OpenTelemetry Collector StatefulSet will use its default value for tolerations. |
| `priorityClassName` _string_ | (Optional) PriorityClassName allows to specify a PriorityClass resource to be used. |


### OpenTelemetryCollectorStatefulSetPodTemplateSpec
...
_Appears in:_
- [OpenTelemetryCollectorStatefulSetSpec](#opentelemetrycollectorstatefulsetspec)

| Field | Description |
| --- | --- |
| `metadata` _[Metadata](#metadata)_ | (Optional) Refer to Kubernetes API documentation for fields of `metadata`. |
| `spec` _[OpenTelemetryCollectorStatefulSetPodSpec](#opentelemetrycollectorstatefulsetpodspec)_ | (Optional) Spec is the OpenTelemetry Collector StatefulSet's PodSpec. |


### OpenTelemetryCollectorStatefulSetSpec
...
| Field | Description |
| --- | --- |
| `minReadySeconds` _integer_ | (Optional) MinReadySeconds is the minimum number of seconds for which a newly created StatefulSet pod should be ready without any of its container crashing, for it to be considered available. If specified, this overrides any minReadySeconds value that may be set on the OpenTelemetry Collector StatefulSet. If omitted, the OpenTelemetry Collector StatefulSet will use its default value for minReadySeconds. |
| `template` _[OpenTelemetryCollectorStatefulSetPodTemplateSpec](#opentelemetrycollectorstatefulsetpodtemplatespec)_ | (Optional) Template describes the OpenTelemetry Collector StatefulSet pod that will be created. |
...
| Field | Description |
| --- | --- |
| `name` _string_ | Name uniquely identifies this exporter. Must be a DNS label. |
| `endpoint` _string_ | Endpoint is the OTLP endpoint URL. Must be https: telemetry and its credentials are never sent in the clear. |
| `protocol` _[OpenTelemetryExporterProtocol](#opentelemetryexporterprotocol)_ | (Optional) Protocol specifies the OTLP transport protocol.<br />Default: grpc. |
| `tls` _[OpenTelemetryExporterTLS](#opentelemetryexportertls)_ | (Optional) TLS configures transport security for this exporter. When omitted, the endpoint is verified against the system roots. |
| `auth` _[OpenTelemetryExporterAuth](#opentelemetryexporterauth)_ | (Optional) Auth configures how requests to this exporter are authenticated. |


### OpenTelemetryExporterAuth
...
| Field | Description |
| --- | --- |
| `headers` _[OpenTelemetryExporterHeader](#opentelemetryexporterheader) array_ | (Optional) Headers are attached to every request to this exporter. Values are read from Secrets rather than set inline. |


### OpenTelemetryExporterHeader
...
_Appears in:_
- [OpenTelemetryExporterAuth](#opentelemetryexporterauth)

| Field | Description |
| --- | --- |
| `name` _string_ | Name is the header name, for example Authorization or DD-API-KEY. |
| `valueFrom` _[OpenTelemetryHeaderValueSource](#opentelemetryheadervaluesource)_ | ValueFrom selects the Secret key holding this header's value. The value is sent verbatim, so a bearer token must include the "Bearer " prefix in the Secret: "Bearer eyJhbGci...", not "eyJhbGci...". |


### OpenTelemetryExporterProtocol
...
| Field | Description |
| --- | --- |
| `caConfigMapName` _string_ | (Optional) CAConfigMapName names a ConfigMap in the tigera-operator namespace holding the CA that signs this exporter's serving certificate, under a tls.crt key. When set, this CA alone is trusted for this exporter; the system roots are not. |
| `clientCertSecretName` _string_ | (Optional) ClientCertSecretName names a Secret in the tigera-operator namespace holding the client keypair to present to this exporter, under tls.crt and tls.key. Setting it turns on mutual TLS for this exporter. |


### OpenTelemetryHeaderValueSource
...
_Appears in:_
- [OpenTelemetryExporterHeader](#opentelemetryexporterheader)

| Field | Description |
| --- | --- |
| `secretKeyRef` _[SecretKeySelector](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#secretkeyselector-v1-core)_ | SecretKeyRef selects a key of a Secret in the tigera-operator namespace. |


### OpenTelemetryLogType
...
| Field | Description |
| --- | --- |
| `types` _[OpenTelemetryLogType](#opentelemetrylogtype) array_ | (Optional) Types specifies which log types to export. Supported values: Audit, DNS, Flows. |
...
| Field | Description |
| --- | --- |
| `state` _[OpenTelemetryMetricsState](#opentelemetrymetricsstate)_ | (Optional) State specifies whether Calico component metrics are scraped and exported via OTLP.<br />Default: Disabled |


### OpenTelemetryMetricsState
...
| Field | Description |
| --- | --- |
| `logs` _[OpenTelemetryLogs](#opentelemetrylogs)_ | (Optional) Logs configures which log types are exported via OTLP. |
| `metrics` _[OpenTelemetryMetrics](#opentelemetrymetrics)_ | (Optional) Metrics configures whether Calico component metrics are exported via OTLP. |
| `exporters` _[OpenTelemetryExporter](#opentelemetryexporter) array_ | (Optional) Exporters configures the OTLP export endpoints. At least one is required, and everything exported is sent to all of them. |
| `openTelemetryCollectorStatefulSet` _[OpenTelemetryCollectorStatefulSet](#opentelemetrycollectorstatefulset)_ | (Optional) OpenTelemetryCollectorStatefulSet configures the OpenTelemetry Collector StatefulSet. |
...
| --- | --- |
| `metadata` _[Metadata](#metadata)_ | (Optional) Refer to Kubernetes API documentation for fields of `metadata`. |
| `spec` _[PodDisruptionBudgetOverrideSpec](#poddisruptionbudgetoverridespec)_ | (Optional) Spec is the specification of the PodDisruptionBudget. |


### PodDisruptionBudgetOverrideSpec
...
| Field | Description |
| --- | --- |
| `minAvailable` _[IntOrString](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#intorstring-intstr-util)_ | (Optional) MinAvailable is the minimum number of pods (as an integer or percentage) that must remain available during a disruption. Mutually exclusive with MaxUnavailable. |
| `maxUnavailable` _[IntOrString](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#intorstring-intstr-util)_ | (Optional) MaxUnavailable is the maximum number of pods (as an integer or percentage) that can be unavailable during a disruption. Mutually exclusive with MinAvailable. If neither MinAvailable nor MaxUnavailable is set, the operator applies its default (MaxUnavailable=1 for calico-typha). |
| `unhealthyPodEvictionPolicy` _[UnhealthyPodEvictionPolicyType](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.32/#unhealthypodevictionpolicytype-v1-policy)_ | (Optional) UnhealthyPodEvictionPolicy defines when unhealthy pods should be considered for eviction. Defaults to IfHealthyBudget (the Kubernetes default) when unset. See https://kubernetes.io/docs/tasks/run-application/configure-pdb/#unhealthy-pod-eviction-policy. |
...
_Appears in:_
- [APIServerDeploymentContainer](#apiserverdeploymentcontainer)
- [CSINodeDriverDaemonSetContainer](#csinodedriverdaemonsetcontainer)
- [CalicoKubeControllersDeploymentContainer](#calicokubecontrollersdeploymentcontainer)
- [CalicoNodeDaemonSetContainer](#caliconodedaemonsetcontainer)
- [CalicoNodeWindowsDaemonSetContainer](#caliconodewindowsdaemonsetcontainer)
- [CalicoWebhooksDeploymentContainer](#calicowebhooksdeploymentcontainer)
- [CalicoWindowsUpgradeDaemonSetContainer](#calicowindowsupgradedaemonsetcontainer)
- [DashboardsJobContainer](#dashboardsjobcontainer)
- [DexDeploymentContainer](#dexdeploymentcontainer)
- [ECKOperatorStatefulSetContainer](#eckoperatorstatefulsetcontainer)
- [EGWDeploymentContainer](#egwdeploymentcontainer)
- [EKSLogForwarderDeploymentContainer](#ekslogforwarderdeploymentcontainer)
- [ESGatewayDeploymentContainer](#esgatewaydeploymentcontainer)
- [ElasticsearchMetricsDeploymentContainer](#elasticsearchmetricsdeploymentcontainer)
- [FluentBitDaemonSetContainer](#fluentbitdaemonsetcontainer)
- [GatewayCertgenJobContainer](#gatewaycertgenjobcontainer)
- [GatewayControllerDeploymentContainer](#gatewaycontrollerdeploymentcontainer)
- [GatewayDaemonSetContainer](#gatewaydaemonsetcontainer)
- [GatewayDeploymentContainer](#gatewaydeploymentcontainer)
- [GoldmaneDeploymentContainer](#goldmanedeploymentcontainer)
- [GuardianDeploymentContainer](#guardiandeploymentcontainer)
- [IntrusionDetectionControllerDeploymentContainer](#intrusiondetectioncontrollerdeploymentcontainer)
- [KibanaContainer](#kibanacontainer)
- [L7LogCollectorDaemonSetContainer](#l7logcollectordaemonsetcontainer)
- [LinseedDeploymentContainer](#linseeddeploymentcontainer)
- [ManagerDeploymentContainer](#managerdeploymentcontainer)
- [OpenTelemetryCollectorStatefulSetContainer](#opentelemetrycollectorstatefulsetcontainer)
- [PacketCaptureAPIDeploymentContainer](#packetcaptureapideploymentcontainer)
- [PolicyRecommendationDeploymentContainer](#policyrecommendationdeploymentcontainer)
...
| Field | Description |
| --- | --- |
| `periodSeconds` _integer_ | (Optional) PeriodSeconds is how often (in seconds) to perform the probe. |
| `timeoutSeconds` _integer_ | (Optional) TimeoutSeconds is the number of seconds after which the probe times out. |
| `failureThreshold` _integer_ | (Optional) FailureThreshold is the minimum consecutive failures for the probe to be considered failed after having succeeded. |
| `initialDelaySeconds` _integer_ | (Optional) InitialDelaySeconds is the number of seconds after the container starts before the probe is initiated. |
...
| Field | Description |
| --- | --- |
| `endpoint` _string_ | Location of the syslog server. example: tcp://1.2.3.4:601 Only the tcp and udp schemes are supported; TLS is selected via the Encryption field rather than the scheme. |
| `packetSize` _integer_ | (Optional) PacketSize defines the maximum size, in bytes, of messages sent to syslog; messages larger than this are truncated. When unset, the syslog output's own default for the RFC5424 format the operator renders applies (2048 bytes). Set this only if you notice long logs being truncated. |
