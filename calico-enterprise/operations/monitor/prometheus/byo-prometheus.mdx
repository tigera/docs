...
- [Scrape metrics from specific components](#scrape-all-enabled-metrics)
- [Scrape metrics from specific components](#scrape-metrics-from-specific-components)
...
{{prodname}} has a feature that will create a service monitor that will scrape all enabled metrics. To enable metrics that
are not enabled by default, please consult the [next subchapter](#scrape-metrics-from-specific-components).

This example illustrates a Prometheus server installed in namespace "external-prometheus" with a `serviceMonitorSelector`
that selects all service monitors with label `k8s-app=tigera-external-prometheus`.
...
3. Verify that the new configuration has been added to your cluster
    ...
    ```
    That's it. You should be seeing the new metrics show up in your prometheus within one minutes. For more information
    on verifying metrics, scroll down to the [Verify BYO Prometheus](#verify-byo-prometheus) section.

### Scrape metrics from specific components

We recommend the previous chapter for scraping all enabled metrics. Read on if you wish to scrape metrics from specific
components, or if you wish to enable metrics that are disabled by default.
