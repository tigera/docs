...
<TabItem label="fluent bit" value="fluent-bit-1">

**Create the service monitor**

Fluent Bit serves its metrics over plain HTTP (port 2020, path `/api/v2/metrics/prometheus`), so no TLS
...
Access to the Fluent Bit metrics port is restricted by the `calico-system.allow-calico-fluent-bit` network policy,
which by default admits only the $[prodname] Prometheus pods in the `tigera-prometheus` namespace. Create a policy
that allows your BYO Prometheus pods to reach port 2020 on the `calico-fluent-bit` pods (see
[Create policy to secure traffic between pods](#create-policy-to-secure-traffic-between-pods)).
