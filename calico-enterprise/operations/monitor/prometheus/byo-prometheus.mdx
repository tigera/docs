...
Access to the Fluent Bit metrics port is restricted by the `calico-system.allow-calico-fluent-bit` network policy, which
by default admits only the `tigera-prometheus` namespace. Create a policy that allows your BYO Prometheus
namespace to reach port 2020 on the `calico-fluent-bit` pods (see
[Create policy to secure traffic between pods](#create-policy-to-secure-traffic-between-pods)).
