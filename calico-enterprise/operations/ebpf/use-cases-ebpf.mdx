...
Enforcing policy on a bridged, VLAN-tagged segment is not possible with the iptables data plane, and the eBPF data plane also raises the minimum kernel version for that feature to 5.18.
