...
The tabs below explain how the operator must be patched with the DNS config so the operator can resolve the apiserver DNS.

<Tabs>
<TabItem label="kOps" value="kOps-10">

For a KOPS cluster, DNS server is 127.0.0.53.
...
<TabItem label="EKS" value="EKS-11">

For an EKS cluster, DNS server is the VPC's primary private IPV4 CIDR range plus two. For example, if you have a VPC with an IPv4 CIDR of 192.168.0.0/16,
DNS server is at 192.168.0.2.

```bash
kubectl patch deployment -n tigera-operator tigera-operator -p '{"spec":{"template":{"spec":{"dnsConfig":{"nameservers":["192.168.0.2"]}}}}}'
```
