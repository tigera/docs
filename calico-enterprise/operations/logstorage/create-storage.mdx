...
In the following example, we create a `StorageClass` to use when explicitly adding `PersistentVolume`s for local disks. This can be performed manually, or using the [sig-storage local static provisioner](https://github.com/kubernetes-sigs/sig-storage-local-static-provisioner).
...
If local persistent volumes are provisioned on a SELinux-enabled host, you can use `/mnt/tigera` host path created by {{prodname}} policy package.
