...
On $[prodname], the standard [Helm install](../getting-started/install-on-clusters/kubernetes/helm.mdx) defaults to native `projectcalico.org/v3` CRD mode, so follow that guide to install. Manifest install is not available for v3 on $[prodname].
