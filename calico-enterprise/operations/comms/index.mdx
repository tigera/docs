...
- **Common Name (CN)**: must match the first DNS name listed in the table below, with two exceptions:
  - `node-certs`: CN = `typha-client`
  - `typha-certs`: CN = `typha-server`
...
| Secret name | DNS name(s) | Namespace | Owner resource |
|---|---|---|---|
