...
The tables below list all operator-managed TLS secrets. The **Deployed to** column shows the namespace where the operator places the secret at runtime. To override, always create your secret in `tigera-operator`.
...
Typha and Node use mutual TLS. If you replace these certificates, ensure all three secrets use certificates signed by the same CA, or include the relevant CA certificates in each trust bundle. Mismatched certificates will break Node-to-Typha communication.
