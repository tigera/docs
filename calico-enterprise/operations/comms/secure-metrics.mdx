...
- `calicoctl` is [installed in your PATH and configured to access the data store]({{site.baseurl}}/maintenance/clis/calicoctl/install).
...
1. Apply labels to any endpoints that should have access to the metrics.
   ...
   can create a [global network set]({{site.baseurl}}/reference/resources/globalnetworkset) using `kubectl`.
...
1. Create host endpoints for each {{site.prodname}} node.
   ...
   > on the [FelixConfiguration resource]({{site.baseurl}}/reference/resources/felixconfig).
   {: .alert .alert-info}
