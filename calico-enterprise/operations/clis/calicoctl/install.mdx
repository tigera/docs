...
1. Use the following commands to pull the `calico` image from the Tigera
   registry. `calicoctl` ships inside this image alongside the other
   $[prodname] command-line tools.

   ```bash
   docker pull $[registry]$[componentImage.calico]
   ```
