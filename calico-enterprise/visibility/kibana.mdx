...
The following logs are generated by {{site.prodname}}. All logs are enabled by default except **l7 logs**, which must be explicity enabled. 
...
In Manager UI, click `>` to expand the left navigation, and select **Kibana**. A new browser tab opens into Kibana. 
...
Tor and VPN-based traffic indicates the use of anonymization techniques in an attempt to mask the origins and destination of network traffic. {{site.prodname}} has built-in capabilities to assist with detecting such traffic and requires minimal [configuration]({{site.baseurl}}/threat/tor-vpn-feed-and-dashboard) to activate.
...
![honeypod-dashboard]({{site.baseurl}}/images/honeypods-dashboard.png)
...
Each dashboard has advanced filtering options if pre-built filters dashboard are insufficient. For example: 
