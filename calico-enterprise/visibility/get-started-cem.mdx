...
Network policy is the primary tool for securing a Kubernetes network. Policy is used to restrict network traffic (egress and ingress) in your cluster so only the traffic that you want to flow is allowed. {variables.prodname} supports these policies:

- {variables.prodname} network policy
- {variables.prodname} global network policy
- Kubernetes policy

{variables.prodname} uses **tiers** (also called, hierarchical tiers) to provide guardrails for managing network policy across teams. Policy tiers allow users with more authority (for example, Dev/ops users) to enforce network policies that take precedence over teams (for example, service owners and developers).

**Policies Board** is the default view for managing tiered policies.

![policy-board](/img/calico-enterprise/policy-export.png)

Users typically use a mix of Policy Board and YAML files. Note that you can export one or all policies in a tier to YAML.

The **Policy Board** lets you filter by name, namespace, policy types and label selectors.

![policy-filters](/img/calico-enterprise/policy-search.png)
...
![endpoints](/img/calico-enterprise/endpoint.png)
...
Network sets and global network sets are {variables.prodname} resources for defining IP subnetworks/CIDRs, which can be matched by standard label selectors in policy (Kubernetes or {variables.prodname}). They are a powerful feature for use/reuse and scaling policy.
...
If you have configured {variables.prodname} for multi-cluster management, you will see the **Managed clusters** option in the left navbar.
...
Compliance tools that rely on periodic snapshots, do not provide accurate assessments of Kubernetes workloads against your compliance standards. {variables.prodname} compliance dashboard and reports provide a complete inventory of regulated workloads, along with evidence of enforcement of network controls for these workloads. Additionally, audit reports are available to see changes to any network security controls.

**Compliance reports** are based on archived flow logs and audit logs for all {variables.prodname} resources, and audit logs for Kubernetes resources in the Kubernetes API server.
...
What changed, who did it, and when? This information is critical for security. Native Kubernetes doesn’t provide an easy way to capture audit logs for pods, namespaces, service accounts, network policies, and endpoints. The {variables.prodname} timeline provides audit logs for all changes to network policy and other resources associated with your {variables.prodname} deployment.
...
How do you know if you have an infected workload? A possible threat? {variables.prodname} detects and alerts on unexpected network behavior that may indicate a security breach. You can create alerts for:
...
{variables.prodname} includes a fully-integrated deployment of Elastic to collect flow log data that drives key features like the Flow Visualizer, metrics in the Dashboard and Policy Board, policy automation, and testing features and security. {variables.prodname} also embeds Kibana so you can view raw log data for the traffic within your cluster.

> From the left navbar, click **Logs**.

**Dashboards**

{variables.prodname} comes with built-in dashboards.
...
You can add threat intelligence feeds to {variables.prodname} to trace network flows of suspicious IP addresses and domains. Then, you can use network policy to block pods from contacting IPs or domains.
