...
To view WAF events in a centralized security events dashboard, go to: **Threat defense**, **Security Events**. For help see, [security-event-management](./security-event-management).
...
If a SQL Injection attack is detected for rule ID 942100, you will see a security event in the dashboard here: **Threat defense**, **Security Events**.
...
For example, if you want to deny traffic with a 403 response, do the following steps: 

1. Modify the modsecurity core rulesets' `SecDefaultAction` to deny rules by commenting and uncommenting these sections in the `crs-setup.conf`, 
which will change the `SecDefaultAction` from `pass` to `deny`.
