...
<!--To review the rules deployed with the WAF, see [Ruleset files](https://github.com/tigera/operator/tree/master/pkg/render/applicationlayer/embed/corerule set/rules).-->
...
The configuration file starts with importing the appropriate rule set config. We use Coraza WAF's recommended [Core Ruleset setup](https://coraza.io/docs/tutorials/corerule set/) files:
...
These files can be customized if desired. 

1. (Recommended) Add all your customizations directly under `tigera.conf`

```bash
kubectl edit cm -n tigera-operator modsecurity-rule set
``` 

1. (Not recommended) Starting your configmap from scratch

```bash
# create your own working copy corerule set
git clone -b v4.0.0-rc2 https://github.com/corerule set/corerule set --depth=1 
cd corerule set

# download a recommend configuration to begin with
wget https://raw.githubusercontent.com/corazawaf/coraza/v4/main/coraza.conf-recommended -O coraza.conf

# create tigera.conf
cat <<EOF > tigera.conf
Include /etc/modsecurity-rule set/coraza.conf
Include /etc/modsecurity-rule set/crs-setup.conf.example
Include /etc/modsecurity-rule set/REQUEST-*.conf

SecRuleEngine DetectionOnly
EOF

## create the config map itself
kubectl create cm --dry-run=client --from-file=tigera.conf --from-file=coraza.conf --from-file=rules --from-file=crs-setup.conf.example -n tigera-operator modsecurity-rule set -o yaml  > rule set.configmap.yaml
...
:::note
the path `/etc/modsecurity-rule set/` is the designated mount path for files found in the configmap 
:::


After you do these steps, the `modsecurity-rule set` ConfigMap will be replaced in the `tigera-operator` namespace,
...
It is important to adhere to [CoreRuleset documentation](https://corerule set.org/docs) on how to edit the behaviour of
 your WAF. A good place to begin at is the [Installing Core Rule Set](https://corerule set.org/docs/deployment/install/).
...
Include /etc/modsecurity-rule set/wordpress-rule-exclusions-config.conf
Include /etc/modsecurity-rule set/wordpress-rule-exclusions-before.conf
...
# Include /etc/modsecurity-rule set/wordpress-rule-exclusions-after.conf
...
kubectl create cm --dry-run=client \
  ...
  -n tigera-operator modsecurity-rule set -o yaml  > rule set.configmap.yaml
