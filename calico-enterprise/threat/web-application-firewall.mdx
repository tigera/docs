...
[ModSecurity module](https://owasp.org/www-project-modsecurity-core-rule-set/). To view the rule set that is bundled with WAF, see [rule-set](https://github.com/coreruleset/coreruleset/blob/v3.4/dev/rules).
...
2. Select the services you want WAF to inspect, and then click **Confirm Selections**.
   ...
   <img
    src='/img/calico-enterprise/waf-select-services-CE3.17-CC-3.16.png'
    alt='WAF services'
    width='600'
  />
...
By default, the WAF OWASP Core Rule set is configured to log all matched rules and block offending requestis using Anomaly Scoring Mode. However, if you want traffic for the service to not be blocked and only logged,
you must edit the modsecurity rules. 

#### Allow traffic

For example, if you want to allow traffic, do the following steps:

1. Anomaly scores could result in Disruptive action. To change the disruption action, see the file `RESPONSE-999-EXCLUSION-RULES-AFTER-CRS.conf.example` section `'Changing the Disruptive Action for Anomaly Mode'`.

Append the lines

```
SecRuleUpdateActionById 949110 "t:none,pass"
SecRuleUpdateActionById 959100 "t:none,pass"
```

2. Rename the file to remove `.example` suffix:
...
Note that switching to pass means the request is now processed and the 403 forbidden error is gone even if a rule is triggered.
For example:
...
1. Create a directory, and download the core rules set files for customization.

```bash
mkdir my-ruleset && cd my-ruleset
curl -L https://github.com/coreruleset/coreruleset/archive/refs/heads/v3.4/dev.zip -o crs.zip
unzip -j crs.zip 'coreruleset-3.4-dev/rules/*'
rm crs.zip
```

2. Download bootstrap files.

  Download the following required bootstrapping configuration files from this [site](https://medium.com/lightbaseio/web-application-firewall-in-go-feat-owasp-modsecurity-core-rule-set-3f97a26e3311). These files are used by Tigera operator:
    - `modsecdefault.conf`
    - `crs-setup.conf`
...
3. If you wish to customize settings on default included rules, see `.example` files for guidance. You must save these files without the .example extension if you wish for these changes to take effect.

4. Go to my-ruleset folder where your core rules set files are located, create a ConfigMap containing all the files downloaded into your new directory and replace the existing rule set with it.
