...
From the **Action** column, select **Enable** or **Disable** to respectively enable or disable WAF for each of the services.
You will be presented with a window containing a kubectl command, which you must execute on a cluster node for the action to take effect.
...
Alternatively, you can annotate the services you want to enable WAF for as follows:
