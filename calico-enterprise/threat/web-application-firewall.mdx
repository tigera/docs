...
Reviewing the default rulset config:
...
The configuration file starts with importing the appropriate ruleset config. We use Coraza WAF's recommended [Core Ruleset setup](https://coraza.io/docs/tutorials/coreruleset/) files:
...
```bash
# create your own working copy coreruleset
git clone -b v4.0.0-rc2 https://github.com/coreruleset/coreruleset --depth=1 
cd coreruleset
...
# create tigera.conf
cat <<EOF > tigera.conf
Include /etc/modsecurity-ruleset/coraza.conf
Include /etc/modsecurity-ruleset/crs-setup.conf.example
Include /etc/modsecurity-ruleset/REQUEST-*.conf

SecRuleEngine DetectionOnly
EOF

## create the config map itself
kubectl create cm --dry-run=client --from-file=tigera.conf --from-file=coraza.conf --from-file=rules --from-file=crs-setup.conf.example -n tigera-operator modsecurity-ruleset -o yaml  > ruleset.configmap.yaml
...
:::note
the path `/etc/modsecurity-ruleset/` is the designated mount path for files found in the configmap 
:::
...
kubectl create cm --dry-run=client \
  ...
  -n tigera-operator modsecurity-ruleset -o yaml  > ruleset.configmap.yaml
