...
Additionally the HTTP request must contain the `Content-Type` HTTP header with a value of either `text/plain` or `application/json`, as well as the `User-Agent` HTTP header with a value having prefix `curl`.
Any HTTP request from the trading app that does not fulfill the outlined criteria would be rejected.
