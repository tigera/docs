...
**To get a new policy recommendation for an existing enforced policy recommendation**, delete or rename the policy so a new staged network policy is recreated on the Policy Recommendations board. 
...
Note that activated staged network policies are not affected by disabling policy recommendations, including enforced policies. Unactivated policy recommendations in the Policies Recommendations are no longer actively updated. 
