...
You can change policy recommendation parameters using the [Policy recommendations resource](../reference/resources/stagednetworkpolicy). For example, you may want to change the interval for flow logs to be processed, or the time for a policy to keep processing flows for until it’s considered stable. You can also filter out namespaces. 
