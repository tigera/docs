...
- The differences between Kubernetes and {{site.prodname}} network policies and when you might want to use each.
...
In contrast, the [Kubernetes network model]({{site.baseurl}}/networking/training/about-kubernetes-networking) defines a "flat"
...
> Note: {{site.prodname}} offers capabilities that can help perimeter firewalls integrate
...
of the specification.  ({{site.prodname}} does implement every feature, and was the original reference implementation of Kubernetes
network policies.)
...
While Kubernetes network policy applies only to pods, {{site.prodname}} network policy can be applied to multiple types of
endpoints including pods, VMs, and host interfaces.

To learn more about {{site.prodname}} network policies, read the [Get started with {{site.prodname}} network
...
Unlike some other network policy implementations, {{site.prodname}} implements the full set of Kubernetes network policy features.

#### Richer network policy
{{site.prodname}} network policies allow even richer traffic control than Kubernetes network policies if you need it. In addition,
{{site.prodname}} network policies allow you to create policy that applies across multiple namespaces using GlobalNetworkPolicy
resources.

#### Mix Kubernetes and {{site.prodname}} network policy
Kubernetes and {{site.prodname}} network policies can be mixed together seamlessly. One common use case for this is to split
responsibilities between security / cluster ops teams and developer / service teams. For example, giving the security /
cluster ops team RBAC permissions to define {{site.prodname}} policies, and giving developer / service teams RBAC permissions to
define Kubernetes network policies in their specific namespaces. As {{site.prodname}} policy rules can be ordered to be enforced
...
For more flexible control and delegation of responsibilities between two or more teams, {{site.prodname}} Enterprise extends this
...
[Calico Enterprise]({{site.baseurl}}/security/tiered-policy) supports hierarchical network policy using policy tiers. RBAC
