...
To see policy information in flow logs, you must enable you must enable flow logs for forwarding decisions in the FelixConfiguration resource:
...
For preDNAT policies, flow logs display the original destination IP and port before DNAT is performed. When traffic originates from outside the cluster (for example, an external client accesses a NodePort IP that forwards to a node in the cluster), the flow log captures the preDNAT source and destination details. Here’s an example:
