...
### Enable template host endpoints

To enable template host endpoints, edit the default KubeControllersConfiguration instance, and update `spec.controllers.node.hostEndpoint.templates` with your custom templates spec

```bash
calicoctl patch kubecontrollersconfiguration default --patch='{"spec": {"controllers": {"node": {"hostEndpoint": {"autoCreate": "Enabled", "templates": [{"generateName": "custom-host-endpoint", "interfaceCIDRs": ["192.168.0.0/16"], "nodeSelector": "!has(node-role.kubernetes.io/control-plane)", "labels": {"custom-label": "template-host-endpoint"}}]}}}}}'
```

If successful, host endpoints are created for nodes matching your node selector:
...
```
:::note
You can disable creation of default host endpoints and only use template host endpoints by updating `spec.controllers.node.hostEndpoint.createDefaultHostEndpoint` to Disabled
:::
