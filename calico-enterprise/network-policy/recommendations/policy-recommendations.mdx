...
- **Stabilization period** is the time to capture enough flow logs to/from the namespace so a policy recommendation is an accurate reflection of the traffic. T

- **Interval** is the frequency to run the policy recommendation to create/refine recommended policies. 

:::tip
For application workloads with less frequent communication, the stabilization period setting may not be long enough to accurate traffic, so you’ll want to increase the time. We recommend that you review your workloads immediately after you enable policy recommendations and adjust the settings accordingly. 
:::
