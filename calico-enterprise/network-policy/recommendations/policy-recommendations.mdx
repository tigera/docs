...
- **Stabilization period** is the time to capture enough flow logs to/from the namespace to so a policy recommendation is an accurate reflection of the traffic. 

- **Interval** is the frequency to run the policy recommendation to create/refine recommended policies. 

:::tip
For application workloads with less frequent communication, the stabilization and interval may not be long enough, so you’ll want to increase one or both settings. We recommend that you review your workloads immediately after you enable policy recommendations and adjust the settings accordingly. 
:::
