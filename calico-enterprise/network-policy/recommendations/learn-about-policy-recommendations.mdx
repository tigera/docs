...
1. Download the [`policy-recommendation-deployments.yaml` manifest]({{tutorialFilesURL}}/policy-recommendation-deployments.yaml).
...
Wait for the recommendations to be generated. Unless otherwise configured, recommendations will take at least 2m30s to be generated, which is default time for the [Processing Interval](../../reference/resources/policyrecommendations.mdx#spec) setting. The recommendations will be listed on the Policy recommendations page, under the Recommendations tab.
...
From:Endpoints [[projectcalico.org/orchestrator == 'k8s']]  Namespaces [[projectcalico.org/name == 'service-ns']]  
To:Ports [Port is 80 ]  
```
...
```
Allow:Protocol is TCP
To:Ports [Port is 80 ]  Endpoints [[projectcalico.org/orchestrator == 'k8s']]  Namespaces [[projectcalico.org/name == 'service-ns']]  
```
...
1. To find the flows that were used to generate the egress to global network set rule, add:
```
source_namespace = "curl-ns" AND  dest_name_aggr = "public-ips"
```

1. To find the flows that generated the egress rule to namespace `kube-system`, define query:
```
source_namespace = "curl-ns" AND  dest_namenamespace = "kube-system"
```
...
Restart the pods in `curl-ns`, to re-establish the connections in your cluster:

```bash
kubectl delete pods -n curl-ns -lapp=curl
```

Search for the same flow logs, and you will notice that the new connections established will contain reference to the staged network policies generated by the Policy Recommendation feature.

In the `policies` field of the flow logs, you will find input like: 
```
0|namespace-isolation|curl-ns/namespace-isolation.staged:curl-ns-recommendation|allow|0
```
indicating that the policy named **curl-ns-recommendation**, will allow traffic defined by this flow after the policy is enforced.
...
Search for the flow log in question, examine the `policies` field and verify that no other enforced policy allows or denies traffic for that flow.
