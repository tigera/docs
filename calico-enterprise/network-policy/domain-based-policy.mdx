...
{{prodname}} supports different DNS policy modes with different peformance and latency implications. The
default mode for iptables/nftables dataplane is `DelayDeniedPacket`, eBPF dataplane defaults to
`Inline` on kernels 5.17 (or RedHat 5.14) and newer and to `NoDelay` on older kernels. Windows
dataplane only support `NodeDelay` mode.

With `NoDelay` mode, newly pods sometimes fail to connect to domains that are allowed by a DNS policy.
