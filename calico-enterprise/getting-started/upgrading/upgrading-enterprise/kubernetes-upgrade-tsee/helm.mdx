...
1. Starting from version 3.20, certain features are made optional. Review the `values.yaml` file for optional features with flags set to `enabled: false`. If you wish to maintain the enable status, update the flag to `enabled: true` in the `values.yaml` before executing the `helm upgrade`.
