...
1. Install the $[prodname] custom resource definitions.
   ...
   Upgrading does not change your cluster's CRD mode, so install the CRDs for the mode your cluster already uses. To check which mode you have, run:

   ```bash
   kubectl api-versions | grep crd.projectcalico.org
   ```

   Output means your cluster uses the aggregation API server. No output means it uses native v3 CRDs. See [Native v3 CRDs](../../../../operations/native-v3-crds.mdx) for background on the two modes.
   ...
   <TabItem label="Native v3 CRDs" value="native-v3">

   If your cluster is based on Kubernetes 1.36 or later:

   ```bash
   kubectl apply --server-side --force-conflicts -f $[filesUrl]/manifests/v3_projectcalico_org.yaml
   kubectl create -f $[filesUrl]/manifests/prometheus-operator-crds.yaml
   kubectl create -f $[filesUrl]/manifests/eck-operator-crds.yaml
   ```
   ...
   ```bash
   kubectl apply --server-side --force-conflicts -f $[filesUrl]/manifests/v3_projectcalico_org-v1beta1.yaml
   kubectl create -f $[filesUrl]/manifests/prometheus-operator-crds.yaml
   kubectl create -f $[filesUrl]/manifests/eck-operator-crds.yaml
   ```
