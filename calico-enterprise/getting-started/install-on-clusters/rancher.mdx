---
description: Install Calico Enterprise on a Rancher Kubernetes Engine (RKE) cluster.
---
...
1. Install the Tigera Operator and custom resource definitions.
   ...
   This install uses the aggregation API server (the operator installs the v1 CRDs from `operator-crds.yaml`). New installs can instead use native v3 CRD mode, where `projectcalico.org/v3` is served directly by CRDs with no aggregation API server (requires Kubernetes 1.32+). To use it, install with [Helm](./kubernetes/helm.mdx) or see [Native v3 CRDs](../../operations/native-v3-crds.mdx).
