...
import DockerContainerService from '@site/calico-enterprise/_includes/content/_docker-container-service.mdx';

import NonClusterBinaryInstall from '@site/calico-enterprise/_includes/content/_non-cluster-binary-install.mdx';
...
1. Configure container to start at boot time.
   The <code>{variables.nodecontainer}</code> container should be started at boot time by your init system and the init system must be configured to restart it if stopped. {variables.prodname} relies on that behavior for certain configuration changes.
