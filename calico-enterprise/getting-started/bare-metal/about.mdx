...
1. Create a `NonClusterHost` custom resource.
   ...
   | ingestionCompression | Optional. Compression for log batches in transit to the log ingestion endpoint. Defaults to `Zstd`. Hosts apply a change without a restart. | `Zstd`, `Gzip`, `None` | string |
...
1. Create a `kubeconfig` file for your non-cluster host or VM:
   ...
   | certfile       | Optional. Path to the file containing the PEM-encoded authority certificates. Use this option if you are providing your own [TLS certificates for $[prodname]](../../operations/comms/index.mdx). If not specified, the Tigera root CA certificate will be used by default. | |
...
1. Start Calico node and log forwarder.
   ...
   The log forwarder reads `ingestionCompression` from the `NonClusterHost` resource, so no host configuration is needed. To override it on one host, set `COMPRESS` to `zstd`, `gzip`, or `none` in `/etc/calico/calico-fluent-bit/calico-fluent-bit.env` and restart the service.

   :::note

   Upgrade your cluster before the packages on your hosts. A host may run an older version than its cluster, never a newer one. Hosts fall back to gzip on a cluster whose `NonClusterHost` resource has no `ingestionCompression` field; every release accepts gzip.
