...
- httpProbe may be used to specify the Probe URLs, HTTP interval and timeout in seconds. `urls` if set, the
  ...
  httpProbe:
    ...
    intervalSeconds: 10
  ```
- Please refer to the [operator reference docs](../../reference/installation/_api.mdx) for details about the Egress Gateway resource type.
...
- When upgrading, create an egress gateway as mentioned [above](#deploy-a-group-of-egress-gateways) which exactly matches
  the current egress gateway deployment.
