...
On-premises, you may want an egress gateway's source IP to be a real, routable address on your local subnet — reachable by external hosts and firewalls directly, without advertising it over BGP and without falling back to the node's host IP. To do this, give egress gateways IPs from a no-encapsulation IP pool within the host subnet and let $[prodname] answer ARP/NDP for them on the local L2 segment:

- [L2 reachability for pods and services without BGP](../configuring/local-subnet-l2-reachability.mdx)
