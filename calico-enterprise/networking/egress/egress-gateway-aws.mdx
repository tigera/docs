...
- ipPools tells {{site.prodname}} IPAM to use one of the "red" IP pools: 
...
```bash
kubectl annotate ns  egress.projectcalico.org/egressGatewayPolicy="egw-policy1"
```
...
- Ensure that either your worker nodes are in a private subnet, or they are using Elastic IPs for their public IP.
  ...
  then, when an Elastic IP is added to an egress gateway, adding the Elastic IP to the node will trigger the
  node to lose its VPC Public IP.  This is a [property of AWS networking](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/elastic-ip-addresses-eip.html#eip-basics);
  a node is not allowed to have a VPC public IP and an Elastic IP (though we have found this is only enforced at boot).
  :::
