...
cluster is in a subnet with 10.224.0.0 as network address, the 10.224.0.1 [is supposed to be](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-networks-faq#are-there-any-restrictions-on-using-ip-addresses-within-these-subnets) the gateway and Azure Route Servers are deployed with
...
kubectl get secret tigera-pull-secret --namespace=calico-system -o yaml | \
   ...
   kubectl apply --namespace=<egress_gw_namespace> -f -
```
...
httpProbe:
  ...
  intervalSeconds: 10
  ```
[comment]: <> (XXX is this not necessary to be updated?)
...
*  IPPool can be specified either by its name (e.g. `-name: egress-ip-red-pool`) or by its CIDR (e.g. `-cidr: 10.10.10.0/31`).
...
```batch
kubectl annotate ns <namespace> egress.projectcalico.org/namespaceSelector="projectcalico.org/name == '<egress_gw_namespace>'"
```
...
metadata:
  annotations:
    ...
    egress.projectcalico.org/namespaceSelector: projectcalico.org/name == '<egress_gw_namespace>'
  name: my-client,
