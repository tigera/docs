...
- [Upgrading egress gateways](#upgrading-egress-gateways)
...
advertise all other IP pools, which does not affect any functionality. However, Azure Route Servers have a [limit](https://learn.microsoft.com/en-us/azure/route-server/overview#route-server-limits) of 1000 advertisements per peers. This limit should not be an issue in a small or medium size cluster, but could be for large size cluster.
In order to prevent hitting the limit, it is possible to create [BGP filters](../configuring/bgp.mdx#configure-a-bgp-filter) to advertise only egress IP pools. As an example, for the IP pool created in the previous section, configure a BGP filter by running the following command:
...
In addition, to prevent {{prodname}} advertising pod IP blocks, it is possible to set `disableBGPExport` to `true` in the default IP pools.
...
Egress Gateway in Azure is supported from v3.16 where deployments are managed by the Tigera Operator.
