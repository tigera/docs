...
annotate a Kubernetes Namespace or Pod to use an egress gateway directly. While, in the second method, a namespace or pod is annotated to
use an Egress Gateway Policy resource which allows you to use an egress gateway for a specific destination or completely skip one.
...
- If no gateway is specified, then egress traffic won't be sent to an egress gateway, and instead would be routed locally.
This is helpful for reaching local endpoints that are not part of a cluster.
- If a gateway field is specified, it is mandatory to set both selector and namespaceSelector. Selector and namespaceSelector both have the same meaning
as explained in the previous section, except that setting namespaceSelector is mandatory.
...
To configure all the pods in a namespace to use an Egress Gateway Policy, for example one named `egw-policy1`, you would annotate that namespace like this:
...
the namespace or pod uses other egress annotations like the ones mentioned in the previous section.
:::
