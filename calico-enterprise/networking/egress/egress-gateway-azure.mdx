...
>**Note**: If the default BGP configuration resource does not exist, you need to create it first. See [BGP configuration](../../reference/resources/bgpconfig.mdx) for more information.
{: .alert .alert-info}
...
- [Domain-based policy](../../security/domain-based-policy) is not supported at egress from egress
...
> **Note**: Since v3.15.0, {{prodname}} also sends health probes to the egress gateway pods from the nodes where
