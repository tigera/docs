...
2.  Prepare this BGPConfiguration resource to [disable the full node-to-node mesh](bgp.mdx#disable-the-default-bgp-node-to-node-mesh):
...
3.  Prepare disabled IPPool resources for the CIDRs from which you will allocate stable
...
4.  Prepare an enabled IPPool resource for your default CIDR for pod IPs. For example:
...
5.  Prepare an EarlyNetworkConfiguration resource to specify the additional information
...
With OpenShift, also add a top level `platform: openshift` setting.
...
6.  Prepare a ConfigMap resource named "bgp-layout", in namespace "tigera-operator", that
...
For a full description, see [Early Network Configuration](../../reference/resources/earlynetworkconfiguration)
:::
