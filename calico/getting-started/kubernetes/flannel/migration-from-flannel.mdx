---
description: Migrate from Flannel to Calico Open Source while preserving the existing VXLAN data plane, gaining Calico IPAM and advanced policy.
---
...
- Cluster must allow for:
  ...
  - Modifying and deleting of the flannel daemon set. For example, it must not be installed using the Kubernetes add-on manager.
