...
Migrate an existing Kubernetes cluster with flannel/Canal to {variables.prodname} networking.

## Value

If you are already using flannel for networking, it is easy to migrate to {variables.prodname}'s native VXLAN networking. {variables.prodname} VXLAN is fully equivalent to flannel vxlan, but you get the benefits of the broader range of features offered by {variables.prodname} with an active maintainer community.
...
Migrating to {variables.prodname} IPAM solves these use cases and more. For advantages of Calico IPAM, see [Blog: Live Migration from Flannel to Calico](https://www.projectcalico.org/live-migration-from-flannel-to-calico/).

### Methods for migrating to {variables.prodname} networking

There are two ways to switch your cluster to use {variables.prodname} networking. Both methods give you a fully-functional {variables.prodname} cluster using VXLAN networking between pods.

- **Create a new cluster using {variables.prodname} and migrate existing workloads**

  If you have the ability to migrate workloads from one cluster to the next without caring about downtime, this is the easiest method: [create a new cluster using {variables.prodname}](../quickstart.mdx).
...
1. Install {variables.prodname}.
   ...
   {`kubectl apply -f {variables.manifestsUrl}/manifests/flannel-migration/calico.yaml`}
   </CodeBlock>

1. Start the migration controller.

   <CodeBlock language='bash'>
   {`kubectl apply -f {variables.manifestsUrl}/manifests/flannel-migration/migration-job.yaml`}
   </CodeBlock>
...
1. Delete the migration controller.
   ...
   <CodeBlock language='bash'>
   {`kubectl delete -f {variables.manifestsUrl}/manifests/flannel-migration/migration-job.yaml`}
   </CodeBlock>
...
If you need to revert a cluster from {variables.prodname} back to flannel, follow these steps.

1. Remove the migration controller and {variables.prodname}.

   <CodeBlock language='bash'>
   {`kubectl delete -f {variables.manifestsUrl}/manifests/flannel-migration/migration-job.yaml
   kubectl delete -f {variables.manifestsUrl}/manifests/flannel-migration/calico.yaml`}
   </CodeBlock>

1. Determine the nodes that were migrated to {variables.prodname}.
...
Learn about [{variables.prodname} IP address management](../../../networking/ipam/index.mdx)
