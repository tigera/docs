...
Since Amazon VPC CNI version 1.14 the CNI itself implements policies (in eBPF). This is not compatible with {{prodname}} network policy enforcement.
The [Amazon VPC CNI polices](https://docs.aws.amazon.com/eks/latest/userguide/cni-network-policy.html) must be disabled, otherwise {{prodname}} network policy enforcement would have undefined behaviour if Kubernetes policies are used as well.
