...
Install {variables.prodname} using the nftables data plane.
...
Installing {variables.prodname} in nftables mode provides a networking and network policy implementation that is compatible with the upstream nftables kube-proxy.
...
### Install {variables.prodname} in nftables dataplane mode
...
Congratulations! You now have a single-host Kubernetes cluster with {variables.prodname} in nftables mode.
