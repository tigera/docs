---
description: Install Calico using the tech-preview nftables data plane.
---
...
Try the tech-preview {{prodname}} nftables data plane.
...
{{prodname}} nftables dataplane is currently in technical preview and may not be suitable for production use. Please report any issues you experience
...
1. Initialize the control plane using the following command.
   ...
   ```
   sudo kubeadm init --pod-network-cidr=192.168.0.0/16 --config=config.yaml
   ```
