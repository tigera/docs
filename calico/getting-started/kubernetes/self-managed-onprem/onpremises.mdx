...
### {{prodname}} operator _recommended_
...
The **etcd** database is not recommended for new Kubernetes installs; some newer Kubernetes-targeted features (such as Service
matches in policy) are not supported with the etcd datastore. However, it is an option if you are running
{{prodname}} as the network plugin for both OpenStack and Kubernetes.
