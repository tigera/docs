...
1. Download the PowerShell script, **install-calico-windows.ps1**.
   ...
   ```powershell
   Invoke-WebRequest {{manifestsUrl}}/node/windows-packaging/install-calico-windows.ps1 -OutFile c:\install-calico-windows.ps1
   ```
...
```powershell
Invoke-WebRequest {{manifestsUrl}}/node/windows-packaging/Install-Containerd.ps1 -OutFile c:\Install-Containerd.ps1
...
```powershell
Invoke-WebRequest {{manifestsUrl}}/node/windows-packaging/PrepareNode.ps1 -OutFile c:\PrepareNode.ps1
