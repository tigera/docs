---
description: Install Calico Open Source with the VPP userspace data plane on a Kubernetes cluster.
---
...
import V3CRDMapVersion from '@site/calico/_includes/content/_v3-crd-map-version.mdx';
...
1. Now that you have an empty cluster configured, you can install the $[prodname] custom resource definitions.
...
4. Now is time to install the VPP data plane components.
...
5. Finally, time to add nodes to the cluster. Since we need to customize the nodes for DPDK, we will use an `eksctl` config file with the `preBootstrapCommands` property to create the worker nodes. The following command will create a managed nodegroup with 2 t3.large worker nodes in the cluster:
