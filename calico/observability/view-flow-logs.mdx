...
* Calico Ingress Gateway is installed, meaning a `GatewayAPI` resource exists on the cluster. See [Create an ingress gateway](../networking/ingress-gateway/create-ingress-gateway.mdx).
...
1. Wait for the Gateway to be programmed and read its address:
   ...
   calico-whisker-gateway   tigera-gateway-class   198.51.100.10   True
   ```
