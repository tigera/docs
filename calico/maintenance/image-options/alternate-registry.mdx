...
1. Retag the images with the name of your registry `$REGISTRY`.
   ...
   ```bash
   docker tag {{releases.0.tigera-operator.registry}}/{{releases.0.tigera-operator.image}}:{{releases.0.tigera-operator.version}} $REGISTRY/{{releases.0.tigera-operator.image}}:{{releases.0.tigera-operator.version}}
   {% for component in releases.0.components -%}
   {%- capture component_name %}{{ component[0] }}{% endcapture -%}
...
1. Push the images to your registry.
   ...
   ```bash
   docker push $REGISTRY/{{releases.0.tigera-operator.image}}:{{releases.0.tigera-operator.version}}
   {% for component in releases.0.components -%}
   {%- capture component_name %}{{ component[0] }}{% endcapture -%}
