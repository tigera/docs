...
policy](../security/get-started/kubernetes-policy/kubernetes-network-policy.mdx) guide.
...
policy](../security/get-started/calico-policy/calico-network-policy.mdx) guide.
...
![Example mix of network policy types](/img/calico/master/example-k8s-calico-policy-mix.svg)
...
out the {{prodname}} [policy for hosts](../security/hosts/index.mdx) guides.
...
deny](../security/get-started/kubernetes-default-deny.mdx) network policies. These ensure that if no other policy is
...
![Example tiers](/img/calico/master/example-tiers.svg)
