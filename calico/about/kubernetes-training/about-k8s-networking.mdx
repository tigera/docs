...
This guide provides optional background education, not specific to {variables.prodname}.
...
{variables.prodname} provides both network and IPAM plugins, but can also integrate and work seamlessly with some other CNI
...
this in the {variables.prodname} [determine best networking option](/networking/determine-best-networking.mdx)
guide.
...
Note that when using {variables.prodname} in on-prem deployments you can also [advertise service IP
addresses](/networking/configuring/advertise-service-ips.mdx), allowing services to be conveniently accessed without
...
When using {variables.prodname}, depending on your environment, you can generally choose whether you prefer to run an
overlay network, or prefer to have fully routable pod IPs. You can read more about this in the {variables.prodname}
[determine best networking option](/networking/determine-best-networking.mdx) guide. {variables.prodname} also
