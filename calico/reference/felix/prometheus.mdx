...
This should match the number of endpoints and netowrk sets in your cluster.
...
selectors should be calculated efficiently even clusters with hundreds of thousands of endpoints.  In general the
CPU used to calculate them should be proportional to the number of endpoints that match them and the churn rate of
_those_ endpoints.

The `optimized="false"` total tracks the number of selectors that could not be optimized.  Those selectors are
much more costly to calculate and the CPU used to calculate them is proportional to the total number of endpoints
in the cluster and the churn rate of all of those.  It is generally OK to have a handful of unoptimized selectors,
but if many selectors are unoptimized the CPU usage can be substantial at high scale.

For more information on writing optimized selectors, see the [EntityRule section](../resources/networkpolicy.mdx#entityrule)
of the NetworkPolicy reference.
...
If `felix_label_index_selector_evals{result="false"}` is many times `felix_label_index_selector_evals{result="true"}`
then it is likely that some selectors in the policy set are not being optimized effectively.
...
as follows (where endpoint means "endpoint or network set" and "parent" is Felix's internal term for resources such
as Kubernetes Namespaces)

- `endpoint|parent-all`: the least efficient fall back strategy for unoptimized selectors. The index
...
- `endpoint|parent-single-value`: the label index was able to limit the scan to only those endpoints/parents that
  has a particular label and value combination.  For example, selector `label == "value"` would only scan items that
...
- `endpoint|parent-multi-value`: the label index was able to limit the scan to only those endpoints/parents that
  has a particular label and one of a few values.  For example, selector `label in {"a", "b")` would only scan items
