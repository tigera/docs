...
If left unset, the {{prodname}} CNI plugin will look for the directory at `/var/run/calico/endpoint-status`. The path `/var/run/calico` is commonly mounted to the {{prodname}} DaemonSet, meaning it can be written to by {{prodname}} containers, and read by the {{prodname}} CNI-plugin, running in the host namespace.
To enable the `endpoint-status` directory, and adjust which {{prodname}} container directory it is written to, the `endpointStatusPathPrefix` option must be configured for [Felix](./felix/configuration.mdx).

To adjust where the {{prodname}} CNI plugin looks for {{prodname}}'s `endpoint-status` directory in the host filesystem, you must set the `endpoint_status_dir` option.
