...
The {variables.prodname} CNI plugins do not need to be configured directly when installed by the operator.
...
The following option allows configuration of the {variables.prodname} datastore type.

- `datastore_type` (default: etcdv3)

The {variables.prodname} CNI plugin supports the following datastore types:
...
To enable the `endpoint-status` directory, and adjust which directory of the Felix container it is written to, the `endpointStatusPathPrefix` option must be configured for [Felix](./felix/configuration.mdx).
