...
By default, the Windows firewall blocks listening on ports. To have {{prodname}} manage the Prometheus metrics ports Windows firewall rules, enabled the `windowsManageFirewallRules` setting in FelixConfiguration:

```bash
kubectl patch felixConfiguration default --type merge --patch '{"spec":{"windowsManageFirewallRules": Enabled}}'
```

[See here for more details](../../reference/resources/felixconfig.mdx). You can also add a Windows firewall rule allowing listening on the Prometheus metrics port instead of having {{prodname}} manage it.
