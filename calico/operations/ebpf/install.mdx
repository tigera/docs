...
You can follow the default [Operator installation](../../getting-started/kubernetes/quickstart) if your cluster meet the following requisites:
...
To enable eBPF mode, add a `calicoNetwork` section inside the `spec` with the `bpfInstallMode` and `linuxDataplane` fields. 
