...
`MKE` supports a number of base OSes; as long as the base OS chosen (such as Ubuntu 20.04 or RHEL 8.2) meets the kernel
requirements, `MKE`-provisioned clusters are supported.

Since `kube-proxy` is not required in eBPF mode, you may wish to disable `kube-proxy` at install time. With `MKE` you
can do that by setting the following when installing the cluster.

```bash
docker container run --rm -it --name ucp mirantis/ucp:<version> install --kube-proxy-mode=disabled --kube-default-drop-masq-bits
```

More details can be found at https://docs.mirantis.com/mke/3.7/install/predeployment/configure-networking/cluster-service-networking-options.html
</TabItem>
