...
To inspect $[prodname]'s internal datastructures, you can use the `calico-node -bpf` tool.
...
## Check if $[prodname] is dropping packets
  ...
  `calico-node -bpf` tool to check the BPF counters. Since the eBPF dataplane is split
...
capabilities of the verifier vary between kernel versions. We do test the eBPF programs with a range of kernels, but it is impossible to test all kernels. You may see errors such as if the verifier rejects a program:
...
- Check if `kube-proxy` is still running. If `kube-proxy` is still running, you must either disable `kube-proxy` or
  ...
  still running, Fielix will continually try to remove the rules, which can cause high CPU usage.
