...
- In eBPF mode, forwarding external client access to services (typically NodePorts) from node to node is implemented using VXLAN encapsulation.
...
# The `calico-node -bpf` tool
  ...
  To inspect $[prodname]'s internal data structures, you can use the `calico-node
  -bpf` tool.  The tool is embedded in the $[nodecontainer] container image and
  displays information about the eBPF data plane from within a `calico-node` pod
  only. Use `kubectl get pod -o wide -n calico-system` to find the name of a
  `calico-node` pod and use the name in the following commands instead of
  `<calico-node-name>`.
...
## Check if $[prodname] is dropping packets
  ...
  `calico-node -bpf` tool to check the BPF counters. Since the eBPF data plane is split
  into programs that are attached to interfaces, you must check the counters on
  the relevant interface. You can either dump counters for all interfaces or use `--iface=<interface name>` to dump
...
`calico-node -bpf` tool to dump the policy that is attached to a specific interface.
...
`tc/tcx` egress hook of the host-side of the caliX veth pair, while ingress
policy for host endpoints is attached to the `tc/tcx` ingress hook of the host
...
capabilities of the verifier differ between kernel versions. We do test the eBPF programs with a range of kernels, but it is impossible to test all kernels. You may see errors such as if the verifier rejects a program:
...
Setting `bpfProfiling` to `Enabled` enables collection of runtime profiling data
...
profiling data can be examined using the `calico-node -bpf profiling
e2e` command. The command resets the profiling data after dumping it.
...
- Check if `kube-proxy` is still running. If `kube-proxy` is still running, you must either disable `kube-proxy` or
  ...
  still running, Felix will continually try to remove the rules, which can cause high CPU usage.
