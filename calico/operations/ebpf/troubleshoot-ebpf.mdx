...
To inspect $[prodname]'s internal data structures, you can use the `calico-node -bpf` tool.
...
## Check if $[prodname] is dropping packets
  ...
  the relevant interface. You can either dump counters for all interfaces or use `--iface=<ifacename>` to dump
...
Release 3.31 introduced runtime profiling of the eBPF programs. The profiling data can be examined using the
`calico-node -bpf profiling e2e` command. The command resets the profiling data after dumping it.
