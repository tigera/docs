...
- [(MKE only) Disable host conntrack bypass](#disable-host-conntrack-bypass)
...
2. If you are running MKE, follow the step procedure in [Modify and existing MKE configuration](https://docs.mirantis.com/mke/current/ops/administer-cluster/configure-an-mke-cluster/use-an-mke-configuration-file.html#modify-an-existing-mke-configuration) to download, edit, and upload your MKE configuration. During the editing step, add the following configuration:
`kube_proxy_mode` to `iptables` and `kube_default_drop_masq_bits ` to `false`   ```
