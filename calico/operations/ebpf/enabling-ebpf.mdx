...
- [Disable host conntrack bypass in MKE](#disable-host-conntrack-bypass-in-MKE)
...
- MKE runs a reverse proxy in each node, which can be used to reach the API server. You should use `proxy.local`
...
Download the cluster configuration as mentioned in https://docs.mirantis.com/mke/3.7/ops/administer-cluster/configure-an-mke-cluster/use-an-mke-configuration-file.html#modify-an-existing-mke-configuration.

Change the `kube_proxy_mode` to `disabled` and `kube_default_drop_masq_bits ` to `true`

Upload the modified cluster configuration as mentioned in the above the link.
...
### Disable host conntrack bypass in MKE

If you running MKE, host conntrack should not be bypassed. This can be done with `kubectl` as follows:

```
kubectl patch felixconfiguration default --patch='{"spec": {"bpfConntrackBypass": false}}'
```
...
1. If you disabled `kube-proxy`, re-enable it (for example, by removing the node selector added above).
   ...
   ```
2. If you are running MKE, download the cluster config as mentioned above, set `kube_proxy_mode` to `iptables` and `kube_default_drop_masq_bits ` to `false`
