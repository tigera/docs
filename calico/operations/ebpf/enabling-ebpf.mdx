...
- VLAN-based traffic without hardware off-loading.
  :::tip
  You can use a VLAN device to connect a node to the cluster by using the `bpfDataIfacePattern` Felix configuration variable if the main device supports VLAN off-loading.
