...
You can choose the automatic eBPF configuration if your cluster meet the following requisites:
 ...
 - Operator can access kubeproxy service and endpoints.

#### Configuring Installation CR

To enable eBPF mode with the auto configuration, change the `spec.calicoNetwork.bpfInstallMode` parameter in the operator's `Installation`
  resource to "Auto", and the `spec.calicoNetwork.linuxDataplane` parameter to "BPF".

```bash
  kubectl patch installation.operator.tigera.io default --type merge -p '{"spec":{"calicoNetwork":{"bpfInstallMode":"Auto","linuxDataplane":"BPF"}}}'
```
