...
Configure {variables.prodname} to pull images from a registry (public or private).

## Value

In many deployments, installing {variables.prodname} in clusters from third-party private repos is not an option. {variables.prodname} offers these public and private registry options, which can be used in any combination:

- **Install from a registry** for use cases like air-gapped clusters, or clusters with bandwidth or security constraints
- **Install from an image path in a registry** if you have pulled {variables.prodname} images to a sub path in your registry
...
- {variables.prodname} is managed by the operator
...
- Use the [Crane command](https://github.com/google/go-containerregistry/blob/main/cmd/crane/README.md) to manage the {variables.prodnameWindows} images, as using `docker pull` commands for these images may not work depending on the operating system and version in which the commands are run.
...
### Push {variables.prodname} images to your registry

To install images from your registry, you must first pull the images from Tigera's registry, retag them with your own registry, and then push the newly-tagged images to your own registry. Use the `crane cp` command instead of pulling+retagging+pushing on the {variables.prodnameWindows} images (`node-windows` and `cni-windows`).
