...
This page describes how to upgrade to {{version}} from {variables.prodname} v3.15 or later. The
...
Starting in {variables.prodname} v3.28, a change in the way UIDs are generated for projectcalico.org/v3 resources requires that you update any OwnerReferences
...
Prior to release v3.23, the {variables.prodname} helm chart itself deployed the `tigera-operator` namespace and required that the helm release was
...
When upgrading from a version of {variables.prodname} v3.22 or lower to a version of {variables.prodname} v3.23 or greater, you must complete the following steps to migrate
...
1. Install the helm chart in the `tigera-operator` namespace.
   ...
   ```
   helm install {variables.prodnamedash} projectcalico/tigera-operator --version {{releaseTitle}} --namespace tigera-operator
   ```
...
1. Download the {{version}} manifest that corresponds to your original installation method.
   ...
   **{variables.prodname} for policy and networking**
   ...
   ```bash
   curl {variables.manifestsUrl}/manifests/calico.yaml -o upgrade.yaml
   ```

   **{variables.prodname} for policy and flannel for networking**

   ```bash
   curl {variables.manifestsUrl}/manifests/canal.yaml -o upgrade.yaml
   ```

   **{variables.prodname} for policy (advanced)**

   ```bash
   curl {variables.manifestsUrl}/manifests/calico-policy-only.yaml -o upgrade.yaml
   ```
...
1. Congratulations! You have upgraded to {variables.prodname} {{version}}.
...
1. Download the {{version}} manifest that corresponds to your original installation method.
   ...
   **{variables.prodname} for policy and networking**
   ...
   ```bash
   curl {variables.manifestsUrl}/manifests/calico-etcd.yaml -o upgrade.yaml
   ```

   **{variables.prodname} for policy and flannel for networking**

   ```bash
   curl {variables.manifestsUrl}/manifests/canal-etcd.yaml -o upgrade.yaml
   ```
