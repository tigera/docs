...
:::note[Your CRD mode is preserved]

Upgrading does not change your cluster's CRD mode. A cluster using the aggregation API server (v1 CRDs) stays on the aggregation API server; a cluster using native v3 CRDs stays on v3. The operator selects the mode from the CRDs present and never switches an existing cluster. To move from the aggregation API server to native v3 CRDs, see [Migrate to native v3 CRDs](../crd-migration.mdx).
...
1. To enable the flow logs API and Calico Whisker (introduced in version 3.30), apply the `Goldmane` and `Whisker` custom resources.
   :::tip
   If you plan to use the observability tools in [Calico Cloud Free Tier](/calico-cloud/free/overview), you need to enable Goldmane to provide flow logs to the console.
   :::
   ```bash
   kubectl apply -f - <<EOF
