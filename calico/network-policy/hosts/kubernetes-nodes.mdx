...
Template host endpoints are similar to automatic host endpoints, but with greater degree of customization. They are created automatically by the calico kube-controllers according to the `spec.controllers.node.hostEndpoint.templates`.
You might want to consider using template host endpoints if you want automatic host endpoints to be created for certain Nodes only, or only for certain Node interfaces.
...
2. Merge the templates spec with kubecontrollersconfiguration

```bash
kubectl patch kubecontrollersconfiguration default --patch-file hostEndpointTemplate.yaml --type merge
```

3. Verify that your kubecontrollersconfiguration is updated
