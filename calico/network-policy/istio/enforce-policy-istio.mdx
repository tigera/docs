...
This tutorial sets up a microservices application, then demonstrates how to use {variables.prodname} application layer policy to mitigate some common threats.
...
2. Install {variables.prodname} on Kubernetes:

- If {variables.prodname} is not installed on Kubernetes, see [Calico on Kubernetes](../../getting-started/kubernetes/quickstart.mdx).
- If {variables.prodname} is already installed on Kubernetes, verify that [Calico networking](../../networking/index.mdx) (or a non-Calico CNI) and {variables.prodname} network policy are installed.
...
We will use a simple microservice application to demonstrate {variables.prodname}
...
{`kubectl apply -f ${variables.tutorialFilesURL}/10-yaobank.yaml`}
</CodeBlock>
...
Although {variables.prodname} & Istio are running in the cluster, we have not defined any authentication
...
We can mitigate both of the above deficiencies with a {variables.prodname} policy.

<CodeBlock language='bash'>
{`wget ${variables.tutorialFilesURL}/30-policy.yaml
