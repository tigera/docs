...
Typha is a {variables.prodname} component which improves scalability and reduces the impact that
large clusters may have on the Kubernetes API. Typha agents must accept connections from other agents on a fixed port.

As part of the {variables.prodname} bootstrap infrastructure, Typha must be available before
