...
### Calico Open Source X.Y.0 early preview release

DD M YYYY

### Calico Open Source X.Y.1 general availability release
...
Adds HPC-based (HostProcess/Privileged containers) install and upgrade for Windows nodes. {{prodname}} will bootstrap Windows worker nodes just like Linux worker nodes. Platform engineers can 1) scale their Windows worker nodes with an auto-scaling node pool without having to bootstrap each node manually and 2) upgrade {{prodname}} deployments without having to manually upgrade each node one at a time.
