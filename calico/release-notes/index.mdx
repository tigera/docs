---
description: Release notes for Calico Open Source
---

import variables from '@site/calico/variables';

# Calico Open Source 3.27 release notes

Learn about the new features, bug fixes, and other updates in this release of {variables.prodname}.

## New features and enhancements

## Technology preview features

## Deprecated and removed features

* In {variables.prodname} 3.28, the Felix configuration parameter `BPFKubeProxyEndpointSlicesEnabled` is deprecated and is planned to be removed in a future release.
  This function no longer needs to be enabled. Now, BPF proxies always accept endpoint slices.

## Bug fixes

<!--
## Security fixes
-->
## Known issues

## Release details

### Calico Open Source 3.28.0 general availability release

DD November 2023

Calico Open Source release 3.28.0 is now generally available.

<!--
### Calico Open Source 3.28.1 bux fix update

DD M YYYY

#### Enhancements

#### Bug fixes

#### How to update

To update an existing installation of Calico Enterprise 3.18, see .

-->
