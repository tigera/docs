...
#### Host process container support in Windows GA

Calico now supports host process containers in Windows. This means that Windows installations no longer need to use our scripts but can instead be installed and managed similar to using native Kubernetes resources.

 - Add new 'windowsManageFirewallRules' field to FelixConfiguration to enable Felix to manage Windows firewall rules needed for prometheus reporting. [calico #8173](https://github.com/projectcalico/calico/pull/8173) (@coutinhop)
 - Add support for Windows host process containers (HPC), with Calico for Windows daemonsets managed by kubernetes. [calico #7857](https://github.com/projectcalico/calico/pull/7857) (@coutinhop)
...
#### IPv6 support in the eBPF dataplane

Calico now supports IPv6 in the eBPF dataplane. This feature is in tech preview. Please visit the documentation for limitations and set up instructions.

 - BPFLogFilters allow specifying fine grained log filters that reduce the amount of logs emitted when debugging. [calico #7607](https://github.com/projectcalico/calico/pull/7607) (@tomastigera)
 - ebpf: only a tiny preamble program is loaded per endpoint device and the rest is executed by generic programs loaded just once. That saves load time due to faster verification and saves memory as the same programs are not loaded many times. [calico #7219](https://github.com/projectcalico/calico/pull/7219) (@tomastigera)
...
The VPP dataplane has been validated in Calico and is ready for general use. Please see our press release with the VPP team for more details.
