...
in all active rules.  This calculation is optimized for certain selector types.
Using the optimized selector types can reduce CPU usage (and policy rendering time) by orders of magnitude.
This becomes important at very high scale (hundreds of active rules, hundreds of thousands of endpoints).
...
- `label in { 'v1', 'v2' }`
- `has(label)`
- `<expresion 1> && <expression 2>` is as optimized if **either** of `<expresion 1>` or `<expression 2>` are
  optimized.

The following are partially optimized:
