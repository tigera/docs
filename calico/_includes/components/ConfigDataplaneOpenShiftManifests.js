...
sed -i "s|<base_domain>|$BASE_DOMAIN|g" ${props.folderName}/01-configmap-kubernetes-services-endpoint.yaml`}
  ...
  template:
    spec:
      ...
      <p>
        As of <em>v3.30</em>, Calico uses the eBPF data plane by default on OpenShift. If you prefer to install Calico with Iptables, you can configure it by running the following command:
      </p>
      <CodeBlock language='bash'>
