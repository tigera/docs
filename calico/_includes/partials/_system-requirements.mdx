...
We recommend using OpenStack Caracal, and our active support and testing of
Calico $[version] with OpenStack is with Caracal.

We generally update our coding and testing every fourth OpenStack release, and
hence the next increment of support in our own coding will be for OpenStack
Gazpacho.  It's possible that Calico will work with intervening releases, but
we do not test that and so cannot say for sure.

Looking back, Calico should also work with any older OpenStack release
(i.e. older than the currently recommended one) that can be run under Python 3.
</If>
