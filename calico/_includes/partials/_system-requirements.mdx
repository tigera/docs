...
Looking back, Calico should also work with any older OpenStack release that can
be run under Python 3.

Looking forwards, Calico might work with a newer OpenStack release, but we
don't know.

We generally update our recommended OpenStack version every fourth OpenStack
release, and hence the next increment of support in our own coding and testing
will be for OpenStack Gazpacho.
