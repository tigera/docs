...
[Upgrade notes](#upgrade-notes-for-existing-deployments) below.
...
- `resync_interval_secs` and `resync_max_interval_secs` now have no effect.
  You can leave them in `neutron.conf` (they will not cause an error), but you
  can also remove them.  If you previously relied on the periodic resync to
  recover from drift, use the startup resync (the default) and run
  `calico-resync` on demand when you need an immediate reconciliation.
