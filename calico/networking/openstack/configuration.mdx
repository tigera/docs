...
The `[calico] startup_resync` option controls whether the $[prodname] driver
reconciles the Neutron database into etcd when Neutron server starts.  See
[Resync between Neutron and etcd](resync.mdx) for details.

| Setting        | Default Value | Meaning                                                                                  |
| -------------- | ------------- | ---------------------------------------------------------------------------------------- |
| startup_resync | always        | Whether to run a full Neutron DB &rarr; etcd resync when Neutron server starts. Set to `never` to suppress it. |

:::note

The `resync_interval_secs` and `resync_max_interval_secs` options that
configured the old periodic resync are deprecated and have no effect.  See
[Resync between Neutron and
etcd](resync.mdx#upgrade-notes-for-existing-deployments).
