...
networking-calico works with Kuryr; this means using Neutron, with the {variables.prodname}
ML2 driver, to provide networking for container workloads.

You can use DevStack to install a single node {variables.prodname}/Kuryr system, with a
...
if you look at the IP routing table and iptables, you will see {variables.prodname} routes to
the containers.

## {variables.prodname} for containers without Kuryr

The {variables.prodname} project also provides networking for containers more directly,
without Neutron and Kuryr as intermediaries. Please see [Getting
Started](../../getting-started/index.mdx) for details.
