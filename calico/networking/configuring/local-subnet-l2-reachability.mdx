...
Traditionally the only way to make these IPs reachable is to [advertise them over BGP](advertise-service-ips.mdx), which requires peering with your network infrastructure. In flat L2 environments — bare-metal labs, edge deployments, `kind` clusters, or networks where you cannot configure the upstream router — BGP may not be an option.
...
calicoctl patch felixconfiguration default --patch \
   '{"spec": {"localSubnetL2Reachability": "PodsAndLoadBalancers"}}'
```
