...
The values of the QoS controls annotations may contain suffixes, such as `k`, `M`, `G`, `T`, `P`, or `E` to represent large numbers. Binary suffixes are also supported, for example `1Ki` represents "1024", and so on. For more details, see the [Quantity type of the resource package](https://pkg.go.dev/k8s.io/apimachinery/pkg/api/resource#Quantity).
...
To configure bandwidth limits for traffic going into or coming out of the pod,  use the `qos.projectcalico.org/ingressBandwidth` and `qos.projectcalico.org/ingressBurst` annotations for ingress, or `qos.projectcalico.org/egressBandwidth` and `qos.projectcalico.org/egressBurst` for egress, to configure bandwidth limits and burst sizes respectively. Limiting the bandwidth enforces that, on average, that will be the maximum bit rate used. Limiting the burst enforces that at any given instant, the maximum burst traffic will not go over the burst limit. Bandwidth values are in bps (bits per second) and constrained from a minimum of `1kbps` to a maximum of `1Pbps` and burst values are in bits and constrained from a minimum of the value assigned to bandwidth in bits to a maximum of `4Gib` (2<sup>32</sup> bits). Burst configuration is optional, if burst is not explicitly configured with an annotation, a default value of `4Gib` is used.
...
To configure packet rate limits for traffic going into or coming out of the pod, use the `qos.projectcalico.org/ingressPacketRate` annotation for ingress, or `qos.projectcalico.org/egressPacketRate` for egress. Limiting the packet rate enforces that, on average, that will be the maximum rate that packets will be sent and/or received by the pod. Packet rate values are in packets per second and constrained from a minimum of `10` to a maximum of `1T` (10<sup>12</sup>) packets per second.

### Configure established connection limits

To configure limits for the number of established connections into or out of a pod, use the `qos.projectcalico.org/ingressMaxConnections` annotation for ingress, or `qos.projectcalico.org/egressMaxConnections` for egress. Limiting the number of connections enforces that the pod cannot accept or initiate more connections that the specified limits. The values are in absolute numbers of simultaneous established connections and constrained from a minimum of `1` to a maximum of 100 * 10<sup>9</sup> (`100G`) connections.
