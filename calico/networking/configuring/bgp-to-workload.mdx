...
The [BGP Guide](./bgp.mdx) provides a general overview of these concepts.
...
Each of the ToR, parent cluster, and nested cluster must have a unique AS number.
...
- If using the IPv6 address, you _must not_ use a link local address (because
  ...
  currently ahs no way to provision the correct scopes). Instead, we recommend
...
Before creating the BGPPeer to peer with the workloads, we create
...
- Because the workloads are running on the $[prodname] cluster, we can assume that
  ...
  need to export any routes to the workloads. Create the following `BGPFilter`,
...
- For added security, it is also wise to limit what routes we'll accept from
  the workloads. Create a `BGPFilter` that only accepts routes that are within
...
Include the BGPFilter defined above in the `BGPPeer` used to peer with the ToR:
...
- Include the `BGPFilter` resources defined above in the `BGPPeer` to control
...
- Ensure that the node-to-node mesh is disabled in the `BGPConfiguration` and
...
- Create a `BGPPeer`(s) that peer with the parent clusters' virtual IP(s):
...
- In the parent cluster: creating a new import `BGPFilter` for each cluster to import its routes.
- In the parent cluster: creating a new, uniquely-named `BGPPeer` for each cluster to peer with the
  parent cluster. You can re-use the shared "no-export" BGPFilter with every nested cluster peering.
- In the parent cluster: updating the `export-to-tor` `BGPFilter` to include the new cluster's CIDRs.
