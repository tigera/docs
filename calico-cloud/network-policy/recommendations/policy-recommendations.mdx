...
Namespaced services with local cluster domains will only generate rules for the service’s namespace. However, if your cluster is a managed cluster instance in an MCM environment, you may also get egress-to-domain rules if the service name suffix isn't the default (svc.cluster.local). It is valid to have a policy with both egress-to-domain and egress-to-namespaces rules that allow the same traffic.
