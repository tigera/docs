...
### Why are egress-to-domain rules being generated for a namespaced service?

Namespaced services with local cluster domains will only generate rules for the service’s namespace. However, if your cluster is a managed cluster instance in an MCM environment, you may also get egress-to-domain rules if your cluster domain differs from the default (cluster.local). It is valid to have a policy with both egress-to-domain and egress-to-namespaces rules that allow the same traffic.
