...
The solution is to define policy for workload services, not for ports used by workloads. For help, see [Policy for services](/network-policy/beginners/services/index.mdx).
...
For help with Pass action rules, see [Get started with tiered policy](/network-policy/policy-tiers/tiered-policy.mdx).
