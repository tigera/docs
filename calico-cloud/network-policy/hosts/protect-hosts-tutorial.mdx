...
allowed by the [failsafe rules](/reference/host-endpoints/failsafe). Because there is no default-deny
...
(except for any [failsafe rules](/reference/host-endpoints/failsafe).
For example, if the
