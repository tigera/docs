...
- [Install FortiGate API key and FortiManager password as secrets](#install-fortigate-api-key-and-fortimanager-password-as-secrets)
...
### Install FortiGate API key and FortiManager password as secrets

1. Store each FortiGate API key as a secret in the `tigera-firewall-controller` namespace.
   For example, the FortiGate device, `prod-east1`, store its API key as a secret name as `fortigate-east1`, with key as `apikey-fortigate-east1`.
