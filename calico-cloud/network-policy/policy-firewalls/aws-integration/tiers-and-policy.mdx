...
- Although you cannot hide specific tiers from non-admin users, you can use [RBAC for tiered policies](/network-policy/policy-tiers/rbac-tiered-policies), to display only a subset of tiers in the {{prodname}} UI. For help, see [displaying only the net-sec tier](/network-policy/policy-tiers/rbac-tiered-policies#user-can-only-view-a-specific-tier).
- To display tiers and associated network policies, but disable write access to those tiers, see [RBAC example fine-grained permissions](/network-policy/policy-tiers/rbac-tiered-policies#fine-grained-rbac-for-policies-and-tiers).
...
- [Enable pod access to AWS metadata](/network-policy/policy-firewalls/aws-integration/metadata-access)
