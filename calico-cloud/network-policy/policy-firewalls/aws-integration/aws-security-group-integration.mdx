...
- {{prodname}} non-cluster hosts, and auto-hostendpoint features. Alternatives to secure host endpoints:
  ...
  - Use {{prodname}} [GlobalNetworkSets](/reference/resources/globalnetworkset) and use custom labels for selection in policy.
...
- Networking provider is [Amazon VPC Networking](/reference/public-cloud/aws#using-aws-networking).
...
- You are not using the {{prodname}} [auto hostendpoints feature](/network-policy/hosts/kubernetes-nodes), and have not created any [host endpoints](/reference/resources/hostendpoint) that have a `spec.node` value that matches any of your Kubernetes nodes.
...
- [Configure fine-grained access control between VPC endpoints and pods](/network-policy/policy-firewalls/aws-integration/tiers-and-policy)
