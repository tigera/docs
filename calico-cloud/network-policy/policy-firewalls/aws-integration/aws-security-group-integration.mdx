...
- You are not using the {{prodname}} [auto hostendpoints feature](/network-policy/hosts/kubernetes-nodes.mdx), and have not created any [host endpoints](/reference/resources/hostendpoint.mdx) that have a `spec.node` value that matches any of your Kubernetes nodes.
...
- [Configure fine-grained access control between VPC endpoints and pods](/network-policy/policy-firewalls/aws-integration/tiers-and-policy.mdx)
