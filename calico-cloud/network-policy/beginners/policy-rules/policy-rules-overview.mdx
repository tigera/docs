...
Read [Get started with Calico policy](/network-policy/beginners/calico-network-policy.mdx) and [Kubernetes policy](/network-policy/get-started/kubernetes-network-policy.mdx), which cover all the basics of using label selectors in policies to select endpoints the policies apply to, or in policy rules.
