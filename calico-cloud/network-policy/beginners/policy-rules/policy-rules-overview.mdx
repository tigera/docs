...
Read [Get started with Calico policy](/network-policy/beginners/calico-network-policy) and [Kubernetes policy](/tutorials/kubernetes-tutorials/kubernetes-network-policy/), which cover all the basics of using label selectors in policies to select endpoints the policies apply to, or in policy rules.
