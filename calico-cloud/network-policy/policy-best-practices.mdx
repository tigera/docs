...
spec:
  ...
  egress:
    ...
    - action: Allow
      ...
      destination:
        ports: ['5432']
        domains: ['*.postgres.database.azure.com']
    - action: Allow
      protocol: TCP
      source: {}
      destination:
        ports: ['443', '80']
        domains: ['*.logic.azure.com']
    - action: Allow
      protocol: TCP
      source: {}
      destination:
        ports: ['443', '80']
        domains: ['*.azurewebsites.windows.net']
    - action: Allow
      protocol: TCP
      source:
        selector: 'app in { "call-archives-api" }||app in { "finwise" }'
      destination:
        domains: ['*.documents.azure.com']
    - action: Allow
      protocol: TCP
      source:
      ...
      destination:
        ports: ['1433']
        domains: ['*.database.windows.net']
    - action: Allow
      source:
        nets: ['65.132.216.26/32', '10.10.10.1/32']
        ports: ['80', '443']
      destination: {}
    - action: Allow
      source:
