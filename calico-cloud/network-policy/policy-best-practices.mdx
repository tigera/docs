...
Because {{prodname}} policy goes well beyond the features in Kubernetes policy, we recommend that you have a basic understanding of [network policy and global network policy](/network-policy/beginners/calico-network-policy.mdx) and how they provide workload access controls. And even though you may not implement the following policies, it is helpful to know the depth of defense that is available in {{prodname}}.

- [Policy for services](/network-policy/beginners/services/index.mdx)
- [Policy integration for firewalls](/network-policy/policy-firewalls/index.mdx)
- [Policy for hosts](/network-policy/hosts/index.mdx)
...
To understand how tiered policy works and best practices, see [Get started with tiered policies](/network-policy/policy-tiers/tiered-policy.mdx).
...
For network set tutorial and best practices, see [Get started with network sets](/network-policy/networksets.mdx).
...
But, be sure to understand the [best practices for creating a default deny policy](/network-policy/default-deny.mdx) to avoid breaking your cluster.

Here are sample [default deny policies](/network-policy/beginners/kubernetes-default-deny.mdx).
...
For examples, see [basic ingress and egress policies](/network-policy/beginners/calico-network-policy.mdx).
...
Network sets allow you to specify CIDRs and/or domains. As noted in [Network set best practices](/network-policy/policy-best-practices.mdx), we do not recommend putting large CIDRs or domains directly in policy. Although nothing stops you from do this in policy, using network sets is more efficient and supports scaling.
...
- [Troubleshoot policies](/network-policy/policy-troubleshooting.mdx)
