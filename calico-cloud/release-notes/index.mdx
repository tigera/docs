...
![tech-preview](/img/calico-cloud/master/tech-preview.svg)
...
To get started see, [Container Threat Detection](/threat/container-threat-detection)

## September 26, 2022

### New feature: Helm

{{prodname}} now supports [installation using Helm](/get-started/connect/install-cluster).
...
To get started see, [Image Assurance](/image-assurance/scan-image-registries).
...
To help meet your compliance requirements, we've added documentation to export logs to a SIEM (syslog, Splunk, or Amazon S3). See [Export logs to a SIEM](/visibility/elastic/archive-storage).
...
{{prodname}} has made the configuration and deployment of anomaly detection jobs for threat detection and performance hotpots more granular, allowing you to selectively enable jobs depending on your use case. See [Detect and alert on anomalies](/threat/security-anomalies).
