...
The default view in Service Graph provides a representation of how namespaces within the cluster are communicating with each other. Each namespace is represented with the standard Kubernetes icon for namespaces. There are some demo namespaces - *hipstershop*, *storefront*, and *acme* - and other Tigera namespaces that are used for {{site.prodname}}. Egress traffic leaving the cluster is represented with globe icons. Under the hood, each of these globe icons is a custom resource called a **NetworkSet**.
...
> Click on the edge between the *hipstershop* namespace, and the *public network* network set. The flow logs are automatically filtered below in the Flows tab, and you can expand any of the rows in that tab to view some of the detailed metadata that {{site.prodname}} provides around workload communication.
...
The layer called **Tigera components** contains namespaces related to {{site.prodname}}.  Click the ellipsis/spillover menu on the right, and select **Hide layer**. 
...
- [Understanding policy tiers]({{site.baseurl}}/get-started/tutorials/policy-tiers)
- [Understanding network sets]({{site.baseurl}}/get-started/tutorials/networksets)