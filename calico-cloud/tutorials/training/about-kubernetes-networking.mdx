...
- Pods can communicate with all other pods in the cluster using pod IP addresses (without
  [NAT]({{site.baseurl}}/get-started/training/about-networking))
...
Kubernetes network implementations use [overlay networks]({{site.baseurl}}/get-started/training/about-networking).
...
overlay network, or prefer to have fully routable pod IPs. {{site.prodname}} also
