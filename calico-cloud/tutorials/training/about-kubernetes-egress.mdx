...
In contrast to ingress traffic, where Kubernetes has the [Ingress](/get-started/training/about-kubernetes-ingress.mdx)
...
achieved using [Network Policy](/get-started/training/about-network-policy.mdx) to define egress rules for each
microservice, often in conjunction with a [default deny](/get-started/training/about-network-policy.mdx#default-deny)
...
the new IP addresses. This limitation can be circumvented using {{prodname}} [Use external IPs or networks rules in policy](/security/beginners/external-ips-policy/), or [DNS policy](/security/beginners/domain-based-policy/) in policy rules.
...
{{prodname}}, the NAT behavior can be configured at a more granular level for particular address ranges using [IP
pools](/reference/resources/ippool). This effectively allows the scope of "non-routable" to be more
...
- [Use external IPs or networks rules in policy](/security/beginners/external-ips-policy/)
- [Restrict a pod to use an IP address in a specific range](/networking/ipam/legacy-firewalls/)
- [Assign IP addresses based on topology](/networking/ipam/assign-ip-addresses-topology/)
- [Egress gateways](/networking/egress/)
