...
In contrast, the [Kubernetes network model](/get-started/training/about-kubernetes-networking.mdx) defines a "flat"
...
policy](/get-started/training/tutorials/kubernetes-network-policy/) guide.

## {{prodname}} network policy

In addition to enforcing Kubernetes network policy, {{prodname}} supports its own
namespaced [NetworkPolicy](/reference/resources/networkpolicy) and non-namespaced
[GlobalNetworkPolicy](/reference/resources/globalnetworkpolicy) resources, which provide additional
...
- ability to reference non-Kubernetes workloads in polices, including matching on
  [NetworkSets](/reference/resources/networkset) in policy rules
...
policy](/security/beginners/calico-network-policy/) guide.
...
deny](/security/beginners/kubernetes-default-deny/) network policies. These ensure that if no other policy is
...
[Calico Cloud](/get-started/tutorials/policy-tiers.mdx) supports hierarchical network policy using policy tiers. RBAC
