...
In contrast, the [Kubernetes network model](/tutorials/training/about-kubernetes-networking/) defines a "flat"
...
policy](/network-policy/beginners/calico-network-policy/) guide.
...
deny](/network-policy/beginners/kubernetes-default-deny/) network policies. These ensure that if no other policy is
...
[Calico Cloud](/network-policy/policy-tiers/tiered-policy/) supports hierarchical network policy using policy tiers. RBAC
