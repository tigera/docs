...
Microsegmentation across namespaces is achieved using {{prodname}} policy labels and selectors with a **Pass** action rule. If you are unfamiliar with policy action rules, see [Tutorial - Understanding policy tiers](/get-started/tutorials/policy-tiers.mdx).
...
- Implement [Global default deny](/workload-access/enterprise-security/default-deny.mdx) security controls
- Implement [Global egress access controls](/workload-access/enterprise-security/global-egress.mdx) for communication with endpoints external to the cluster in the security tier. These controls govern the communication of all applications for a given tenant with the outside world.
