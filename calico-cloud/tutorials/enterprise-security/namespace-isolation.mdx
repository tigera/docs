...
Microsegmentation across namespaces is achieved using {{prodname}} policy labels and selectors with a **Pass** action rule. If you are unfamiliar with policy action rules, see [Tutorial - Understanding policy tiers](/network-policy/policy-tiers/tiered-policy/).
...
- Implement [Global default deny](/tutorials/enterprise-security/default-deny/) security controls
- Implement [Global egress access controls](/tutorials/enterprise-security/global-egress/) for communication with endpoints external to the cluster in the security tier. These controls govern the communication of all applications for a given tenant with the outside world.
