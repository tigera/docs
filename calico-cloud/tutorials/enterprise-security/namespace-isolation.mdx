...
canonical_url: '/workload-access/enterprise-security/namespace-isolation'
---

In this article you will learn how to create {{site.prodname}} global network policies to isolate namespaces for ingress traffic. 
...
Microsegmentation across namespaces is achieved using {{site.prodname}} policy labels and selectors with a **Pass** action rule. If you are unfamiliar with policy action rules, see [Tutorial - Understanding policy tiers]({{site.baseurl}}/get-started/tutorials/policy-tiers).
...
![multi-tenancy-ingress]({{site.baseurl}}/images/multi-tenancy-ingress.png)
...
- Implement [Global default deny]({{site.baseurl}}/workload-access/enterprise-security/default-deny) security controls 
- Implement [Global egress access controls]({{site.baseurl}}/workload-access/enterprise-security/global-egress) for communication with endpoints external to the cluster in the security tier. These controls govern the communication of all applications for a given tenant with the outside world. 
