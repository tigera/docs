...
{{prodname}} anomaly detection allows you to proactively determine if there is an issue (or not), and potentially resolve problems before service levels are compromised. Anomaly detection uses {{prodname}} Elasticsearch logs ([flow](/visibility/elastic/flow/index) logs, [L7](/visibility/elastic/l7/index) logs, and [DNS](/visibility/elastic/dns/index) logs) to learn the behavior of cluster nodes, pods, services, and other entities that send log records (applications, load balancers, databases, etc.).
...
For a list of anomalies that are enabled by default, see [Anomaly detection reference](/reference/anomaly-detection).
...
- To use L7/HTTP anomaly detectors, you must enable [L7 logs](/visibility/elastic/l7/configure) on the cluster
...
![anomaly-detection-enable-alert](/img/calico-enterprise/master/anomaly-detection-enable-alert.png)
...
![anomaly-detection-single-alert](/img/calico-enterprise/master/anomaly-detection-single-alert.png)

**Service Graph**

From the left navbar, click Service Graph. Monitor security performance in the bottom panel.

![anomaly-detection-service-graph-alerts-tab](/img/calico-enterprise/master/anomaly-detection-service-graph-alerts-tab.png)
...
- [Anomaly detection reference](/reference/anomaly-detection)
- [Global Alert reference](/reference/resources/globalalert)
