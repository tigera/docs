---
title: Block anonymization attacks
...
1. Add threat feed to the cluster.
   For EJR VPN,
   ```shell
   kubectl apply -f {{url}}{{baseUrl}}/manifests/threatdef/ejr-vpn.yaml
   ```
   For Tor Bulk Exit Feed,
   ```shell
   kubectl apply -f {{url}}{{baseUrl}}/manifests/threatdef/tor-exit-feed.yaml
   ```
...
- [Block suspicious IPs](suspicious-ips.mdx)
