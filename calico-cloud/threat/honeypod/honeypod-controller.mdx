...
- [Packet capture feature](../../visibility/packet-capture.mdx) to collect honeypod traffic in clusters.
...
For help, see [PacketCapture](../../reference/resources/packetcapture.mdx).
...
If you’ve customized or created your own honeypods, be sure to modify the included `capture-honey` [PacketCapture](../../reference/resources/packetcapture.mdx) manifest to target your honeypods.
...
```bash
kubectl apply -f {{url}}{{baseUrl}}/manifests/threatdef/honeypod/controller.yaml
```

For OpenShift deployments, the controller requires privileged access:

```bash
kubectl apply -f {{url}}{{baseUrl}}/manifests/threatdef/honeypod/controller_os.yaml
```
...
Refer to the [Snort documentation](https://www.snort.org/documents) for information on how to write Snort signatures.
...
- [Packet capture](../../visibility/packet-capture.mdx)
