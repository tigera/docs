...
- [Packet capture feature](/visibility/packetcapture) to collect honeypod traffic in clusters.
...
[Honeypods are configured](/threat/honeypod/honeypods) for clusters, and alerts are generated when the honeypods are accessed.
...
The following manifest enables packet capture on default [honeypods](/threat/honeypod/honeypods).
Be sure to modify the namespace and selector if honeypods are placed elsewhere.
For help, see [PacketCapture](/visibility/packetcapture).
...
If you’ve customized or created your own honeypods, be sure to modify the included `capture-honey` [PacketCapture](/visibility/packetcapture) manifest to target your honeypods.
