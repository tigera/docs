...
If you have a program running within the cluster and want to negate the generation of any runtime logs for that process, thus negating an Security Events for that process, then you can add an entry into the RuntimeExceptionList of the Runtime Security resource through the kubectl :
...
Setting the `matching` field to `regex` lets you pass in a regular expression to capture more than 1 value for each field. Whereas `exact` would require each field to be an exact match.

Supported fields are outlined in the Runtime Security resource
