...
- **{{prodname}} Manager** with built-in Elasticsearch and Kibana instances
...
- A `tigera-network-admin` role with full permissions to create and modify resources. For help, see [Log in to {{prodname}} Manager](/operations/cnx/authentication-quickstart/).

- To view Elasticsearch resources in {{prodname}} Manager, users must have [minimum permissions](/network-policy/policy-tiers/rbac-tiered-policies).
...
- Configure [RBAC for tiered policies](/network-policy/policy-tiers/rbac-tiered-policies).
- Learn more about the [ManagementCluster](/reference/installation/api#operator.tigera.io/v1.ManagementCluster) resource.
