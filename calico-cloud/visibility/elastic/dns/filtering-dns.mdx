...
1. Create a `filters` directory with a file named `dns` with the contents of
   ...
   If you are also adding [flow filters](/visibility/elastic/flow/filtering) also add the `flow` file
   to the directory.
...
describes how to write fluentd filters. The [DNS log schema](/visibility/elastic/dns/dns-logs) can be referred to
