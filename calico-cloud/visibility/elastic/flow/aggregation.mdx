...
The following is a sample flow log entry using aggregation by pod prefix for allowed traffic. For flow log fields and filter parameters, see [Filter flow logs](/visibility/elastic/flow/filtering).
...
Before [changing the default aggregation level](/reference/resources/felixconfig#aggregationkind), note the following:
...
- Verify that the parameters that you want to see in your aggregation level, are not already [filtered](/visibility/elastic/flow/filtering)
...
- [Archive logs to storage](/visibility/elastic/archive-storage)
- [Configure RBAC for Elasticsearch logs](/visibility/elastic/rbac-elasticsearch)
