...
For allowed flows, the default aggregation level is 2, `AnyProcessInSamePodPrefix` and for denied flows the default aggregation level is 1,
`AnyProcessInSameSourcePod`.
...
The source port is usually ephemeral and does not convey useful information. By suppressing the source port, `AnyProcessInSameSourcePod` aggregation
