...
- Calico CNI
- [Configure BGP peering](/networking/configuring/bgp) between {{prodname}} and your network infrastructure
...
1. Determine the service cluster IP range. (Or ranges, if your cluster is [dual stack](/networking/ipam/ipv6).)
...
1. Based on above results, update or create a BGPConfiguration.
   ...
   **Create default BGPConfiguration**.
   ...
   For help see, [BGP configuration resource](/reference/resources/bgpconfig).
...
- [Apply policy to services exposed externally as cluster IPs](/network-policy/beginners/services/services-cluster-ips)
