...
- Pods can communicate with all other pods in the cluster using pod IP addresses (without
  [NAT](/networking/training/about-networking))
...
addresses](/networking/configuring/advertise-service-ips), allowing services to be conveniently accessed without
...
Kubernetes network implementations use [overlay networks](/networking/training/about-networking#overlay-networks).
...
allows you to [configure outgoing NAT](/networking/configuring/workloads-outside-cluster) for specific IP address
