...
Or, see the [reference documentation on host-local IPAM](/reference/component-resources/configuration#using-host-local-ipam).
...
- [IP Pool](/reference/resources/ippool)

There are several other ways to leverage {{prodname}} IPAM including:

- [Assign addresses based on topology](/networking/ipam/assign-ip-addresses-topology)
- [Use a specific address for a pod](/networking/ipam/use-specific-ip)
- [Migrate from one IP pool to another](/networking/ipam/migrate-pools)
...
- [Change IP address block size](/reference/resources/ippool)
