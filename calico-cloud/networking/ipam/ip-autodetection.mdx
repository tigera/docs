...
- [Changes to cross subnet packet encapsulation](/networking/configuring/vxlan-ipip)
...
[NodeAddressAutodetection](/reference/installation/api#operator.tigera.io/v1.NodeAddressAutodetection) in the operator Installation reference
and for more details see the [node configuration](/reference/component-resources/node/configuration#ip-autodetection-methods) reference.
...
You can configure specific IP address and subnet for a node by disabling IP autodetection and then updating the [Node resource](/reference/resources/node).
...
- For details on autodetection methods, see the [node configuration](/reference/component-resources/node/configuration) reference.
- For calicoctl environment variables, see [Configuring {{nodecontainer}}](/reference/component-resources/node/configuration)
