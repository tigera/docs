...
even a [specific IP](/networking/ipam/use-specific-ip.mdx) for a new pod, but this requires predicting how many pods
...
pool](/networking/ipam/legacy-firewalls.mdx) and to perform an SNAT on the traffic passing through them. Hence, any
...
- Set `ipipMode` or `vxlanMode` to `Always` if the pod network has [IPIP or VXLAN](/networking/configuring/vxlan-ipip.mdx) enabled.
...
the [egress gateway maintenance guide](/networking/egress/egress-gateway-maintenance.mdx).
...
- [Additional configuration for egress gateway maintenance](/networking/egress/egress-gateway-maintenance.mdx)
