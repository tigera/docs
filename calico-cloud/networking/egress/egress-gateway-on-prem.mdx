...
{{prodname}}'s own policy (including [DNS policy](/network-policy/domain-based-policy)) and
...
{{prodname}} allows specifying an [IP pool](/reference/resources/ippool) for each pod or namespace, and
even a [specific IP](/networking/ipam/use-specific-ip) for a new pod, but this requires predicting how many pods
...
pool](/networking/ipam/legacy-firewalls) and to perform an SNAT on the traffic passing through them. Hence, any
...
that pod. It will be the **node IP** if the pod IP came from an [IP pool](/reference/resources/ippool) with `natOutgoing: true`, and the **pod IP** if
...
pool](/reference/resources/ippool) with `natOutgoing: true`, and the egress
...
- [Domain-based policy](/network-policy/domain-based-policy) is not supported at egress from egress
...
- Set `ipipMode` or `vxlanMode` to `Always` if the pod network has [IPIP or VXLAN](/networking/configuring/vxlan-ipip) enabled.
...
expressions](/reference/resources/networkpolicy#selector) as the selector fields in
{{prodname}} [network policy](/reference/resources/networkpolicy#entityrule).
...
the [egress gateway maintenance guide](/networking/egress/egress-gateway-maintenance).

## Above and beyond

Please see also:

- The `egressIP...` fields of the [FelixConfiguration resource](/reference/resources/felixconfig#spec).
- [Additional configuration for egress gateway maintenance](/networking/egress/egress-gateway-maintenance)
