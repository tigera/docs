...
- _If no egress gateway is configured_ and the pod IP came from an [IP pool](/reference/resources/ippool)
...
resource. The NodeAddressAutodetection options, `canReach` and `cidrs` are suitable. See [Installation reference](/reference/installation/api). If using the `cidrs` option, set the CIDRs list to include only the
...
ensure that you have an entry in an [IP reservation](/reference/resources/ipreservation) for its first
...
- The `egressIP...` and `aws...` fields of the [FelixConfiguration resource](/reference/resources/felixconfig#spec).
- [Troubleshooting egress gateways](/networking/egress/troubleshoot).
