...
For a detailed explanation on the various components within `{{noderunning}}` please see [Anatomy of a `{{noderunning}}` container](/reference/architecture/components).
...
Refer to [Configuring Prometheus](/operations/monitor/prometheus/index.mdx) for information on how to create a new Alerting rule or updating the scraping interval for how often Prometheus collects the metrics.
...
- [Secure {{prodname}} Prometheus endpoints](/operations/comms/secure-metrics.mdx)
- [Configuring Prometheus](/operations/monitor/prometheus/index.mdx)
