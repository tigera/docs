...
against your compliance standards. {{prodname}} compliance dashboard and reports provide a complete inventory of regulated workloads, along with evidence of enforcement of network controls for these workloads. Additionally, audit reports are available to see changes to any network security controls.

## Features

This how-to guide uses the following {{prodname}} features:

- Predefined **compliance reports** (Inventory, Network Access, Policy Audit) that are installed with {{prodname}}
- A **GlobalReport** resource to schedule reports, specify which nodes to include, and manually run reports
- The **compliance dashboard** in {{prodname}} Manager to view and export reports from Elasticsearch

## Concepts

### Compliance reports at a glance

Compliance reports are based on archived flow logs and audit logs for all of your {{prodname}} resources, plus any audit logs you've configured for Kubernetes resources in the Kubernetes API server:
...
![compliance-reporting](/img/calico-cloud/master/compliance-reporting.png)
...
- [Configure audit logs for Kubernetes resources](../visibility/elastic/audit-overview.mdx)
  ...
  Although all {{prodname}} resources are configured for compliance reports, you must configure Kubernetes resources for audit logs through the Kubernetes API for a complete view of your resources.
...
Report permissions are granted using the standard Kubernetes RBAC based on ClusterRole and ClusterRoleBindings. The following table outlines the required RBAC verbs for each resource type for a specific user action.
...
- Paul has permission to create/modify/delete the report schedules and configuration but does not have permission to export generated reports from the UI.
- Candice has permission to list and export generated reports from the UI, but cannot modify the report schedule or configuration.
...
| endpoints       | **Optional**. For inventory and network-access reports, specifies the endpoints to include in the report. The policy-audit report restricts audit logs to include only policies that apply to the selected endpoints. If not specified, the report includes all endpoints and audit logs. |
...
1. Download the pod template corresponding to your installation method.
   ...
   ```
   curl -O {{url}}{{baseUrl}}/manifests/compliance-reporter-pod.yaml
   ```
...
- For report field descriptions, see [Compliance Reports](../reference/compliance-reports/index.mdx)
