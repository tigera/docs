---
title: Schedule and run CIS benchmark reports
description: Configure reports to assess compliance for all assets in a Kubernetes cluster.
---

## Big picture

Use {{prodname}} CIS benchmark compliance reports to assess compliance for all assets in a Kubernetes cluster.
...
{{prodname}} CIS benchmark compliance reports provide this view into your Kubernetes clusters. Plus, the reports strengthen your threat detection capability, providing a more comprehensive view than looking only at network data.

## Features

This how-to guide uses the following {{prodname}} features:

- Predefined CIS benchmark compliance reports that are installed with {{prodname}}
- A **GlobalReport** resource to schedule reports and set thresholds
- The {{prodname}} compliance dashboard to view and export reports from Elasticsearch.

## Concepts

### Kubernetes CIS benchmarks

As part of {{prodname}} installation, each node runs a pod named compliance-benchmarker that executes Kubernetes CIS benchmarks every hour (default). We recommend that you review the CIS benchmark best practices for securing cluster component configurations here: [CIS benchmarks downloads](https://learn.cisecurity.org/benchmarks).

### Configure Global Report

The {{prodname}} **global report** is the resource for configuring CIS benchmark reports including scheduling, filtering, setting thresholds, and exporting reports in .csv format.
...
- For other predefined compliance reports, see [Compliance Reports](../reference/compliance-reports/index.mdx).
