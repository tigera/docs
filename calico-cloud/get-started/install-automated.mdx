...
| `installer.calicoCloudVersion` | string | `$[cloudUserVersion]` | The version of Calico Cloud you're installing. This also selects the operator image, which is tagged with that version. |
...
:::important
The operator image is pulled at the tag named by `installer.calicoCloudVersion`, so [mirror that version's operator image](setup-private-registry.mdx) first. With `installer.imagePath` unset, it is pulled from the `calico-cloud-public` path in your registry rather than `tigera`.
:::
