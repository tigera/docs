...
{{prodname}} architecture is based on the Calico Enterprise multi-cluster management feature. {{prodname}} manages the control plane, and you connect your clusters (called **managed clusters**) to the control plane. Communication between the {{prodname}} control plane and managed clusters is secured using TLS tunnels. 
