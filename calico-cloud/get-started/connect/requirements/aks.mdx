...
To allow existing AKS managed clusters with Calico to be managed in the same deployment as new {{prodname}} clusters without conflict, you must change the `networkPlugin` to `none`.
:::
