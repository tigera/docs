...
The cluster you are using to install {{prodname}} does not have a CNI plugin installed, the CNI is incompatible. If your cluster has functional pod networking and you see this message, it is likely that kubelet has been configured to use kubenet networking, which is not compatible with {{prodname}}. You can use a different cluster, or re-create your cluster with [compatible networking](/get-started/connect/requirements/system-requirements/).
...
For more information on operator custom resources see the [Installation API reference](/reference/installation/api).
