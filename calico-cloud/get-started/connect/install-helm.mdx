...
1. Optional: To make changes to what features are enabled during installation, paste the command to a text editor and append the `--set` option any of the following key-value pairs.
   ...
   --set installer.components.compliance.enabled=true
   ```
   In this example, this command connects the cluster to Calico Cloud with Image Assurance, Runtime Security, Security Posture Dashboard, Packet Capture, and Compliance Reports features enabled.
