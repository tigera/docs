...
### Check {{ site.prodname }} installation

Installing {{site.prodname}} on your Kubernetes cluster is managed by the {{ site.prodname }} operator. The {{ site.prodname }} operator is deployed as a Deployment in the `calico-cloud` namespace, and records status in a custom resource named `installer`.
...
After `state` is `complete`, {{site.prodname}} is properly installed.

### Check logs for fatal errors

Check that the {{ site.prodname }} operator is running and that logs do not have any fatal errors.
...
```bash
kubectl get installers.operator.tigera.io --namespace calico-cloud -o yaml
```
...
To upload diagnostics to {{ site.prodname }} support automatically, run the following command:
...
If you have verified all of the above steps and still have issues, you can move on to the [Tigera Operator checklist](operator-checklist).
