...
canonical_url: '/get-started/connect/checklist'
---
...
The cluster you are using to install {{site.prodname}} does not have a CNI plugin installed, the CNI is incompatible. If your cluster has functional pod networking and you see this message, it is likely that kubelet has been configured to use kubenet networking, which is not compatible with {{site.prodname}}. You can use a different cluster, or re-create your cluster with [compatible networking]({{site.baseurl}}/get-started/connect/system-requirements).
...
If you see messages like, `Unable to connect to the server: x509: certificate signed by unknown authority`, or you suspect there are issues due to proxies in your environment, add the `--insecure-tls` flag to your installation invocation. For example:

```bash
curl -s https://tigera-installer.calicocloud.io/... | bash -s -- --insecure-tls
```
>**Note**: When you enable this flag, some certificate validations are skipped.
{: .alert .alert-info}

### Check Calico Cloud installation

Installing {{site.prodname}} on your Kubernetes cluster is managed by the Tigera operator. The Tigera operator is deployed as a ReplicaSet in the `tigera-operator` namespace, and records status in a custom resource named, `tigerastatus`. The operator get its configuration from several Custom Resources (CRs); the central one is the Installation CR.
...
If all components show a status of "Available" = TRUE, {{site.prodname}} is properly installed.

**Issue: {{site.prodname}} is not installed**

If {{site.prodname}} is not installed, you'll get the following error. Install {{site.prodname}} on the node using the `curl` command that you got from Support.
...
**Issue: {{site.prodname}} components are missing or are degraded**

If some of the {{site.prodname}} components are Available = FALSE or DEGRADED = TRUE, run the following command and contact Support with the following output.

```bash
kubectl get tigerastatus -o yaml
```

>**Note**: If you are using the **AWS or Azure CNI plugin**, a degraded state is likely because you do not have enough pod capacity on your nodes. To fix this, see [Check pod capacity](#check-pod-capacity).
{: .alert .alert-info}

**Sample output**

In the following example, the typha component has an issue because it is showing `AVAILABLE: FALSE`, and `DEGRADED: TRUE`. To understand details of {{site.prodname}} components, see [Deep dive into custom resources](#deep-dive-into-custom-resources).
...
For more information on operator custom resources see the [Installation API reference]({{site.baseurl}}/reference/installation/api).
...
`log-collector` collects flow and other logs and forwards them to {{site.prodname}}. Check the pods and logs in the `tigera-fluentd` namespace. You should have one pod running on each of your nodes.
...
The `management-cluster-connection` is required for your managed clusters to connect to the {{site.prodname}} backend. If it is not available or degraded, check the pods and logs in the `tigera-guardian` namespace.
...
The high-level components {{site.prodname}} needs to run are:
