...
3. Update the [LogCollector](../../reference/installation/api.mdx#logcollector)
   ...
   $[prodname] writes one directory per log type under `bucketPath`, and names each object
   for the UTC day it covers:
   ...
   Flow logs from non-cluster hosts are archived under `non_cluster_flows/` instead of
   being mixed into `flows/`.

   :::note

   If you are upgrading from a release that collected logs with Fluentd, these keys have
   changed. Fluentd wrote flat keys with the date concatenated onto the log type, as in
   `<bucketPath>/flows20260101_1.gz`. Update any downstream tooling that matches the old
   pattern.
   ...
   spec:
     additionalStores:
       syslog:
         ...
         # (Optional) Maximum message size in bytes. Defaults to 2048.
         packetSize: 2048
