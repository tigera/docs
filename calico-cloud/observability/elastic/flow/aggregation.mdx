...
$[prodname] enables flow log aggregation by default, and uses a default aggregation level that balances log volume with comprehensive visibility. The
...
For allowed flows, the default aggregation level is 2, `AggregateIPsAndSourcePorts` and for denied flows the default aggregation level is 1,
`AggregateSourcePorts`.  Aggregation levels are configured using the numeric levels.

The following table summarizes the aggregation levels for flow log traffic:

| **Level** | **Name**                            | **Description**                                                   |
|-----------|-------------------------------------|-------------------------------------------------------------------|
| 0         | AggregateNothing                    | No aggregation.  Create separate flow logs for each distinct 5-tuple (protocol, source and destination IP and port) observed. |
| 1         | AggregateSourcePorts                | Aggregate flow data relating to multiple connections which only differ in source port. This reduces log volume by discarding source port information, which is typically ephemeral and of little value. |
| 2         | AggregateIPsAndSourcePorts          | In addition to the above, aggregate flows which have related sources and destinations into a single log.  Specifically pods created by the same pod controller (Deployments/ReplicaSets etc) will be combined and identified by their common pod prefix, IP addresses in the same NetworkSet will be identified by their shared NetworkSet, and when no more precise identity is known arbitrary IP addresses will be combined under public(`pub`) and private (`pvt`) as defined in RFC1918. |
| 3         | AggregateIPsAndPorts                | In addition to the above, aggregate destination ports together. This is intended to reduce log volumes in situations where a lot of network probing is expected (for example by `nmap`) but is not typically needed. |
...
The source port is usually ephemeral and does not convey useful information. By suppressing the source port, `AggregateSourcePorts` aggregation
type minimizes the flow logs generated for applications that make many connections to the same destination. The two flows originating from `client-a` without aggregation are combined into one.

In Kubernetes, pod controllers (Deployments, DaemonSets, ReplicaSets etc.) automatically name the pods they create with a common prefix.
For example, the pods `nginx-1` and `nginx-2` are created by the ReplicaSet `nginx`. That prefix is used to aggregate flow log entries and is indicated with an asterisk * at the end of the name. The three flows originating from `client-a` and `client-b` are combined into a single flow log. This aggregation level is called `AggregateIPsAndSourcePorts`.

Finally, `AggregateIPsAndPorts` combines flows between matching pod prefixes that target distinct destination ports, as seen in the last example row below.
Destination port information is often valuable, so this level should only be used after log analysis indicates it will be necessary and beneficial. 

Note that currently aggregation is performed on a per node basis, but this behavior may change in future.  Certain other fields are always kept separate
...
- Note that currently the numeric levels must be used instead of the names in configuration.
