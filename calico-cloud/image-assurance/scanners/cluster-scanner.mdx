...
- Modify your [Image Assurance](../reference/installation/ia-api.mdx#image-assurance.operator.tigera.io/v1.ImageAssuranceSpec) installation resource to include the `exclusions.namespaces` field. List each namespace name you want to exclude.
...
In this example, the workloads in `kube-system` and `dev-qa` namespaces are excluded from future image scans. 
