...
**Deleted Results**: Deleting a scan result removes it from the **All Scanned Images** tab. However, if the image is currently running, its result will still appear in the **Running Images** tab, with the number of instances tracked.
