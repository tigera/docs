...
The export generates a CSV file with image and CVE metadata for the current list of images you have selected (based on your current filter options).

> **Note**:
> - Images with no associated CVEs will not be included in the exported data (regardless of whether or not it is included by the filters).
> - **Export one row for each image and CVE ID** exports a unique image + CVE combination for each row. For example, if an image has 10 CVE’s, 10 rows are outputted (1 for each CVE).
> - **Export one row per image** exports one row for each image, with all associated CVE’s condensed into a single column.
{: .alert .alert-info}
