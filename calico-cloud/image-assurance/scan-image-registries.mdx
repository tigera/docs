...
Scan images for vulnerabilities using {{site.prodname}} Image Assurance.

### Value

{{site.prodname}} Image Assurances helps you identify vulnerabilities in workloads that you deploy to Kubernetes clusters. A vulnerability can be a weakness in an application (for example, a design flaw or an implementation bug) that allows attackers to cause additional harm. With Image Assurance you can:
...
1. Download the latest version of the CLI scanner.
   ...
   **Linux**
   ```shell
   curl -Lo tigera-scanner {% include clidownloadurl %}/image-assurance-scanner-cli-linux-amd64
   ```
   **MacOS**
   ```shell
   curl -Lo tigera-scanner {% include clidownloadurl %}/image-assurance-scanner-cli-darwin-amd64
   ```
2. Set the executable flag on the binary.
   ...
   ```
   >**Note**: You must download and set the executable flag each time you get a new version of the CLI.
   {: .alert .alert-info}
...
3. Verify the CLI scanner works correctly by running the version command.
   ...
   {{site.imageassuranceversion}}
   ```

#### Scan images using CLI

You can scan images and report results back to {{site.prodname}}, or scan images locally without reporting results to {{site.prodname}}.
...
- `--apiurl` - {{site.prodname}} API URL path. You can get this URL in Manager UI, **Image Assurance**, **Scan settings**. 
- `--token` - secure API or authorization token to make requests to {{site.prodname}} API URL. You can get this URL in Manager UI, **Image Assurance**, **Scan settings**.
...
>**Note**: Key names must match the full name of arguments passed to the CLI. The configuration precedence order is options > environment variables > file configuration.
{: .alert .alert-info} 
...
--- |-----------|----------------------| ---
--apiurl | -a        | CC_API_URL                 | {{site.prodname}} API URL path. You can get this URL in Manager UI, Image Assurance, Scan settings. 
--token | -t        | CC_TOKEN                 | Secure API or authorization token to make requests to {{site.prodname}} API URL. 
...
To see the latest version, see [Release notes]({{site.baseurl}}/release-notes/).
...
The left panel lists scanned images (assuming you set up the CLI scanner to report scan results to {{site.prodname}}).
...
The left panel lists images running in your cluster. Because you can see the CVEs associated with running pods, you can assess pod vulnerability. The columns, **Clusters** and **Running Instances**, show the number of running instances in clusters that are connected to {{site.prodname}}.
...
>**Note**: In the Scan Results and Runtime tabs, the **Registry path** field may be blank if {{site.prodname}} cannot access this metadata. For example, images from Docker Hub do not specify the registry in the image metadata. 
{: .alert .alert-info}  

**How Pass, Warn, and Fail scores map to CVSS scores**

{{site.prodname}} image scoring is based on the Common Vulnerability Scoring System v3 (CVSS Scores). 
You can change the CVSS score threshold between a Pass and a Warn, and between a Warn and a Fail. Currently, only {% include open-new-window.html text='CVSS v3 scores' url='https://nvd.nist.gov/vuln-metrics/cvss/v3-calculator' %} are displayed. CVEs that do not have a CVSS v3 score (too old to have one, or are too new to be assigned one), display a blank score in the UI, and **N/A** in the CLI.
...
The default threshold values (shown below) will work for the majority of {{site.prodname}} deployments. However, you may need to change the defaults because of security requirements.  

![scan-settings]({{site.baseurl}}/images/scan-settings.png)
...
>**Note**: Images with no associated CVEs are not included in the exported data (regardless of whether or not it is included by the filters).
{: .alert .alert-info}

### Next steps

- [Set up alerts on vulnerabilities]({{site.baseurl}}/image-assurance/set-up-alerts)

- Create [policy]({{site.baseurl}}/image-assurance/install-the-admission-controller) to block vulnerable containers from deploying to your cluster
