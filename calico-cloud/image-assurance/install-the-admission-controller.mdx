...
1. Generate the certificate key pair.
   ...
   ```bash
   export URL="{% include clouddownloadurl %}/manifests" && curl ${URL}/generate-open-ssl-key-cert-pair.sh | bash
   ```
...
You can also use modify the Allow rule to match an absolute time. For help, see [ContainerAdmissionPolicies]({{site.baseurl}}/reference/resources/containeradmissionpolicy).
