...
Microsegmentation across namespaces is achieved using {{prodname}} policy labels and selectors with a **Pass** action rule. If you are unfamiliar with policy action rules, see [Tutorial - Understanding policy tiers](/get-started/tutorials/policy-tiers).
