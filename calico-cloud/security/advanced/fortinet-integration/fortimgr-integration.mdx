...
- Pull secret that you used during {{prodname}} installation
...
- Login access to [{{prodname}} Manager UI

**Recommended**

- Experience with [ tiered policy](../../../get-started/tutorials/policy-tiers.mdx) and [ global network policy](../../../reference/resources/globalnetworkpolicy.mdx)
...
Create a [{{prodname}} tier](../../../get-started/tutorials/policy-tiers.mdx) in the {{prodname}} Manager UI for each Kubernetes cluster you want to secure. We recommend that you create a new tier (rather than reusing an existing tier) for all global network policies created by the {{prodname}} integration controller.
...
1. In this [FortiManager ConfigMap manifest](/manifests/fortimanager-device-configmap.yaml), add your FortiManager device information in the data section: `tigera.firewall.fortimanager-policies`. For example:
   ...
   | tier                       | Tier name you created in Manager UI                                                                              |
...
1. Apply the manifest.
   ...
   ```bash
   kubectl apply -f {{url}}{{baseUrl}}/manifests/fortimanager-device-configmap.yaml
   ```
...
1. Apply the manifest.
   ...
   ```bash
   kubectl apply -f {{url}}{{baseUrl}}/manifests/fortimanager.yaml
   ```
