...
A description of the {{prodname}} architecture can be found in our [architectural overview](/reference/architecture/overview). However, a brief discussion of the routing and data paths is useful for
the discussion.
...
- A BGP fabric where each of the TOR switches (and their subsidiary compute servers) are a unique [Autonomous System (AS)](/reference/architecture/design/<https:/en.wikipedia.org/wiki/Autonomous_System_(Internet)>)
