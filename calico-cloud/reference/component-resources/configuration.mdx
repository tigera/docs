...
The `host-local` IPAM plugin can be configured by setting the `Spec.CNI.IPAM.Plugin` field to `HostLocal` on the [operator.tigera.io/Installation](/reference/installation/api.mdx#operator.tigera.io/v1.Installation) API.
...
match a configured [IPPool](/reference/resources/ippool.mdx) resource.
...
`nodeSelector` field. Check out the [IP pool resource document](/reference/resources/ippool.mdx)
for more details.
