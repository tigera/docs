...
| CALICO_IPV4POOL_NODE_SELECTOR      | Controls the NodeSelector for the IPv4 Pool created at start up. [Default: `all()`]                                                                                                                         | [selector](/reference/resources/ippool#node-selector)                         |
...
| CALICO_IPV6POOL_NODE_SELECTOR      | Controls the NodeSelector for the IPv6 Pool created at start up. [Default: `all()`]                                                                                                                         | [selector](/reference/resources/ippool#node-selector)                         |
...
BGP configuration for Calico nodes is normally configured through the [Node](/reference/resources/node), [BGPConfiguration](/reference/resources/bgpconfig), and [BGPPeer](/reference/resources/bgppeer) resources.
...
retrieve the [Node resource](/reference/resources/node) configured for this node if it exists, or to create a new node resource representing the node if it does not. It is
also used to associate the node with per-node [BGP configuration](/reference/resources/bgpconfig), [felix configuration](/reference/resources/felixconfig), and endpoints.
...
[node resource configuration](/reference/resources/node)
...
in the cluster, see [decommissioning a node](/operations/decommissioning-a-node).
