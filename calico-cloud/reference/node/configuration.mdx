---
title: Configure cnx-node
description: Customize cnx-node using environment variables.
---

The `{{nodecontainer}}` container is deployed to every node (on Kubernetes, by a DaemonSet), and runs three daemons:
...
also used to associate the node with per-node BGP configuration, [felix configuration](../resources/felixconfig.mdx), and endpoints.
...
in the cluster, see decomissioning a node.
