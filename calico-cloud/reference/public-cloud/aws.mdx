...
See the [IP pool configuration reference](/reference/resources/ippool)
...
is configured correctly, follow the [Configuring IP-in-IP guide](/networking/configuring/vxlan-ipip)
...
you must configure outgoing NAT on your [{{prodname}} IP pool](/reference/resources/ippool).
...
We recommend using the AWS VPC CNI plugin with [federation](/multicluster/overview/) as it provides seamless IP connectivity
...
1. By default, the AWS CNI plugin performs SNAT for any packet routed outside the VPC. You must disable SNAT
   ...
   Required for [federation](/multicluster/overview/).
