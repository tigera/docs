...
Container traffic routing can be enabled by setting [IP-in-IP encapsulation](/networking/configuring/vxlan-ipip) and NAT outgoing on the configured {{prodname}} IP pools.

See the [IP pool configuration reference](/reference/resources/ippool) for information on how to configure {{prodname}} IP pools.
...
To allow {{prodname}} networked containers to reach resources outside of GCE, you must configure outgoing NAT on your [{{prodname}} IP pool](/reference/resources/ippool).
