...
[{{prodname}} over IP fabrics](/reference/architecture/design/l3-interconnect-fabric.mdx).
...
> [BGPConfiguration resource](/reference/resources/bgpconfig.mdx). That value can be overridden per-node by using the `bgp` field of
> the [node resource](/reference/resources/node.mdx).
