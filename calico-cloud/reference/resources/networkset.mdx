...
only applies to [network policies](/reference/resources/networkpolicy.mdx)
in that namespace. Two resources are in the same namespace if the `namespace`
value is set the same on both. (See [GlobalNetworkSet](/reference/resources/globalnetworkset.mdx) for non-namespaced network sets.)
...
[network policy](/reference/resources/networkpolicy.mdx) rule, it includes
