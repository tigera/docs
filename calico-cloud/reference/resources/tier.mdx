...
A tier resource (`Tier`) represents an ordered collection of [NetworkPolicies](/reference/resources/networkpolicy)
and/or [GlobalNetworkPolicies](/reference/resources/globalnetworkpolicy).
...
- If a [NetworkPolicy](/reference/resources/networkpolicy) or [GlobalNetworkPolicy](/reference/resources/globalnetworkpolicy) in the Tier `Allow`s or `Deny`s the packet, then evaluation is done: the packet is handled accordingly.
- If a [NetworkPolicy](/reference/resources/networkpolicy) or [GlobalNetworkPolicy](/reference/resources/globalnetworkpolicy) in the Tier `Pass`es the packet, the next Tier containing a Policy that applies to the endpoint processes the packet.
