...
| `header`                      | Whether or not the document contains a header row.                        | boolean   |
...
| `disableRecordSizeValidation` | Disable row size checking. Mutually exclusive with `recordSize`.          | boolean   |
...
| optional | Whether the pull can proceed without the referenced value | If the referenced value does not exist, `true` means omit the header. `false` means abort the entire pull until it exists | boolean   | `false` |
