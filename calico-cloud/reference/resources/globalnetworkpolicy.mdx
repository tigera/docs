...
`GlobalNetworkPolicy` is not a namespaced resource. `GlobalNetworkPolicy` applies to [workload endpoint resources](/reference/resources/workloadendpoint.mdx) in all namespaces, and to [host endpoint resources](/reference/resources/hostendpoint.mdx).
...
See [network policy resource](/reference/resources/networkpolicy.mdx) for namespaced network policy.
...
GlobalNetworkPolicies are organized into [tiers](/reference/resources/tier.mdx), which provide an additional layer of ordering—in particular, note that the `Pass` action skips to the
