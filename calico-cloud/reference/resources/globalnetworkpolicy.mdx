...
import Httpmatch from '@site/calico-cloud/_includes/content/_httpmatch.mdx';
...
`GlobalNetworkPolicy` is not a namespaced resource. `GlobalNetworkPolicy` applies to workload endpoint resources in all namespaces. Select a namespace in a `GlobalNetworkPolicy` in the standard selector by using
...
`GlobalNetworkPolicy` resources can be used to define network connectivity rules between groups of {{prodname}} workload endpoints.
...
must be enabled in order to use the following match criteria.
