...
import SelectorScopes from '@site/calico-cloud/_includes/content/_selector-scopes.mdx';
...
`GlobalNetworkPolicy` is not a namespaced resource. `GlobalNetworkPolicy` applies to [workload endpoint resources](/reference/resources/workloadendpoint) in all namespaces, and to [host endpoint resources](/reference/resources/hostendpoint).
...
See [network policy resource](/reference/resources/networkpolicy) for namespaced network policy.
...
GlobalNetworkPolicies are organized into [tiers](/reference/resources/tier), which provide an additional layer of ordering—in particular, note that the `Pass` action skips to the
...
See [Policy for hosts](/network-policy/hosts/index)
