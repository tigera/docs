...
| wireguard            | WireGuard configuration for this node. This is applicable only if WireGuard is enabled in [Felix Configuration](/reference/resources/felixconfig). |                 | [WireGuard](#wireguard)      |
...
| Field                   | Description                                                                                        | Accepted Values                                                                                                                                                                       | Schema  | Default |
| ----------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ------- |
| asNumber                | The AS Number of your `{{nodecontainer}}`.                                                         | Optional. If omitted the global value is used (see [example modifying Global BGP settings](/networking/configuring/bgp) for details about modifying the `asNumber` setting). | integer |
