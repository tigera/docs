...
See [global network policy resource](/reference/resources/globalnetworkpolicy) for non-namespaced network policy.
...
NetworkPolicies are organized into [tiers](/reference/resources/tier), which provide an additional layer of ordering—in particular, note that the `Pass` action skips to the
