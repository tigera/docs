...
See [global network policy resource](/reference/resources/globalnetworkpolicy.mdx) for non-namespaced network policy.
...
NetworkPolicies are organized into [tiers](/reference/resources/tier.mdx), which provide an additional layer of ordering—in particular, note that the `Pass` action skips to the
