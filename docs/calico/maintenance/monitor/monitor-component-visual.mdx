...
```bash
kubectl apply -f {{data.versions.first.manifests_url}}/manifests/grafana-dashboards.yaml
```
...
By executing below command, you will delete all Calico monitoring resources, including the ones created by following this tutorial, *and* the [monitor component metrics](monitor-component-metrics.mdx) guide.
