...
1. Create the following manifest, which will install the API server as a deployment in the `calico-apiserver` namespace.
   ...
   ```
   kubectl create -f {{data.versions.first.manifests_url}}/manifests/apiserver.yaml
   ```
...
```
   kubectl delete -f {{data.versions.first.manifests_url}}/manifests/apiserver.yaml
   ```
