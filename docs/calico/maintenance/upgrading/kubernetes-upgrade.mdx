...
This page describes how to upgrade to {{page.version}} from {{prodname}} v3.0 or later. The
...
<HostEndpointsUpgrade prodname='{{prodname}}' orch="Kubernetes" />
...
1. Install the helm chart in the `tigera-operator` namespace.
   ...
   ```
   helm install {{prodname | downcase}} projectcalico/tigera-operator --version {{data.versions[0].title}} --namespace tigera-operator
   ```
...
1. Run the helm upgrade:
   ...
   ```bash
   helm upgrade {{prodname | downcase}} projectcalico/tigera-operator
   ```
...
1. Download the {{page.version}} operator manifest.
   ...
   ```bash
   curl {{data.versions.first.manifests_url}}/manifests/tigera-operator.yaml -O
   ```
...
1. Download the {{page.version}} manifest that corresponds to your original installation method.
   ...
   **{{prodname}} for policy and networking**
   ```bash
   curl {{data.versions.first.manifests_url}}/manifests/calico.yaml -O
   ```

   **{{prodname}} for policy and flannel for networking**
   ```bash
   curl {{data.versions.first.manifests_url}}/manifests/canal.yaml -O
   ```

   **{{prodname}} for policy (advanced)**
   ```bash
   curl {{data.versions.first.manifests_url}}/manifests/calico-policy-only.yaml -O
   ```
...
1. If you have [enable application layer policy](../../security/istio/app-layer-policyindex.mdx),
...
1. Congratulations! You have upgraded to {{prodname}} {{page.version}}.
...
1. Download the {{page.version}} manifest that corresponds to your original installation method.
   ...
   **{{prodname}} for policy and networking**
   ```bash
   curl {{data.versions.first.manifests_url}}/manifests/calico-etcd.yaml -O
   ```

   **{{prodname}} for policy and flannel for networking**
   ```bash
   curl {{data.versions.first.manifests_url}}/manifests/canal-etcd.yaml -O
   ```
...
1. If you have [enabled application layer policy](../../security/istio/app-layer-policyindex.mdx),
   follow [the instructions below](#upgrading-if-you-have-application-layer-policy-enabled) to complete your upgrade. Skip this if you are not using Istio with {{prodname}}.
...
If you have [enabled application layer policy](../../security/istio/app-layer-policyindex.mdx),
...
1. Update the Istio sidecar injector template to use the new version of Dikastes. Replace `<your Istio version>` below with
   ...
   ```bash
   kubectl apply -f {{data.versions.first.manifests_url}}/manifests/alp/istio-inject-configmap-<your Istio version>.yaml
   ```
...
<AutoHostendpointsMigrate prodname='{{prodname}}' orch="Kubernetes" />
