...
See [Enforce network policy using Istio tutorial](/security/tutorials/app-layer-policy/enforce-policy-istio) to learn how application layer policy provides second-factor authentication for the mythical Yao Bank.
...
- [calicoctl is installed and configured](../../maintenance/clis/calicoctl/install.mdx)
...
```bash
kubectl apply -f {{data.versions.first.manifests_url}}/manifests/csi-driver.yaml
```
...
```bash
curl {{data.versions.first.manifests_url}}/manifests/alp/istio-inject-configmap-1.10.yaml -o istio-inject-configmap.yaml
kubectl patch configmap -n istio-system istio-sidecar-injector --patch "$(cat istio-inject-configmap.yaml)"
```

[View sample manifest]({{data.versions.first.manifests_url}}/manifests/alp/istio-inject-configmap-1.10.yaml)

</TabItem>
<TabItem label="Istio v1.9.x" value="Istio v1.9.x-1">

```bash
curl {{data.versions.first.manifests_url}}/manifests/alp/istio-inject-configmap-1.9.yaml -o istio-inject-configmap.yaml
kubectl patch configmap -n istio-system istio-sidecar-injector --patch "$(cat istio-inject-configmap.yaml)"
```

[View sample manifest]({{data.versions.first.manifests_url}}/manifests/alp/istio-inject-configmap-1.9.yaml)
...
```bash
kubectl apply -f {{data.versions.first.manifests_url}}/manifests/alp/istio-app-layer-policy-envoy-v3.yaml
```
[View sample manifest]({{data.versions.first.manifests_url}}/manifests/alp/istio-app-layer-policy-envoy-v3.yaml)
...
- [Enforce network policy using Istio tutorial](/security/tutorials/app-layer-policy/enforce-policy-istio)
- [Use HTTP methods and paths in policy rules](/security/http-methods)
