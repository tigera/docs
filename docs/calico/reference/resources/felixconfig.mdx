...
| ipipMTU                            | The MTU to set on the tunnel device. Zero value means auto-detect. See [Configuring MTU](/networking/mtu) | int | int | `0` |
...
| policySyncPathPrefix               | File system path where Felix notifies services of policy changes over Unix domain sockets. This is only required if you're configuring [application layer policy](/security/app-layer-policy). Set to `""` to disable. | string | string | `""` |
...
| sidecarAccelerationEnabled         | Enable experimental acceleration between application and proxy sidecar when using [application layer policy](/security/app-layer-policy). [Default: `false`] | boolean | boolean | `false` |
...
| bpfEnabled                         | Enable eBPF dataplane mode.  eBPF mode has some limitations, see the [HOWTO guide](../../maintenance/ebpf/enabling-ebpf.mdx) for more details. | true, false | boolean | false |
