...
| policySyncPathPrefix               | File system path where Felix notifies services of policy changes over Unix domain sockets. This is only required if you're configuring [application layer policy](../../security/istio/app-layer-policyindex.mdx). Set to `""` to disable. | string | string | `""` |
...
| sidecarAccelerationEnabled         | Enable experimental acceleration between application and proxy sidecar when using [application layer policy](../../security/istio/app-layer-policyindex.mdx). [Default: `false`] | boolean | boolean | `false` |
