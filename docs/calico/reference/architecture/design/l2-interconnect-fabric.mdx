...
Carrier](https://datatracker.ietf.org/doc/html/rfc1149) as a transport for {{prodname}}
...
2.  High rate of *churn*. In a classical Ethernet data center fabric,
    ...
    *churn* in a {{prodname}} network's Ethernet fabric would be the addition
...
![A diagram showing the Ethernet spine planes. Each color represents a distinct Ethernet network, transporting a unique IP network.](/img/calico/l2-spine-planes.png)
...
[^1]: In this document (and in all {{prodname}} documents) we tend to use the
