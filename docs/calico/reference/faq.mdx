...
key `log_level`.  See [Configuring the {{prodname}} CNI plugins](cni-plugin/configuration.mdx) for more information.
...
By default, all enabled IP pools are used. However, you can specify which IP pools to use for IP address management in the [CNI network config](cni-plugin/configuration.mdx#ipam),
or on a per-pod basis using [Kubernetes annotations](cni-plugin/configuration.mdx#using-kubernetes-annotations).
...
See the [Requesting a specific IP address](cni-plugin/configuration.mdx#requesting-a-specific-ip-address) section in the CNI plugin reference documentation for more details.
