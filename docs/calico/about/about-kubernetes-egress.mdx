...
Sets](/security/external-ips-policy), or Calico Enterprise's support for [domain
names](/security/calico-enterprise/egress-access-controls) in policy rules.
...
layer, including [L5-7 rules](/security/http-methods), as another alternative to using IP addresses in rules. To
learn more about the benefits of this kind of approach, read our [Adopt a zero trust network model for security
](/security/adopt-zero-trust) guide.
...
- [Adopt a zero trust network model for security](../security/adopt-zero-trust.mdx)
- [Use external IPs or networks rules in policy](/security/external-ips-policy)
- [Enforce network policy using Istio](/security/app-layer-policy)
- [Use HTTP methods and paths in policy rules](/security/http-methods)
- [Restrict a pod to use an IP address in a specific range](/networking/legacy-firewalls)
- [Assign IP addresses based on topology](/networking/assign-ip-addresses-topology)
- <EnterpriseLink href='../security/calico-enterprise/egress-access-controls'>Advanced egress access controls</EnterpriseLink>
