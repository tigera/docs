...
Sets](../security/policy-rules/external-ips-policy.mdx), or Calico Enterprise's support for [domain
names](../security/calico-enterprise/egress-access-controls.mdx) in policy rules.
...
layer, including [L5-7 rules](../security/istio/http-methods.mdx), as another alternative to using IP addresses in rules. To
learn more about the benefits of this kind of approach, read our [Adopt a zero trust network model for security
](../security/adopt-zero-trust.mdx) guide.
...
- [Use external IPs or networks rules in policy](../security/policy-rules/external-ips-policy.mdx)
- [Enforce network policy using Istio](../security/istio/app-layer-policyindex.mdx)
- [Use HTTP methods and paths in policy rules](../security/istio/http-methods.mdx)
