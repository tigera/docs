...
<EnvironmentFile prodname='{{prodname}}' nodecontainer='{{nodecontainer}}' install="container" target="calico/node" />

### Step 2: Configure the init system

Use an init daemon (like systemd or upstart) to start the the {{nodecontainer}} image as a service using the EnvironmentFile values.
...
ExecStart=/usr/bin/docker run --net=host --privileged \
 ...
 -v /etc/pki:/pki \
 {{page.registry}}{{page.imageNames["calico/node"]}}:{{data.versions.first.title}} /bin/calico-node -felix
