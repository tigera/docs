...
### On-prem
The most common network setup for {{prodname}} on-prem is non-overlay mode using [BGP to peer](/networking/bgp) with the physical network (typically top of rack routers) to make pod IPs routable outside of the cluster.  (You can of course configure the rest of your on-prem network to limit the scope of pod IP routing outside of the cluster if desired.)  This setup provides a rich range of advanced {{prodname}} features, including the ability to advertise Kubernetes service IPs (cluster IPs or external IPs), and the ability to control IP address management at the pod, namespace, or node level, to support a wide range of possibilities for integrating with existing enterprise network and security requirements.
...
- [Configure BGP peering](/networking/bgp)
- [Configure overlay networking](/networking/vxlan-ipip)
- [Advertise Kubernetes service IP addresses](/networking/advertise-service-ips)
- [Customize IP address management](ipam/index.mdx)
- [Interoperate with legacy firewalls using IP ranges](/networking/legacy-firewalls)
