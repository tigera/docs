...
of what Neutron generally allows (see [IP addressing and connectivity](/networking/openstack/connectivity)) and because it's a purely layer 3 model (see [The {{prodname}} data path](../../reference/architecture/data-path.mdx), not all Neutron API calls will have the same effect as they would with other backends.
