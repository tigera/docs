...
to *understand* that {{prodname}} networks were different in this detail.
...
However work is still needed to make {{prodname}} networks report `l2_adjacency
...
{{prodname}} provides *exactly* the same connectivity between instances on different
...
{{prodname}} tenant network is *not* connected to a Neutron router, or if there are
