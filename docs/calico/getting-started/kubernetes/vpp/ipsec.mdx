...
- [IP-in-IP encapsulation](/networking/vxlan-ipip) configured between the nodes
...
In order to verify that the traffic is encrypted, open a VPP debug CLI session to check the configuration with [calivppctl](../../../maintenance/troubleshoot/vpp.mdx)
```bash
...
You can also [capture the traffic](../../../maintenance/troubleshoot/vpp.mdx#tracing-packets) flowing between the nodes to verify that it is encrypted.
