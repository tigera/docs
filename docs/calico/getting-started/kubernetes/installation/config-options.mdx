...
- [Installation](/reference/installation/api#operator.tigera.io/v1.Installation): a singleton resource with name "default" that
  configures common installation parameters for a {{prodname}} cluster.
- [APIServer](/reference/installation/api#operator.tigera.io/v1.Installation): a singleton resource with name "default" that
...
[Configuring {{nodecontainer}}](../../../reference/node/configuration.mdx).
...
manifest.  For more information, see [Configuring {{nodecontainer}}](../../../reference/node/configuration.mdx).
...
as Azure, you may wish to switch to [Calico's VXLAN encapsulation mode](/networking/vxlan-ipip).
...
```
      - name: dikastes
        image: {{page.registry}}{{page.imageNames["calico/dikastes"]}}:{{data.versions.first.components["calico/dikastes"].version}}
...
Refer to the
[Calico ConfigMap manifest]({{data.versions.first.manifests_url}}/manifests/alp/istio-inject-configmap-1.4.2.yaml) for an
