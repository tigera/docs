...
1. First, install the operator on your cluster.
   ...
   ```
   kubectl create -f {{data.versions.first.manifests_url}}/manifests/tigera-operator.yaml
   ```

1. Download the custom resources necessary to configure {{prodname}}

   ```
   curl {{data.versions.first.manifests_url}}/manifests/custom-resources.yaml -O
   ```
...
1. Download the {{prodname}} networking manifest for the Kubernetes API datastore.
   ...
   ```bash
   curl {{data.versions.first.manifests_url}}/manifests/calico-typha.yaml -o calico.yaml
   ```
1. <PodCidrSed yaml="calico" />
...
1. Download the {{prodname}} networking manifest for etcd.
   ...
   ```bash
   curl {{data.versions.first.manifests_url}}/manifests/calico-etcd.yaml -o calico.yaml
   ```
1. <PodCidrSed yaml="calico" />
...
The geeky details of what you get:
<GeekDetails prodname='{{prodname}}' details='Policy:Calico,IPAM:Calico,CNI:Calico,Overlay:IPIP,Routing:BGP,Datastore:etcd' />
...
- [Secure Calico component communications](../../../security/comms/crypto-auth.mdx)
- [Secure hosts by installing Calico on hosts](../../bare-metal/about.mdx)
- [Secure pods with Calico network policy](../../../security/get-started/calico-policy/calico-network-policy.mdx)
- If you are using {{prodname}} with Istio service mesh, get started here: [Enable application layer policy](../../../security/istio/app-layer-policyindex.mdx)
