...
- If you are using the default BGP networking with full-mesh node-to-node peering with no encapsulation, go to [Configure BGP peering](/networking/bgp) to get traffic flowing between pods.
...
- [Secure pods with Calico network policy](/security/calico-network-policy)
- If you are using {{prodname}} with Istio service mesh, get started here: [Enable application layer policy](/security/app-layer-policy)
