...
- Kubernetes cluster meets these requirements:
  - Kubernetes is installed *without* a CNI plugin **OR** cluster is running a compatible CNI for {{prodname}} to run in policy-only mode
...
- {{prodname}} can manage `cali` and `tunl` interfaces on the hosts.
  ...
  [Configure NetworkManager](../../maintenance/troubleshoot/troubleshooting.mdx#configure-networkmanager).
...
1. Add any other customizations you require to `values.yaml`.  You might like to refer to the [helm docs](https://helm.sh/docs/) or run
   ...
   ```
   helm show values projectcalico/tigera-operator --version {{data.versions[0].title}}
   ```
...
1. Install the Tigera {{prodname}} operator and custom resource definitions using the Helm chart:
   ...
   ```
   helm install {{prodname | downcase}} projectcalico/tigera-operator --version {{data.versions[0].title}} -f values.yaml --namespace tigera-operator
   ```
...
**Required**
- [Install and configure calicoctl](../../maintenance/calicoctl/install.mdx)

**Recommended tutorials**
- [Secure a simple application using the Kubernetes NetworkPolicy API](../../security/get-started/kubernetes-policy/kubernetes-policy-basic.mdx)
- [Control ingress and egress traffic using the Kubernetes NetworkPolicy API](../../security/get-started/kubernetes-policy/kubernetes-policy-advanced.mdx)
- [Run a tutorial that shows blocked and allowed connections in real time](../../security/get-started/kubernetes-policy/kubernetes-demo.mdx)
