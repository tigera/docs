...
- IP addresses for every node in the cluster. This is required to establish secure tunnels between the nodes. {{prodname}} can automatically do this using [IP autodetection methods](/networking/ip-autodetection).
...
```
For OpenShift, add the Felix configuration with WireGuard enabled [under custom resources](/getting-started/openshift/installation#provide-additional-configuration).
...
We recommend that you review and modify the MTU used by {{prodname}} networking when WireGuard is enabled to increase network performance. Follow the instructions in the [Configure MTU to maximize network performance](/networking/mtu) guide to set the MTU to a value appropriate for your network.
...
- [Secure {{prodname}} component communications](../security/comms/index.mdx)
- [Configure MTU to maximize network performance](/networking/mtu)
