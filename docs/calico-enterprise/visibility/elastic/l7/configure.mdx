...
L7 logs require a minimum of 1 additional GB of log storage per node, per one-day retention period. Adjust your [Log Storage](https://docs.tigera.io/maintenance/logstorage/adjust-log-storage-size) before you start tasks in the next section. 
...
1. Configure L7 log aggregation, retention, and reporting. 
    ...
    For help, see [Felix Configuration documentation](/reference/felix/configuration#calico-enterprise-specific-configuration).
...
1. Create or update the [ApplicationLayer](/reference/installation/api#operator.tigera.io/v1.ApplicationLayer) resource named, `tigera-secure` to include a logCollection section of the file, `_api.html` [here](/reference/installation/api). 
