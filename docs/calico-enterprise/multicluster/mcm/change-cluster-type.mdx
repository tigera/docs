...
1. Apply the [ManagementCluster](/reference/installation/api) CR.
...
1. Delete all managed cluster resources in your cluster.
   ...
   Although installation automatically cleans up credentials for managed clusters, the management cluster Elasticsearch still retains managed cluster data based on the retention value that you specify in your [LogStorage](/reference/installation/api).
...
1. Ensure a persistent volume is provisioned to store log data for the standalone cluster. 
  See [Configure storage for logs and reports](../../getting-started/create-storage.mdx).
...
1. Install the Tigera custom resources. 
   For more information, see [the installation reference](/reference/installation/api).
   ```bash
