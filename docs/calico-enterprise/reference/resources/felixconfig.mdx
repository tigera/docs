...
See [Configuring Felix](/reference/felix/configuration) for more details.
...
| policySyncPathPrefix               | File system path where Felix notifies services of policy changes over Unix domain sockets. This is required only if you're configuring [L7 logs](/visibility/elastic/l7/configure), or [egress gateways](../../networking/egress/index.mdx). Set to `""` to disable. | string | string | `""` |
...
| wireguardMTU                       | MTU set on the WireGuard interface created by Felix. Zero value means auto-detect. See [Configuring MTU](/networking/mtu). | int | int | 0 |
