...
`StagedGlobalNetworkPolicy` is not a namespaced resource. `StagedGlobalNetworkPolicy` applies to [workload endpoint resources](/reference/resources/workloadendpoint) in all namespaces, and to [host endpoint resources](hostendpoint.mdx).
