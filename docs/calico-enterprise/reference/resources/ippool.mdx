...
| nodeSelector | Selects the nodes where {{prodname}} IPAM should assign pod addresses from this pool.  Can be overridden if a pod [explicitly identifies this IP pool by annotation](/reference/cni-plugin/configuration#using-kubernetes-annotations). | | [selector](#node-selector) | all() |
...
{{prodname}} supports Kubernetes [annotations that force the use of specific IP addresses](../cni-plugin/configuration#requesting-a-specific-ip-address). These annotations take precedence over the `allowedUses` field.
...
[Configuring IP-in-IP](/networking/vxlan-ipip).
...
[Assign IP addresses based on topology guide.](/networking/assign-ip-addresses-topology).
