...
key `log_level`.  See [Configuring the {{prodname}} CNI plugins](/reference/cni-plugin/configuration) for more information.
...
By default, all enabled IP pools are used. However, you can specify which IP pools to use for IP address management in the [CNI network config](/reference/cni-plugin/configuration#ipam),
or on a per-pod basis using [Kubernetes annotations](/reference/cni-plugin/configuration#using-kubernetes-annotations).
...
See the [Requesting a specific IP address](/reference/cni-plugin/configuration#requesting-a-specific-ip-address) section in the CNI plugin reference documentation for more details.
...
{{prodname}}'s core principle is that *applications* and *workloads*
...
You can securely configure port mapping by following [External connectivity](../networking/external-connectivity.mdx).
...
[External connectivity](../networking/external-connectivity.mdx) explains in more detail
