...
To deploy {{prodname}} in [Google Compute Engine (GCE)][GCE], you must ensure
...
and NAT outgoing on the configured {{prodname}} IP pools.
...
running {{prodname}} in policy-only mode.  Kubernetes GCE cloud provider integration
simplifies route configuration by enabling Kubernetes to handle creating
routes.

## Enabling workload-to-WAN traffic

To allow {{prodname}} networked containers to reach resources outside of GCE,
you must configure outgoing NAT on your [{{prodname}} IP pool][IPPool].
...
[IPIP]: /networking/configuring/vxlan-ipip/[IPPool]: ../resources/ippool.mdx[GCE]: https://cloud.google.com/compute/
