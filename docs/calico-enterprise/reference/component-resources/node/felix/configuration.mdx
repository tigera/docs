...
| `PolicySyncPathPrefix`            | `FELIX_POLICYSYNCPATHPREFIX`            | File system path where Felix notifies services of policy changes over Unix domain sockets. This is required only if you're configuring [L7 logs](/visibility/elastic/l7/configure), or [egress gateways](../../../../networking/egress/index.mdx). Set to `""` to disable. [Default: `""`] | string |
...
See [Enable the eBPF dataplane](../../../../maintenance/ebpf/enabling-ebpf.mdx) for step-by step instructions to enable this feature.
...
|------------------------------|------------------------------------|---------|-----------------------------------------|
| `DropActionOverride`         | `FELIX_DROPACTIONOVERRIDE`         | `Drop`  | How to treat packets that are disallowed by the current {{prodnamew}} policy.  For more detail please see below. |
