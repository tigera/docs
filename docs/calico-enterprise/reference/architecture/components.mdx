...
`{{nodecontainer}}` can be regarded as a helper container that bundles together the
various components required for networking containers with {{prodname}}.  The key
components are:
...
The [calico repository](https://github.com/projectcalico/calico) contains the Dockerfile for `{{nodecontainer}}` along with various
configuration files that are used to configure and "glue" these components
together.

:::note

`{{nodecontainer}}` may be run in *policy only mode* in which Felix
...
The Felix daemon is the heart of {{prodname}} networking.  Felix's primary job is to
...
There are two BIRD processes running in the `{{nodecontainer}}` container.  One for
...
{{prodname}} uses a fork of the main BIRD repo, to include an additional feature
required for IPIP support when running {{prodname}} in a cloud environment.  Refer
...
{{prodname}} uses a fork of the main confd repo which includes an additional change
