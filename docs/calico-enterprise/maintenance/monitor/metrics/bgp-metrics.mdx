...
For a detailed explanation on the various components within `{{noderunning}}` please see [Anatomy of a `{{noderunning}}` container](../../../reference/architecture/components.mdx).
...
- [Secure {{prodname}} Prometheus endpoints](../../../security/comms/secure-metrics.mdx)
