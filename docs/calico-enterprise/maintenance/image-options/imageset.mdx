...
docker inspect {{ operator.registry }}/{{ operator.image }}:{{ operator.version }} -f '{{range .RepoDigests}}{{printf "%s\n" .}}{{end}}'
```
...
Create an [ImageSet](/reference/installation/api#operator.tigera.io/v1.ImageSet) manifest file named `imageset.yaml` like the following:

```yaml
apiVersion: operator.tigera.io/v1
kind: ImageSet
metadata:
  name: calico-{{data.versions.first.title}}
spec:
  images:
...
1. Set the name for your ImageSet to `calico-<version>` (Example: `calico-{{data.versions.first.title}}`).
...
1. Add the correct digest for each image. If you are using a private registry, ensure you pull the image from the private registry and use the digest associated with the private registry.
     ...
     1. Use the digest from the image that matches the repo/image you will use.
        If you are using a private registry or have specified an [imagePath](/reference/installation/api#operator.tigera.io/v1.Installation)
...
3. Verify that the correct ImageSet is being used. In Installation status, check that the `imageset` field is set to the ImageSet you created.
   ...
   ```
       imageSet: calico-{{data.versions.first.title}}
   ```
...
Before upgrading to a new release or downgrading, you must create a new [ImageSet](/reference/installation/api#operator.tigera.io/v1.ImageSet)
...
The [status.imageset](/reference/installation/api#operator.tigera.io/v1.InstallationStatus) field of
