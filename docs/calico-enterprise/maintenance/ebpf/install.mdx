...
- Linux distribution/kernel:
  ...
  - Another [supported distribution](/getting-started/install-on-clusters/kubernetes/requirements/) with Linux kernel v5.3 or above.
...
**Unsupported platforms**
  ...
  - RKE
   ...
   >**Note**:
   eBPF supports AKS with Calico CNI and {{prodname}} network policy. However, with [AKS with Azure CNI and {{prodname}} network policy](/getting-started/install-on-clusters/kubernetes/aks/#install-aks-with-azure-cni-networking), kube-proxy cannot be disabled so the performance benefits of eBPF are lost. However, there are other reasons to use eBPF other than performance gains, as described in [eBPF use cases](use-cases-ebpf.mdx). 
   {: .alert .alert-info}
