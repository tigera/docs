...
but which otherwise meets the {{prodnameWindows}} Kubernetes [cluster requirements](/getting-started/windows-calico/kubernetes/requirements). This guide was tested with RKE v1.18.9.
- One or more Windows nodes that meet the [requirements](/getting-started/windows-calico/kubernetes/requirements).
...
1. Update the `calicoNetwork` options, ensuring that the correct pod CIDR is set. (Rancher uses `10.42.0.0/16` by default.)
   ...
   **BGP**
   ...
   For more information on configuration options available in this manifest, see [the installation reference](/reference/installation/api).
...
1. Finally, follow the {{prodnameWindows}} [quickstart guide for Kubernetes](/getting-started/windows-calico/quickstart#install-calico-enterprise-for-windows-manually)
...
- [Try the basic policy demo](/getting-started/windows-calico/demo)
