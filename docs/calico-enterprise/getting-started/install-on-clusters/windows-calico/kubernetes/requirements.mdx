...
If Calico CNI with VXLAN is used, BGP must be disabled. See the [installation reference](/reference/installation/api#operator.tigera.io/v1.BGPOption).

:::

### Kubernetes version 

See the [Kubernetes requirements](/getting-started/kubernetes/requirements#kubernetes-requirements).
...
- At least four Linux Kubernetes worker nodes to run {{prodname}}'s cluster-wide components that meets [Linux system requirements](/getting-started/kubernetes/requirements), and is installed with {{prodname}} v3.5.0+
...
[Install {{prodnameWindows}}](/getting-started/windows-calico/kubernetes/standard)
