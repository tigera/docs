...
Install {{prodnameWindows}} on Kubernetes clusters. The standard installation for {{prodnameWindows}} requires more time and expertise to configure. If you need to get started quickly, we recommend the [Quickstart](/getting-started/windows-calico/quickstart)
...
- Linux and Windows nodes [meet requirements](/getting-started/windows-calico/kubernetes/requirements)
...
- If using {{prodname}} networking:
   ...
   - Install and configure [calicoctl](../../../../maintenance/clis/calicoctl/install.mdx)
...
1. [Install Calico Enterprise for networking and policy](/getting-started/kubernetes/) for Kubernetes datastore (kdd).
...
1. Modify VXLAN as described in [Overlay Networking](/networking/vxlan-ipip) guide. Note the following:
...
2. Edit the install configuration file, `config.ps1` as follows:
   ...
   | $env:KUBECONFIG | Location of the kubeconfig file {{prodname}} should use to access the Kubernetes API server. To set up a secure kubeconfig with  the correct permissions for {{prodnameWindows}}, see [Create a kubeconfig](/getting-started/windows-calico/kubeconfig) for {{prodnameWindows}}. |
...
- [Create a kubeconfig](/getting-started/windows-calico/kubeconfig)
- [Review network policy limitations in Windows](/getting-started/windows-calico/limitations)
