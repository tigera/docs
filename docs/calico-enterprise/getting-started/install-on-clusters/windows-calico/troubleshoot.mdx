...
If the error includes `loading config file <path-to-kubeconfig\>`, follow the instructions in [Set environment variables](/getting-started/windows-calico/kubernetes/standard#install-calico-enterprise-for-windows-and-kubernetes-on-windows-nodes) to update the `KUBECONFIG` environment variable to the path of your kubeconfig file.

### Felix starts, but does not output logs

By default, Felix waits to connect to the datastore before logging (in case the datastore configuration intentionally disables logging). To start logging at startup, update the [FELIX_LOGSEVERITYSCREEN environment variable](/reference/felix/configuration#general-configuration) to "info" or "debug" level.
