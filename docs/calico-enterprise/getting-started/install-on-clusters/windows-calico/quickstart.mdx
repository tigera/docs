...
Review the [Linux requirements](/getting-started/install-on-clusters/kubernetes/requirements/) and the [{{prodnameWindows}} requirements](kubernetes/requirements.mdx).
...
The following steps install a Kubernetes cluster on a single Windows node with a [minimum](/getting-started/install-on-clusters/kubernetes/requirements/#node-requirements) of 4 Linux worker nodes.
...
- **Kubernetes BGP**
  ...
  The geeky details of what you get by default:
  <GeekDetails prodname='{{prodname}}' details='Policy:Calico,IPAM:Calico,CNI:Calico,Overlay:No,Routing:BGP,Datastore:Kubernetes' />
...
<CalicoWindowsInstall prodname='{{prodname}}' prodnameWindows='{{prodnameWindows}}' networkingType="vxlan" />

</TabItem>
<TabItem label="Kubernetes BGP" value="Kubernetes BGP-4">

<CalicoWindowsInstall prodname='{{prodname}}' prodnameWindows='{{prodnameWindows}}' networkingType="windows-bgp" />
