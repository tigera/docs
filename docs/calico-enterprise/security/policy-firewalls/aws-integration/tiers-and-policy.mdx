...
To avoid accidentally modifying the above tiers and associated network policies, do not allow admin and non-admin users to view and modify them. 
  - Although you cannot hide specific tiers from non-admin users, you can use [RBAC for tiered policies](/security/rbac-tiered-policies), to display only a subset of tiers in the {{prodname}} UI. For help, see [displaying only the net-sec tier](/security/rbac-tiered-policies#user-can-only-view-a-specific-tier).
  - To display tiers and associated network policies, but disable write access to those tiers, see [RBAC example fine-grained permissions](/security/rbac-tiered-policies#fine-grained-rbac-for-policies-and-tiers).
...
- [Enable pod access to AWS metadata](/security/aws-integration/metadata-access)
