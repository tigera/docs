...
This guide is a variation of the [simple policy demo](/security/tutorials/kubernetes-policy-basic) intended to introduce the extra features of {{prodname}} to people already familiar with Project Calico for Kubernetes.
...
[installation guides](/getting-started/kubernetes/),
or by [upgrading an existing cluster](/maintenance/upgrade-to-tsee).

The key steps in moving to {{prodname}} are to change to the {{prodname}} version of calico-node, update its configuration, download [calicoq](/reference/calicoq) and deploy Prometheus.
...
information about the policies for endpoints on a given host.
   ...
   The full calicoq documentation is [here](/reference/calicoq).
...
In {{prodname}} Manager, head to the dashboard view. You will see graphs associated with allowed packets/bytes and denied packets/bytes. The graphs represent the rates at which packets/bytes are being allowed or denied and are time windowed.
...
1. You can clean up the demo by deleting the demo namespace.
   ...
   This was just a simple example of the Kubernetes NetworkPolicy API and how {{prodname}} can secure your Kubernetes cluster.  For more
