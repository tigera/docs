...
The solution is to define policy for workload services, not for ports used by workloads. For help, see [Policy for services](/security/services).
...
For help with Pass action rules, see [Get started with tiered policy](tiered-policy.mdx).