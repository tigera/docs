...
To understand how tiered policy works and best practices, see [Get started with tiered policies](tiered-policy.mdx).
...
Here are sample [default deny policies](/security/kubernetes-default-deny).
...
For examples, see [basic ingress and egress policies](/security/calico-network-policy).
...
For details, see [Policy life cycle tools](policy-lifecycle/index.mdx).    
