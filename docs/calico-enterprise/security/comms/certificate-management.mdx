...
temporarily remove [the logstorage resource](/reference/installation/api#operator.tigera.io/v1.LogStorage)
before following the steps to enable certificate management and then re-apply afterwards. For detailed steps on 
re-creating logstorage, read more on [how to create a new Elasticsearch cluster](../../visibility/troubleshoot.mdx#how-to-create-a-new-cluster).

Currently, this feature is not supported in combination with [Multi-cluster management](../../multicluster/mcm/create-a-management-cluster.mdx).
...
1. Modify your [the installation resource](/reference/installation/api#operator.tigera.io/v1.Installation)
