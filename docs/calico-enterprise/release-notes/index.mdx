...
---
<div className='git-hash' id="{{data['hash']}}">
</div>

The following table shows component versioning for {{prodname}}  **{{ page.version }}**.
...
[Release archive]({{downloadsurl}}/ee/archives/release-{{release.title}}-{{release.tigera-operator.version}}.tgz){: data-proofer-ignore=""} with Kubernetes manifests. Based on Calico {{data.versions.first.calico.minor_version}}.
{% endunless %}
