...
1. Add any labels on existing all-interfaces host endpoints to their corresponding {{include.orch}} nodes. {{prodname}} manages labels on automatic host endpoints by syncing
...
2. Enable auto host endpoints by following the [enable automatic host endpoints how-to guide](../../security/hosts/kubernetes-nodes.mdx#enable-automatic-host-endpoints).
   ...
   ```
3. Delete old all-interfaces host endpoints. You can distinguish host endpoints managed by {{prodname}} from others in several ways. First, automatic host endpoints
