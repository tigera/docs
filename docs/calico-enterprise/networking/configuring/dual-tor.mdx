...
**{{nodecontainer}}** run as a container, pre-Kubernetes
...
designs](/reference/architecture/design/l3-interconnect-fabric).  One of
...
podman run --privileged --net=host \
    ...
    {{page.registry}}{{imageNames["node"]}}:latest
```
...
ExecStart=/bin/podman run --rm --privileged --net=host --name=calico-early -v /etc/calico-early:/etc/calico-early -e CALICO_EARLY_NETWORKING=/etc/calico-early/details.yaml {{page.registry}}{{imageNames["node"]}}:latest
[Install]
...
With OpenShift, follow [our documentation](/getting-started/openshift) as
far as the option to [provide additional configuration](/getting-started/openshift/installation).
...
Follow [our documentation](/getting-started/kubernetes/generic-install) as
