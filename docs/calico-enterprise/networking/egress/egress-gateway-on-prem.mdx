...
{{prodname}}'s own policy (including [DNS policy](../../security/domain-based-policy.mdx)) and
...
even a [specific IP](/networking/use-specific-ip) for a new pod, but this requires predicting how many pods
...
pool](/networking/legacy-firewalls) and to perform an SNAT on the traffic passing through them.  Hence any
...
The source port of an outbound flow through an egress gateway can generally *not* be
...
name](/security/domain-based-policy) here, because the gateway's node will not see
...
- Set `ipipMode` or `vxlanMode` to `Always` if the pod network has [IPIP or VXLAN](/networking/vxlan-ipip) enabled. 
