...
See the [OpenShift installer documentation](https://cloud.redhat.com/openshift/install) for more information
...
Override the OpenShift networking to use {{prodname}} and update the AWS instance types to meet the [system requirements](/getting-started/openshift/requirements):
...
To include [{{prodname}} resources](../../../calico-enterprise/reference/resources/index.mdx) during installation, edit `manifests/02-configmap-calico-resources.yaml` in order to add your own configuration.

> **Notes**: If you have a directory with the {{prodname}} resources, you can create the file with the command:
> ```
...
{{prodname}} requires storage for logs and reports. Before finishing the installation, you must [create a StorageClass for {{prodname}}](../../../calico-enterprise/getting-started/create-storage.mdx).
...
Download the Tigera custom resources. For more information on configuration options available in this manifest, see [the installation reference](/reference/installation/api).
...
<OpenShiftPrometheusOperator prodname='{{prodname}}' operation="install" />
...
{% if include.clusterType == "managed" %}
<ConfigureManagedCluster prodname='{{prodname}}' kubectlCmd="oc" />
