...
- Linux kernel 3.10 or later with [required dependencies](#kernel-dependencies).
  ...
  known to work well with {{prodname}} and {{include.orch}}.{% if include.orch == "Kubernetes" or include.orch == "host protection" %}
  - CentOS 8
...
- {{prodname}} must be able to manage `cali*` interfaces on the host. When IPIP is
  ...
  [Preventing NetworkManager from controlling {{prodname}} interfaces](../../../calico-enterprise/maintenance/troubleshoot/troubleshooting.mdx#configure-networkmanager)
...
- If your Linux distribution comes with installed Firewalld or another iptables manager it should be disabled. 
  ...
  More information about configuration at [Security for host](../../../calico-enterprise/security/hosts/index.mdx).
  
  :::

- In order to properly run Elasticsearch, nodes must be configured according to the
  [Elasticsearch system configuration documentation.](https://www.elastic.co/guide/en/elasticsearch/reference/current/system-config.html)
...
Ensure that your hosts and firewalls allow the necessary traffic based on your configuration. See [Component architecture](../../../calico-enterprise/reference/architecture/overview.mdx) to view the following components. 
