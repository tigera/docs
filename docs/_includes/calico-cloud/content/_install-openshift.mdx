...
To include [{{prodname}} resources](../../../calico-cloud/reference/resources/index.mdx) during installation, edit `manifests/02-configmap-calico-resources.yaml` in order to add your own configuration.
...
{{prodname}} requires storage for logs and reports. Before finishing the installation, you must [create a StorageClass for {{prodname}}](/getting-started/create-storage).
...
To secure the components that make up {{prodname}}, install the following set of network policies.
