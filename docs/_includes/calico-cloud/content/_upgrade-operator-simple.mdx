...
1. If you previously [installed using a private registry](/getting-started/private-registry), you will need to
   [push the new images](/getting-started/private-registry/private-registry-regular#push-calico-enterprise-images-to-your-private-registry)
   and then [update the manifest](/getting-started/private-registry/private-registry-regular#run-the-operator-using-images-from-your-private-registry)
...
1. If your cluster has OIDC login configured, follow these steps:
   ...
   ```
   If both of these resources are present, you can continue with the next step. Otherwise, use the instructions to [configure an identity provider](/getting-started/cnx/configure-identity-provider) to configure OIDC.

   d) Follow [configure an identity provider](/getting-started/cnx/configure-identity-provider).

1. If your cluster is a management cluster using v3.1 or older, apply a [ManagementCluster](/reference/installation/api#operator.tigera.io/v1.ManagementCluster)
   CR to your cluster.
   ```bash
   kubectl apply -f - <<EOF
...
1. If your cluster is v3.7 or older, apply a new [Monitor](/reference/installation/api#operator.tigera.io/v1.Monitor)
   CR to your cluster.
...
1. Install the new network policies to secure {{prodname}} component communications.
