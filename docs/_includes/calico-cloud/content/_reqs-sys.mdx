...
- {{prodname}} must be able to manage `cali*` interfaces on the host. When IPIP is
  ...
  [Preventing NetworkManager from controlling {{prodname}} interfaces](/maintenance/troubleshoot/troubleshooting#configure-networkmanager)
...
- If your Linux distribution comes with installed Firewalld or another iptables manager it should be disabled. 
  ...
  More information about configuration at [Security for host](/security/hosts).
...
Ensure that your hosts and firewalls allow the necessary traffic based on your configuration. See [Component architecture](/reference/architecture/overview) to view the following components. 
