...
{{prodname}}'s own policy (including [DNS policy](../../security/beginners/domain-based-policy.mdx)) and
...
name](../../security/beginners/domain-based-policy.mdx) here, because the gateway's node will not see
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: egress-gateway
        image: {{page.registry}}{% component_image("egress-gateway") %}
        env:
        - name: EGRESS_POD_IP
          valueFrom:
            fieldRef:
