...
You must be using {{prodname}} IPAM.
...
If it is present, you are using {{prodname}} IPAM. If the IPAM is set to something else, or the 10-calico.conflist file does not exist, you cannot use these features in your cluster.
...
For help configuring {{prodname}} CNI and {{prodname}} IPAM, see [Configuring the {{prodname}} CNI Plugins](../../reference/cni-plugin/configuration.mdx).
