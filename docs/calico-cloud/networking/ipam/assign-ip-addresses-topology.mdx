...
Node selection-based IP address assignment is exactly what it sounds like: node labels are set, and {{prodname}} uses node selectors to decide which IP pools to use when assigning IP addresses to the node.
...
[{{prodname}} IPAM](../../reference/cni-plugin/configuration.mdx)
