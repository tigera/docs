...
A best practice is to develop an allow list of "known-good" domains that particular applications or services must access, and then [enforce this allow list with network policy](/security/domain-based-policy).
