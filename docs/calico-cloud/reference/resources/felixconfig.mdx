...
| awsSecondaryIPSupport              | Controls whether Felix will create secondary AWS ENIs for AWS-backed IP pools.  This feature is documented in the egress gateways on AWS guide].  Should only be enabled on AWS. | `Enabled`, `Disabled` | string | `Disabled` |
...
| ipipEnabled                        | Whether Felix should configure an IPinIP interface on the host. Set automatically to `true` by `{{nodecontainer}}` or `calicoctl` when you create an IPIP-enabled pool. | boolean | `false` |
| ipipMTU                            | The MTU to set on the tunnel device. Zero value means auto-detect.  | int | int | `0` |
...
| policySyncPathPrefix               | File system path where Felix notifies services of policy changes over Unix domain sockets. This is only required if you're configuring application layer policy, or egress gateways. Set to `""` to disable. | string | string | `""` |
...
| serviceLoopPrevention              | When service IP advertisement is enabled, prevent routing loops to service IPs that are not in use, by dropping or rejecting packets that do not get DNAT'd by kube-proxy.  Unless set to "Disabled", in which case such routing loops continue to be allowed. | `Drop`, `Reject`, `Disabled` | string | `Drop` |
| sidecarAccelerationEnabled         | Enable experimental acceleration between application and proxy sidecar when using application layer policy. [Default: `false`] | boolean | boolean | `false` |
...
| wireguardMTU                       | MTU set on the WireGuard interface created by Felix. Zero value means auto-detect. | int | int | 0 |
...
| bpfEnabled                         | Enable eBPF dataplane mode.  eBPF mode has a number of limitations. ).  This is a tech preview feature and subject to change in future releases. | true, false | boolean | false |
