...
| natOutgoing | When enabled, packets sent from {{prodname}} networked containers in this pool to destinations outside of this pool will be masqueraded. | true, false | boolean | `false` |
...
| nodeSelector | Selects the nodes where {{prodname}} IPAM should assign pod addresses from this pool.  Can be overridden if a pod [explicitly identifies this IP pool by annotation](../cni-plugin/configuration.mdx#using-kubernetes-annotations). | | [selector](#node-selector) | all() |
...
{{prodname}} supports Kubernetes [annotations that force the use of specific IP addresses](../cni-plugin/configuration.mdx#requesting-a-specific-ip-address). These annotations take precedence over the `allowedUses` field.
...
Assign IP addresses based on topology guide..
