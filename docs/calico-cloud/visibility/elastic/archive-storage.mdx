...
The default log vendor is Elasticsearch. To meet compliance storage requirements, you can export {{prodname}} logs to SIEMs like syslog, Splunk, or Amazon S3.
...
3. Update the [LogCollector](/reference/installation/api#operator.tigera.io/v1.LogCollector)
   resource named, `tigera-secure` to include an [S3 section](/reference/installation/api#operator.tigera.io/v1.S3StoreSpec)
...
1. Update the [LogCollector](/reference/installation/api#operator.tigera.io/v1.LogCollector)
   resource named `tigera-secure` to include a [Syslog section](/reference/installation/api#operator.tigera.io/v1.SyslogStoreSpec)
...
2. You can control which types of {{prodname}} log data you would like to send to syslog. 
   The [Syslog section](/reference/installation/api#operator.tigera.io/v1.SyslogStoreSpec) 
   ...
   Refer to the [Syslog section](/reference/installation/api#operator.tigera.io/v1.SyslogStoreSpec) for more details on what data each log type represents.
   ...
   The log type `IDSEvents` is only supported for a cluster that has [LogStorage](/reference/installation/api#operator.tigera.io/v1.LogStorage) configured. It is because intrusion detection event data is pulled from the corresponding LogStorage datastore directly.
...
3. Update the
   [LogCollector](/reference/installation/api#operator.tigera.io/v1.LogCollector)
   resource named `tigera-secure` to include
   a [Splunk section](/reference/installation/api#operator.tigera.io/v1.SplunkStoreSpec)
...
## Forward global alerts to a SIEM

{{prodname}} supports setting up global alerts on the following logs:
...
To forward global alerts to a SIEM, notify Support in the Contact page in Manager UI, and open a [Support ticket](https://support.tigera.io/s/login/?ec=302&startURL=%2Fs%2F) with the following information:

- Endpoint to your SIEM/Syslog server setup. For example:

   `syslog: endpoint: tcp://rsyslog-service.default.svc:522`
