...
- **Manager UI** with built-in Elasticsearch and Kibana instances
...
- A `tigera-network-admin` role with full permissions to create and modify resources. 

- To view Elasticsearch resources in {{prodname}} Manager, users must have [minimum permissions](../../security/rbac-tiered-policies.mdx).
...
- Configure [RBAC for tiered policies](../../security/rbac-tiered-policies.mdx).
