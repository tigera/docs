...
To avoid accidentally modifying the above tiers and associated network policies, do not allow admin and non-admin users to view and modify them. 
  - Although you cannot hide specific tiers from non-admin users, you can use [RBAC for tiered policies](/security/rbac-tiered-policies), to display only a subset of tiers in the {{prodname}} UI. For help, see [displaying only the net-sec tier](../../rbac-tiered-policies.mdx#user-can-only-view-a-specific-tier).
  - To display tiers and associated network policies, but disable write access to those tiers, see [RBAC example fine-grained permissions](../../rbac-tiered-policies.mdx#fine-grained-rbac-for-policies-and-tiers).
