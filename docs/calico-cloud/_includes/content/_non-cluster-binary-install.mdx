...
1. Use the following command to download the {{nodecontainer}} image.
   ...
   ```bash
   docker pull {{page.registry}}{% component_image("cnx-node") %}
   ```
...
1. Create a temporary {{nodecontainer}} container.
   ...
   ```bash
   docker create --name container {{page.registry}}{% component_image("cnx-node") %}
   ```
...
<EnvironmentFile prodname='{{prodname}}' nodecontainer='{{nodecontainer}}'  />
...
See [Felix configuration](../../reference/felix/configuration.mdx) for help with environment variables.
