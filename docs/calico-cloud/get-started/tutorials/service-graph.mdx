...
For more examples of using network sets, see [Secure egress access from workloads to destinations outside the cluster](../../workload-access/applications/egress-controls.mdx), and [Understanding network sets](networksets.mdx).
...
> Click on the edge between the *hipstershop* namespace, and the *public network* network set. The flow logs are automatically filtered below in the Flows tab, and you can expand any of the rows in that tab to view some of the detailed metadata that {{prodname}} provides around workload communication.
