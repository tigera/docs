...
the new IP addresses. This limitation can be circumvented using {{prodname}} [Use external IPs or networks rules in policy](/security/external-ips-policy), or [DNS policy](/security/domain-based-policy) in policy rules.
