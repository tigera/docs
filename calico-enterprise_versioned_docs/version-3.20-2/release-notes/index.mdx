...
#### Known issues
* **Important:** If you are using the Authentication resource, the Dex pod has a problem starting due to an entrypoint bug. While we are working on a patch,
    replace the tigera-operator deployment with the following image: `quay.io/tigera/operator:v1.36.10.1`.
