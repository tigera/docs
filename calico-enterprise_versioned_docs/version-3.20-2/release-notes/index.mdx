...
* The eBPF dataplane will fall back to non-CO-RE if CO-RE does not load. This will affect older kernel versions, including AKS and EKS.
* Security updates.
...
* Fixes an issue where the Manager UI reports "Invalid value" when editing and saving staged policies.
* Security updates.
