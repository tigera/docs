...
| `logSeverity` _[LogLevel](#loglevel)_ | (Optional) LogSeverity defines the logging level of the Egress Gateway.<br />Default: Info |
...
| Field | Description |
| --- | --- |
| `name` _string_ | Name is an enum which identifies the Manager Deployment container by name.<br />Supported values are: tigera-voltron, tigera-manager, tigera-es-proxy |
...
| Field | Description |
| --- | --- |
| `endpoint` _string_ | Location of the log ingestion point for non-cluster hosts. example: https://1.2.3.4:443 |
