...
You may notice that the `kubernetes-control-plane` and `kubernetes-worker` are in waiting status due to missing CNI. This status can be prevented by ensuring setting `ignore-missing-cni=true` in the `kubernetes-control-plane` and `kubernetes-worker` charms
...
:::note
- `ignore-missing-cni=true` allows the `kubernetes-control-plane` and `kubernetes-worker` charms to be ready without waiting for a CNI plugins to be installed since $[prodname] will provide its own
:::

To allow setup the CNI to be taken care of by $[prodname], the `kubernetes-control-plane` charm also needs to allow privileged pods. Run
```bash
