...
:::note
- `ignore-missing-cni=true` allows the `kubernetes-control-plane` and `kubernetes-worker` charms by running: to be ready without waiting for a CNI plugins to be installed since $[prodname] will provide its own.
:::
