...
import InstallGeneric from '@site/calico-enterprise_versioned_docs/version-3.20-2/_includes/components/InstallGeneric';
...
## Prepare a compatible cluster for $[prodname] using a custom bundle

:::caution
The default Charmed Kubernetes bundle includes a managed calico charm that is currently unsupported by the operator to easily migrate to $[prodname].
...
1. Get a copy of the bundle file located in [Charmed Kubernetes Github Releases](https://github.com/charmed-kubernetes/bundle/tree/main/releases).

    For example, to get the default bundle for the v1.31 charmed kubernetes release:

    ```bash
    curl -o charmed-kubernetes-bundle.yaml -L https://raw.githubusercontent.com/charmed-kubernetes/bundle/refs/heads/main/releases/1.31/bundle.yaml
    ```
...
## Setup Juju and deploy the cluster using your custom bundle file

1. Configure juju with a default credential by adding a new credential or use an existing credential:
   ```
...
1. Create the Charmed Kubernetes cluster by specifying the bundle file you created in the `juju deploy` command:
   ```
...
1. Ensure the applications and units are active in the model by running:
   ...
   App                       Version  Status   Scale  Charm                     Channel      Rev  Exposed  Message
    containerd                1.6.38   active       5  containerd                1.31/stable   90  no       Container runtime available
    easyrsa                   v3.0.9   active       1  easyrsa                   1.31/stable   74  no       Certificate Authority connected.
    etcd                      3.4.37   active       3  etcd                      1.31/stable  788  no       Healthy with 3 known peers
    kubeapi-load-balancer     1.18.0   active       1  kubeapi-load-balancer     1.31/stable  196  yes      Ready
    kubernetes-control-plane  1.31.x   waiting      2  kubernetes-control-plane  1.31/stable  652  no       Waiting for 3 kube-system pods to start
    kubernetes-worker         1.31.x   active       3  kubernetes-worker         1.31/stable  369  yes      Ready
    ```
   :::
