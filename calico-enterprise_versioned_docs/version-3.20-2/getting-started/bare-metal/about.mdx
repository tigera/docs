...
1. Install Calico node and fluent-bit log forwarder packages.
    ...
    To pin a specific version of `calico-node` so that it won't be automatically upgraded, use the `versionlock` plugin for dnf. Note that this won't install calico-node if it isn't installed already.
