...
| Field | Description |
| --- | --- |
| `name` _string_ | Name is an enum which identifies the calico-kube-controllers Deployment container by name.<br />Supported values are: calico-kube-controllers |
...
| `ipPools` _[IPPool](#ippool) array_ | (Optional) IPPools contains a list of IP pools to create if none exist. At most one IP pool of each address family may be specified. If omitted, a single pool will be configured if needed. |
