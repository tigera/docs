...
<li>Some application layer features are not working as expected for {variables.prodname} installations with the following deployment types:
  <ul><li>AKS clusters with Azure CNI for networking and {variables.prodname} for network policy</li>
...
{variables.prodname} 3.19 contains breaking changes for installations that use the Calico API server.

:::

* ***Breaking change***: Upgrading from Calico Enterprise 3.18 or earlier {variables.prodname} will alter the UID of all `projectcalico.org/v3` resources.
