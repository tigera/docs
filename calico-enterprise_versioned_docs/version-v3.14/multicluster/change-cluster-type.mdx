...
1. Replace the network policies to secure the standalone cluster with network policies for the managed cluster.
   ```bash-plain-text
   kubectl delete -f {{url}}{{baseUrl}}/manifests/tigera-policies.yaml
   kubectl create -f {{url}}{{baseUrl}}/manifests/tigera-policies-managed.yaml
   ```
...
1. Ensure a persistent volume is provisioned to store log data for the standalone cluster.
   See [Configure storage for logs and reports](../maintenance/logstorage/create-storage.mdx).
...
1. Install the Tigera custom resources.
   ...
   ```bash-plain-text
   kubectl apply -f {{url}}{{baseUrl}}/manifests/custom-resources.yaml
   ```
1. Delete the network policies that secure managed clusters.
   ```bash-plain-text
   kubectl delete -f {{url}}{{baseUrl}}/manifests/tigera-policies-managed.yaml
   ```
