...
- [Install and configure calicoq](../../maintenance/clis/calicoq/installing.mdx)
...
1. If RBAC is enabled, apply this manifest.
   ...
   ```bash-plain-text
   kubectl apply -f \
   {{url}}{{baseUrl}}/getting-started/kubernetes/installation/federation-rem-rbac-kdd.yaml
   ```
...
1. Apply the following manifest to create a service account called `tigera-federation-remote-cluster`.
   ...
   ```bash-plain-text
   kubectl apply -f \
   {{url}}{{baseUrl}}/getting-started/kubernetes/installation/federation-remote-sa.yaml
   ```
