...
import OpenshiftManifests from '@site/calico-enterprise_versioned_docs/version-v3.14/_includes/content/_openshift-manifests.mdx';
...
If your cluster has Windows nodes and uses custom TLS certificates for log storage, prior to upgrade, prepare and apply new certificates for [log storage](../../../maintenance/comms/log-storage-tls.mdx) that include the required service DNS names.
...
1. If your cluster is management or standalone cluster using v3.8 or older, follow these steps:
   ...
   ```bash-plain-text
   kubectl apply -f {{url}}{{baseUrl}}/manifests/ocp/tigera-policies-es-access.yaml
   ```
...
1. Update the network policies.
   ...
   ```bash-plain-text
   oc apply -f {{url}}{{baseUrl}}/manifests/ocp/tigera-policies-managed.yaml
   ```

   a. If your cluster is a standalone or management cluster:

   ```bash-plain-text
   oc apply -f {{url}}{{baseUrl}}/manifests/ocp/tigera-policies.yaml
   ```
...
1. Remove unused policies in your cluster.
   ...
   ```bash-plain-text
   kubectl delete -f {{url}}{{baseUrl}}/manifests/default-tier-policies-managed.yaml
   ```

   For other clusters, run this command:

   ```bash-plain-text
   kubectl delete -f {{url}}{{baseUrl}}/manifests/default-tier-policies.yaml
   ```
