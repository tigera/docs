...
1. Install the {{prodname}} custom resource definitions.
   ...
   ```bash-plain-text
   kubectl apply -f {{url}}{{baseUrl}}/manifests/operator-crds.yaml
   kubectl apply -f {{url}}{{baseUrl}}/manifests/prometheus-operator-crds.yaml
   kubectl apply -f {{url}}{{baseUrl}}/manifests/eck-operator-crds.yaml
   ```
