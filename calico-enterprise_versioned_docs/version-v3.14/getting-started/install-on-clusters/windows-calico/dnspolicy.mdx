...
{{prodnameWindows}} DNS policy is enabled by default and is configured the same way as [DNS policy for Linux](../../../security/domain-based-policy.mdx). Use the following Windows-specific parameters for specifying the file to preserve learned DNS information and extra TTL in addition to DNS TTL.
