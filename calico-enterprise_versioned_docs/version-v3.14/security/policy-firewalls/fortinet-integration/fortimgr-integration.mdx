...
- Experience with [ tiered policy](/security/policy-tiers/tiered-policy/) and [ global network policy](../../../reference/resources/globalnetworkpolicy.mdx)
...
Create a [{{prodname}} tier](/security/policy-tiers/tiered-policy/) in the {{prodname}} Manager UI for each Kubernetes cluster you want to secure. We recommend that you create a new tier (rather than reusing an existing tier) for all global network policies created by the {{prodname}} integration controller.
