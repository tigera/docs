...
| -------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------- | ------------ | ------------------------------ |
| flow     | Network flows for workloads: source and destination namespaces, pods, labels, and policies     | $[prodname] node (Felix)               | `flows`      | `tigera_secure_ee_flows.*`     |
...
| bgp      | $[prodname] networking BGP peering and route propagation                                      | $[prodname] node (BIRD)                | `ee_bgp`     | `tigera_secure_ee_bgp.*`       |
| dns      | DNS lookups and responses from $[prodname] domain-based policy                                | $[prodname] node (Felix)               | `ee_dns`     | `tigera_secure_ee_dns.*`       |
