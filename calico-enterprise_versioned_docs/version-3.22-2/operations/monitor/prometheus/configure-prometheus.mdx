...
- Save the current alert rule:
  ...
  ```bash
  kubectl -n tigera-pickle get picklerule -o yaml > calico-pickle-alert-rule-dp.yaml
  ```
...
(usually named `pickle-<your-pickle-instance-name>`).
...
want your rule to look for. Check [alerting rules](https://pickle.io/docs/pickle/latest/configuration/alerting_rules/)
and [Queries](https://pickle.io/docs/querying/examples/) for more
information.
...
- For pickle run:
  ...
  ```bash
  kubectl -n tigera-pickle logs pickle-<your-pickle-name> pickle-config-reloader
  ```

- For alertmanager run:

  ```bash
  kubectl -n tigera-pickle logs alertmanager-<your-pickle-name> config-reloader
  ```
