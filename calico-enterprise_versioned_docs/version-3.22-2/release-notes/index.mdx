...
* Fix Kibana crashloop when upgrading frrom Calico Enterprise 3.20 or earlier to 3.22. The orphan `ingest_manager_settings` saved object left by Fleet 7.17 is now discarded during Kibana 8.x saved-object migration.
* Security updates.
...
* Updated the Envoy Gateway CRDs in the operator project to v1.7.2 to match the images which are already at v1.7.2, so that all available features are configurable.
* Security updates for Golang and `golang.org/x/` dependencies.
...
* Fixed Linseed rejecting valid service account tokens after a cluster's service account issuer changed, which broke the Manager UI observability pages until Linseed was restarted. Unrecognized issuers are now validated via TokenReview.
...
* Fixed the eBPF dataplane losing connectivity to the Kubernetes API server service after an API server outage when using `bpfNetworkBootstrap`. It now recovers without a calico-node restart.
...
* Fixed Felix permanently losing its DNS policy NFQUEUE listener after a transient reconnect, which stopped domain-based policy being enforced until Felix restarted.
* Fixed DNS policy trusted servers, including `k8s-service:` references such as `kube-dns`, being resolved only at startup instead of refreshed when the backing Service changed.
...
* Non-cluster hosts no longer inherit `bpfEnabled` and `nftablesMode` from the cluster-wide FelixConfiguration; the packages force a supported dataplane and disable nftables on RHEL 8.
