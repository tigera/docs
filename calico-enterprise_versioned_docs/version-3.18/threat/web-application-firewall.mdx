...
[ModSecurity module](https://owasp.org/www-project-modsecurity-core-rule-set/). To view the rule set that is bundled with WAF, see [rule-set](https://github.com/tigera/operator/blob/master/pkg/render/applicationlayer/modsec-core-ruleset).
...
The rules are not overridden during upgrade, you will have to manage deploying updates of the OWASP CoreRuleSet to your cluster over time. 

If you modify rules, it is recommended to keep your main version in git or similar version control systems. If you enable WAF, modify rules, and then disable WAF again then the rule modifications will be lost. 
...
A Security Event is created for [you to review in the UI](#view-waf-events) for every request in which WAF detects a rule match, regardless of whether it was ultimately allowed or denied. This can greatly help in tuning later.  
...
1. Edit the crs-setup.conf file and set the `tx.inbound_anomaly_score_threshold` to the value of your choice: 
   ```bash
     SecAction \
      "id:900110,\
      phase:1,\
