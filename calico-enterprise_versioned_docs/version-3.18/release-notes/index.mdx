...
We've added a new Severity Events Management dashboard in Manager UI for threat detection. The dashboard provides useful context on suspicious activity detected in your cluster, alongside the Kubernetes context so you can see which workloads are affected. [Security event management](../threat/security-event-management).
