...
1. Export the service port number
  ...
  ```bash
  export EXT_NODE_PORT=<your-load-balancer-external-node-port>
  ```
...
1. Export the service port number
  ...
  ```bash
  export MANAGEMENT_CLUSTER_ADDR=<your-management-cluster-addr>:$EXT_SERVICE_PORT
  ```
...
If the external service is a LoadBalancer on AWS, then set the necessary service annotations:
...
See [Create a network load balancer for AWS](https://docs.aws.amazon.com/eks/latest/userguide/network-load-balancing.html) for more information.
