...
A federated service (also called a backing service), is a set of services with consolidated endpoints. {variables.prodname} discovers services across all clusters (both local cluster and remote clusters) and creates a "federated service" on the local cluster that encompasses all of the individual services.
