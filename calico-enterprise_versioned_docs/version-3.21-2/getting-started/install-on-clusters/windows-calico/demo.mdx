...
Windows Server 2022 (build 20348.169 or newer).

:::note

Windows Server 2022 (build older than 17763.1432) do not support [direct server return](https://techcommunity.microsoft.com/t5/networking-blog/direct-server-return-dsr-in-a-nutshell/ba-p/693710). This means that policy support is limited to only pod IP addresses.
...
The powershell and porter pod manifests below use images based on `mcr.microsoft.com/windows/servercore:ltsc2022`.
...
The pwsh and porter pod manifests below use images based on mcr.microsoft.com/windows/servercore:ltsc2022. If you are using a more recent Windows Server version, update the manifests to use a servercore image that matches your Windows Server version.
