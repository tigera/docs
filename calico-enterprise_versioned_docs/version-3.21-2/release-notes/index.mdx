...
* Fixed felix panic when upgrading to v3.21.1 in eBPF mode.
* Fixed an issue so Flow logs now correctly track packet and byte counts for forwarded traffic (fwd reporter), ensuring accurate volume metrics for complete traffic visibility and analysis.
