---
description: Reference for the KubeControllersConfiguration resource in Calico Enterprise that controls behavior of the kube-controllers component.
---
...
A $[prodname] Kubernetes controllers configuration resource (`KubeControllersConfiguration`) configures the controllers that run inside the `calico-kube-controllers` container. These controllers monitor the Kubernetes API and reconcile cluster state with the $[prodname] datastore.

The `calico-kube-controllers` container can run the following controllers; each is enabled by including a corresponding block under `spec.controllers`, and disabled by omitting the block.
...
- $[prodname] automatically creates a resource named `default` containing the configuration settings, only the name `default` is used and only one object of this type is allowed. You can use [calicoctl](../clis/calicoctl/overview.mdx) to view and edit these settings
