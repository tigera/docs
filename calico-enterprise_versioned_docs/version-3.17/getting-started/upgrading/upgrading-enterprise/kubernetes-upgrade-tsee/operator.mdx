...
import UpgradeOperatorSimple from '@site/calico-enterprise_versioned_docs/version-3.17/_includes/components/UpgradeOperatorSimple';
...
For all other platforms or versions older than v3.11.2, upgrading {variables.prodnameWindows} can be performed out-of-band with the {variables.prodname} upgrade of the cluster. For each Windows node, [uninstall](../../../install-on-clusters/windows-calico/maintain.mdx#uninstall-calico-enterprise-for-windows-from-windows-nodes) the {variables.prodnameWindows} services, copy over the latest {variables.prodnameWindows} installation archive, then proceed with the [installation](../../../install-on-clusters/windows-calico/quickstart.mdx#install-calico-enterprise-for-windows-manually).
