...
- Flow log are continuously monitored for policy recommendations
...
You can now view the policies **Policies Board**.

In the left navbar, click **Policies**.

Policy recommendations are added to the **namespace-isolation** tier. Note the following:

- Staged network policy recommendations work like any other staged network policies.
- You cannot move policy recommendation staged policies.
...
- **Processing Interval** is the frequency to run policy recommendation to create/refine recommended policies.
...
#### Re-learn activate recommendations
...
To generate a new recommendation for an enforced policy, delete or rename the staged network policy on the **Policy** board.
...
**Notes**:
This option is not available on managed clusters of a MCM instance.
