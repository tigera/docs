...
This topic lists {variables.prodname} anomaly detectors and their descriptions.

:::note

If you have detectors currently running that are not listed on this page, they will not be managed on {variables.prodname} and will not show up on Manager UI. Delete them manually through `kubectl` by running the command: `kubectl delete globalalert tigera.io.detector.<detector-name>`, replacing `_` in the `<detector-name>` with `-`.

:::

## Security anomaly detectors

The following detectors are primarily searching for [security anomalies](../threat/anomaly-detection/security-anomalies.mdx) related to malicious activities.
