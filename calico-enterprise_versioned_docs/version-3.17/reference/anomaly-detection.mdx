...
If you have detectors currently running that are not listed on this page, they will not be managed on $[prodname] and will not show up on the web console. Delete them manually through `kubectl` by running the command: `kubectl delete globalalert tigera.io.detector.<detector-name>`, replacing `_` in the `<detector-name>` with `-`.
