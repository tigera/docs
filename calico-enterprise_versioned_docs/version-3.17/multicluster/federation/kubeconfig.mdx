...
Each cluster in a cluster mesh acts as both a local and remote cluster.
...
1. If RBAC is enabled, apply this manifest.
   ...
   kubectl apply -f \
   \{variables.filesUrl}/manifests/federation-rem-rbac-kdd.yaml
   </CodeBlock>
...
1. Apply the following manifest to create a service account called `tigera-federation-remote-cluster`.
   ...
   <CodeBlock language='bash'>
   kubectl apply -f \
   \{variables.filesUrl}/manifests/federation-remote-sa.yaml
   </CodeBlock>
...
- Service accounts (if you configure {variables.prodname} federated services)
