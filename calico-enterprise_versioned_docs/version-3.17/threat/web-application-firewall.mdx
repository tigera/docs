...
Protect cloud-native applications from application layer attacks with {variables.prodname} Workload-based Web Application Firewall (WAF).
...
{variables.prodname} WAF allows you to selectively run service traffic within your cluster, and protect intra-cluster traffic from common HTTP-layer attacks such as SQL injection, and cross-site request forgery. To increase protection, you can use {variables.prodname} network policies to enforce security controls on selected pods on the host.
...
### About {variables.prodname} WAF

WAF is deployed in your cluster as an Envoy DaemonSet. {variables.prodname} proxies selected service traffic through Envoy, checking HTTP requests using the industry-standard 
...
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| DetectionOnly | Traffic is not denied nor dropped. {variables.prodname}  will log events. | No
| On     | Denies HTTP traffic. {variables.prodname}  will log the event in Elasticsearch. | Yes          |
...
By default, {variables.prodname} ships with CoreRuleset v3.x with the following setup files pre-loaded:
