...
All you need to do is install the anomaly detection within your cluster. If there are any security or performance anomalies, you will automatically get alerts in the web console.

## Concepts

### About anomalies

$[prodname] anomaly detection allows you to proactively determine if there is an issue (or not), and potentially resolve problems before service levels are compromised. Anomaly detection uses $[prodname] Elasticsearch logs ([flow](../../visibility/elastic/flow/index.mdx) logs, [L7](../../visibility/elastic/l7/index.mdx) logs, and [DNS](../../visibility/elastic/dns/index.mdx) logs) to learn the behavior of cluster nodes, pods, services, and other entities that send log records (applications, load balancers, databases, etc.).
...
To persist storage for the training models [configure a storage class](storage.mdx)

## How To

- [Enable and disable anomaly detectors](#enable-and-disable-anomaly-detectors)
- [Monitor anomaly alerts in the web console](#monitor-anomaly-alerts-in-manager-ui)

### Enable and disable anomaly detectors

By default all Anomaly Detectors are disabled. To enable/disable a detector, follow these steps in the web console.
...
![anomaly-detection-enable-alert](/img/calico-enterprise/anomaly-detection-enable-alert.png)

### Monitor anomaly alerts in the web console

You can monitor anomaly alerts on the Alerts page and/or Service Graph.

**Monitor anomalies in the web console**
