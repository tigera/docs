...
{`kubectl apply -f {variables.filesUrl}/manifests/threatdef/honeypod/controller.yaml`}
</CodeBlock>

For OpenShift deployments, the controller requires privileged access:

<CodeBlock language='bash'>
{`kubectl apply -f {variables.filesUrl}/manifests/threatdef/honeypod/controller_os.yaml`}
</CodeBlock>

### Add custom Snort rules

You can add custom Snort rules to the controller using a ConfigMap. By default, {variables.prodname} uses the [Snort Community rule set](https://www.snort.org/downloads/#rule-downloads).
