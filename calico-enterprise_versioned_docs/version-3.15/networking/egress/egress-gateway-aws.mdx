...
- This annotation tells {variables.prodname} IPAM to use one of the "red" IP pools:
...
- The following resource request tells Kubernetes to only schedule the gateway to a node with available AWS IP capacity;
  it also tells {variables.prodname} to use an AWS-backed IP pool for allocating the IP address:
