...
- Adding graceful shutdown to Typha improves the "happy case" for upgrade, ensuring that shutting down a Typha instance doesn't cause a thundering herd of `calico-node` subcomponent restarts. Instead, clients are disconnected slowly over a configured interval, thus limiting the rate of `calico-node` subcomponent restarts.
