...
## About {variables.prodname} architecture

The following diagram shows the components that comprise a Kubernetes on-premises deployment using the {variables.prodname} CNI for networking and network policy.

**Tip**: For best visibility, right-click on the image below and select "Open image in new tab"

<ModalImage
  small='/img/calico-enterprise/architecture-ee.svg'
  medium='/img/calico-enterprise/architecture-ee.svg'
  alt='Architecture'
  imageBackgroundColor='white'
/>

Calico open-source components are the foundation of {variables.prodname}. {variables.prodname} provides value-added components for visibility and troubleshooting, compliance, policy lifecycle management, threat and anomaly detection, and multi-cluster management.

## {variables.prodname} components
...
Translates the orchestrator APIs for managing networks to the {variables.prodname} data-model and datastore.

For cloud providers, {variables.prodname} has a separate plugin for each major cloud orchestration platform. This allows {variables.prodname} to tightly bind to the orchestrator, so users can manage the {variables.prodname} network using their orchestrator tools. When required, the orchestrator plugin provides feedback from the {variables.prodname} network to the orchestrator. For example, providing information about Felix liveness, and marking specific endpoints as failed if network setup fails.
...
Handles listing of required Kubernetes and {variables.prodname} configuration and pushes snapshots to Elasticsearch. Snapshots give you visibility into configuration changes, and how the cluster-wide configuration has evolved within a reporting interval.
...
**Main task**: Consists of a controller that handles integrations with threat intelligence feeds and {variables.prodname} custom alerts, and an installer that installs the Kibana dashboards for viewing jobs through the Kibana UI.

### kube-controllers

**Main task**: Monitors the Kubernetes API and performs actions based on cluster state. The {variables.prodname} kube-controllers container includes these controllers:
...
**Main task**: Collects and forwards {variables.prodname} logs (flows, DNS, L7) to Elasticsearch. Open source data collector for unified logging. [fluentd open source](https://www.fluentd.org/).

### Prometheus

**Main task**: The default monitoring component for collecting {variables.prodname} policy metrics. It can also be used to collect metrics on calico/nodes from Felix. Prometheus is an open-source toolkit for systems monitoring and alerting. [Prometheus metrics](../component-resources/node/felix/prometheus.mdx), and [Configure Prometheus](../../operations/monitor/index.mdx).
...
- **BGP route reflector configuration**
  ...
  For redundancy, you can seamlessly deploy multiple BGP route reflectors. Note that BGP route reflectors are involved only in control of the network: endpoint data does not passes through them. When the {variables.prodname} BGP client advertises
...
**Main task**: Command line interface used largely during pre-installation for CRUD operations on {variables.prodname} objects. `kubectl` is the recommended CLI for CRUD operations. calicoctl is available on any host with network access to the {variables.prodname} datastore as either a binary or a container. Requires separate installation. [calicoctl](../clis/calicoctl/index.mdx)).

### calico-node

**Main task**: Bundles key components that are required for networking containers with {variables.prodname}:
...
**Main task**: Provides {variables.prodname} networking for Kubernetes clusters.
...
**Main task**: Monitors {variables.prodname} datastore for changes to BGP configuration and global defaults such as AS number, logging levels, and IPAM information. An open source, lightweight configuration management tool.
...
**Main task**: The datastore for the {variables.prodname} CNI plugin. The Kubernetes API datastore:
...
- Uses Kubernetes audit logging to generate audit logs of changes to {variables.prodname} resources
...
- **ACL programming**
  ...
  Programs ACLs into the Linux kernel to ensure that only valid traffic can be sent between endpoints, and that endpoints cannot circumvent {variables.prodname} security measures.
...
`{variables.nodecontainer}` can be run in _policy only mode_ where Felix runs without BIRD and confd. This provides policy management without route distribution between hosts, and is used for deployments like managed cloud providers.

:::

### IPAM plugin

**Main task**: Uses {variables.prodname}’s IP pool resource to control how IP addresses are allocated to pods within the cluster. It is the default plugin used by most {variables.prodname} installations. It is one of the {variables.prodname} [CNI plugins](../component-resources/configuration.mdx).
...
**Main task**: A Kubernetes component that validates and configures data for the API objects (for example, pods, services, and others). Proxies requests for {variables.prodname} API resources to the Kubernetes API server through an aggregation layer.

### kubectl

**Main task**: The recommended command line interface for CRUD operations on {variables.prodname} and Calico objects. [kubectl](https://kubernetes.io/docs/reference/kubectl/overview/).
