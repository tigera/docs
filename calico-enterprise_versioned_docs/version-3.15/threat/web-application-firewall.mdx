...
With {variables.prodname} WAF, you gain visibility into internal east/west traffic at the HTTP layer in Manager UI Service Graph, Kibana, and Alerts page.

### {variables.prodname} WAF implementation

{variables.prodname} WAF is deployed in your cluster as an Envoy daemonset. {variables.prodname} proxies selected service traffic through Envoy, checking HTTP requests using the industry-standard ModSecurity module.
...
The OWASP Core Rule set that is bundled with {variables.prodname} WAF, has reasonable defaults to get started.
...
The two bootstrapping files `modsecdefault.conf` and `crs-setup.conf` MUST be named lowercase i.e. lowercase "m" and lowercase "c" respectively in order to ensure they are loaded into ModSec before any `REQUEST-*.conf` Core Rules Set files. Presence of these two files is required and enforced by the operator.
