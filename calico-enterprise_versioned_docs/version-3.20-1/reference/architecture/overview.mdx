...
Calico open-source components are the foundation of {variables.prodname}. {variables.prodname} provides value-added components for visibility and troubleshooting, compliance, policy lifecycle management, threat detection, and multi-cluster management.
...
<code>{variables.nodecontainer}</code> can be run in _policy only mode_ where Felix runs without BIRD and confd. This provides policy management without route distribution between hosts, and is used for deployments like managed cloud providers.
