...
A node resource (`Node`) represents a node running {variables.prodname}. When adding a host
to a {variables.prodname} cluster, a node resource needs to be created which contains the
configuration for the <code>{variables.nodecontainer}</code> instance running on the host.

When starting a <code>{variables.nodecontainer}</code> instance, the name supplied to the instance should
match the name configured in the Node resource.

By default, starting a <code>{variables.nodecontainer}</code> instance will automatically create a node resource
...
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------- | --------------- | ---------------------------- | ------- |
| bgp                  | BGP configuration for this node. Omit if using {variables.prodname} for policy only.                                                      |                 | [BGP](#bgp)                  |
...
| Field                   | Description                                                                                        | Accepted Values                                                                                                                                                                       | Schema  | Default |
| ----------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ------- |
| asNumber                | The AS Number of your <code>{variables.nodecontainer}</code>.                                                         | Optional. If omitted the global value is used (see [example modifying Global BGP settings](../../networking/configuring/bgp.mdx) for details about modifying the `asNumber` setting). | integer |
| ipv4Address             | The IPv4 address and subnet exported as the next-hop for the {variables.prodname} endpoints on the host    | The IPv4 address must be specified if BGP is enabled.                                                                                                                                 | string  |
| ipv6Address             | The IPv6 address and subnet exported as the next-hop for the {variables.prodname} endpoints on the host    | Optional                                                                                                                                                                              | string  |
...
| --------------------- | ------------- | ------ | -------- | ------------------------------------------------------------------ |
| Kubernetes API server | No            | Yes    | Yes      | <code>{variables.nodecontainer}</code> data is directly tied to the Kubernetes nodes. |
