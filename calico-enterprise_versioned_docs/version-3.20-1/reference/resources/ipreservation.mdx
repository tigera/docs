...
An IP reservation resource (`IPReservation`) represents a collection of IP addresses that {variables.prodname} should
not use when automatically assigning new IP addresses. It only applies when {variables.prodname} IPAM is in use.
...
{variables.prodname} may become significantly slower when searching for free IPAM blocks.
...
{variables.prodname} supports Kubernetes [annotations that force the use of specific IP addresses](../component-resources/configuration.mdx#requesting-a-specific-ip-address). These annotations override any `IPReservation`s that
are in place.
