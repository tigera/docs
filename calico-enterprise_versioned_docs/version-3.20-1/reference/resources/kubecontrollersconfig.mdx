...
A {variables.prodname} [Kubernetes controllers](../component-resources/kube-controllers/configuration.mdx) configuration resource (`KubeControllersConfiguration`) represents configuration options for the {variables.prodname} Kubernetes controllers.
...
- {variables.prodname} automatically creates a resource named `default` containing the configuration settings, only the name `default` is used and only one object of this type is allowed. You can use [calicoctl](../clis/calicoctl/overview.mdx) to view and edit these settings
