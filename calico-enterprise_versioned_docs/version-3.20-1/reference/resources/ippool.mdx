...
An IP pool resource (`IPPool`) represents a collection of IP addresses from which {variables.prodname} expects
...
| natOutgoing                        | When enabled, packets sent from {variables.prodname} networked containers in this pool to destinations outside of any Calico IP pools will be masqueraded.                                                                                          | true, false                                                                                                                                                                                                            | boolean                    | `false`                                       |
| disabled                           | When set to true, {variables.prodname} IPAM will not assign addresses from this pool.                                                                                                                                                               | true, false                                                                                                                                                                                                            | boolean                    | `false`                                       |
| disableBGPExport _(since v3.11.0)_ | Disable exporting routes from this IP Pool’s CIDR over BGP.                                                                                                                                                                                 | true, false                                                                                                                                                                                                            | boolean                    | `false`                                       |
| nodeSelector                       | Selects the nodes where {variables.prodname} IPAM should assign pod addresses from this pool. Can be overridden if a pod [explicitly identifies this IP pool by annotation](../component-resources/configuration.mdx#using-kubernetes-annotations). |                                                                                                                                                                                                                        | [selector](#node-selector) | all()                                         |
...
Do not use a custom `blockSize` until **all** {variables.prodname} components have been updated to a version that
...
{variables.prodname} supports Kubernetes [annotations that force the use of specific IP addresses](../component-resources/configuration.mdx#requesting-a-specific-ip-address). These annotations take precedence over the `allowedUses` field.

### AWS-backed pools

{variables.prodname} supports IP pools that are backed by the AWS fabric. This feature was added in order
...
{variables.prodname} will only route using IP-in-IP if the IP address of the destination node is in a different
subnet. The subnet of each node is configured on the node resource (which may be automatically
determined when running the <code>{variables.nodecontainer}</code> service).
...
Hosts running {variables.prodname} is asymmetric and may cause traffic to be filtered due to
...
{variables.prodname} will only route using VXLAN if the IP address of the destination node is in a different
subnet. The subnet of each node is configured on the node resource (which may be automatically
determined when running the <code>{variables.nodecontainer}</code> service).
...
To prevent an IP pool from being used automatically by {variables.prodname} IPAM, while still allowing
