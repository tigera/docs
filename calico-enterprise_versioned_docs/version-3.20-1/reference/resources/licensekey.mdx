...
A License Key resource (`LicenseKey`) represents a user's license to use {variables.prodname}. Keys are
...
When you add {variables.prodname} to an existing Kubernetes cluster or create a
new OpenShift cluster, you must apply your license key to complete the installation
and gain access to the full set of {variables.prodname} features.

When your license key expires, you must update it to continue using {variables.prodname}.
