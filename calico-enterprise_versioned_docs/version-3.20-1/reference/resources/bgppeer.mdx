...
which the node(s) in a {variables.prodname} cluster will peer.
Configuring BGP peers allows you to peer a {variables.prodname} network
with your datacenter fabric (e.g. ToR). For more
information on cluster layouts, see {variables.prodname}'s documentation on
[{variables.prodname} over IP fabrics](../architecture/design/l3-interconnect-fabric.mdx).
...
KeyRef tells {variables.prodname} where to get a BGP password. The referenced Kubernetes
secret must be in the same namespace as the {variables.nodecontainer} pod.
...
determines which <code>{variables.nodecontainer}</code>s will attempt to establish a BGP session with that peer.
If <code>{variables.nodecontainer}</code> has a `listenPort` set in `BGPConfiguration`, it will be used in peering.
