...
determines which `{variables.nodecontainer}`s will attempt to establish a BGP session with that peer.
If `{variables.nodecontainer}` has a `listenPort` set in `BGPConfiguration`, it will be used in peering.
