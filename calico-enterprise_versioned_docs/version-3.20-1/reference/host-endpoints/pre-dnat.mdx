...
Translation), which is useful if it is more convenient to specify {variables.prodname} policy
...
not), it makes sense to express {variables.prodname} policy to expose or secure particular
Services in terms of the corresponding NodePorts. But that is only possible if
the {variables.prodname} policy is applied before DNAT changes the NodePort to something
