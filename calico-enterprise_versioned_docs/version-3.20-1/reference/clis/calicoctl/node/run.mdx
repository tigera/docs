...
<code>{variables.nodecontainer}</code> service, alternatively, the addresses may either be explicitly
...
If you are starting the <code>{variables.nodecontainer}</code> container directly (and not using the
`calicoctl run` helper command), the options are passed in environment
variables. These are described in
[Configuring <code>{variables.nodecontainer}</code>](../../../component-resources/node/configuration.mdx).
