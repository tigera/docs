...
The command line tool, `calicoctl`, makes it easy to manage {variables.prodname}
network and security policy, as well as other {variables.prodname} configurations.
...
{variables.prodname} resources. This section is a command line reference for
...
The following table lists supported aliases for {variables.prodname} resources when using `calicoctl`. Note that all aliases
