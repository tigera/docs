...
The {variables.prodname} CNI plugins do not need to be configured directly when installed by the operator. For a complete operator
...
The {variables.prodname} CNI plugin is configured through the standard CNI
[configuration mechanism](https://github.com/containernetworking/cni/blob/master/SPEC.md#network-configuration)

A minimal configuration file that uses {variables.prodname} for networking
...
If the <code>{variables.nodecontainer}</code> container on a node registered with a `NODENAME` other than the node hostname, the CNI plugin on this node must be configured with the same `nodename`:
...
The {variables.prodname} CNI plugin supports the following datastore:
...
When using {variables.prodname} IPAM, the following flags determine what IP addresses should be assigned. NOTE: These flags are strings and not boolean values.
...
By default, {variables.prodname} IPAM will assign IP addresses from all the available IP pools.
...
you must also run `{variables.imageNames.kubeControllers}` with the policy, profile, and workloadendpoint controllers enabled.
...
When using `type: k8s`, the {variables.prodname} CNI plugin requires read-only Kubernetes API access to the `Pods` resource in all namespaces.
</TabItem>
</Tabs>

### Enabling policy setup timeout

The {variables.prodname} CNI plugin can be configured to prevent new pods from starting their containers until one of the following conditions occurs:
...
<TabItem label="Manifest" value="Manifest-0">

The policy setup timeout can be configured by setting the `policy_setup_timeout_seconds` option in the CNI config.
...
The {variables.prodname} CNI plugin reads Felix's `endpoint-status` directory to determine when the dataplane has been programmed for a pod.
If left unset, the {variables.prodname} CNI plugin will look for the directory at `/var/run/calico/endpoint-status`. The path `/var/run/calico` is commonly mounted to the {variables.prodname} DaemonSet, meaning it can be written to by the Felix container, and read by the (host-namespace) {variables.prodname} CNI plugin.
To enable the `endpoint-status` directory, and adjust which directory of the Felix container it is written to, the `endpointStatusPathPrefix` option must be configured for [Felix](node/felix/configuration.mdx).

To adjust where the {variables.prodname} CNI plugin looks for the `endpoint-status` directory in the host filesystem, you must set the `endpoint_status_dir` option.
...
{variables.prodname} supports the host-local IPAM plugin's `routes` field as follows:

- If there is no `routes` field, {variables.prodname} will install a default `0.0.0.0/0`, and/or `::/0` route into the pod (depending on whether the pod has an IPv4 and/or IPv6 address).

- If there is a `routes` field then {variables.prodname} will program _only_ the routes in the routes field into the pod. Since {variables.prodname} implements a point-to-point link into the pod, the `gw` field is not required and it will be ignored if present. All routes that {variables.prodname} installs will have {variables.prodname}'s link-local IP as the next hop.

{variables.prodname} CNI plugin configuration:
...
When making use of the `usePodCidr` or `usePodCidrIPv6` options, the {variables.prodname} CNI plugin requires read-only Kubernetes API access to the `Nodes` resource.

#### Configuring node and typha

When using `host-local` IPAM with the Kubernetes API datastore, you must configure both {variables.nodecontainer} and the Typha deployment to use the `Node.podCIDR` field by setting the environment variable `USE_POD_CIDR=true` in each.
...
In addition to specifying IP pools in the CNI config as discussed above, {variables.prodname} IPAM supports specifying IP pools per-namespace or per-pod using the following [Kubernetes annotations](https://kubernetes.io/docs/concepts/overview/working-with-objects/annotations/).
...
The {variables.prodname} CNI plugin supports specifying an annotation per namespace.
...
You can also request a specific IP address through [Kubernetes annotations](https://kubernetes.io/docs/concepts/overview/working-with-objects/annotations/) with {variables.prodname} IPAM.
There are two annotations to request a specific IP address:

- `cni.projectcalico.org/ipAddrs`: A list of IPv4 and/or IPv6 addresses to assign to the Pod. The requested IP addresses will be assigned from {variables.prodname} IPAM and must exist within a configured IP pool.
...
You can request a floating IP address for a pod through [Kubernetes annotations](https://kubernetes.io/docs/concepts/overview/working-with-objects/annotations/) with {variables.prodname}.
...
The CNI 0.3.0 [spec](https://github.com/containernetworking/cni/blob/spec-v0.3.0/SPEC.md#network-configuration-lists) supports "chaining" multiple CNI plugins together. {variables.prodname} supports the following Kubernetes CNI plugins, which are enabled by default. Although chaining other CNI plugins may work, we support only the following tested CNI plugins.

**Port mapping plugin**

{variables.prodname} is required to implement Kubernetes host port functionality and is enabled by default.
...
To disable it, remove the portmap section from the CNI network configuration in the {variables.prodname} manifests.
...
To disable it, remove the bandwidth section from the CNI network configuration in the {variables.prodname} manifests.
...
{variables.prodname} IPAM will not reassign IP addresses to workloads
