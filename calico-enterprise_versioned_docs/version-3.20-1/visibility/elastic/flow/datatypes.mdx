...
{variables.prodname} sends the following data to Elasticsearch.
...
| `action`                          | keyword           | - `allow`: {variables.prodname} accepted the flow.<br />- `deny`: {variables.prodname} denied the flow.                                                                                                                                                                                                                                                  |
...
* `<policy name>` is the name of the policy/profile; its format depends on the type of policy:
  ...
  * `<tier>.<name>` for {variables.prodname} `GlobalNetworkPolicy`.
  * `<namespace>/knp.default.<name>` for Kubernetes `NetworkPolicy`.
  * `<namespace>/<tier>.<name>` for {variables.prodname} `NetworkPolicy`.
