...
Beyond using filtering to suppress flow logs, {variables.prodname} provides controls to aggregate flow logs. Although aggressive aggregation levels reduce
...
{variables.prodname} enables flow log aggregation for pod/workload endpoints by default, and uses an aggressive aggregation level to reduce log volume. The
