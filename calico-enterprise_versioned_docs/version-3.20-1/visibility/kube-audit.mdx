...
**Kubernetes resources** are used in {variables.prodname} compliance reports and other audit-related features, but they are not enabled by default. You must enable Kubernetes resources through the Kubernetes API server. If you miss this step, some compliance reports will not work, and audit trails will not provide a complete view to your security team.
...
Like {variables.prodname} audit logs, Kubernetes audit logs are displayed in Manager UI in the Timeline dashboard, Kibana dashboard (indexed by, `tigera_secure_ee_audit_kube`), and provide the core data for compliance reports.
...
<Tabs>
<TabItem label="Kubernetes on-premises" value="Kubernetes on-premises-0">
...
### Enable Kubernetes audit logs for {variables.prodname}

The following updates require a restart to the Kubernetes API Server.

To enable Kubernetes resource audit logs to be read by {variables.prodname} in fluentd, follow these steps.
...
1. Enable the **Audit** option, and click **Update**.
   <img
     src='/img/calico-enterprise/audit-log.png'
     alt='Audit Log'
     width='300'
   />
...
1. Under **Logging**, **Cloudwatch**, make a note of the URL value for a later step, then click the link.
   <img
     src='/img/calico-enterprise/cloudwatch-url.png'
     alt='Cloudwatch Logging'
     width='400'
   />
1. In the **Log Streams** list, make note of the common prefix (for example, kube-apiserver-audit) for a later step.
   <img
     src='/img/calico-enterprise/log-streams.png'
     alt='Log Streams'
     width='400'
   />
...
1. On the **Add user** page, make these changes:
   ...
   <img
     src='/img/calico-enterprise/programmatic-access.png'
   ...
   <img
     src='/img/calico-enterprise/cloudwatch-readonly.png'
     alt='Cloudwatch URL'
     width='400'
   />
...
### Update {variables.prodname} log collector with EKS values
...
1. Update the `tigera-secure` LogCollector resource with values from the EKS configuration.
   ...
   - `fetchInterval`: Interval in seconds for {variables.prodname} to get logs from Cloudwatch. Default: 60 seconds, this fetches 1MB every 60 seconds, adjust it based number on CRUD operations performed on cluster resource.
...
### Configure authentication between {variables.prodname} and Cloudwatch logs

In this step, you add AWS authentication information to enable {variables.prodname} to get logs from the EKS Cloudwatch instance.
...
<TabItem label="AWS using kOps" value="AWS using kOps-2">
...
Follow these instructions to enable audit logs for [AWS using kOps](https://kops.sigs.k8s.io/cluster_spec/#audit-logging).
