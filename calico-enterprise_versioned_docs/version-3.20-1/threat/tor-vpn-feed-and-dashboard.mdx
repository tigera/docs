...
### The {variables.prodname} Tor-VPN dashboard
...
- Log in to {variables.prodname} Manager, and go to **kibana**, select **dashboard**, and select **Tor-VPN Dashboard**.
...
We recommend that you turn down the aggregation of flow logs sent to Elasticsearch for configuring threat feeds. If you do not adjust flow logs, {variables.prodname} aggregates over the external IP addresses for allowed traffic, and threat feed searches will not provide useful results (unless the traffic is denied by policy). Go to: [FelixConfiguration](../reference/resources/felixconfig.mdx) and set the field, **flowLogsFileAggregationKindForAllowed** to **1**.

## How to

In this section we will look at how to add Tor and VPN feeds to {variables.prodname}. Installation process is straightforward as below.

1. Add threat feed to the cluster.
   For VPN Feed,
   <CodeBlock language='shell'>
   {`kubectl apply -f {variables.filesUrl}/manifests/threatdef/vpn-feed.yaml`}
   </CodeBlock>
   For Tor Bulk Exit Feed,
   <CodeBlock language='shell'>
   {`kubectl apply -f {variables.filesUrl}/manifests/threatdef/tor-exit-feed.yaml`}
   </CodeBlock>
2. Now, you can monitor the Dashboard for any malicious activity. The dashboard can be found at {variables.prodname} Manager, go to "kibana" and then go to "Dashboard". Select "Tor-VPN Dashboard".
