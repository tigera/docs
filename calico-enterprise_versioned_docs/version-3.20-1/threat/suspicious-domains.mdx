...
Add threat intelligence feeds to {variables.prodname} to trace DNS queries involving suspicious domains.
...
In addition to allow-lists, you can use threat feeds to monitor your cluster for DNS queries to known malicious or suspicious domain names. {variables.prodname} monitors DNS queries and generates alerts for any that are listed in your threat feed.
...
To add threat feeds to {variables.prodname} for automatic updates (default is once a day), the threat feed(s) must be available using HTTP(S), and return a newline-separated list of domain names.
...
4. In {variables.prodname} Manager, go the “Security Events” page to view events that are generated when an endpoint in the cluster queries a name on the list.
