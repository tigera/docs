...
{variables.prodname} is a security solution with full-stack observability for cloud-native applications running on containers and Kubernetes. Built upon the Calico CNI and network policy, {variables.prodname} works across all multi-cloud and hybrid environments with any combination of VMs, containers, Kubernetes, cloud instances, hosts, and bare metal servers.

![calico-enterprise](/img/calico/calico-enterprise.svg)

## Best fit

The best fit for {variables.prodname} is **enterprise teams** who need full control to customize their networking security deployment to meet regulatory and compliance requirements for Kubernetes at scale.
...
For a detailed list of {variables.prodname} features, see [Tigera product comparison](../about/product-comparison.mdx)

## Going into production with {variables.prodname}
...
Tigera's Customer Success has invested heavily in custom and self-service training to address these obstacles. Guided by their best-practices-to-production workflows, you can keep progressing and join the growing list of companies who are in production with {variables.prodname}.
...
- To install {variables.prodname} on a cluster in 15 minutes, see [Quickstart](../getting-started/install-on-clusters/kubernetes/quickstart.mdx)
