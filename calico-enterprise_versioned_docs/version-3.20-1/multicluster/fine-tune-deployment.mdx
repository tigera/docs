...
- All users that log in to the {variables.prodname} Manager UI must use a valid service account or user account in the management cluster.
...
{variables.prodname} log data is stored within Elasticsearch indexes. The indexes have the following naming scheme:

```bash
<log-type>.<cluster-name>.<date>
```

A standalone cluster uses the cluster name cluster for Elasticsearch indexes. This is also the name used by a management cluster. For a managed cluster, its cluster name is the value chosen by the user at the time of registration, through the {variables.prodname} Manager.
...
1. Log in to {variables.prodname} Manager.
