...
- You are decommissioning a host running <code>{variables.nodecontainer}</code> or removing it from your
  cluster.
...
Provide guidance on how to remove a host that is part of a {variables.prodname} cluster
and clean up the associated [Node resource](../reference/resources/node)
information.

## Prerequisites

- Prior to removing any Node resource from the datastore the <code>{variables.nodecontainer}</code>
