...
- **Get the certificate and key pair for the $[prodname] web console**
  Generate the certificate using any X.509-compatible tool or from your organization's Certificate Authority. The certificate must have Common Name or Subject Alternate Names that match the IPs or DNS names that will be used to [access the web console](../cnx/access-the-manager.mdx).
