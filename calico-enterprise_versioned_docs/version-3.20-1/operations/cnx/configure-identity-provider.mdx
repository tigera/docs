...
Configure an external identity provider (IdP), create a user, and log in to {variables.prodname} Manager UI.
...
- **Claims**: Every time your IdP issues a token for a valid user, these claims add metadata about the user. {variables.prodname} uses this to determine the username.
...
1. Optional: Google OIDC does not support the groups claim. However, {variables.prodname} leverages [Dex IdP](https://dexidp.io/docs/connectors/google/) to add groups if you configure a service account.
...
1. Create values for some required variables. `MANAGER_URL` is the URL where {variables.prodname} Manager will be accessed,
...
### (Optional) Allow {variables.prodname} URIs in your IdP

Most IdPs require redirect URIs to be allowed to redirect users at the end of the OAuth flow to the {variables.prodname} Manager or to Kibana. Consult your IdP documentation for authorizing your domain for the respective origins and destinations.
...
- If logging into Kibana fails with a `cookie not present` error, update the browser settings to allow third-party cookies, as {variables.prodname} Manager UI uses Kibana's cookies during login.
