...
attempting to use {variables.prodname} networking.

NetworkManager manipulates the routing table for interfaces in the default network
namespace where {variables.prodname} veth pairs are anchored for connections to containers.
This can interfere with the {variables.prodname} agent's ability to route correctly.

The procedure for configuring NetworkManager to ignore {variables.prodname} interfaces
...
1. Check the interfaces that NetworkManager ignores.
   ...
   If this does not to prevent NetworkManager from interfering with {variables.prodname} networking, try disabling NetworkManager. If disabling NetworkManager does not stop it from interfering with {variables.prodname} networking, you may need to remove NetworkManager. This will require manual network configuration.
...
## Error: {variables.nodecontainer} is not ready: BIRD is not ready: BGP not established with 10.0.0.1
...
query, and will fail to generate alerts if nested aggregations
