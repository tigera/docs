...
import EbpfValue from '@site/calico-enterprise_versioned_docs/version-3.20-1/_includes/content/_ebpf-value.mdx';
...
Install the eBPF dataplane during the initial installation of {variables.prodname}.
...
You’ll see a lot of non-networking projects leveraging eBPF, but for {variables.prodname} our focus is on networking,
...
For best pod-to-pod performance, we recommend using an underlying network that doesn't require {variables.prodname} to use an overlay. For example:
...
Installing {variables.prodname} normally consists of the following steps, which are covered by the main installation
guide:

- Create a cluster suitable to run {variables.prodname}
...
- [Create a cluster](#create-a-suitable-cluster) suitable to run {variables.prodname} **with the added requirement that the nodes must use a recent
  enough kernel**.
- [**Create a config map with the "real" address of the API server.**](#create-kubernetes-service-endpoint-config-map) This allows the operator to install {variables.prodname}
...
fully supported. Since Red Hat have backported the eBPF features required by {variables.prodname} the Red Hat kernel
...
AKS does not support disabling `kube-proxy` so it's necessary to tell {variables.prodname} not to try to clean up
...
In eBPF mode, {variables.prodname} takes over from `kube-proxy`. This means that, like `kube-proxy`, it needs to be able
to reach the API server _directly_ rather than by using the API server's `ClusterIP`. To tell {variables.prodname} how
...
the address of the load balancer. This prevents {variables.prodname} from being disconnected if the API servers IP changes.
...
<CodeBlock language='bash'>
  curl -o custom-resources.yaml {variables.filesUrl}/manifests/custom-resources.yaml
</CodeBlock>
...
{variables.prodname} not to clean up `kube-proxy`'s iptables rules. If you didn't disable `kube-proxy` when starting
...
away any manual changes made to its configuration. To ensure `kube-proxy` and {variables.prodname} don't fight, set
