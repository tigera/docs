...
import DomainNames from '@site/calico-enterprise_versioned_docs/version-3.20-1/_includes/content/_domain-names.mdx';
...
In these cases, the pod tries to connect to a domain before {variables.prodname} has finished processing the DNS policy that allows the connection.
