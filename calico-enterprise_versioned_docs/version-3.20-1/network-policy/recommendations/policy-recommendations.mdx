...
{variables.prodname} makes it easy for platform operators to implement namespace isolation without experience in authoring network policy or detailed knowledge of how application workloads are communicating. {variables.prodname} analyzes the flow logs that are generated from workloads, and automatically recommends and stages policies for each namespace that can be used for isolation.
...
- Not enough traffic history 
   ...
   If you recently installed {variables.prodname}, you may not have enough traffic history. Workloads must run for some time (around 5 days) to get “typical network traffic” for applications.
