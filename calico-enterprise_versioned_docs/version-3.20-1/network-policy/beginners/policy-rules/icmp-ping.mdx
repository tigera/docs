...
Use {variables.prodname} network policy to allow and deny ICMP/ping messages.
...
it to learn about your network, or for DoS attacks. Using {variables.prodname} network policy, you can control where ICMP is used. For example, you can:
...
{variables.prodname} network policy also lets you deny and allow ICMP traffic based on specific types and codes. For example, you can specify ICMP type 5, code 2 to match specific ICMP redirect packets.
...
In any "deny-all" {variables.prodname} network policy, be sure to specify a lower order (**order:200**) than regular policies that might allow traffic.
