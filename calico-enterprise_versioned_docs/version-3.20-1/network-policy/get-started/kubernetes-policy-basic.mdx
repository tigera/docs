...
This guide provides a simple way to try out Kubernetes `NetworkPolicy` with {variables.prodname}. It requires a Kubernetes cluster configured with {variables.prodname} networking, and expects that you have `kubectl` configured to interact with the cluster.
