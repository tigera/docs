...
{variables.prodname}'s eBPF dataplane [native service handling](#calico-ebpf-native-service-handling) (rather than kube-proxy) which preserves source IP address.
...
or by using {variables.prodname}'s eBPF dataplane [native service handling](#calico-ebpf-native-service-handling) (rather
...
{variables.prodname} supports advertising service Cluster IPs, or External IPs for services configured with one. If you are
...
When combined with services of type `LoadBalancer` or with {variables.prodname} service IP address advertising, traffic is
...
As an alternative to using Kubernetes standard kube-proxy, {variables.prodname}'s [eBPF dataplane](../../operations/ebpf/enabling-ebpf.mdx)
