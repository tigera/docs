...
{variables.prodname} requires storage for logs and reports. Before finishing the installation, you must [create a StorageClass for {variables.prodname}](../../../operations/logstorage/create-storage).
...
In order to use {variables.prodname}, you must install the license provided to you by Tigera support representative. Before applying the license, wait until the Tigera API server is ready with the following command:
...
### Install {variables.prodname} resources
