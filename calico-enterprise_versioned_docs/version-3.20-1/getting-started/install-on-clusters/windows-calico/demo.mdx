...
This guide provides a simple demo to illustrate basic pod-to-pod connectivity and the application of network policy in a {variables.prodnameWindows} cluster. We will create client and server pods on Linux and Windows nodes, verify connectivity between the pods, and then we'll apply a basic network policy to isolate pod traffic.

## Prerequisites

To run this demo, you will need a [{variables.prodnameWindows} cluster](operator.mdx) with
...
spec:
  ...
  - name: porter
    ...
    env:
    - name: SERVE_PORT_80
      value: This is a {variables.prodnameWindows} demo.
    imagePullPolicy: IfNotPresent
  nodeSelector:
    kubernetes.io/os: windows
EOF
...
StatusCode        : 200
StatusDescription : OK
Content           : This is a {variables.prodnameWindows} demo.
RawContent        : HTTP/1.1 200 OK
                    Content-Length: 49
                    Content-Type: text/plain; charset=utf-8
                    Date: Fri, 21 Aug 2020 22:45:46 GMT

                    This is a {variables.prodnameWindows} demo.
Forms             :
