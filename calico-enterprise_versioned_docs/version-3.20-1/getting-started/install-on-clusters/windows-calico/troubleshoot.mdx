...
When using the {variables.prodname} CNI plugin, each {variables.prodname} IPAM block (or the single podCIDR in host-local IPAM mode), is represented as a HNS l2bridge network. Use the following command to inspect the networks.

<CodeBlock language='powershell'>
ipmo -DisableNameChecking \{variables.rootDirWindows}\libs\hns\hns.psm1
Get-HNSNetwork
</CodeBlock>

**Examine pod endpoints**

Use the following command to view the HNS endpoints on the system. There should be one HNS endpoint per pod networked with {variables.prodname}:
...
### After initializing {variables.prodnameWindows}, AWS metadata server is no longer reachable
...
### Installation stalls at "Waiting for {variables.prodname} initialization to finish"
...
### Windows Server 2019 insider preview: after rebooting a node, {variables.prodnameWindows} fails to start, the tigera-node.err.log file contains errors
...
- Stop and then start {variables.prodnameWindows} using the `stop-calico.ps1` and `start-calico.ps1` scripts
...
If kubelet is already running when {variables.prodnameWindows} is installed, the creation of the container vSwitch can cause kubelet to lose its connection and then persistently fail to reconnect to the API server.
To resolve this, restart kubelet after installing {variables.prodnameWindows}.
...
If using {variables.prodname} networking, check that the {variables.prodname} IP pool you are using has IPIP mode disabled (set to "Never). IPIP is not supported on Windows. To check the IP pool, you can use `calicoctl`:
...
If the error includes `loading config file <path-to-kubeconfig\>`, follow the instructions in [Set environment variables](manual-install/standard.mdx#install-calico-enterprise-for-windows-and-kubernetes-on-windows-nodes) to update the `KUBECONFIG` environment variable to the path of your kubeconfig file.
...
### {variables.prodname} BGP mode: connectivity issues, Linux calico/node pods report unready
...
- Check the logs for the confd service in the configured log directory for errors
  (default {variables.rootDirWindows}\logs).
