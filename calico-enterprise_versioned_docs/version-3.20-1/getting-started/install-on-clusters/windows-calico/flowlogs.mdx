...
Configure {variables.prodnameWindows} flow log data for visibility and troubleshooting Windows workloads in Kubernetes clusters.

## Value

{variables.prodnameWindows} includes a fully-integrated deployment of Elasticsearch to collect flow log data that drives
...
{variables.prodnameWindows} provides the same support for flow logs as Linux, with these exceptions:
...
{variables.prodnameWindows} flow logs are enabled and configured the same way as [Flow logs for Linux](../../../visibility/elastic/overview.mdx). Use the following Windows-specific parameters for specifying file directories and paths.
