...
- At least four Linux Kubernetes worker nodes to run {variables.prodname}'s cluster-wide components that meets [Linux system requirements](../requirements.mdx), and is installed with {variables.prodname} v3.5.0+
- Must not be running in eBPF mode
- VXLAN or BGP without encapsulation is supported if using {variables.prodname} CNI. IPIP ({variables.prodname}'s default encapsulation mode) is not supported. Use the following command to turn off IPIP.
...
For operator-managed Linux {variables.prodname} clusters, three Linux worker nodes are required to meet high-availability requirements for Typha.
...
The manual method for installing {variables.prodnameWindows} is deprecated in favor of using [the Operator and Windows HostProcess containers (HPC)](operator.mdx#install-calico-enterprise-for-windows-using-the-operator). Support for this method will be dropped in a future {variables.prodname} version.
