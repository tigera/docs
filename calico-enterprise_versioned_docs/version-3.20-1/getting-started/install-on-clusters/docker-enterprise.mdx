...
Install {variables.prodname} on a Mirantis Kubernetes Engine (MKE) cluster (formerly Docker Enterprise).
...
- A [compatible MKE cluster](../compatibility.mdx#mke) with:
  ...
  - CNI flag set to unmanaged, `--unmanaged-cni` so UCP does not install the default {variables.prodname} networking plugin
...
1. Install any extra [{variables.prodname} resources](../../reference/resources/index.mdx) needed at cluster start using [calicoctl](../../reference/clis/calicoctl/overview.mdx).
