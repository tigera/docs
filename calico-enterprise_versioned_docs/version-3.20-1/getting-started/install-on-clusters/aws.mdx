...
Install {variables.prodname} with a self-managed Kubernetes cluster using Kubernetes Operations (kOps) on AWS. kOps is a cluster management tool that provisions cluster VMs and installs Kubernetes. It is a good default choice for most because it gives you access to all {variables.prodname} [flexible and powerful networking features](../../networking/index.mdx). However, other options may work better for your environment.
...
- AWS VPC CNI for networking with {variables.prodname} network policy
...
1. The provisioned kOps cluster will assign its own set of pod network CIDR in the kube-proxy instance different than the one {variables.prodname} expects. To set the cluster cidr for the kube-proxy to match the one expected by {variables.prodname} edit the cluster config `kops edit cluster <name-of-your-cluster>` and add the `kubeProxy` config with the `clusterCIDR` expected by the default {variables.prodname} installation.
   ...
   For more advanced pod networking CIDR configuration, the requirement is to have `ipPools` CIDR set by the {variables.prodname} installation to match cluster CIDR set in kube-proxy. Calico's `ipPools` setting is obtainable in the Installation resource `kubectl get installation -o yaml` and can be configured by editing the operator manifest found in the [install instructions for {variables.prodname}](kubernetes/generic-install.mdx).

   :::

1. The default size of the provisioned instance groups for the cluster might not be sufficient for the full installation of kubernetes and {variables.prodname}. To increase the size of the instance groups run `kops edit ig <name-of-instance-group-in-your-cluster> --name <name-of-your-cluster>` and edit the following fields accordingly.
...
1. KOps does not install any CNI when the flag `--networking cni` or `spec.networking: cni {}` is used. In this case the user is expected to install the CNI separately.
   To Install {variables.prodname} follow the [install instructions for {variables.prodname}](kubernetes/generic-install.mdx).
...
You can use Amazon’s VPC CNI plugin for networking, and {variables.prodname} for network policy. The advantage of this approach is that pods are assigned IP addresses associated with Elastic Network Interfaces on worker nodes. The IPs come from the VPC network pool, and therefore do not require NAT to access resources outside the Kubernetes cluster.
...
After the cluster is up and ready, [Install {variables.prodname}](kubernetes/generic-install.mdx).
...
- [Try out {variables.prodname} network policy](../../network-policy/beginners/calico-network-policy.mdx)
