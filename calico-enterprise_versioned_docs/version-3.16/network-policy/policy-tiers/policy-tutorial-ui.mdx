...
| NetworkSet         | An arbitrary set of IP subnetworks/CIDRs or domains that can be matched by standard label selectors in Kubernetes or {variables.prodname} network policy. Network sets are a {variables.prodname} namespaced resource.                                                |
| GlobalNetwork Set  | An arbitrary set of IP subnetworks/CIDRs or domains that can be matched by standard label selectors in Kubernetes or {variables.prodname} network policy. Global network sets are a {variables.prodname} global resource.                                             |
...
| HostEndpoint (HEP) | Physical or virtual interfaces attached to a host that runs {variables.prodname}. HEPs enforce {variables.prodname} policy on the traffic that enters or leaves the host’s default network namespace through the interfaces. HEPs are a {variables.prodname} global resource. |
...
There are three main parts to every {variables.prodname} policy:
...
Suppose we want to match traffic to the pod labeled `BE`, and the {variables.prodname} `networkset-1`.
...
Not only is the pod labeled `BE` included, but also the {variables.prodname} `networkset-1`.
...
{variables.prodname} policies can have order values that control the order of precedence. For both network policies and global network policies, {variables.prodname} applies the policy with the lowest value first.

![policy-ordering](/img/calico-enterprise/policy-ordering.png)

### Mixing Kubernetes and {variables.prodname} policies

Kubernetes and {variables.prodname} policies work side by side without a problem. However, Kubernetes network policies cannot assign an order value, so {variables.prodname} will set an implicit order value of 1000 to any Kubernetes network policies.
...
### {variables.prodname} policies with no order value

{variables.prodname} policies with order values take precedence. Policies without order values take lesser precedence and are processed alphabetically.
...
When a new connection is processed by {variables.prodname}, each tier that contains a policy that selects the endpoint processes the connection. Tiers are sorted by their order - the smallest number first. Policies in each tier are then processed in order from the smallest to largest.
...
It is also important to understand that {variables.prodname} policy action rules affect how traffic and connections are processed. Let's go back to the drop-down menu on the Create New Policy Rule page.
