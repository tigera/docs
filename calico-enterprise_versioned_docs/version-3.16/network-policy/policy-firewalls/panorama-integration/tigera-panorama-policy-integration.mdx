...
Use the {variables.prodname} Panorama policy integration to leverage {variables.prodname} network policy and observability/troubleshooting tools for Kubernetes workloads.

## Value

The {variables.prodname} integration for Panorama:

- Dynamically translates Panorama policy into {variables.prodname} global network policy and hierarchical policy tiers. This lets you extend Kubernetes to existing Panorama workflows with security zones to secure both East-West and North-South traffic.
- Maps Panorama Address Groups to {variables.prodname} global network sets (equivalent to Panorama “group IP addresses”), to centrally manage endpoints external to Kubernetes in Panorama. Security and platform operators can use the Manager UI Service Graph tool to troubleshoot clusters/workloads, and see how applications and microservices are being consumed.

## Concepts

### About the Panorama policy integration

{variables.prodname} global network policies are a key resource for securing ingress and egress traffic. The policy integration also includes a Panorama-specific firewall tier. {variables.prodname} policy tiers are a key microsegmentation feature that let you group policies and enforce higher precedence policies that cannot be circumvented by other teams.

The following example shows Panorama policy rules mapped into a {variables.prodname} GlobalNetworkPolicy.
...
{variables.prodname} global network sets are a key resource for defining DNS policy along with global network policy for securing egress traffic. Network sets are visible in Manager UI from the left nav, or using `kubectl`.

Deployed concurrently to policy integration, the Calico Enterprise address-groups controller integrates Panorama Address Groups, and {variables.prodname} automatically maps the values to a {variables.prodname} global network set.

The following example shows Address Groups mapped into a Calico Enterprise GlobalNetworkSet. After you verify that you have a GlobalNetworkSet, that's all you need. You can start using {variables.prodname}'s robust network policy!
...
```bash
kubectl create -f {variables.filesUrl}/manifests/tigera-policy-integration.yaml
```
...
4. In the Manager UI, click Network Policies in the left navbar and verify that the GlobalNetworkPolicies are created
   ...
   For any Panorama rule to be reflected as a GlobalNetworkPolicy in Kubernetes, at least one source or destination **Zone** must be defined.
...
The cause is either a connection failure between {variables.prodname} and Panorama. Look in the debug logs for a `Client.Timeout` error similar to the following:
