...
The {variables.prodname} anomaly detection engine analyzes patterns and alerts on potential
...
{variables.prodname} anomaly detection allows you to proactively determine if there is an issue (or not), and potentially resolve problems before service levels are compromised. Anomaly detection uses {variables.prodname} Elasticsearch logs ([flow](../../visibility/elastic/flow/index.mdx) logs, [L7](../../visibility/elastic/l7/index.mdx) logs, and [DNS](../../visibility/elastic/dns/index.mdx) logs) to learn the behavior of cluster nodes, pods, services, and other entities that send log records (applications, load balancers, databases, etc.).
