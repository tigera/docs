...
Configure Felix for syncing WAF policy. If you have already [configured L7 logs](../visibility/elastic/l7/configure.mdx), you can ignore this step.
...
2. From the **Action** column, select **Enable** or **Disable** to respectively enable or disable WAF for each of the services.
   ...
   <img src='/img/calico-enterprise/waf-select-services-CE3.16.png' alt='WAF services' width='600' />

3. On the Enable WAF window, a kubectl command appears.

   <img src='/img/calico-enterprise/waf-kubectl-copy-command-CE3.16.png' alt='WAF services' width='600' />
...
Modify the modsecurity core rule sets' `SecDefaultAction` to deny rules by commenting and uncommenting these sections in the `crs-setup.conf`,
...
The file names MUST be in all lowercase or they will fail to upload. They are loaded into ModSec before any REQUEST-*.conf Core Rules Set files.
