...
{{prodname}}'s {variables.nodecontainer} image can be run in an "early networking" mode,
...
{{prodname}}'s {variables.nodecontainer} image normally runs as a Kubernetes pod, but
