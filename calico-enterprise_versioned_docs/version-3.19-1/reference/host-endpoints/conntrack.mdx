...
{variables.prodname} uses Linux's connection tracking ('conntrack') as an important
optimization to its processing. It generally means that {variables.prodname} only needs to
...
packets in the same flow, without {variables.prodname} rechecking every packet.

This can, however, make it look like a {variables.prodname} policy is not working as it
...
the {variables.prodname} policy has been changed.

Per {variables.prodname}'s current implementation, there are two workarounds for this:
...
Then you should observe that the new {variables.prodname} policy is enforced for new packets.
