...
This guide describes how to use {variables.prodname} to secure the network interfaces
...
{variables.prodname} supports the same rich security policy model for host endpoints (host
...
{variables.prodname} does not support setting IPs or policing MAC addresses for host
interfaces, it assumes that the interfaces are configured by the
underlying network fabric.

{variables.prodname} distinguishes workload endpoints from host endpoints by a configurable
...
Felix](../component-resources/node/felix/configuration.mdx).
...
{variables.prodname} blocks all traffic to/from workload interfaces by default;
allowing traffic only if the interface is known and policy is in place.
However, for host endpoints, {variables.prodname} is more lenient; it only polices
traffic to/from interfaces that it's been explicitly told about. Traffic
to/from other interfaces is left alone.

You can use host endpoint policy to secure a NAT gateway or router. {variables.prodname}
