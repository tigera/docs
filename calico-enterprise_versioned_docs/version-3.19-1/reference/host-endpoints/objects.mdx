...
For each host endpoint that you want {variables.prodname} to secure, you'll need to
...
the {variables.prodname} node running on the host that owns the interface; in most cases this
...
By default, {variables.prodname} has a failsafe in place that allows certain
