...
endpoint, {variables.prodname} will default to denying traffic to/from that endpoint,
...
Therefore, we recommend creating a failsafe {variables.prodname} security policy that
...
- apiVersion: projectcalico.org/v3
  ...
  spec:
    ...
    - action: Allow
      protocol: TCP
      destination:
        nets: [<your etcd IP>/32]
        ports: [<your etcd ports>]
    - action: Allow
      protocol: TCP
      destination:
        nets: [<your Kubernetes API server IP]
        ports: [<your Kubernetes API server IP ports>]
    - action: Allow
      protocol: UDP
      destination:
        ports: [67]
...
If you are using {variables.prodname} for networking workloads, you should add
