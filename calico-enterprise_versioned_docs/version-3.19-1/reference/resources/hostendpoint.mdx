...
import Endpointport from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/content/_endpointport.mdx';

A host endpoint resource (`HostEndpoint`) represents one or more real or virtual interfaces
attached to a host that is running {variables.prodname}. It enforces {variables.prodname} policy on
...
Each host endpoint may include a set of labels and list of profiles that {variables.prodname}
will use to apply
[policy](networkpolicy.mdx)
to the interface. If no profiles or labels are applied, {variables.prodname} will not apply
any policy.
...
If a host endpoint is created and network policy is not in place, the {variables.prodname} default is to deny traffic to/from that endpoint (except for traffic allowed by failsafe rules).
For a named host endpoint (i.e. a host endpoint representing a specific interface), {variables.prodname} blocks traffic only to/from the interface specified in the host endpoint. Traffic to/from other interfaces is ignored.
...
For a wildcard host endpoint (i.e. a host endpoint representing all of a host's interfaces), {variables.prodname} blocks traffic to/from _all_ interfaces on the host (except for traffic allowed by failsafe rules).
...
{variables.prodname} provides a default profile resource named `projectcalico-default-allow` that consists of allow-all ingress and egress rules.
...
When rendering security rules on other hosts, {variables.prodname} uses the
