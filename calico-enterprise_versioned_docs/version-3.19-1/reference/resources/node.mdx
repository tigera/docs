...
configuration for the `{variables.nodecontainer}` instance running on the host.

When starting a `{variables.nodecontainer}` instance, the name supplied to the instance should
match the name configured in the Node resource.

By default, starting a `{variables.nodecontainer}` instance will automatically create a node resource
...
| Field                   | Description                                                                                        | Accepted Values                                                                                                                                                                       | Schema  | Default |
| ----------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ------- |
| asNumber                | The AS Number of your `{variables.nodecontainer}`.                                                         | Optional. If omitted the global value is used (see [example modifying Global BGP settings](../../networking/configuring/bgp.mdx) for details about modifying the `asNumber` setting). | integer |
...
| --------------------- | ------------- | ------ | -------- | ------------------------------------------------------------------ |
| Kubernetes API server | No            | Yes    | Yes      | `{variables.nodecontainer}` data is directly tied to the Kubernetes nodes. |
