...
import Ipnat from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/content/_ipnat.mdx';

A workload endpoint resource (`WorkloadEndpoint`) represents an interface
connecting a {variables.prodname} networked container or VM to its host.

Each endpoint may specify a set of labels and list of profiles that {variables.prodname} will use
...
plugin such as the {variables.prodname} CNI plugin. In general, we recommend that you only
