...
- Translating hierarchies of physical firewalls directly into {variables.prodname} policy.
...
When a new connection is processed by {variables.prodname}, each tier that contains a policy that applies to the endpoint processes the packet.
...
All Policies created by {variables.prodname} orchestrator integrations are created in the default (last) Tier.
