...
{variables.prodname} supports threat feeds that give either

- a set of IP addresses or IP prefixes, with content type IPSet, or
- a set of domain names, with content type DomainNameSet

For each IPSet threat feed, {variables.prodname} automatically monitors flow logs for members of the set.
...
For each DomainNameSet threat feed, {variables.prodname} automatically monitors DNS logs for queries (QNAME) or answers (RR NAME or RDATA) that contain members of the set.
...
You can configure {variables.prodname} to pull updates from your threat feed using a [`pull`](#pull) stanza in
...
with a [global network set](globalnetworkset.mdx). This global network set will have the name `threatfeed.<threat feed name>`
...
KeyRef tells {variables.prodname} where to get the value for a header. The referenced Kubernetes object
