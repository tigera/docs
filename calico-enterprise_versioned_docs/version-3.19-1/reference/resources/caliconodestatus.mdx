...
A Calico node status resource (`CalicoNodeStatus`) represents a collection of status information for a node that {variables.prodname} reports back to the user for use during troubleshooting.
...
Calico node status resource is only valid when {variables.prodname} BGP networking is in use.
