...
One of {variables.prodname}’s key features is how packets flow between workloads in a
data center, or between a workload and the Internet, without additional
encapsulation.

In the {variables.prodname} approach, IP packets to or from a workload are routed and
firewalled by the Linux routing table and iptables or eBPF infrastructure on the
workload’s host. For a workload that is sending packets, {variables.prodname} ensures
...
The direct routes are set up by a {variables.prodname} agent named Felix when it is
...
routing and iptables or eBPF. There’s a great deal more to {variables.prodname} in terms of
