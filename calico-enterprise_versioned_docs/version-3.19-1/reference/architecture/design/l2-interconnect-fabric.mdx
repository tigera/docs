...
Any technology that is capable of transporting IP packets can be used as the interconnect fabric in a {variables.prodname} network. This means that the standard tools used to transport IP, such as MPLS and Ethernet can be used in a {variables.prodname} network.

The focus of this article is on Ethernet as the interconnect network. Most at-scale cloud operators have converted to IP fabrics, and that infrastructure will work for {variables.prodname} as well. However, the concerns that drove most of those operators to IP as the interconnection network in their pods are largely ameliorated by {variables.prodname}, allowing Ethernet to be viably considered as a {variables.prodname} interconnect, even in large-scale deployments.
...
If you weren't ready to turn off your Ethernet data center network before this, I bet you are now. Before you do, however, let's look at how {variables.prodname} can mitigate these issues, even in very large deployments.

## How does {variables.prodname} tame the Ethernet daemons?

First, let's look at how {variables.prodname} uses an Ethernet interconnect fabric. It's important to remember that an Ethernet network _sees_ nothing on the other side of an attached IP router, the Ethernet network just _sees_ the router itself. This is why Ethernet switches can be used at Internet peering points, where large fractions of Internet traffic is exchanged. The switches only see the routers from the various ISPs, not those ISPs' customers' nodes. We leverage the same effect in {variables.prodname}.

To take the issues outlined above, let's revisit them in a {variables.prodname}
context.

- Large numbers of endpoints

  In a {variables.prodname} network, the Ethernet interconnect fabric only sees the routers/compute servers, not the
...
- High rate of churn
  ...
  destroyed, or moved. In a large data center, with hundreds of thousands of endpoints, this _churn_ could run into tens of events per second, every second of the day, with peaks easily in the hundreds or thousands of events per second. In a {variables.prodname} network, however, the _churn_ is very low. The only event that would lead to _churn_
...
- High volume of broadcast traffic
  ...
  Because the first (and last) hop for any traffic in a {variables.prodname} network is an IP hop, and IP hops terminate
...
With these considerations in mind, it should be evident that an Ethernet connection fabric in {variables.prodname} is not only possible, it is practical and should be seriously considered as the interconnect fabric for a {variables.prodname}
network.

As mentioned in the IP fabric post, an IP fabric is also quite feasible for {variables.prodname}, but there are more considerations that must be taken into account. The Ethernet fabric option has fewer architectural considerations in its design.

## A brief note about Ethernet topology

As mentioned elsewhere in the {variables.prodname} documentation, because {variables.prodname} can use most of the standard IP tooling, some interesting options regarding fabric topology become possible.

We assume that an Ethernet fabric for {variables.prodname} would most likely be constructed as a _leaf/spine_ architecture. Other options are possible, but the _leaf/spine_ is the predominant architectural model in use in
scale-out infrastructure today.

Because {variables.prodname} is an IP routed fabric, a {variables.prodname} network can use [ECMP](https://en.wikipedia.org/wiki/Equal-cost_multi-path_routing) to distribute traffic across multiple links (instead of using Ethernet techniques such as MLAG). By leveraging ECMP load balancing on the {variables.prodname} compute servers, it is possible to build the fabric out of multiple _independent_ leaf/spine planes using no technologies other than IP routing in the {variables.prodname} nodes, and basic Ethernet switching in the interconnect fabric. These planes would operate completely independently and could be designed such that they would not share a fault domain. This would allow for the catastrophic failure of one (or more) plane(s) of Ethernet interconnect fabric without the loss of the pod (the failure would just decrease the amount of interconnect bandwidth in the pod). This is a gentler failure mode than the pod-wide IP or Ethernet failure that is possible with today's designs.
...
In this document (and in all {variables.prodname} documents) we tend to use the term _endpoint_ to refer to a virtual machine, container, appliance, bare metal server, or any other entity that is connected to a {variables.prodname} network. If we are referring to a specific type of endpoint, we will call that out (such as referring to the behavior of VMs as distinct from containers).
