...
`calicoq endpoint <substring>` shows you the {variables.prodname} policies and profiles that
relate to endpoints whose full ID includes `<substring>`. It displays, for
each endpoint:

- the policies and profiles that apply to that endpoint (that {variables.prodname} uses to
...
applies. The untracked policy is listed first because {variables.prodname} enforces
...
- [NetworkPolicy](../../resources/networkpolicy.mdx) and
  ...
  for more information about the {variables.prodname} policy model.
