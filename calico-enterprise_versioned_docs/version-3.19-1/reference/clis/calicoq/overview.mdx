...
`calicoq` is the {variables.prodname} policy query utility. It is a command line tool that
makes it easy to check your {variables.prodname} security policies.
See [Installing calicoq](../../../operations/clis/calicoq/index.mdx) for
how to download and install `calicoq`.

## Datastore configuration

`calicoq` works by querying the {variables.prodname} datastore. For this configuration calicoq uses exactly the same
...
- The [endpoint](endpoint.mdx)
  command shows you the {variables.prodname} policies and profiles that relate to specified
  endpoints.
