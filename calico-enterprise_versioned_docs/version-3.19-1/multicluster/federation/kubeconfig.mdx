...
Secure cross-cluster traffic with identity-aware policy, and leverage {variables.prodname} to establish the required cross-cluster networking.
...
RemoteClusterConfiguration creates this connection in one direction. If you want identity-aware policy on both sides (i.e. both clusters) of a connection, or you want {variables.prodname} to establish cross-cluster overlay networking, you need to create a RemoteClusterConfiguration for both directions.
...
## Before you begin
**Required**
- [Install {variables.prodname}](../../getting-started/install-on-clusters/kubernetes/index.mdx)
...
Federation of workload endpoint identities requires [Pod IP routability](./overview#pod-ip-routability) between clusters. If your clusters are using a supported overlay networking mode, {variables.prodname} can automatically meet this requirement when clusters are connected.

#### {variables.prodname} multi-cluster networking
{variables.prodname} can automatically extend the overlay networking in your clusters to establish pod IP routes across clusters and thus meet the requirement for Pod IP routability. Only VXLAN overlay is supported at this time.

Ensure the following requirements are met if utilizing {variables.prodname} multi-cluster networking to achieve pod IP routability:
...
Alternatively, you can meet the requirement for Pod IP routability by configuring {variables.prodname} with BGP or with VPC routing to establish unencapsulated Pod IP routes in your environment.
...
1. Create the ServiceAccount used by remote clusters for authentication:
   ...
   kubectl apply -f \{variables.filesUrl}/manifests/federation-remote-sa.yaml
   </CodeBlock>

1. If RBAC is enabled, create the ClusterRole and ClusterRoleBinding used by remote clusters for authorization:

   <CodeBlock language='bash'>
   kubectl apply -f \{variables.filesUrl}/manifests/federation-rem-rbac-kdd.yaml
   </CodeBlock>
...
In this setup, the cluster mesh will be configured to meet the pod IP routability requirement by establishing routes between clusters using [{variables.prodname} multi-cluster networking](#calico-enterprise-multi-cluster-networking).
...
1. If RBAC is enabled in cluster A, create a Role and RoleBinding for {variables.prodname} to use to access the secret that contains the kubeconfig for cluster B:
   ```bash
...
1. If you have no IP pools in cluster A with NAT-outgoing enabled, skip this step.
   ...
   Otherwise, if you have IP pools in cluster A with NAT-outgoing enabled, and workloads in that pool will egress to workloads in cluster B, you need to instruct {variables.prodname} to not perform NAT on traffic destined for IP pools in cluster B.
...
:::caution
 This tutorial sets up RemoteClusterConfigurations in both directions. This is required for {variables.prodname} to manage multi-cluster networking, and also ensures you can write identity-aware policy on both sides of a cross-cluster connection. Unidirectional connections can be made at your own discretion.
:::
...
If all requirements were met for {variables.prodname} to establish multi-cluster networking, you can test the functionality by establishing a connection from a pod in a local cluster to the IP of a pod in a remote cluster. Ensure that there is no policy in either cluster that may block this connection.
...
If you do not see the each of the resource types above, overlay routing was not successfully enabled in your cluster. Verify that you followed the [setup](#create-remoteclusterconfigurations) correctly for overlay routing, and that the cluster is using a version of {variables.prodname} that supports multi-cluster networking.
