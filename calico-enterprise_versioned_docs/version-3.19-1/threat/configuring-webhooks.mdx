...
# Create webhooks for security event integrations
...
You can configure {variables.prodname} webhooks to post security alerts directly to Slack, Jira, or your own integration.
...
![Example Slack integration alert](/img/calico-enterprise/slack-integration.jpg "Example Slack integration alert")

## Before you begin

Your target application must be configured to receive data from the {variables.prodname} webhook.

* **Slack**. You must have a webhook URL for the Slack app that you want {variables.prodname} to send alerts to.
...
* **Jira**. You must have an API token for an Atlassian user account that has write permissions to your Jira instance.
  ...
  * A Jira project key. This is the Jira project where your {variables.prodname} webhook creates new issues. This user associated with your API token must have write permissions to this project.
* **Generic JSON**. You must have a webhook URL for any other application you want the {variables.prodname} webhook to send alerts to.
