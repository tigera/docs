...
import License from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/content/_license.mdx';

## Big picture

Use the Prometheus monitoring and alerting tool to get {variables.prodname} license metrics.

## Value

Platform engineering teams need to report licensing usage on third-party software (like {variables.prodname}) for their CaaS/Kubernetes platforms. This is often driven by compliance, but also to mitigate risks from license expiration or usage that may impact operations. For teams to easily access these vital metrics, {variables.prodname} provides license metrics using the Prometheus monitoring and alerting tool.

## Concepts

### About Prometheus

The Prometheus monitoring tool scrapes metrics from instrumented jobs and displays time series data in a visualizer (such as Grafana). For {variables.prodname}, the “jobs” that Prometheus can harvest metrics from the License Agent component.

### About License Agent

The **License Agent** is a containerized application that monitors the following {variables.prodname} licensing information from the Kubernetes cluster, and exports the metrics through the Prometheus server:
...
1. Apply the manifest.
   <CodeBlock language=''>
    kubectl apply -f \{variables.filesUrl}/manifests/licenseagent.yaml
   </CodeBlock>
