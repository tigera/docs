...
Adjust the size of the {variables.prodname} log storage during or after installation.

## Value

By default, {variables.prodname} creates the log storage with a single node. This makes it easy to get started using {variables.prodname}.
...
In the following example, {variables.prodname} is configured to install 3 nodes that have 200Gi of storage each with 1 replica. Whenever the storage size is modified, resourceRequirements must be revisited respectively to support these changes.
