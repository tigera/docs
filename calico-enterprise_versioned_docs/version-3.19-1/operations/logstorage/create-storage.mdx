...
import PersistentStorageTerms from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/content/_persistent-storage-terms.mdx';
...
Before installing {variables.prodname}, you must configure persistent storage for flow logs, DNS logs, audit logs, and compliance reports.

## Concepts

Before configuring a storage class for {variables.prodname}, the following terms will help you understand storage interactions.
...
Before installing {variables.prodname}, create a storage class named, `tigera-elasticsearch`.
...
If local persistent volumes are provisioned on an SELinux-enabled host, you can use the `/mnt/tigera` host path created by the {variables.prodname} policy package.
...
In the following example for an AKS cloud provider integration, the **StorageClass** tells {variables.prodname} to use LRS disks for log storage.
:::note
...
In the following example for a GKE cloud provider integration, the **StorageClass** tells {variables.prodname} to use the GCE Persistent Disks for log storage.
