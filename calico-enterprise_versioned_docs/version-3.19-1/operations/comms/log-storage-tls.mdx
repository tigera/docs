...
Provide TLS certificates to secure access to {variables.prodname} to the log storage.
...
By default, the {variables.prodname} log storage uses self-signed certificates on connections. To provide TLS certificates,
get the certificate and key pair for the {variables.prodname} log storage using any X.509-compatible tool or from your organization's
...
To provide TLS certificates for use by {variables.prodname} components during deployment, you must create a secret before applying the 'custom-resource.yaml' or before creating the LogStorage resource. Use the following command to create a secret:
...
If the {variables.prodname} log storage already exists, you must manually delete the log storage pods one by one
after updating the secret. These pods will be in the `tigera-elasticsearch` namespace with the prefix, `tigera-secure-es`.
Other {variables.prodname} components will not be unable to communicate with log storage until the pods are restarted.
