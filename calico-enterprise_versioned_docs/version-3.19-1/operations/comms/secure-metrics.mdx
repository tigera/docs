...
## About securing access to {variables.prodname}'s metrics endpoints

When using {variables.prodname} with Prometheus metrics enabled, we recommend using network policy
to limit access to {variables.prodname}'s metrics endpoints.

## Prerequisites

- {variables.prodname} is installed with Prometheus metrics reporting enabled.
...
to {variables.prodname}'s Prometheus metrics. Choosing an approach depends on your requirements.
...
- [Using a deny-list approach](#using-a-deny-list-approach)
  ...
  {variables.prodname} policy. This approach allows you to restrict access to specific ports, while leaving other
...
1. Create a network policy that denies unwanted traffic to the {variables.prodname} metrics endpoints.
1. Apply labels to allow access to the Prometheus metrics.

### Example for {variables.nodecontainer}

This example shows how to limit access to the {variables.nodecontainer} Prometheus metrics endpoints.
...
1. Create a default network policy to allow host traffic
   ...
   metadata:
     name: default-host
   spec:
     # Select all \{variables.prodname} nodes.
...
1. List the nodes on which {variables.prodname} is running with the following command.
...
1. Create host endpoints for each {variables.prodname} node.
   ...
   Note the use of a label to indicate that this host endpoint is running {variables.prodname}. The
...
1. Create a network policy that restricts access to the {variables.nodecontainer} Prometheus metrics port.
   ...
   all {variables.prodname} workload endpoints, host endpoints, and global network sets that do not have the label, as well as any
   other network endpoints unknown to {variables.prodname}.
...
1. Apply labels to any endpoints that should have access to the metrics.
   ...
   {variables.prodname}'s Prometheus metrics endpoints on each node. To grant access, simply add this label to the
   desired endpoints.
...
If your {variables.prodname} installation uses the Kubernetes API datastore and has greater than 50 nodes, it is likely
...
If your {variables.prodname} installation exposes metrics from kube-controllers, you can limit access to those metrics
...
The above policy is installed in the calico-system namespace. If your cluster has {variables.prodname} installed
...
1. Create host endpoints for each node that you'd like to secure.
1. Create a network policy that allows desired traffic to the {variables.prodname} metrics endpoints.
1. Apply labels to allow access to the Prometheus metrics.
...
1. Create host endpoints for each {variables.prodname} node.
   ...
   {variables.prodname} allows some traffic as a failsafe even after applying this policy. This can
...
1. Create a network policy that allows access to the {variables.nodecontainer} Prometheus metrics port.
   ...
   all {variables.prodname} workload endpoints, host endpoints, and global network sets that have the label will be allowed access.
