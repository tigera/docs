...
## Connections from {variables.prodname} components to kube-apiserver (Kubernetes and OpenShift)

We recommend enabling TLS on kube-apiserver, as well as the client certificate and JSON web token (JWT)
authentication modules. This ensures that all of its communications with {variables.prodname} components occur
over TLS. The {variables.prodname} components present either an X.509 certificate or a JWT to kube-apiserver
...
Tigera {variables.prodname} Manager's web interface, run from your browser, uses HTTPS to securely communicate
with the {variables.prodname} Manager, which in turn, communicates with the Kubernetes and {variables.prodname} API
servers also using HTTPS. Through the installation steps, secure communication between
{variables.prodname} components should already be configured, but secure communication through your web
...
{variables.prodname} Manager through a
...
at the {variables.prodname} Manager. This means that the TLS certificates used to secure traffic
between your web browser and the {variables.prodname} Manager do not need to be shared or related
to any other TLS certificates that may be used elsewhere in your cluster or when
configuring {variables.prodname}. The flow of traffic should look like the following:

![{variables.prodname} Manager traffic diagram](/img/calico-enterprise/cnx-tls-mgr-comms.svg)
...
browser and the {variables.prodname} Manager.

:::

To properly configure TLS in the {variables.prodname} Manager, you will need
...
matches the host name/DNS entry/IP address that is used to access the {variables.prodname} Manager
(i.e. what it says in the browser address bar).

:::

## Configuring {variables.prodname} Manager UI TLS

Configure manager TLS based on how you've deployed {variables.prodname}
...
{variables.prodname} Manager. During installation of the {variables.prodname} Manager, you should have run
...
you will need to kill the {variables.prodname} Manager pod so that it is restarted to uptake the new
certificates.
...
- **Untrusted Certificate Authority**: Your browser may not display `Secure` because
  ...
  the certificates that the {variables.prodname} Manager is using. This is generally caused by using
  self-signed certificates (either generated by Kubernetes or manually). If you have
  certificates signed by a recognized CA, we recommend that you use them with the {variables.prodname}
  ...
  to be imported into every browser you access {variables.prodname} Manager from.

- **Mismatched Common Name or Subject Alternative Name**: If you are still having issues
  securely accessing {variables.prodname} Manager with TLS, you may want to make sure that the Common Name
  or Subject Alternative Name specified in your certificates matches the host name/DNS
  entry/IP address that is used to access the {variables.prodname} Manager (i.e. what it says in the browser
  address bar). In Google Chrome you can check the {variables.prodname} Manager certificate with Developer Tools
  ...
  Subject Alternative Name and reconfigure {variables.prodname} Manager following the steps above.
...
proxies etc., between user web browsers and the {variables.prodname} Manager. If you do so, configure your proxy
such that {variables.prodname} Manager receives a HTTPS (TLS) connection, not unencrypted HTTP.
...
- re-originate a TLS connection from your proxy to {variables.prodname} Manager, as it expects TLS

If you do not require TLS termination, configure your proxy to "pass thru" the TLS to {variables.prodname} Manager.
