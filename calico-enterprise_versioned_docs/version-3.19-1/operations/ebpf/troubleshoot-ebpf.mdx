...
- Either {variables.prodname}'s eBPF mode or `kube-proxy` must be active on a host for services to function. If you
  disabled `kube-proxy` when enabling eBPF mode, verify that eBPF mode is actually functioning. If {variables.prodname}
  detects that the kernel is not supported, it will fall back to standard dataplane mode (which does not support
  services).

  To verify that eBPF mode is correctly enabled, examine the log for a `{variables.noderunning}` container; if
...
- In DSR mode, {variables.prodname} requires that the underlying network fabric allows one node to respond on behalf of
  another.
...
Since BPF maps contain binary data, the {variables.prodname} team wrote a tool to examine {variables.prodname}'s BPF maps.
The tool is embedded in the {variables.nodecontainer} container image. To run the tool:

- Find the name of the {variables.nodecontainer} Pod on the host of interest using
...
If you notice `{variables.noderunning}` using high CPU:
...
- {variables.prodname} supports endpoint slices, similarly to `kube-proxy`. If your Kubernetes cluster supports endpoint
  slices and they are enabled, then you can enable endpoint slice support in {variables.prodname} with the
  `bpfKubeProxyEndpointSlicesEnabled` configuration flag.

## eBPF program debug logs

{variables.prodname}'s eBPF programs contain optional detailed debug logging. Although th logs can be very verbose (because
...
- `ens192---E` is the {variables.prodname} log tag. For programs attached to interfaces, the first part contains the
...
- Verify that you are using the best networking mode for your cluster. If possible, avoid using an overlay network;
  a routed network with no overlay is considerably faster. If you must use one of {variables.prodname}'s overlay modes,
