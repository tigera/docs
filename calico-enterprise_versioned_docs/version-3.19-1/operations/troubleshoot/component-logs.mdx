...
View and collect {variables.prodname} logs.
...
### {variables.nodecontainer} logs

The {variables.nodecontainer} logs contain log output from the following subcomponents:
...
## View logs for a {variables.nodecontainer} instance
...
1. Modify logSeverityScreen to desired value.
   ...
   For a global change set the name to "default".
   For a node-specific change set the name to the node name, e.g., "{variables.prodname}-Node-1".
