...
- You are decommissioning a host running `{variables.nodecontainer}` or removing it from your
  cluster.
...
- Prior to removing any Node resource from the datastore the `{variables.nodecontainer}`
