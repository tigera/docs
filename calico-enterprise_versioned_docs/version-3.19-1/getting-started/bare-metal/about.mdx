...
import DockerContainerService from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/content/_docker-container-service.mdx';

import NonClusterBinaryInstall from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/content/_non-cluster-binary-install.mdx';
...
Secure non-cluster hosts by installing {variables.prodname} network policy.
...
A non-cluster host is a computer that is running an application that is _not part of a Kubernetes cluster_. But you can protect hosts using the same {variables.prodname} network policy that you use for your Kubernetes cluster. In the following diagram, the Kubernetes cluster is running full {variables.prodname} with networking (for pod-to-pod communications) and network policy; the non-cluster host uses {variables.prodname} network policy only for host protection.
...
To learn how to restrict traffic to/from hosts using {variables.prodname} network policy see, [Protect hosts](../../network-policy/hosts/protect-hosts.mdx).
...
- Kubernetes API datastore is up and running and is accessible from the host
  ...
  If {variables.prodname} is installed on a cluster, you already have a datastore.

- Non-cluster host meets {variables.prodname} [system requirements](requirements.mdx)
...
1. Configure container to start at boot time.
   The `{variables.nodecontainer}` container should be started at boot time by your init system and the init system must be configured to restart it if stopped. {variables.prodname} relies on that behavior for certain configuration changes.
...
Using {variables.prodname} network policy-only mode, you must ensure that the non-cluster host can directly communicate with your Kubernetes cluster. Here are some vendor tips:
