...
Install {variables.prodname} on RKE2 using the Rancher UI (Rancher Manager).

## Before you begin

:::note

To install {variables.prodname} using Rancher UI, you must provision a base RKE2 cluster with Calico Open Source, then upgrade to {variables.prodname}. This is required because Rancher UI does not provide an option to set the RKE2 CNI value as `none`, which is required to install a non-default CNI like {variables.prodname}. 
...
- [Upgrade to {variables.prodname}](#install-calico-enterprise)
...
### Upgrade to {variables.prodname}

1. In Rancher UI, open a `kubectl` shell for the cluster.

2. Follow the [steps to upgrade Calico to {variables.prodname} in the `kubectl` shell](../upgrading/upgrading-calico-to-calico-enterprise/upgrade-to-tsee/standard.mdx)
