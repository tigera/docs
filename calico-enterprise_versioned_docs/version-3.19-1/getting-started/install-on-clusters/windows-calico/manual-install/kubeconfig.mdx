...
Create kubeconfig for Windows nodes for manual installations of {variables.prodnameWindows}.

## How to

In a manual installation of {variables.prodnameWindows}, {variables.prodname} requires a kubeconfig file to access the API server. This section describes how to find an existing `calico-node` service account used by {variables.prodname} on Linux side, and then to export the service account token as a kubeconfig file for {variables.prodname} to use.

:::note

In general, the node kubeconfig as used by kubelet does not have enough permissions to access {variables.prodname}-specific resources.
...
Note: if {variables.prodname} is installed in kube-system, update the `namespace` in the above command.
...
Copy this config file to the windows node `{variables.rootDirWindows}\calico-kube-config` and set the KUBECONFIG environment variable in `config.ps1` to point to it.
