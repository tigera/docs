...
import InstallGKETemp from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/components-temp/InstallGKE-temp.js';

# Google Kubernetes Engine (GKE)

## Big picture

Install {variables.prodname} on a GKE managed Kubernetes cluster.

<!-- CE supports only GKE CNI, even though OS/Calico supports Calico CNI. Verified with Shaun, Casey, and Sujeet in 3.15. So docs will continue to be misaligned so be careful during merges. Note that OS support of Calico CNI is only in v1 of the dataplane; in v2, Cillium is the supported CNI. Another misalignment is that CE install docs list all platforms supported; OS/Calico lists of supported CNIs based on an opinionated view. -->

## Before you begin

**CNI support**

GKE CNI with {variables.prodname} network policy:
...
- Cluster has these Networking settings:
  ...
  - GKE control plane access to TCP ports 5443, 8080 and 9090
    The GKE control plane must be able to access the {variables.prodname} API server which runs with pod networking on TCP ports 5443 and 8080, and the {variables.prodname} Prometheus server which runs with pod networking on TCP port 9090. For multi-zone clusters and clusters with the "master IP range" configured, you will need to add a GCP firewall rule to allow access to those ports from the control plane nodes.
...
1. [Install {variables.prodname}](#install-calico-enterprise)
1. [Install the {variables.prodname} license](#install-the-calico-enterprise-license)

<InstallGKETemp clusterType='standalone' />
...
- [Get started with {variables.prodname} network policy](../../network-policy/beginners/calico-network-policy.mdx)
