---
description: Enable Calico network policy in EKS.
---

import variables from '@site/calico-enterprise_versioned_docs/version-3.19-1/variables';

import InstallEKSTemp from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/components-temp/InstallEKS-temp.js';

# Amazon Elastic Kubernetes Service (EKS)

## Big picture

Install {variables.prodname} on an EKS managed Kubernetes cluster.

## Before you begin

**CNI support**

- Calico CNI for networking with {variables.prodname} network policy

  The geeky details of what you get by default:

  <GeekDetails
    prodname='{variables.prodname}'
    details='Policy:Calico,IPAM:Calico,CNI:Calico,Overlay:IPIP,Routing:BGP,Datastore:Kubernetes'
  />

- AWS CNI networking with {variables.prodname} network policy

  The geeky details of what you get by default:

  <GeekDetails
    prodname='{variables.prodname}'
    details='Policy:Calico,IPAM:AWS,CNI:AWS,Overlay:No,Routing:VPC Native,Datastore:Kubernetes'
  />

**Required**

- A [compatible EKS cluster](../compatibility.mdx#eks)

- Cluster meets [system requirements](requirements.mdx)

- A [Tigera license key and credentials](calico-enterprise.mdx)

- [Install kubectl](https://kubernetes.io/docs/tasks/tools/install-kubectl/)

## How to

1. [Option A: Install with Amazon VPC networking](#install-eks-with-amazon-vpc-networking)
1. [Option B: Install with Calico CNI networking](#install-eks-with-calico-networking)
1. [Install the {variables.prodname} license](#install-the-calico-enterprise-license)

<InstallEKSTemp clusterType='standalone' />

## Next steps

- [Configure access to {variables.prodname} Manager UI](../../operations/cnx/access-the-manager.mdx)

**Recommended**

- [Video: Everything you need to know about Kubernetes pod networking on AWS](https://www.projectcalico.org/everything-you-need-to-know-about-kubernetes-pod-networking-on-aws/)
- [Get started with {variables.prodname} network policy](../../network-policy/beginners/calico-network-policy.mdx)
- [Enable default deny for Kubernetes pods](../../network-policy/beginners/kubernetes-default-deny.mdx)
