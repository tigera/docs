...
import UpgradeOperatorSimpleTemp from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/components-temp/UpgradeOperatorSimple-temp.js';
...
Verify that your Kubernetes cluster is using a version of {variables.prodname} installed with the operator, by running
...
For AKS only, upgrades beginning from {variables.prodname} v3.11.2 to a newer version will automatically upgrade {variables.prodnameWindows}. During the upgrade, Windows nodes will be tainted so new pods will not be scheduled until the upgrade of the node has finished. The {variables.prodnameWindows} upgrade status can be monitored with: `kubectl get tigerastatus calico -oyaml`

For all other platforms or versions older than v3.11.2, upgrading {variables.prodnameWindows} can be performed out-of-band with the {variables.prodname} upgrade of the cluster. For each Windows node, [uninstall](../../../install-on-clusters/windows-calico/manual-install/maintain.mdx#uninstall-calico-enterprise-for-windows-from-windows-nodes) the {variables.prodnameWindows} services, copy over the latest {variables.prodnameWindows} installation archive, then proceed with the [installation](../../../install-on-clusters/windows-calico/manual-install/quickstart.mdx#install-calico-enterprise-for-windows-manually).

### Multi-cluster management

For {variables.prodname} v3.5 and v3.7, upgrading multi-cluster management setups must include updating all managed and management clusters.
...
<UpgradeOperatorSimpleTemp upgradeFrom='Enterprise' />
...
If {variables.prodname} was installed directly onto the AKS cluster, follow the upgrade instructions in the **Kubernetes** tab.

:::

These AKS upgrade instructions apply only to AKS clusters which were upgraded to
{variables.prodname} from Calico. Check whether the cluster was upgraded from Calico by
