...
Ensure that your {variables.prodname} OpenShift cluster is running a supported version of [OpenShift Container Platform](../../compatibility.mdx#openshift), and the {variables.prodname} operator version is v1.2.4 or greater.
...
During upgrade, the {variables.prodname} LogStorage CR is temporarily removed so Elasticsearch can be upgraded. Features
...
<InstallOpenShiftManifestsTemp upgradeFrom='Enterprise' />
...
1. <OpenShiftPrometheusOperatorTemp operation='upgrade' />
...
1. Remove unused policies in your cluster.
   ...
   kubectl delete -f \{variables.filesUrl}/manifests/default-tier-policies-managed.yaml
   </CodeBlock>

   For other clusters, run this command:

   <CodeBlock language='bash'>
   kubectl delete -f \{variables.filesUrl}/manifests/default-tier-policies.yaml
   </CodeBlock>
