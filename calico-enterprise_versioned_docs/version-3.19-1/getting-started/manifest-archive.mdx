...
import ReleaseArchiveTableTemp from "@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/components-temp/ReleaseArchiveTable-temp";

Installing the most recent patch version of {variables.prodname} {variables.version} ensures that you have the latest bug fixes and improvements.
...
- `enterprise-resources` contains the {variables.prodname} resources.
- `upgrade-manifests` folder contains all the manifests needed for upgrading {variables.prodname}.
...
1. Install {variables.prodname} resources:
