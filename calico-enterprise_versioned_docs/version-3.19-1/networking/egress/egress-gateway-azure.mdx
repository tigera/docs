...
using {variables.prodname} IPAM. egress gateway pods use dedicated IPPools to use as source IP addresses, which enables static configuration
...
{variables.prodname} CNI and IPAM.
...
a network virtual appliance, like {variables.prodname}, to dynamically configure Azure fabric by exchanging routing information
using Border Gateway Protocol (BGP). {variables.prodname} can establish BGP sessions with an Azure Route Server in a VNet
...
{variables.prodname} CNI and IPAM are required. The ability to control the egress gateway’s IP is a feature of {variables.prodname} CNI and IPAM. Azure VNet CNI does not support that feature, so it is incompatible with egress gateways.
...
It is possible to establish BGP connections between all {variables.prodname} nodes and Azure Route Servers, but
...
The default {variables.prodname} **node-to-node BGP mesh** may be turned off to enable other BGP topologies.
...
Usually, in the default installation of {variables.prodname} in a public cloud, BGP is disabled as it is not used.
However, running egress gateways in Azure relies on BGP connections between {variables.prodname} and Azure
...
and then restart the {variables.prodname} daemonSet:
...
It is only necessary to advertise the egress IP pool, created in the previous section, to Azure Route Servers. By default {variables.prodname}
...
Even with the BGP filter, {variables.prodname} still advertises pod IP blocks. To prevent {variables.prodname} advertising pod IP blocks, it is possible to set
`disableBGPExport` to `true` in the default IP pools.

### Configure route reflectors

{variables.prodname} nodes can be configured to act as route reflectors. To do this, each node that you want to act as a route reflector
...
Finally, Add the IP addresses of Azure Route Servers, usually there are two, as peers to route reflectors. For example, if the {variables.prodname}
...
- It is mandatory to set `reachableBy` field set to the gateway of the subnet {variables.prodname} cluster is running in for
...
* This annotation tells {variables.prodname} IPAM to use one of the "egress-ip-red-pool" IP pool.
...
Since v3.15.0, {variables.prodname} also sends health probes to the egress gateway pods from the nodes where their clients are located.
