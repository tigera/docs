...
Once the updated egress gateway deployment rolls out, we're ready to observe the gateway maintenance annotations {variables.prodname} adds to your dependent workloads. Let's simulate cluster maintenance by deleting an egress gateway pod. It should take 60 seconds to terminate - the amount of time defined by `terminationGracePeriodSeconds`.
...
{variables.prodname} selects a subset of replicas in such a way as to evenly distribute load across the whole replica set.
...
**The following are annotations that {variables.prodname} sets automatically on any egress-dependent pods:**
