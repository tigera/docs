...
Configure the maximum transmission unit (MTU) for your {variables.prodname} environment.

## Value

Optimize network performance for workloads by configuring the MTU in {variables.prodname} to best suit your underlying network.
...
### MTU and {variables.prodname} defaults
...
By default, {variables.prodname} will auto-detect the correct MTU for your cluster based on node configuration and enabled networking modes. This guide explains how you can override auto-detection
...
The following table lists common MTU sizes for {variables.prodname} environments. Because MTU is a global property of the network path between endpoints, you should set the MTU to the minimum MTU of any path that packets may take.

**Common MTU sizes**

| Network MTU            | {variables.prodname} MTU | {variables.prodname} MTU with IP-in-IP (IPv4) | {variables.prodname} MTU with VXLAN (IPv4) | {variables.prodname} MTU with VXLAN (IPv6) | {variables.prodname} MTU with WireGuard (IPv4) | {variables.prodname} MTU with WireGuard (IPv6) |
| ---------------------- | ---------------- | ------------------------------------- | ---------------------------------- | ---------------------------------- | -------------------------------------- | -------------------------------------- |
...
- If using flannel with VXLAN, use the “{variables.prodname} MTU with VXLAN” column in the table above for common sizes.

### Configure MTU

:::note

The updated MTU used by {variables.prodname} only applies to new workloads.

:::

For Operator installations, edit the {variables.prodname} operator `Installation` resource to set the `mtu`
