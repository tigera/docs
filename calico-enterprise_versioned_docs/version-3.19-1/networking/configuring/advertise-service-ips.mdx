...
Enable {variables.prodname} to advertise Kubernetes service IPs outside a cluster. {variables.prodname} supports advertising a service’s cluster IPs and external IPs.
...
Just as {variables.prodname} supports advertising **pod IPs** over BGP, it also supports advertising Kubernetes **service IPs** outside a cluster over BGP. This avoids the need for a dedicated load balancer. This feature also supports equal cost multi-path (ECMP) load balancing across nodes in the cluster, as well as source IP address preservation for local services when you need more control.

## Concepts

### BGP makes it easy

In Kubernetes, all requests for a service are redirected to an appropriate endpoint (pod) backing that service. Because {variables.prodname} uses BGP, external traffic can be routed directly to Kubernetes services by advertising Kubernetes service IPs into the BGP network.
...
{variables.prodname} implements the Kubernetes **externalTrafficPolicy** using kube-proxy to direct incoming traffic to a correct pod. Advertisement is handled differently based on the service type that you configure for your service.
...
- Calico CNI
- [Configure BGP peering](bgp.mdx) between {variables.prodname} and your network infrastructure
...
In earlier versions of {variables.prodname}, and for IPv4 only, service cluster IP advertisement was configured via the environment variable CALICO_ADVERTISE_CLUSTER_IPS.
...
1. Identify the external IP ranges of all services that you want to advertise outside of the {variables.prodname} cluster.
...
The following steps will configure {variables.prodname} to advertise Service `status.LoadBalancer.Ingress.IP` addresses.
...
Service LoadBalancer address allocation is outside the current scope of {variables.prodname}, but can be implemented with an external controller.
...
1. Configure MetalLB to provision addresses by creating the following IPAddressPool, replacing `x.x.x.x/16` with the CIDR given to {variables.prodname} in the steps above. Please note IPAddressPool requires Metallb `0.13+`.
...
For a tutorial on how service advertisement works with {variables.prodname}, see the blog [Kubernetes Service IP Route Advertisement](https://www.projectcalico.org/kubernetes-service-ip-route-advertisement/).
