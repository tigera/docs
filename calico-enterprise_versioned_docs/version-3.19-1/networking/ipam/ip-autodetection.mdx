...
Configure IP autodetection for {variables.prodname} nodes to ensure the correct IP address is used for routing.

## Value

When you install {variables.prodname} on a node, an IP address and subnet is automatically detected. {variables.prodname} provides several ways to configure IP/subnet autodetection, and supports configuring specific IPs for:
...
For internode routing, each {variables.prodname} node must be configured with an IPv4 address and/or an IPv6 address. When you install {variables.prodname} on a node, a node resource is automatically created using routing information that is detected from the host. For some deployments, you may want to update autodetection to ensure nodes get the correct IP address.
...
By default, {variables.prodname} uses the **firstFound** method; the first valid IP address on the first interface (excluding local interfaces such as the docker bridge). However, you can change the default method to any of the following:
...
- **Kubernetes Node IP**
  ...
  {variables.prodname} will select the first internal IP address listed in the Kubernetes node's `Status.Addresses` field.
...
- **Source address used to reach an IP or domain name**
  ...
  {variables.prodname} will choose the IP address that is used to reach the given "can reach" IP address or domain. For example:
...
- **Including matching interfaces**
  ...
  {variables.prodname} will choose an address on each node from an interface that matches the given [regex](https://pkg.go.dev/regexp).
  For example:
...
- **Excluding matching interfaces**
  ...
  {variables.prodname} will choose an address on each node from an interface that does not match the given [regex](https://pkg.go.dev/regexp).
  For example:
...
- **Including CIDRs**
  ...
  {variables.prodname} will select any IP address from the node that falls within the given CIDRs. For example:
...
- For calicoctl environment variables, see [Configuring {variables.nodecontainer}](../../reference/component-resources/node/configuration.mdx)
- [Node resource](../../reference/resources/node.mdx)
- [Reference documentation for calicoctl patch](../../reference/clis/calicoctl/patch.mdx)
