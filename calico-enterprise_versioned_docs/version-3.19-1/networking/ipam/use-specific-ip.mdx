...
import DetermineIpam from '@site/calico-enterprise_versioned_docs/version-3.19-1/_includes/content/_determine-ipam.mdx';

## Big picture

Choose the IP address for a pod instead of allowing {variables.prodname} to choose automatically.
...
IP pools are ranges of IP addresses from which {variables.prodname} assigns pod IPs. Static IPs must be in an IP pool.
...
The address must be within a configured {variables.prodname} IP pool and not currently in use. The annotation must be present when the pod is created; adding it later has no effect.
...
by default, {variables.prodname} may decide to use the IP address that you select for another workload or for an internal
...
- To prevent {variables.prodname} from using IPs from a certain pool for internal IPIP and/or VXLAN tunnel addresses, you
  can set the `allowedUses` field on the [IP Pool](../../reference/resources/ippool.mdx) to `["Workload"]`.

## Additional resources

For help configuring {variables.prodname} CNI and {variables.prodname} IPAM, see [Configuring the {variables.prodname} CNI Plugins](../../reference/component-resources/configuration.mdx).
