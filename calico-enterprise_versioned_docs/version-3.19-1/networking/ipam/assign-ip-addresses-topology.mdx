...
If you have workloads in different regions, zones, or rack, you may want them to get IP addresses from the same IP pool. This strategy is useful for reducing the number of routes that are required in the network, or to meet requirements imposed by an external firewall device or policy. {variables.prodname} makes it easy to do this using an IP pool resource with node labels and node selectors.
...
By installing {variables.prodname} without setting the default IP pool to match,
running `calicoctl get ippool -o wide` shows that {variables.prodname} created its
