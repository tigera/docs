...
**IP pools** are ranges of IP addresses from which {variables.prodname} assigns pod IPs. By default, {variables.prodname} creates an IP pool for the entire Kubernetes pod CIDR, but you can change this to break the pod CIDR up into several pools. You can control which pool {variables.prodname} uses for each pod using node selectors, or annotations on the pod or the pod’s namespace.
...
For help configuring {variables.prodname} IPAM, see [Configuring the {variables.prodname} CNI Plugins](../../reference/component-resources/configuration.mdx).
