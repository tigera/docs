...
Secure Kubernetes nodes with host endpoints managed by {variables.prodname}.

## Value

{variables.prodname} can automatically create host endpoints for your Kubernetes nodes. This means {variables.prodname} can manage the lifecycle of host endpoints as your cluster evolves, ensuring nodes are always protected by policy.
...
Each host has one or more network interfaces that it uses to communicate externally. You can represent these interfaces in {variables.prodname} using host endpoints and then use network policy to secure them.

{variables.prodname} host endpoints can have labels, and they work the same as labels on workload endpoints. The network policy rules can apply to both workload and host endpoints using label selectors.
...
{variables.prodname} creates a wildcard host endpoint for each node, with the host endpoint containing the same labels and IP addresses as its corresponding node.
{variables.prodname} will ensure these managed host endpoints maintain the same labels and IP addresses as its node by periodic syncs.
