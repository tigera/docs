...
This guide is a variation of the [simple policy demo](../get-started/kubernetes-policy-basic.mdx) intended to introduce the extra features of {variables.prodname} to people already familiar with Project Calico for Kubernetes.
...
The key steps in moving to {variables.prodname} are to change to the {variables.prodname} version of calico-node, update its configuration, download [calicoq](../../reference/clis/calicoq/index.mdx) and deploy Prometheus.
