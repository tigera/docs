...
{variables.prodname} network policy uses standard Kubernetes Services that allow you to expose services within clusters to external clients in the following ways:
...
- {variables.prodname} cluster IP advertisement
- **HostEndpoints**
- **GlobalNetworkPolicy**
  - applyOnForward
  - preDNAT
...
Cluster IPs were originally designed for use within the Kubernetes cluster. {variables.prodname} allows you to advertise Cluster IPs externally -- so external clients can use them to access services hosted inside the cluster. This means that {variables.prodname} ingress policy can be applied at **one or both** of the following locations:
...
Using **local mode**, the original source address of external traffic is preserved, and you can define policy directly using standard {variables.prodname} network policy.

1. Create {variables.prodname} **NetworkPolicies** or **GlobalNetworkPolicies** that select the same set of pods as your Kubernetes Service.
...
Or, you can add rules that allow specific host traffic including Kubernetes and {variables.prodname}. Without this rule, normal host traffic is blocked.
