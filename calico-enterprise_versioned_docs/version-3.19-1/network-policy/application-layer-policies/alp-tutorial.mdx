...
This tutorial shows how to use {variables.prodname} application layer policy to restrict ingress traffic for applications and microservices.
...
## Set up
- A {variables.prodname} cluster is running with application layer policy enabled
...
With a {variables.prodname} policy, you can mitigate risks to the banking application.
...
Let's examine this policy piece by piece. First, notice that an application layer policy looks like a regular {variables.prodname} global network policy. 
