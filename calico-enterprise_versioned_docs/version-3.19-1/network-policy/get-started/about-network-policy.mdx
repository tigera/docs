...
This guide provides educational material that is not specific to {variables.prodname}.

:::

Kubernetes and {variables.prodname} provide network policy APIs to help you secure your workloads.

In this guide you will learn:

- What network policy is and why it is important.
- The differences between Kubernetes and {variables.prodname} network policies and when you might want to use each.
...
Note: {variables.prodname} offers capabilities that can help perimeter firewalls integrate
...
of the specification. ({variables.prodname} does implement every feature, and was the original reference implementation of Kubernetes
network policies.)

To learn more about Kubernetes network policies, read the [Get started with Kubernetes network
policy](kubernetes-network-policy.mdx) guide.

## {variables.prodname} network policy

In addition to enforcing Kubernetes network policy, {variables.prodname} supports its own
...
While Kubernetes network policy applies only to pods, {variables.prodname} network policy can be applied to multiple types of
endpoints including pods, VMs, and host interfaces.

To learn more about {variables.prodname} network policies, read the [Get started with {variables.prodname} network
policy](../beginners/calico-network-policy.mdx) guide.

## Benefits of using {variables.prodname} for network policy

### Full Kubernetes network policy support

Unlike some other network policy implementations, {variables.prodname} implements the full set of Kubernetes network policy features.

### Richer network policy

{variables.prodname} network policies allow even richer traffic control than Kubernetes network policies if you need it. In addition,
{variables.prodname} network policies allow you to create policy that applies across multiple namespaces using GlobalNetworkPolicy
resources.

### Mix Kubernetes and {variables.prodname} network policy

Kubernetes and {variables.prodname} network policies can be mixed together seamlessly. One common use case for this is to split
responsibilities between security / cluster ops teams and developer / service teams. For example, giving the security /
cluster ops team RBAC permissions to define {variables.prodname} policies, and giving developer / service teams RBAC permissions to
define Kubernetes network policies in their specific namespaces. As {variables.prodname} policy rules can be ordered to be enforced
...
For more flexible control and delegation of responsibilities between two or more teams, {variables.prodname} Enterprise extends this
...
As {variables.prodname} policies can be enforce on host interfaces, you can use them to protect your Kubernetes nodes (not
just your pods), including for example, limiting access to node ports from outside of the cluster. To learn more, check
out the {variables.prodname} [policy for hosts](../hosts/index.mdx) guides.

### Extendable with {variables.prodname}
...
deny](../beginners/kubernetes-default-deny.mdx) network policies. These ensure that if no other policy is
defined that explicitly allows traffic to/from a pod, then the traffic will be denied. As a result, anytime a team
deploys a new pod, they are forced to also define network policy for the pod. It can be useful to use a {variables.prodname}
