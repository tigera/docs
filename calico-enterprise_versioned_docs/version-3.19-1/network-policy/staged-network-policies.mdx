...
{variables.prodname} staged network policy resources lets you test the traffic impact of the policy as if it were enforced, but without changing traffic flow. You can also preview the impacts of a staged policy on existing traffic. By verifying that correct flows are allowed and denied before enforcement, you can minimize misconfiguration and potential network disruption.
...
and staged network policies. Adjust permissions for your environment. As with {variables.prodname} network policy and global network policies, the RBAC for {variables.prodname} staged network policy and staged global network policy is tier-dependent.
...
You can also use custom resources to stage Kubernetes and {variables.prodname} policies, and apply them using `kubectl`. Here are sample YAML files.
