---
description: Enable hostendpoint reporting in flow logs.
---

import variables from '@site/calico-enterprise_versioned_docs/version-3.19-1/variables';

# Enable HostEndpoint reporting in flow logs

## Big picture

Enable {variables.prodname} flow logs to report HostEndpoint information.

## Value

Get visibility into the network activity at the HostEndpoint level using {variables.prodname} flow logs.

## Before you begin

**Limitations**

HostEndpoint reporting is only supported on Kubernetes nodes.

## Features

This how-to guide uses the following {variables.prodname} features:

- **Flow logs**
- **HostEndpoint** resource
- **FelixConfiguration** resource

## How to

### Enable HostEndpoint reporting

To enable reporting HostEndpoint metadata in flow logs, use the following command:

```
 kubectl patch felixconfiguration default -p '{"spec":{"flowLogsEnableHostEndpoint":true}}'
```

## Additional resources

- [Protect Kubernetes nodes](../../../network-policy/hosts/kubernetes-nodes.mdx)
