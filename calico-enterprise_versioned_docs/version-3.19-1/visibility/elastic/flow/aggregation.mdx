...
The underlying reason for this overlap is a dependency on Linux conntrack, which provides the lifetime of stats that {variables.prodname} tracks across different protocols (TCP, ICMP, UDP). For example, for UDP and ICMP, {variables.prodname} waits for a conntrack entry to timeout before it considers a “connection” closed, and this is usually greater than 10 seconds.
