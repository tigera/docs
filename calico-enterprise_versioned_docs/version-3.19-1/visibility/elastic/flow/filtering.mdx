...
Filter {variables.prodname} flow logs.

## Value

Filter {variables.prodname} flow logs to suppress logs of low significance, and troubleshoot threats.
...
{variables.prodname} flow logs provide continuous records of every single packet sent/received by all pods in your Kubernetes cluster. Note that flow logs do not contain all packet data; only the number of packets/bytes that were sent between specific IP/ports, and when. In the previous monitoring tool example, {variables.prodname} flow logs could see the packets running to/from the bitcoin mining network.

{variables.prodname} flow logs tell you when a pod is compromised, specifically:
