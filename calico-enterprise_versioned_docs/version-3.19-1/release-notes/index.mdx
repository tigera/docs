...
We've updated the Endpoints page with a new flow logs panel so you can view and filter for Endpoints associated with denied traffic. Flow logs include the source, destination, ports, protocols, and other key forms for metadata. We've also updated the Policy Board to highlight policies with denied traffic.
