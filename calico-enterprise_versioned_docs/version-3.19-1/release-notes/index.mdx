...
### Improvements to Endpoints page in Manager UI

We've updated the Endpoints page so you can view and filter for Endpoints that are associated with denied traffic and associated flow logs. Denied traffic results includes the source, destination, ports, protocols, and other key forms for metadata associated with those flows. You can then go to the Policy Board and see that the policies that have been highlighted as having denied traffic.

### Improvements to Security events dashboard

We've added the following improvement to the [Security events dashboard](../threat/security-event-management.mdx).
...
- Added threat feed alerts
   ...
   If you have implemented global threat feeds for suspicious activity (domains or suspious IPs), alerts are now visible in the Security Overview dashboard. See [Trace and block suspicious IPs](.../threat/suspicious-ips).
...
* [Manual install for Windows](../getting-started/install-on-clusters/windows-calico/manual-install/) will be deprecated in a future release. Starting in in v3.18.1, the [standard installation is operator-based](../getting-started/install-on-clusters/windows-calico/operator).
