...
Use {variables.prodname} flow logs to trace external IP addresses that access
clusters.

## Value

Anomaly detection is key to threat protection. {variables.prodname} not only
...
To expose external IP addresses in flow logs, {variables.prodname} must also
track these intermediaries that function between the internet and the
cluster. When configured, {variables.prodname} flow logs for traffic that has
...
- {variables.prodname} flow logs are running **with flow log aggregation level of 1 or 0**.
...
logs, you need to set up the ingress log collector and configure {variables.prodname} to accept the
...
1. Download the patch file to patch-ingress.yaml.
   ...
   ```
   curl {variables.filesUrl}/manifests/ingress/patch-ingress.yaml -O
   ```
...
#### Step 2: Enable communication with {variables.prodname}

In order for the logs captured by the ingress log collector to be correlated with connection flow
logs properly, we need to enable communication between the ingress log collector and {variables.prodname}. Follow
...
- [{variables.prodname} installed](../getting-started/index.mdx)
...
1. Download the appropriate patch file for enabling the Policy Sync API on Felix.
   ```
   curl {variables.filesUrl}/manifests/ingress/patch-flexvol.yaml -O
   ```
...
1. Add the following json template to the logging format for your chosen ingress
   ...
   **OpenShift router**
   To view OpenShift router logs, set up a syslog server (See the
...
1. Set the log file path to `/var/log/calico/ingress/ingress.log`.
   ...
   **NGINX Inc-maintained NGINX ingress controller**
   ...
   NGINX template to tell NGINX to log to a file. To do this, set
