...
> Go to the Alerts page to view events that are generated when an endpoint in the cluster queries a name on the list. For more information, see [Manage alerts](../observability/alerts.mdx).
...
4. In the $[prodname] web console, go the “Alerts” page to view events that are generated when an endpoint in the cluster queries a name on the list.
