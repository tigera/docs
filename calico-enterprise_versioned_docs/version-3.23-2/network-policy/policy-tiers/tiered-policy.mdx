...
The `kube-admin` tier replaces the `adminnetworkpolicy` tier from earlier releases. Support for the
upstream `AdminNetworkPolicy` (`policy.networking.k8s.io/v1alpha1`) resource is removed; existing
`AdminNetworkPolicy` resources must be migrated to `ClusterNetworkPolicy` with `spec.tier: Admin`
before upgrade.
...
The `kube-baseline` tier replaces the `baselineadminnetworkpolicy` tier from earlier releases.
Support for the upstream `BaselineAdminNetworkPolicy` resource is removed; existing
`BaselineAdminNetworkPolicy` resources must be migrated to `ClusterNetworkPolicy` with
`spec.tier: Baseline` before upgrade.
