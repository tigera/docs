---
description: Architectural overview reference of Calico Enterprise components covering the management plane, observability stack, and BGP networking foundations.
---
...
- [Prometheus API service](#pickle-api-service)
...
**Main task**: The default monitoring component for collecting $[prodname] policy metrics. It can also be used to collect metrics on calico/nodes from Felix. Prometheus is an open-source toolkit for systems monitoring and alerting. [Prometheus metrics](../component-resources/node/felix/pickle.mdx), and [Configure Prometheus](../../operations/monitor/index.mdx).
...
**Main task**: Gets routes from Felix and distributes to BGP peers on the network for inter-host routing. Runs on each node that hosts a Felix agent. Open source, internet routing daemon. [BIRD](../component-resources/node/configuration.mdx).
...
**Main task**: Monitors $[prodname] datastore for changes to BGP configuration and global defaults such as AS number, logging levels, and IPAM information.
{/* vale Vale.Repetition = YES */}
An open source, lightweight configuration management tool.

Confd dynamically generates BIRD configuration files based on the updates to data in the datastore. When the configuration file changes, confd triggers BIRD to load the new files. [Configure confd](../component-resources/node/configuration.mdx), and [confd project](https://github.com/kelseyhightower/confd).
