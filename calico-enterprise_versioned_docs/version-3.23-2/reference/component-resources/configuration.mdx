---
description: Reference for configuring the CNI plugin in Calico Enterprise covering operator-managed and manifest-mode CNI configuration options.
---
...
The $[prodname] CNI plugin reads Felix's `endpoint-status` directory to determine when the data plane has been programmed for a pod.
