---
description: Prometheus metrics reference for Felix in Calico Enterprise covering counters and gauges exposed for data plane health and policy evaluation.
---
...
- `endpoint|parent-label-name`: the label index was able to limit the scan to only those endpoints/parents that
  have a particular label (but was unable to limit it to a particular subset of values).  For example, `has(label)`
...
Felix exports statistics for monitoring load balancing.
...
Can be 1 or 0 (true or false). Indicates whether Felix's node has been marked for maintenance.