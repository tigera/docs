---
description: Reference for the calicoctl get command in Calico Enterprise, used to list resources in plain, YAML, JSON, or wide output formats.
---
...
Options:
  ...
  --show-policy-activities     Show policy activity timestamps (only for policy
...
### Showing policy activity

Pass `--show-policy-activities` to append policy-activity timestamps to
the table output. For each listed policy, calicoctl adds three columns:

- `LAST EVALUATED` — when the current generation of the policy was last
  evaluated against live traffic.
- `LAST EVALUATED (ANY GEN)` — when any generation of the policy (current
  or previous) was last evaluated.
- `LAST EVALUATED GEN` — which generation the `LAST EVALUATED (ANY GEN)`
  timestamp refers to.

This is useful for confirming that a staged or enforced policy is being
exercised by live traffic before you promote or clean it up.

The flag:

- Is only supported for policy kinds: `globalnetworkpolicy`,
  `networkpolicy`, `stagedglobalnetworkpolicy`, `stagednetworkpolicy`.
- Is only supported with the `ps` (default) and `wide` table output
  formats. It is not supported with `yaml`, `json`, `custom-columns`,
  or `go-template` output.

Example — show activity for all staged global network policies:
...
`calicoctl get` does not support `-l` / `--selector`. To scope output to
a specific tier or label set, filter the output by inspecting the table
columns, or use `kubectl get ... -o name` to resolve the matching
resource names and invoke `calicoctl get` on each one.
