...
| network | Name of the [Network](network.mdx) resource to which this peer belongs. When set, BIRD programs routes received from this peer into the routing table associated with the named `Network` (rather than the main table). Used for VRF networks. Mutually exclusive with `externalNetwork`. | - | string | |
