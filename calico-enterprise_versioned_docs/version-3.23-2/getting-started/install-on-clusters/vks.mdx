...
The Addon framework expects the `AddonConfig` name to use the format `<cluster-name>-<addon-name>`. If the name does not match this format, the framework ignores your `AddonConfig` and creates a new one with default values instead.
