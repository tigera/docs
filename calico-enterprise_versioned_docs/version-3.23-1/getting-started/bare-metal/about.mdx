...
- The following operating systems are supported for non-cluster hosts and VMs:
  * RHEL 8 and RHEL 9 (on x86-64 v3 chip architectures only)
...
1. Download the Calico Enterprise repository to work with your Linux distro.
   ...
   <TabItem label="Debian/Ubuntu" value="debian-1">
   1. Add the Tigera GPG key:
      ```bash
      curl -fsSL $[debianKeyUrl] | sudo gpg --dearmor -o /usr/share/keyrings/tigera-keyring.gpg
      ```
   1. Add the Calico Enterprise repo to your sources list:
      ```bash
      echo "deb [signed-by=/usr/share/keyrings/tigera-keyring.gpg] $[debianKeyUrl]/ ./" | \
      sudo tee /etc/apt/sources.list.d/calico-enterprise.list
      ```
   </TabItem>
   </Tabs>
