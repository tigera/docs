...
- {/* CORE-13089 */} If the trunk device is not named correctly under `hostConnections` in the `Network` resource and the trunk device matches the "BPF data interface regex" then it will lose connectivity.  This is because it will be treated as an L3 device and RPF checks will fail. Always name the trunk under `hostConnections`.
- {/* CORE-13462 */} Calico only checks the settings on the bridge at bring-up, if the configuration is changed after Felix first examines the bridge, it will not detect the configuration drift. Pin the bridge's settings in your node configuration so nothing rewrites them.
...
- {/* CORE-13456 */} When a `Network` is deleted, or it no longer applies to a node due to label changes, Felix fails to clean up all the state that it added to the bridge.  Remove the VLANs from the bridge yourself after deleting a `Network` or changing node labels.
