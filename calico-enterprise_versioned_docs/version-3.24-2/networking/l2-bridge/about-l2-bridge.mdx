...
L2 bridge networking attaches KubeVirt VMs and pods to a VLAN your physical network already carries, using a VLAN-aware Linux bridge on each node.
This page covers why you would do that, how the pieces fit together, and which decisions you cannot change after the cluster is built.
...
Its address is in DNS and named in firewall rules, license servers are bound to it, monitoring dashboards chart it, and other applications hold it in their own configuration.
...
Each one you create adds work for $[prodname] on every node it selects, so several Networks carrying one VLAN each cost more than one Network carrying several VLANs.
...
Tags are added and removed at the edges, so the workload never sees them: $[prodname] tags a frame on its way in, and strips the tag on its way out.
...
If the node has an interface $[prodname] can take over, one that is not carrying the node's own IP address, then $[prodname] creates and owns the bridge.
...
On a bridge you built, those become two sources of truth.
If they diverge, the next time your host configuration is reapplied it can strip a VLAN $[prodname] added, and traffic on that VLAN stops without anything reporting it.

If this is your case, go to [Connect workloads to an existing VLAN](connect-vlan.mdx).
...
If this is your case, start with [Prepare a bridge on single-uplink nodes](byo-bridge.mdx), then come back to [Connect workloads to an existing VLAN](connect-vlan.mdx) for the rest.
...
Nodes commonly end up this way for reasons that have nothing to do with $[prodname].
The node may be a physical server with a fixed set of cards, or carry a bonded uplink kept for redundancy.
It may run a standardized image that cannot be varied, or sit on one of the L2 VLANs by deliberate choice.
...
Moving that address onto a bridge is not a single operation.
Afterwards, the component that manages it has to be doing DHCP or static addressing on the *new* interface, or the address does not come back after a reboot.
...
Attaching the first workload can therefore lower the MTU of a bridge you built, and of any VLAN device above it.
See [Prepare a bridge on single-uplink nodes](byo-bridge.mdx) for how to pin it.
...
If your host is on its **own VLAN** while workloads are on others, the address goes on a VLAN sub-device of the bridge.
That is a device layered on top of the bridge, stripping the tag for that one VLAN.
...
Workloads on an L2 network also carry labels identifying the network they are on.
Policy can therefore select a specific interface of a workload attached to more than one network.
...
Felix does everything on the host side, in a fixed order.
It installs the policy program, programs the forwarding and neighbor entries, and locks the port down *before* the interface joins the bridge.
...
It validates the source MAC and source IP address of everything a workload sends, and checks the destination address of what it receives.
It also validates the sender fields in ARP, filters unexpected Ethernet types, and drops broadcast and multicast traffic.
Reverse path filtering on pod traffic is strictly enforced.

The goal is a plain one: a workload on an L2 network should be at least as safe as a virtual machine on a traditional VLAN.
It should in fact be safer, because its traffic is also subject to your network policy.
...
Traffic between a workload on the pod network and a workload on an L2 network goes out to your external router and back — **including when both are on the same node**.
...
The eBPF data plane sees the frame the same way in both directions, so one implementation covers every case.
It also opens the door to bypassing parts of the bridge path entirely in a later release.

## What $[prodname] must own

$[prodname] has to know each workload's IP address, because policy decisions and reverse path filtering both depend on it.
An address $[prodname] does not know about is one it cannot enforce, and cannot tell apart from a spoofed one.
