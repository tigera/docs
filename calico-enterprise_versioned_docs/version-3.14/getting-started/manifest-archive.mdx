...
1. In the patch release archive, find the `ocp-manifests` folder. This contains three folders `install-manifests`,
   ...
   `enterprise-resources` contains the {{prodname}} resources. `upgrade-manifests` folder contains all the manifests needed for upgrading {{prodname}}.
...
1. Apply patches.
   ...
   To install {{prodname}} resources apply the following command.
