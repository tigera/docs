...
{{prodname}} supports federated endpoint identity for OpenShift. However, the [federated services feature](services-controller.mdx)
cannot be used, because OpenShift currently does not allow users to disable the `RestrictedEndpointsAdmission` plugin.
...
In this section, we'll create the kubeconfig files that local clusters will use to connect to the remote cluster. For each cluster in the federation, follow these steps.
