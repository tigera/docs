...
## Overview
Calico enables Kubernetes users to segment their environment using network policies. With Calico, richer network policies compared to the Kubernetes standard can be applied from a global to a namespace scope, with ordering, and can protect hosts. This enables users to protect and divide their cluster into smaller segments, and apply network policy at a level of granularity appropriate for the application or environment.

Traditional or legacy firewall approaches (which Calico can integrate with) may protect the perimeter but do not protect lateral movement within a cluster, and may not have the ability to adequately protect such a dynamic and ephemeral environment as Kubernetes. Applying a segmentation approach enables security and protection of individual workloads, tenants, and namespaces. Network policies are the tool of choice for achieving network security over traditional firewalls due to their ability to apply to Kubernetes objects based on label selectors and not IP addresses.

Microsegmentation has the advantage of scalability. As networks or clusters grow and evolve, identity-aware microsegmentation (Calico has wider support for more selectors) allows for the addition of new segments without disrupting or exposing the existing network infrastructure. 

Microsegmentation can be achieved with Project Calico (the open source CNI), however Calico Enterprise and Calico Cloud have a richer set of functionality that makes this easier, including observability tools and policy editors and visualization. 
...
**Cluster**: The cluster can be regarded as a distinct security domain, as both platform and security teams often implement cluster-wide controls governing egress and ingress. These controls might include restricting egress connectivity to public domains or specifying the trusted sources allowed to send ingress traffic to the cluster. 

**Tenants**: Customers, business units, or teams that share the same Kubernetes infrastructure can be considered as tenants in the cluster. Each tenant is a distinct security domain as tailored controls may have to be applied to isolate tenants and ensure that they meet regulatory requirements such as PCI and HIPAA.
