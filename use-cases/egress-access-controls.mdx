...
* It restricts access to unknown endpoints protects can prevent data exfiltration.
...
In one of our blogs, we demonstrate how [implementing egress access controls in Kubernetes is enough to thwart an exploit of the log4shell vulnerability.](https://www.tigera.io/blog/how-network-security-policies-can-protect-your-environment-from-future-vulnerabilities-like-log4j/)
...
With the native network policy in Kubernetes, egress rules can be defined to limit communication from pods to other entities by defining a destination, using label selectors or IP addresses, and over which port and protocol.
...
* Network policy management using Calico’s policies board help you visualize policies that are applied to a cluster, group policies into hierarchical tiers, and control the order of evaluation and see the volume of egress traffic that is being allowed or denied.
...
You can find out more in our documentation about [staged policies](https://docs.tigera.io/calico-cloud/reference/resources/stagednetworkpolicy) or the [log action](https://docs.tigera.io/calico/latest/network-policy/get-started/calico-policy/calico-network-policy#generate-logs-for-specific-traffic).
...
Network sets can be applied through kubectl or created in the Calico Enterprise or Calico Cloud Manager UI.
